...
export interface ConversationManagerConfig {
  ...
  /** 每个 conversation 的最大待处理消息数。默认 5。 */
...
export class ConversationManager implements ConversationCommitProjection {
  ...
  constructor(
    ...
    if (this.durableTurnExecutorCb && !this.onTurnCommitted) {
      throw new Error(
        "ConversationManager: `onTurnCommitted` callback is required when `durableTurnExecutor` is provided",
      );
    }
   ...
   *
   * detach 内部先 cancelAll → pending 的 resolved 事件送达
   * Renderer/Bridge → 清索引。
   */
  ...
  /** 释放所有运行时资源（宿主关闭时调用）。async —— 透传各会话末窗 onWindowClose。 */
