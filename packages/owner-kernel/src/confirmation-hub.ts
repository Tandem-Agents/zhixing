/**
 * ConfirmationHub —— 聚合 per-runtime broker 的 owner 级 facade
 *
 * `AgentRuntime` 每次创建都 `new ConfirmationBroker()`——进程中存在 N+1 个
 ...
 *
 * 不变量：
 *   - 同一 conversationId 至多 attach 一个 broker（`conversationIndex` 唯一约束）
 *   - brokerId 全局唯一（`attach` 重复抛错）
 *   - detach 顺序"cancelAll → 取消订阅 → 清索引"——保证 pending 的
...
export class ConfirmationHub {
   ...
   * 注册一个 broker。
   *
   * @throws {Error} 当 brokerId 已存在或 conversationId 已被其它 broker 占用。
   *                 这两个都是**运行时编程错误**——生产里不应触发。
   */
  attach(
    brokerId: BrokerId,
   ...
   * 依赖 conversationIndex 唯一约束保证每个 conversationId 至多一个 broker。
   */
