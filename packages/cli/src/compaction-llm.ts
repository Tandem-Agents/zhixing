 ...
 *
 * 把 secondary 角色的"消费流式响应、拼接 text_delta"模式抽成独立可测单元——
 * 让 ContextEngine 的 callLLM 注入点和角色路由解耦：
 *
 *   ContextEngine ←─ callLLM (CompactLLMFn) ─── createCompactionFlush(roles)
 *                                                      │
 *                                                      └─→ roles.secondary.chat({...})
 *
 * 路由决策（走 secondary 而非 main）是 secondary-llm-capability.md §二.2 的核心
 * spec 承诺；抽出此 helper 让该承诺在单测中可以反向 assert（roles.main.chat 不
 * 应被调用），防止未来 refactor 把 callsite 错绑到 main 而无人发现。
 *
 ...
 *
 * **关键契约**：实现固定走 `roles.secondary.chat`。secondary 角色的核心价值是
 ...
 * 不是核心价值。
 *
 * secondary 兜底机制（resolveSecondaryRole）：用户没显式配 llm.secondary 时，
 * secondary 自动用 main 实例 + main.model。隔离价值仍保留——createCompactionFlush
