 ...
 * 轻且稳。Windows 经 shell(cmd.exe)拉起(`.cmd` / 无扩展名脚本不是可直接 spawn 的 PE 映像),
 * 无闪靠 `windowsHide` + detached;**spawn 失败必监听 `'error'` 事件兜底,绝不让逃生口
 * 的拉起失败崩掉主进程**(详见 `defaultSpawn`)。
 */
 ...
 * `onError` 接收 spawn 失败(ENOENT / 权限等)的 error:既兜底防止 unhandled error 崩进程,
 * 又把失败回传给 caller,让编辑屏据此提示"没打开、在这儿改"而非假装已打开。
 */
...
/**
 * 构造跨平台 spawn 调用(纯逻辑,可测)。
 *
 * Windows:编辑器常是 `.cmd` / `.bat` 包装或无扩展名脚本(VS Code 的 `code` 是 bash 脚本),
 * 都不是可直接 spawn 的 PE 映像 —— 必须经 shell(cmd.exe),由它按 PATHEXT 补全 `.cmd` 并执行。
 * shell 模式下 Node 不自动加引号,故对含空格的程序路径(`...\Microsoft VS Code\bin\code`)与
 * 文件路径手动加引号,避免被空格切断成多个 token。非 Windows 原样直 spawn、不需 shell。
 */
...
const defaultSpawn: EditorSpawn = (command, args, onError) => {
  const inv = buildSpawnInvocation(command, args, process.platform);
  const child = spawn(inv.command, inv.args, {
    detached: true,
    stdio: "ignore",
    // 无窗口启动:.cmd / .bat / shell 拉起的控制台都隐藏,等价 .NET CreateNoWindow。
    windowsHide: true,
    shell: inv.shell,
  });
  // 关键:监听 'error' —— spawn 失败(编辑器不存在 / 路径错 / 权限)会 emit 'error' 事件,
  // **不监听则成为 unhandled error event、直接崩掉整个 zhixing 进程**。回传给 caller →
  // 既兜底防崩,又让编辑屏据此把"自动打开"降级为"给路径请用户手动打开",而非假装已打开。
  child.on("error", onError);
  // 不阻塞、不等关闭 —— 回屏靠 mtime 比对重读,而非等子进程退出。
  child.unref();
};

/** 用解析到的编辑器打开文件(拉起即返回);spawn 失败经 `onError` 回传。 */
export function openInEditor(
  ...
  spawnFn(resolved.command, [...resolved.baseArgs, file], onError);
}
