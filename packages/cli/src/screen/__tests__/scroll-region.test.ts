...
describe("ScrollRegion · 视觉行级 tail state（段间空行幂等保证基础）", () => {
  ...
  it("suspend / resume：tail state 保留（alt-screen 协议下 main buffer 由终端原子保管）", () => {
    // 新契约：suspend / resume 仅切 flag，不动内部状态。alt-screen 切换由
    // ScreenController 层 emit `\x1b[?1049h`，main buffer 内容（含 cursor 位置）
    // 由终端原子保存；resume `\x1b[?1049l` 后终端恢复——内部 tail state 字段
    // 与现实匹配（不需要 reset）。
    //
    // 旧协议（destructive DECSTBM clear）会 reset tail state 配合可视区被擦；
    // 新协议保留 region 可视区内容，tail state 也对应保留。
    const { region } = makeRegion();
    region.attachInput(0, "");
    region.appendInline("mid-line content"); // currentRowHasVisible=true
    // appendInline 写入纯可视字符（无 \n）→ currentRowHasVisible=true,
    // trailingBlankRows 维持 attachInput 初始值 1（无 \n 触发递增）
    region.suspend();
    expect(region.state.currentRowHasVisible).toBe(true); // 保留
    expect(region.state.trailingBlankRows).toBe(1); // 保留 attachInput 初值
    region.resume();
    expect(region.state.currentRowHasVisible).toBe(true); // 仍保留
    expect(region.state.trailingBlankRows).toBe(1);
  });
...
describe("ScrollRegion · suspend / resume", () => {
  // 新协议（alt-screen 路径）：scroll-region.suspend()/resume() 仅切内部 flag,
  // **不**做任何 destructive emit。main buffer 内容由 ScreenController 层 emit
  // `\x1b[?1049h` 后由终端原子保管；resume 时 ScreenController emit `\x1b[?1049l`
  // 由终端原子恢复。scroll-region.resume 仅 emit 防御性 DECSTBM re-set（兜底
  // 终端不保存 DECSTBM 跨 buffer 切换的边界 case）。

  it("suspend 不做任何 emit——仅切 flag（main buffer 内容由 ScreenController 层 alt-screen 协议保管）", () => {
    const { region, buffer } = makeRegion({ rows: 24 });
    region.attachInput(3, "<chrome>");
    buffer.value = "";
    region.suspend();
    // 关键：旧协议的 destructive region clear + cursor 跳转**全部消除**
    expect(buffer.value).toBe("");
    expect(region.state.suspended).toBe(true);
    expect(region.state.attached).toBe(true);
  });

  it("suspend 保留内部状态——segment 字段 / handle / tail state 不重置", () => {
    // 新协议下 suspend 不做状态 reset：main buffer 由终端原子保管，内容、cursor、
    // segment 边界全部由 alt-screen 切换保护。caller 契约保证 suspend 期间不持有
    // 活跃 segment（LLM 流式与 modal 不并发），无需 scroll-region 防御性清理。
    const { region } = makeRegion({ rows: 24 });
    region.attachInput(0, "");
    const seg = region.beginReplaceableSegment();
    seg.replace("a\nb");
    const beforeSegTop = region.state.segmentTopRow;
    region.suspend();
    const s = region.state;
    expect(s.segmentTopRow).toBe(beforeSegTop); // 保留
    expect(s.segmentBottomRow).not.toBeNull();
    expect(s.segmentRemainingRows).not.toBeNull();
  });
  ...
  it("suspend chromeHeight=0 同样不 emit——纯 flag 切换", () => {
    const { region, buffer } = makeRegion({ rows: 10 });
    region.attachInput(0, "");
    buffer.value = "";
    region.suspend();
  ...
  it("resume 仅 emit 防御性 DECSTBM——不做 destructive clear、不动内部状态", () => {
    const { region, buffer } = makeRegion({ rows: 24 });
    region.attachInput(3, "");
    region.writeScrollLine("a"); // 写入让 regionTailRow / filledRows 推进
    const tailBeforeSuspend = region.state.regionTailRow;
    const filledBeforeSuspend = region.state.regionFilledRows;
    region.suspend();
    buffer.value = "";
    region.resume();
    // chrome=3 → scrollBottom=21，resume emit `\x1b[1;21r` 防御性 DECSTBM
    expect(buffer.value).toBe("\x1b[1;21r");
    const s = region.state;
    expect(s.suspended).toBe(false);
    // 内部状态保留——alt-screen 由终端恢复 main buffer，内部跟踪与现实一致
    expect(s.regionTailRow).toBe(tailBeforeSuspend);
    expect(s.regionFilledRows).toBe(filledBeforeSuspend);
    expect(s.scrollBottom).toBe(21); // chromeHeight 不变
  });
  ...
  it("resume 后可正常写入", () => {
    ...
    // writeScrollLine 写入后 regionTailRow=2（在 row 1 写完 \n 后 cursor 到 row 2）
    expect(region.state.regionTailRow).toBe(2);
  });

  it("未 suspended 时 resume 抛错", () => {
    const { region } = makeRegion({ rows: 5 });
    region.attachInput(2, "");
    expect(() => region.resume()).toThrow(/not suspended/);
  });
});

describe("ScrollRegion · rebuild（运行时整屏重建）", () => {
  it("未 attached 调 rebuild 抛错（前提：运行中重建，非首次）", () => {
    const { region } = makeRegion({ rows: 24 });
    expect(() => region.rebuild(3, "<chrome>")).toThrow(/not attached/);
  });

  it("chromeHeight 非法抛错（复用 establishLayout 校验）", () => {
    const { region } = makeRegion({ rows: 24 });
    region.attachInput(3, "");
    expect(() => region.rebuild(-1, "")).toThrow(/must be ≥ 0/);
    expect(() => region.rebuild(30, "")).toThrow(/no room/);
  });

  it("清屏后重建：与 attachInput 同构 emit（DECSTBM + cursor(1,1) + chrome + cursor(1,1)）+ region 视空回顶", () => {
    const { region, buffer } = makeRegion({ rows: 24, cols: 80 });
    region.attachInput(3, "");
    const seg = region.beginReplaceableSegment();
    seg.replace("a\nb\nc");
    ...
    buffer.value = "";
    region.rebuild(3, "<chrome>"); // rows 24, chromeHeight 3 → scrollBottom 21
    // caller 已清屏（ScreenController 前置 ANSI_FIRSTATTACH_SEQUENCE），故
    // resetRegionState 回顶 (1,1) 与现实一致。emit 与 attachInput 同构。
    expect(buffer.value).toBe(
      "\x1b[1;21r" + "\x1b[1;1H" + "<chrome>" + "\x1b[1;1H",
    );
    const s = region.state;
    expect(s.scrollBottom).toBe(21);
    // resetRegionState：region 视空、cursor 回顶、segment 全清
    expect(s.regionTailRow).toBe(1);
    expect(s.regionFilledRows).toBe(0);
    expect(s.segmentTopRow).toBeNull();
    expect(s.segmentBottomRow).toBeNull();
    expect(s.segmentRemainingRows).toBeNull();
    expect(s.committedLogicalRows).toBe(0);
    // 运行时重建：生命周期标志不变（仍 attached）
    expect(s.attached).toBe(true);
  });

  it("rebuild 后从 region 顶写入（resetRegionState 回顶语义，对齐 firstAttach）", () => {
    const { region, buffer } = makeRegion({ rows: 24, cols: 80 });
    region.attachInput(3, "");
    region.writeScrollLine("旧内容");
    region.rebuild(3, "<chrome>");
    buffer.value = "";
    region.appendInline("新内容");
    // 清屏重建后 region 视空，写入从 region 顶 (1,1) 起
    expect(buffer.value).toBe("\x1b[1;1H新内容");
  });

  it("chromeHeight=0 时不 emit chrome 字节段", () => {
    const { region, buffer } = makeRegion({ rows: 24 });
    region.attachInput(0, "");
    buffer.value = "";
    region.rebuild(0, "");
    // 无 chrome：DECSTBM 1..24 + cursor(1,1)，无 chromeBytes 段
    expect(buffer.value).toBe("\x1b[1;24r" + "\x1b[1;1H");
  });

  it("attachInput / rebuild emit 序列同构（establishLayout 单一来源，杜绝漂移）", () => {
    const a = makeRegion({ rows: 24, cols: 80 });
    a.region.attachInput(3, "<chrome>");
    const attachEmit = a.buffer.value;

    const b = makeRegion({ rows: 24, cols: 80 });
    b.region.attachInput(3, ""); // 先 attach（rebuild 前提）
    b.buffer.value = "";
    b.region.rebuild(3, "<chrome>");
    // attachInput 首次（含 attached 前的几何/emit）与 rebuild 的 emit 完全一致
    expect(b.buffer.value).toBe(attachEmit);
  });
});
