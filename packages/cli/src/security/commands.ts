...
function printTrustHelp(): void {
  console.log();
  console.log(chalk.bold("  /trust — 权限规则管理"));
  console.log();
  console.log("  /trust list [scope]      列出规则（scope: session/workspace/global）");
  console.log("  /trust revoke <id>       按 id 前缀撤销规则");
  console.log("  /trust reset             清除当前工作区的所有规则");
  console.log("  /trust reset all         清除所有规则（含 global，需二次确认）");
  console.log();
}
...
function listRules(args: string[], opts: TrustOptions): void {
  ...
  if (workspacePath) {
    console.log(chalk.dim(`  当前工作区: ${workspacePath}`));
  }
  console.log(
...
async function resetRules(args: string[], opts: TrustOptions): Promise<void> {
  ...
  if (isAll) {
    ...
    store.resetAll();
    console.log(chalk.green("  ✓ 所有规则已清除\n"));
    return;
  }
  ...
  console.log(chalk.green(`  ✓ 已清除 ${wsRules.length} 条规则\n`));
}
...
export function handleSecurityCommand(
  ...
  if (subcommand === "rules") {
    return showPolicyRules(pipeline);
  }
  if (subcommand === "help") {
    return printSecurityHelp();
  }
  return showSecurityOverview(pipeline);
}

function printSecurityHelp(): void {
  console.log();
  console.log(chalk.bold("  /security — 安全系统状态"));
  console.log();
  console.log("  /security             状态概览（默认）");
  console.log("  /security rules       列出当前生效的策略规则");
  console.log();
}

function showSecurityOverview(pipeline: SecurityPipeline): void {
  const store = pipeline.getPermissionStore();
  const workspaceId = pipeline.getWorkspaceId();
  const workspacePath = pipeline.getWorkspace();
  ...
  console.log(chalk.bold("│"));
  console.log(`${chalk.bold("│")} ${chalk.bold("── 确认追踪 ──")}`);
  if (trackerSnapshot.length === 0) {
    console.log(`${chalk.bold("│")} ${chalk.dim("(无累计)")}`);
  } else {
...
function showPolicyRules(pipeline: SecurityPipeline): void {
  const rules: SecurityRule[] = pipeline.getPolicyEngine().getActiveRules();
  console.log();
  console.log(chalk.bold(`  策略规则 (${rules.length} 条)`));
  console.log(chalk.dim("  ─────────────────────────────────────────────────────"));
  ...
  console.log(chalk.dim("  ─────────────────────────────────────────────────────"));
  console.log(chalk.dim("  [!] = bypassImmune（任何配置都无法覆盖）"));
  console.log();
}
