...
describe("tool-render-strategy · 单一事实源契约", () => {
  ...
  it("未注册工具(read/write/bash/grep/glob 等)兜底 = 'default'", () => {
    for (const name of ["read", "write", "bash", "grep", "glob", "edit", "memory", "web_fetch"]) {
  ...
  it("Object.freeze 运行期不变量:isFrozen=true + 任意 mutate 在 strict 模式抛 TypeError", () => {
    ...
    // 渲染"的全局事实源,任何运行期 mutate 都会破坏 renderer.handleEvent 与
    // setupSubAgentStatus 的双侧渲染契约(回归 P0 双重渲染问题)
