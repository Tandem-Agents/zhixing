 ...
 *
 * 废弃中间 CompactInfo 类型，直接产出 CompactMarker（core 的权威类型）。
 ...
 *
 * 比"仅返回 getter"的老签名多一个 `dispose` 成员，调用方可显式取消 EventBus 订阅。
 * 当前所有使用点都是"每 run 新 bus + run 结束 bus 自然 GC"模式，dispose 非必须调用；
 * 但当未来 eventBus 被跨 run 共享时（如集中式 EventBus 架构），不 dispose 会造成
 * listener 泄漏。API 形态提前到位，杜绝这类隐蔽 bug。
 */
