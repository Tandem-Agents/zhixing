...
import {
  ...
  type BackupTargetConfiguration,
  type BackupTargetConfigurationRepository,
} from "./backup-target-config.js";

const BACKUP_TARGET_CONFIGURATION_SEGMENTS = Object.freeze([
  "distributed-runtime",
  "recovery-backup-targets.json",
] as const);

interface StoredBackupTargetConfiguration {
  readonly v: 1;
  readonly currentTargetId: string;
  readonly bindings: readonly BackupTargetBinding[];
}

class FileBackupTargetConfiguration implements BackupTargetConfigurationRepository {
  readonly #file: string;

  constructor(zhixingHome: string) {
    this.#file = path.join(path.resolve(zhixingHome), ...BACKUP_TARGET_CONFIGURATION_SEGMENTS);
  }
  ...
  async load(): Promise<BackupTargetConfiguration | undefined> {
    ...
    if (
      ...
      !isRecord(value) ||
      value.v !== 1 ||
      typeof value.currentTargetId !== "string" ||
    ...
    return { currentTargetId: value.currentTargetId, bindings };
  }

  async select(binding: BackupTargetBinding): Promise<void> {
    const current = await this.load();
    const bindings = new Map(
      (current?.bindings ?? []).map((candidate) => [candidate.targetId, candidate]),
    );
    ...
    await this.#write({
      ...
      bindings: [...bindings.values()].sort((left, right) =>
        left.targetId.localeCompare(right.targetId, "en-US")),
    });
  }

  async #write(value: StoredBackupTargetConfiguration): Promise<void> {
...
/** The sole physical composition of the P11 backup-target configuration repository. */
export function createBackupTargetConfigurationInfrastructure(
  zhixingHome: string,
): BackupTargetConfigurationRepository {
  return projectBackupTargetConfigurationRepository(
    new FileBackupTargetConfiguration(zhixingHome),
  );
}
