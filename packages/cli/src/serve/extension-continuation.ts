...
export function extensionContinuationText(operation: ExtensionOperation, deviceId: string): string {
  ...
  if (operation.phase === "preparing") return `扩展接入已耐久接纳：${fact}\n原请求：${operation.source.request}\n加载“外部能力接入”技能，读取 extension guide 返回的安装包资料，查证来源后准备独立候选，再用 extension_connect 提交。不要再创建接入请求。`;
  ...
  return `扩展接入结果：${fact}\n原请求：${operation.source.request}\n请如实告知结果。ready 表示本人、双向收发和文本确认已验证；其他状态不能宣告可用。`;
}
...
export function createExtensionContinuation(input: { manager: ConversationManager; communication: ConversationCommunicationTransport; deviceId: string }) {
  return { notify: async (operation: ExtensionOperation): Promise<unknown> => {
    ...
    if (isLocalConversationId(operation.source.conversationId)) {
      const receipt = await input.communication.invoke(operation.source.conversationId, { action: "send", conversationId: operation.source.conversationId, operationId: turnId, input: text });
      return { kind: "message", receipt };
    }
    const origin = operation.source.returnAddress as TurnOrigin | undefined;
    const principal = `conversation:${operation.source.conversationId}`;
    const accepted = await input.manager.admitDurableTurn({ conversationId: operation.source.conversationId, input: text,
      invocation: { kind: "agent", source: "interactive" }, surfacePrincipal: principal,
      options: { source: "interactive", surfacePrincipal: principal, turnContext: { turnId,
        ...(origin ? { turnOrigin: origin, ...(origin.target ? { emissionTarget: origin.target } : {}) } : {}) } } });
    ...
    const continuation = operation.continuation as { kind: string; turnId?: string; receipt?: { messageId: string } } | undefined;
    if (continuation?.kind === "message" && continuation.receipt?.messageId) {
      const status = await input.communication.invoke(operation.source.conversationId, { action: "observe", conversationId: operation.source.conversationId,
    ...
    const run = await input.manager.findDurableRunByIngress(operation.source.conversationId, continuation.turnId, "interactive");
