...
export function createMcpManagementTools(
  ...
  const discover: ToolDefinition = {
    ...
    async call(input, ctx) {
      if (input.action === "presets")
        return { content: JSON.stringify(MCP_PRESETS) };
      if (input.action === "status")
        return {
          content: JSON.stringify(
            (await discovery.snapshot()).map(
  ...
  const connect: ToolDefinition = {
    ...
    async call(input, ctx) {
      ...
      if (
        ...
        return {
          content:
            "接入需在支持耐久交接的主对话或工作场景中确认；当前运行可继续使用已有能力。",
          isError: true,
        };
  ...
  const delegate: ToolDefinition = {
    name: "mcp_delegate",
    description:
      "将需新增能力的原任务与公开接入方案交给主设备的独立对话承接，结果自动返回。只交接必要材料，不含凭据或无关历史；不会在本机安装，也不代表接入已获准。调用后结束本轮。",
    inputSchema: connect.inputSchema,
    isReadOnly: false,
    isParallelSafe: false,
    boundaries: [
    ...
    async call(input, ctx) {
      ...
      if (
        ...
        return {
          content: "能力承接需在耐久主对话或工作场景中发起。",
          isError: true,
        };
      validateMcpCandidate(input.candidate);
      validateWorksceneTaskHandoff(input.handoff);
      if (!input.handoff.remaining.length)
        return { content: "没有需要续接的任务，无需交接", isError: true };
      ctx.abortSignal?.throwIfAborted();
      emitPostTurnControlIntent({
        kind: "delegate_mcp",
        candidate: structuredClone(input.candidate),
      ...
      return {
        content:
          "已记录能力承接请求，本轮成功提交后交接；尚未安装或生效。请结束本轮。",
      };
    },
  };
