...
// ─── EventBus 事件 payload 契约 ───
//
// caller(cli REPL render / serve render / 测试) 依赖 `interrupt:warn` payload
// 字段做诊断展示 (elapsedMs / timeoutMs / chunksReceived)。锚定字段避免无意
// 改动破坏诊断链路。core 不直接 console——所有终端可见性由 caller 渲染层负责。

describe("wrapStreamWithWatchdog: interrupt:warn payload 契约", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  afterEach(() => {
  ...
  it("warn 触发 → payload 携带 kind / elapsedMs / timeoutMs / chunksReceived", async () => {
    const c = createInterruptController();
    const bus = createEventBus<AgentEventMap>();
    const warns: AgentEventMap["interrupt:warn"][] = [];
    bus.on("interrupt:warn", (e) => warns.push(e));
  ...
  it("abort 触发 → AbortReason 携带 idle-timeout / chunksReceived (不在 warn 事件)", async () => {
    const c = createInterruptController();
    const wrapped = wrapStreamWithWatchdog(neverEndingStream(), c, {
      idleTimeoutMs: 60_000,
      warnThresholdRatio: 0.5,
    ...
    // abort 后续可见性由 emitRunEnd → interrupt:fired 单点收敛，
    // watchdog 自身只调 abortWithReason 写 reason 到 controller signal
    expect(c.signal.aborted).toBe(true);
    const reason = c.signal.reason as { kind: string; chunksReceived: number };
    expect(reason.kind).toBe("idle-timeout");
    expect(reason.chunksReceived).toBe(0);
  });

  it("payload chunksReceived 反映已收到的 chunk 数 (不是 0)", async () => {
    const c = createInterruptController();
    const bus = createEventBus<AgentEventMap>();
    const warns: AgentEventMap["interrupt:warn"][] = [];
    bus.on("interrupt:warn", (e) => warns.push(e));
    ...
    // 第二个 chunk 后再走 30s → warn 触发, payload chunksReceived=2
    await vi.advanceTimersByTimeAsync(30_000);
    expect(warns).toHaveLength(1);
    expect(warns[0]?.chunksReceived).toBe(2);
