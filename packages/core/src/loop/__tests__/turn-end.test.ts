 ...
 *   - 副作用顺序（段切换 → tokens 快照 → 换代触发）
 *   - messages 链式传递
 *   - terminal 短路语义
 *   - 可选依赖缺省退化
 *
 * 不重复测 SegmentManager 自身行为（自有独立测试），
...
describe("runTurnEnd · 副作用串行编排", () => {
  ...
  it("段评估收到 caller 原始 messages", async () => {
...
describe("runTurnEnd · 可选依赖缺省", () => {
  it("messages 直传给 segmentManager", async () => {
    const original = [msg("original")];
    const seg = makeSeg({
      decision: { kind: "pass", reason: "below-optimal" },
      modified: false,
  ...
  it("无 segmentManager → 段切换步骤 no-op，原样返回 messages", async () => {
    const outcome = await runTurnEnd(baseParams({}));

    expect(outcome.kind).toBe("ok");
    if (outcome.kind === "ok") {
      expect(outcome.messages).toEqual([msg("hi")]);
    }
  });
