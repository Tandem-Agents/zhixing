...
export function selectExecutorForEnvironment(
  ...
  const ordered = [...candidates].sort((left, right) => {
    const leftPreferred = left.executorId === preferredExecutorId ? 0 : 1;
    const rightPreferred = right.executorId === preferredExecutorId ? 0 : 1;
    return (
  ...
  if (requirement.workspace) {
    for (const candidate of capable) {
      const workspace = candidate.descriptor.workspaces.find(
