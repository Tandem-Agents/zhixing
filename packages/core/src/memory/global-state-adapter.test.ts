...
describe("AnchorMemoryGlobalStateAdapter", () => {
  ...
  it("condenses journal sources atomically and rebuilds from the authority log", async () => {
    const fixture = await createFixture();
    await fixture.adapter.initializeStagedPublishing();
    ...
    const sources = await readEntries(fixture.adapter, { kind: "personal" }, "journal");
    await fixture.adapter.mutate(
      {
    ...
    await expect(
      readEntries(reopened, { kind: "personal" }, "journal"),
    ).resolves.toMatchObject([{ id: "2026-01", content: "monthly summary" }]);
    const records = (await fixture.log.readAll()).flatMap((commit) => commit.entries);
    expect(records.filter((record) => record.body.t === "memory-journal-condensed"))
      .toHaveLength(1);
    expect(JSON.stringify(records)).not.toContain("memory-materialized");
  });
