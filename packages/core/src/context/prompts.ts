...
/** Auto-compact 后注入的续写消息 */
export function buildContinuationMessage(summary: string): string {
  return `[对话已压缩]
以下是之前对话的摘要，涵盖了核心目标、技术上下文、文件变更、问题追踪、待办事项、当前进度和关键标识符。
...
/** 手动 /compact 后注入的续写消息 */
export function buildManualCompactMessage(summary: string): string {
  return `[对话已压缩]
以下是之前对话的摘要：
