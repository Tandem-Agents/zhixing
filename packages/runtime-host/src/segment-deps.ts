...
export interface RuntimeHostSegmentDepsInput {
...
/**
 * 瞬态装配变体 —— 不把 segmentMeta persistence 接到 conversation
 * repo，故注 no-op（SegmentPersistence 的失败语义本就允许 segmentMeta 缺写：
 * 它是独立观测元数据流，缺失不影响段切换语义完成度）。taskListReader 照常
 * 复用 TaskListService——会话的 in-progress 守卫与宿主执行面同源。
 */
export function createTransientSegmentDeps(input: {
  readonly taskListService: TaskListService;
}): RuntimeHostSegmentDeps {
  return {
    taskListReader: createTaskListReaderFromService(input.taskListService),
