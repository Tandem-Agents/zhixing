 ...
 *     推给当前 observer 连接。隐私安全：多客户端共享宿主时，B 的观察不会
 *     看到 A 的 confirmation 请求细节。
 *   - 无 conversationId（scheduler ephemeral 兜底）：广播给当前宿主的全部
 *     authenticated 连接。
 *
 * 推送 schema：
...
export function createConfirmationBridge(
  ...
  const notifyTargets = (
    targets: Iterable<RpcNotificationConnection>,
    method: string,
    params: unknown,
  ): void => {
  ...
  const resolveTargets = (conversationId?: string): RpcNotificationConnection[] => {
    if (conversationId) {
    ...
    // 无 conversationId 的宿主任务没有会话 observer，通知当前宿主的已认证连接。
...
function canReceiveFullRequest(
  entry: HubEntry,
  conn: RpcNotificationConnection,
): boolean {
