 ...
 *
 * 不引入 LLM 调用判定意图(spec §0.4):控制意图必须**确定性**,不能 model drift /
 * 无意触发。关键词覆盖 95% 主动取消场景已足够;复杂自然语言意图保留给后续 P2。
 *
 * 匹配语义:
...
/** 末尾标点/空白 trim;与 confirmation/match.ts TRAILING_PUNCT_RE 同源,IM 用户习惯 */
...
export interface DefaultIntentClassifierOptions {
  ...
  /**
   * 与本 classifier 共存的 confirmation 词集,用于启动期静态互斥校验(INV-R2)。
 ...
 *
 * 词集互斥是 INV-R2 不变量,不能运行时容忍 —— 冲突会让同一文本在 pending
 * confirmation 与 in-flight turn 场景下产生不同语义,UX 歧义。fail-fast 暴露
 * 配置错误优于让用户在生产遇到不一致行为。
 */
