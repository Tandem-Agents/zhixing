...
import { buildLogMethods } from "./logs.js";
...
import {
  ...
  buildSessionUsageMethod,
  buildSessionSecurityMethod,
 ...
 *
 * 注意：session.* 方法需要 ctx.server.conversation 被注入；scheduler.* 同理。
...
export const DEVICE_LOCAL_RPC_METHODS = Object.freeze([
  "logs.search", "logs.read", "logs.status", "logs.apply-policy",
  "auth",
  "health",
  "mcp.pending",
  "extensions.list",
