 ...
 *
 * 职责边界(只做 3 件事):
 *   1. wallClockTimeoutMs 通过外置 setTimeout → AbortController.signal,
 *      与 parentSignal 一起作为 loop 的两路独立中断源(loop 内部分别走
 *      external / parent-abort kind,诊断时可区分)
 ...
 *     parent abort / wallClock 走 reason="aborted" + 对应 abortReason.kind
 *   - 仅 setTimeout / pipeline 装配等基础设施异常会泡出去,由 runChildAgent
 *     的 try/catch 兜底转 status="failed"
 *
 * 资源契约:
 *   - finally 块 clearTimeout 是硬约束 —— 否则 setTimeout 句柄阻止进程退出,
 *     且单次 dispatch 完成后泄漏的句柄会跨 dispatch 累积
 */
...
export interface SubAgentLoopResult {
  ...
  usage: TokenUsage;
  /** budget exceeded —— max_turns 触发为 true,其他终止为 false / undefined */
...
export async function runSubAgentLoop(
  ...
  // wall-clock 通过外置 controller 注入 —— loop 视其为 abortSignal(external kind),
  // 与 parentSignal(parent-abort kind)是两路独立中断源。
  ...
  try {
    ...
    const { yields, result } = await drainAgentLoop({
      ...
      abortSignal: wallClockController.signal,
    ...
    return {
      ...
      budgetExceeded: result.reason === "max_turns",
      abortReason:
