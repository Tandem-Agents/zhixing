 ...
 *
 * 安全模型四件套:
 *   - **裁判独立**:领域应用经独立 LLM 端口审查(单发调用、不带对话上下文)——
 ...
 *
 * 两阶段状态机、安全裁决、token/TTL、artifact binding、staging 清理与 durable
 * admit 全部由 Skill Catalog application 拥有；本工具只做输入 binding 与产品呈现。
 */
    ...
    async call(input, context): Promise<ToolResult> {
      ...
      const mode: SkillMode =
        input.mode === "work" || input.mode === "main"
          ...
          : defaultMode;
      try {
        const outcome = await application.admit({
          ...(srcPath ? { source: { kind: "local-path", path: srcPath } } : {}),
          ...(token ? { admissionToken: token } : {}),
          mode,
          operationId: context?.toolCallId,
        });
        return renderAdmissionOutcome(outcome);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        return {
          content: token
            ? `接入失败:${message}(已丢弃,需重新审查)`
            : `接入失败:${message}`,
          isError: true,
        };
      }
...
function renderAdmissionOutcome(outcome: SkillCatalogAdmissionOutcome): ToolResult {
  switch (outcome.kind) {
    case "missing-input":
      return {
        content:
    ...
    case "needs-confirm":
      return {
        content:
          ...
          `裁判理由:${outcome.reason}\n${renderThreats(outcome.threats)}\n` +
          "用户明确同意接入后,带 admissionToken 重调本工具完成接入(有时效):" +
          outcome.admissionToken,
        isError: false,
      };
    ...
    case "admitted":
      return {
        content: `已记录接入技能「${outcome.name}」(id: ${outcome.id})；本轮成功完成后入库。`,
        isError: false,
      };
  }
}
...
function renderThreats(threats: readonly ContentThreat[]): string {
  ...
  return (
    "静态扫描信号:\n" +
    threats.map((threat) =>
      `- ${threat.category}/${threat.rule}: ${threat.excerpt}`
    ).join("\n")
  );
}
