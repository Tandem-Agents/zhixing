...
## 六、Compaction 摘要 Prompt 模板

### 6.1 必需章节（5 段）

OpenClaw Safeguard 模式要求摘要包含 5 个章节：
...
加上固定指令："Produce a compact, factual summary with these exact section headings"、"Do not omit unresolved asks from the user."
...
- 用对话的主要语言写摘要正文
- 聚焦事实：讨论了什么、做了什么决策、当前状态
- 保持必需的章节结构不变
- 不翻译/修改代码、文件路径、标识符、错误信息
```

### 6.3 分块与合并

多段摘要的合并指令（`MERGE_SUMMARIES_INSTRUCTIONS`）：
...
`auditSummaryQuality` 检查三项：
1. **必需章节是否齐全**：5 个 `##` 标题逐一验证
2. **标识符是否保留**（strict 模式）：检查 UUID/hash/API key 等是否出现在摘要中
3. **最近用户请求是否反映**：token 重叠度检查

不通过时追加 "Fix all issues..." 指令重试。

### 6.5 Compaction 后续写

压缩完成后不是直接续对话，而是注入 `[Post-compaction context refresh]` 系统事件：

```
Session was just compacted. The conversation summary above is a hint,
NOT a substitute for your startup sequence.
Run your Session Startup sequence — read the required files before
responding to the user.
```

附加 AGENTS.md 的关键片段（如 `Session Startup`、`Red Lines` 部分）。
...
每行 `JSON.parse` 后：
- **普通消息**：`{ message: AgentMessage, id?: string }` — `message` 字段是实际消息内容
- **Compaction 边界**：`{ type: "compaction", timestamp: string, id?: string }` — 合成为 `role: "system"` 的分割标记

### 7.2 存储路径

```
<stateDir>/agents/<agentId>/sessions/<sessionId>.jsonl
<stateDir>/agents/<agentId>/sessions/sessions.json    # 索引
```

带 Topic 时文件名变为 `<sessionId>-topic-<topicId>.jsonl`。

`sessionId` 规则：`^[a-z0-9][a-z0-9._-]{0,127}$`

### 7.3 sessions.json 索引

顶层为 `Record<sessionKey, SessionEntry>`，`SessionEntry` 包含 `sessionId`、`updatedAt`、`sessionFile`、模型覆盖等。索引与 JSONL 分离维护。

### 7.4 API

通过网关 RPC 暴露 `sessions.list`、`sessions.patch`、`sessions.reset` 等方法。
