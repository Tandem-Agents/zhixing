...
**意图**：简化配置管理，一个活动路径 = 一个配置上下文。

**实现**：
- `resolveConfigPath()` 始终返回一个文件路径
- 环境变量 `OPENCLAW_CONFIG_PATH` 最高优先
- `$include` 用于模块化拆分，但逻辑入口仍只有一个
- 缺失文件返回空对象 `{}`，不自动创建

**评价**：对单机单用途场景简洁有效，但**缺乏项目级配置发现**。用户在不同项目间切换时需手动更换环境变量或修改配置，体验差。

**对知行的启示**：不采用纯单路径模型。需要全局 + 项目级自动发现的多层配置。但借鉴其「环境变量覆盖路径」的灵活性。

### 7. JSON5 配置格式

**意图**：允许配置文件中写注释，提升可读性。

**实现**：`json5` 库解析，用户可在 `openclaw.json` 中写 `// 注释` 和尾逗号。

**评价**：对配置文件来说比纯 JSON 好很多。注释对用户理解各字段含义至关重要。

**对知行的启示**：值得借鉴。JSON5 或 JSONC 都是好选择。
