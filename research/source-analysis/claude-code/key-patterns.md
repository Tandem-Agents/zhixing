...
**意图**：支持企业→项目→个人的配置继承和覆盖。

**实现**：5 层级联（Managed → CLI → Local → Project → User），字段级合并，数组类配置拼接去重。

**评价**：层级设计是 Claude Code 配置系统最优秀的部分。`settings.local.json` 自动 gitignore 的细节很贴心。但 5 层对个人产品偏多，Managed 层是企业噪音。

**对知行的启示**：借鉴项目共享 + 个人覆盖的分离，但精简为 3 层（全局→项目→本地）。丢弃企业 Managed 和 CLI 参数层。

### 9. 配置与记忆的分离

**意图**：Settings（行为与护栏）和 CLAUDE.md（知识与上下文）是两个不同的关注点。

**实现**：
- `settings.json`：控制权限、模型、hooks、环境变量等**行为**
- `CLAUDE.md`：包含项目约定、架构说明、代码风格等**知识**

**评价**：分离概念清晰。但文件类型不同（JSON vs Markdown）、位置不同（`.claude/` vs 项目根）、加载机制不同，增加了用户心智负担。

**对知行的启示**：概念值得借鉴——但可以考虑将记忆统一在配置文件中用 `systemPrompt` 字段承载（MVP 阶段），后续再分离为独立的记忆系统。

### 10. `/config` + `/status` 配置可发现性

**意图**：让用户知道当前生效的配置是什么、来自哪里。

**实现**：
- `/config`：交互式 UI 查看/修改配置
- `/status`：显示每个字段的来源文件

**评价**：这是配置 UX 的最佳实践。特别是 `/status` 的来源追溯——当配置出问题时，用户能立即定位是哪一层覆盖了值。

**对知行的启示**：必须实现类似的配置来源追溯能力。可以作为 `zhixing config show` 命令。
