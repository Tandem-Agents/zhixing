...
> **所属系统**: Claude Code | **分析状态**: ✅ 核心架构 + 配置系统已分析（2026-04-07 更新）
...
Claude Code 拥有业界最精细的多层配置系统。

#### 配置文件层级（优先级从高到低）

| 层级 | 位置 | 用途 | 可提交 |
|------|------|------|--------|
| 企业托管 | `managed-settings.json` / MDM / 注册表 | 企业合规策略 | N/A |
| CLI 参数 | 命令行 | 单次运行覆盖 | — |
| 项目本地 | `<project>/.claude/settings.local.json` | 个人覆盖 | ✗ (自动 gitignore) |
| 项目共享 | `<project>/.claude/settings.json` | 团队默认 | ✓ |
| 用户全局 | `~/.claude/settings.json` | 跨项目默认 | — |

#### 独立的运行时状态文件

`~/.claude.json` 是**运行时状态**（OAuth、编辑器模式、通知偏好等），**不是**用户应编辑的设置文件。这个命名容易与 `settings.json` 混淆。

#### 记忆系统（与配置分离）

`CLAUDE.md` 系列属于「知识与上下文」，与 `settings.json` 的「行为与护栏」分离：

| 文件 | 位置 | 用途 |
|------|------|------|
| `CLAUDE.md` | 项目根或 `~/.claude/` | 项目约定、架构说明、风格 |
| `CLAUDE.local.md` | 项目根 | 个人本地指令 |
| `.claude/rules/` | 项目内 | 模块化规则 |
...
// 主要配置项
{
  ...
  "availableModels": [...],
  "env": { ... },
  "apiKeyHelper": "op read op://vault/claude/key",
  "hooks": { ... },
  "sandbox": { ... },
}
```
...
- **`/config`**：交互式 Settings UI，分栏查看和修改
- **`/status`**：显示当前生效配置的来源（哪个文件/环境变量覆盖了哪个字段）
- **手动编辑**：直接编辑 JSON 文件
- **JSON Schema**：`schemastore.org` 提供自动补全和校验

#### 首次运行

- 不自动创建 `settings.json`
- 通过 Authentication 流程完成登录
- `/config` 作为统一配置入口
- 自动备份配置文件（保留最近 5 份带时间戳的备份）

#### 设计评价

| 维度 | 评价 |
|------|------|
| **层级设计** | ✅ 优秀。项目共享 + 个人覆盖的分离非常实用 |
| **Local vs Shared** | ✅ `settings.local.json` 自动 gitignore 是贴心细节 |
| **交互体验** | ✅ `/config` + `/status` 提供了良好的可发现性 |
| **文件散落** | ⚠️ `settings.json`、`.claude.json`、`CLAUDE.md`、`.mcp.json` 分布太散 |
| **企业复杂度** | ⚠️ Managed settings 对个人用户是噪音 |
| **命名** | ⚠️ `~/.claude.json`（状态）vs `~/.claude/settings.json`（设置）易混淆 |
| **格式** | ❌ 纯 JSON 不支持注释，对配置文件不友好 |
