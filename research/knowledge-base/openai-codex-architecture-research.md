...
> **范围**：独立分析当前开源 Codex 的产品定位、技术栈、核心运行时、线程协议、工具执行、安全、会话、子 Agent、扩展、客户端边界与架构代价；不讨论其他项目如何借鉴或迁移，也不把未开源的 IDE 与 Codex cloud 实现推断为仓库事实。
...
- **用户要把事情做好，而不是使用一个 Agent**：业务应用保留专业界面、上下文、规则、数据、工具与授权体验，Harness 退到背后成为可嵌入、可控制的智能执行底座，而不是迫使所有工作进入通用聊天框。
...
官方默认的 `workspace-write` 关闭网络并限制工作区外写入，整棵命令进程树继承同一边界。macOS 由 Seatbelt 强制，Linux/WSL2 由 bubblewrap（不可用时启用基于 user namespace 的内置 helper）强制；Windows 优先使用低权限专用账户、文件权限、防火墙和私有桌面，无法完成提权安装时降级为 restricted token + ACL 并明确降低保证。这个结构优于把“请谨慎”写进 Prompt：模型即使判断错误，未获权限的效果仍由系统边界拦截。
...
Codex 的产品愿景不是用统一聊天框取代专业软件，而是让 Agent 在用户已经熟悉的产品和工作流中完成任务：应用从当前业务对象直接提供上下文与工具，用户在有实际后果的行动前做决定。

| 层面 | 责任 | 架构支撑 |
|---|---|---|
| 业务应用 | 界面、业务上下文、规则、系统记录、MCP 工具与审批体验 | 用户操作任务与业务对象，不必从零编写 Prompt |
| App Server | 连接产品与运行时，传递会话、事件、中断和审批请求/响应 | 双向协议把产品体验与 Rust 内部实现解耦 |
| Core | Agent Loop、上下文、工具执行、持久化、沙箱与授权策略执行 | 所有产品面共享同一执行事实和安全不变量 |

这里必须区分：业务应用拥有审批的产品表达与用户决定，App Server 负责传递请求和响应，Core 才负责暂停执行并落实策略。正是这条责任链，使 Harness 可以隐藏在专业产品背后而不丢失控制权。
...
- **任务优先、Harness 后置**：用户在熟悉的专业界面中完成工作；产品保留语义、记录与控制，Codex 提供连续、可观察、受约束的执行能力。
...
- [官方产品定位：Codex as a platform](https://developers.openai.com/blog/codex-as-a-platform)
