...
Codex 的核心架构是以 Rust `codex-core` 将每个 Thread 建模为提交/事件双队列驱动的状态化 Agent Runtime，统一拥有模型—工具循环、沙箱审批、耐久历史与子 Agent 图，再由 TUI/Exec 直接调用，或经 App Server 与 CLI JSONL 以 Thread/Turn/Item 语义投影给 IDE、SDK 和其他客户端。业务产品由此可以保留自己的界面、上下文、规则、记录、工具与授权体验，让 Harness 退到背后成为可嵌入、可控制的智能执行底座。

## NVIDIA AVO

AVO 的核心架构是让通用编码 Agent 以历史成果谱系、领域知识、客观评价函数和执行工具为输入，自主循环观察、假设、修改、验证与纠错，只把正确且不退步的结果提交为新基线，并由持久记忆和停滞监督器维持长时程进展。
...
- **综合最强：OpenAI Codex**
  Thread Runtime、双队列协议、工具主链、沙箱审批、耐久历史、多 Agent 和多客户端投影形成完整闭环；App Server 又把这套执行能力交给专业产品而不夺走其业务语义与用户界面。架构最成熟，但也最重。

- **长时程可验证自主优化最有突破性：NVIDIA AVO**
  评价器定义事实，成果谱系保存进步，持久记忆积累经验，监督器打破停滞，形成了现有对象中最清楚的持续改进闭环；但它仍是未完整公开的研究架构，不能按产品成熟度与其他对象并列排名。
...
因此，如果问题是“做一个成熟、可靠、面向用户的顶级 Coding Agent，哪套纯架构最好”，目前答案是 **Codex**；如果问题是“用最少概念构建可塑造的 Agent Harness”，答案是 **Pi**；如果问题是“构建一切皆插件的 Agent 平台”，答案是 **DeepSeek Harness**；如果问题是“围绕客观目标持续实验并积累可验证进步”，答案是 **AVO**。

乔布斯式判断下，真正的最优不是最灵活，而是**让用户直接把事情做好，同时把 Agent 与内部复杂度完全藏住**。按这个标准，Codex 当前综合领先；Pi 最有架构美感；DeepSeek Harness 的插件思想和 AVO 的长时程改进闭环分别代表两种架构突破。现有一句话架构对比可以用于定位，但还不能单独承担“裁定最优”的功能。
