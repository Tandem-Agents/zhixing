...
#### PR-A3：砍 `ContextCompiler` 框架 + `ToolResultAnchorStage`
...
#### PR-A4：砍 `tier-compressor` + `manageWindow` + `TierThresholds` + `onTurnComplete` 主路径改造
...
#### PR-B1：`ModelCapability` 接口 + 常量 + `estimateTools` API
...
#### PR-B2：`AgentRoleProfile.enabledTools` + tools[] 装配重构
...
#### PR-B3：`CompactMarker` + `Conversation` type 字段扩展 + `/clear` 扩展
...
#### PR-B4：`TurnContextInjector` 段切换分支
