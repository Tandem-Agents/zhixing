...
```
                ...
                │   ──────────────────────────────     │
                │   • 创建/运行/级联中断/资源回收      │
                │   • 工具子集装配(profile.enabledTools)│
                │   • AgentRoleProfile prompt 渲染     │
                │   • hierarchical EventBus + lineage  │
                │   • 子用量回写(tool_result usage)   │
                │   • 子执行 = 父 turn 一次 Task 调用  │
...
```
runChildAgent({
  // AgentRoleProfile 由 subAgentProfile(task) 派生(决策 #8)
  task,           // 任务文本进 system prompt "Your Role" 段
  budget,         // resolveSubAgentBudget(maxTurns / maxTokens / timeout)
  parentBus,      // hierarchical EventBus,自动注入 lineage(决策 #6)
  parentSignal,   // parentSignal 透传,loop 内派生 child controller 注入 parent-abort kind(决策 #10)
  parentBroker,   // 子 ConfirmationBroker 引父 broker id 作审计血缘(决策 #5)
})
→ 返回结构化 ChildAgentResult 三态(completed / failed / aborted)
```
...
| 限制 | 含义 | v1 值 | 来源 |
|---|---|---|---|
| maxConcurrent | 同 turn 内并发 sub-agent 数 | **3** | 由 sub-agent budget 解析,具体配置形态待 spec |
| maxTokensPerSub | 单个 sub-agent token 预算 | 待对标三家 | `resolveSubAgentBudget`(`budget.ts`) |
| maxDepth | 递归深度 | **1** | sub profile.enabledTools 不含 Task,天然封顶 |
...
| 项 | 决策 |
|---|---|
| 子 agent 底座 | `runChildAgent` 一次性派发 + `subAgentProfile` 装配 + `parentSignal` 透传(子 abort 不反向)+ loop 内 `createInterruptController({ parent })` 父 abort 级联 |
| 唯一 surface | Task 工具(同步并行委托) |
| 同 turn 并发上限 | **3**;dispatcher 已落地并发:全 `isParallelSafe===true` 且 N≥2 → `runParallelBatch`(`Promise.allSettled`),否则串行回退 |
| 递归深度 | **1**(sub profile.enabledTools = read/glob/grep,不含 Task,天然无法再派子) |
...
| token / cost 归属 | 子用量经 `tool_result` 末尾 `<usage>` trailer 回写;CLI `/usage` 由 `parse-task-usage` best-effort 解析为拆分视图 |
| abort 父→子级联 | `AbortReason.parent-abort` typed kind + loop 内 `createInterruptController({ parent })` 自动级联,Esc 杀整 turn |
| transcript | 子 agent 不写独立 Turn、不持久化;整个子执行 = 父 turn 里的一次 Task 工具调用 |
| confirmation | 子独立 `ConfirmationBroker`(`{ parentBrokerId, sourceAgentId }` 作审计血缘)+ server `ConfirmationHub` 聚合;non-interactive resolver 由 `budget.confirmationPolicy` 决定(默认 fail-to-deny) |
| channel UX | hierarchical EventBus + lineage 自动注入 → channel adapter 各自渲染。v1 只做 CLI 状态条 + RPC 事件透传 |
| 模块归属 | `@zhixing/orchestrator` 包:createAgentRuntime / 子 agent lifecycle / Task 工具执行委托 |
...
- 实现交给决策 #10 的工具子集机制:sub profile.enabledTools 不含 Task,子 agent 拿不到 Task 工具,天然无法再派子
- v1 默认 1,放宽与否待 spec(不绑定具体配置 key)

### B. state / 持久化 / 中断

#### 决策 4:持久化 — 子不写独立 Turn,子执行收敛进父 turn

- 子 agent 不写独立 Turn、不持久化:整个子执行 = 父 turn 里的一次 Task 工具调用,子结果作为 `tool_result` 进父 turn 的消息流,随父 turn 一并 `commitTurn`
- 不学 Claude Code sidechain 文件:多文件读写 + resume 协议要二次开发,daemon 重启恢复复杂
- 不学 OpenClaw 独立 sessionFile / 物理 fork:跨 session 检索关联子结果工程量大
- 不引入 Turn schema 父子链字段:子既不独立落盘,transcript 不需要 `agentRole / subAgentId / parentTurnId`;子执行的可观测性靠 EventBus lineage(决策 #6),用量靠 `<usage>` trailer(决策 #7)
- daemon 重启零额外恢复协议:子不持久化,无未完成子 turn 需恢复
...
- 我们 [packages/server/src/confirmation/hub.ts](packages/server/src/confirmation/hub.ts) `ConfirmationHub` 已有多 broker 聚合机制 —— 子用独立 `ConfirmationBroker`,审计 / RPC 路由自动复用
- 子 broker 选项扩展:`{ parentBrokerId, sourceAgentId }` —— 仅作审计血缘元信息(emit / snapshot 时透传),子 broker 不读父 broker 状态;父已 `alwaysAllow` 的规则走共享 SecurityPipeline 自动命中,根本不进 broker
- 子 broker 的 non-interactive resolver 由 `budget.confirmationPolicy` 决定(`resolveSubAgentResolver`):默认走 fail-to-deny 安全姿态,避免子 agent 在无人值守下偷调危险工具
- 具体策略枚举与配置形态待 spec(不绑定具体配置 key)
...
#### 决策 7:Token 归属 — 子用量经 tool_result `<usage>` trailer 回写

- 子用量不进父 Turn.usage canonical 三字段:子执行收敛进一次 Task 调用(决策 #4),用量随 `tool_result` 回传
- Task 工具在 `tool_result` 末尾输出 `<usage>tokens:.., tool_uses:.., duration_ms:.., sub_id:..</usage>` trailer —— 既是 LLM 可读诊断,也是 CLI 解析协议
- CLI `/usage` 由 [packages/cli/src/parse-task-usage.ts](packages/cli/src/parse-task-usage.ts) best-effort 解析该 trailer 为子用量拆分视图;格式不匹配 graceful 降级为空 entries,不影响主用量呈现
- trailer 格式由 orchestrator `task.ts` 的 `formatUsageTag()` 单一真相源 + 单测守护;cli 侧是只读协议输出,非反向调内部 API

#### 决策 8:Identity — `AgentRoleProfile` 数据结构

- 不再硬编码 `buildIdentity("Zhixing")`,抽象为数据 `AgentRoleProfile`
- 实际字段:`name / role / instructions / constraints / tone? / enabledTools / capabilities?`
  - `enabledTools`:**工具装配的唯一权威源** —— session 按此从 builtin 实例化,子 agent 按此过滤 parent tools
  - `capabilities`(`canSpawnSubAgents / userFacing`):forward-looking 预留元数据,**当前未被任何代码消费**(不参与 prompt 渲染 / 调度路由),供未来 RoleTask 按能力分类注册角色
- system-prompt 拼装由 profile 渲染:主 agent 用 `mainProfile()`,子 agent 用 `subAgentProfile({ subAgentId, task })`,未来 RoleTask 各角色各一个 profile
- 子 agent profile 注入约束(`constraints` + `instructions` 的 "Your Role" 段):任务全文进 instructions;constraints 含"输出只给主 agent 看 → self-contained""尽量少用工具调用,够用即收尾""拿不到 Task 工具不能再派子""专注任务,不主动发起用户对话"
- 同时也是 v2+ 角色化(researcher / critic / planner / writer / executor)的底座

### C. 执行 / 工具

#### 决策 9:Tool dispatch — `isParallelSafe` 并发已落地

- v1 承诺"3 并发"。Dispatch 串行 = 产品撒谎
- 已落地:[packages/core/src/loop/tool-executor.ts](packages/core/src/loop/tool-executor.ts) 按 toolCalls 分组 —— N≥2 且全部 `isParallelSafe===true` 且均已注册 → `runParallelBatch`(`Promise.allSettled` 真并发);否则 `runSerialBatch` 串行回退
- Task 工具 `isParallelSafe: true` —— "同 turn 派多个 Task 并发跑"已在 dispatch 层兑现
- fail-closed:任一工具 `isParallelSafe!==true` 或未注册,整批回退串行;N=1 并发原语无收益,直接串行

#### 决策 10:工具子集 — `profile.enabledTools` 白名单

- 黑名单方式(三家)随工具数增长 drift,新工具加进来必须记得维护黑名单
- 改为:由 `AgentRoleProfile.enabledTools` 白名单声明 —— profile 是工具装配的唯一权威源,不会 drift
- v1 配置:
  - 主 profile.enabledTools = read / write / edit / glob / grep / bash / memory / web_fetch / Task
  - 子 profile.enabledTools = read / glob / grep —— 不含 Task(天然封死决策 #3 递归),不含 memory(子 agent 不污染主记忆),只读探索类
- 子 agent 装配:`parentTools.filter(t => new Set(profile.enabledTools).has(t.name))`(`factory.ts`)—— 与主 agent 装配同机制

#### 决策 11:初始 user message — 极短,task 文本只在 system prompt

- task 文本只放 system prompt(`profile.instructions`)
- 子 agent 初始 user message 固定模板,极短:`Begin. Your task is in the system prompt under "Your Role".`
- OpenClaw 已论证(issue #72019):重复 task 让 input token 翻倍 + prompt cache miss
- 子 agent 看到 system prompt 已有 task 全文,不需要再读一遍

### D. 模块边界

#### 决策 12:`@zhixing/orchestrator` 包

- core 是基础设施(transcript / event / confirmation / interrupt / agent-loop / context-engine)—— **稳定层**
- orchestrator 是基础设施的**组合应用层**,已落地:
  - `createAgentRuntime`(`orchestrator/src/runtime/create-agent-runtime.ts`)
  - 子 agent lifecycle:`runChildAgent` spawn / run / cleanup / abort cascade(`orchestrator/src/subagent/factory.ts`)
  - Task 工具的执行委托(`orchestrator/src/tools/task.ts` 调 `runChildAgent`)
  - 未来:BackgroundAgent / RoleTask / WorkflowTask / BatchTask 都归此层
- 拒绝把 orchestrator 塞进 core(职责正交不该混)
- 拒绝留在 cli(server 曾通过 cli 工厂跨包反向依赖,已顺势归位本包)
...
### 5. token 透明到分桶
一个 turn 用了 60K token,明确拆出"主 LLM 5K + Task#1 35K + Task#2 12K + Task#3 8K"。**用量透明、可分桶到每个子 agent 是一等公民诉求**。实现:子用量经 Task 的 `tool_result` `<usage>` trailer 回写,CLI `/usage` 由 `parse-task-usage` 解析为拆分视图;trailer 格式由 orchestrator 单测守护,cli 侧只读协议输出。后续 spec 阶段会论证是否把子用量进一步收敛进父 Turn metadata 形成 canonical 单一来源。
...
不硬编码 `buildIdentity()`,改为 `AgentRoleProfile` 数据结构 + 模板渲染。子 profile 的 `instructions` "Your Role" 段写入任务全文,`constraints` 注入边界约束(详见决策 #8):输出只给主 agent 看(self-contained)、尽量少用工具调用够用即收尾、拿不到 Task 工具不能再派子、专注任务不主动发起用户对话。

同一抽象 v2+ 给 RoleTask 复用(researcher / critic / writer 各自一个 profile)。智能体行为塑造层面的 prompt 工程精度,业界三家都没做到位。

### 8. 不会无限套娃 — `profile.enabledTools` 白名单
不靠维护黑名单(三家通病:新工具加进来必须记得改黑名单,drift 风险),改为 `AgentRoleProfile.enabledTools` 白名单作工具装配唯一权威源。子 profile.enabledTools 不含 Task —— 子 agent 拿不到 Task 工具,直接堵死递归,等价于深度 1,**单一真相源不会 drift**。v1 默认深度 1,放宽与否待 spec。比 hermes 的 `max_spawn_depth + role` 更干净,比 claudecode 给"内部用户"开后门更纯粹。

### 9. transcript 一份存,统一持久化
不学 claudecode 写独立 sidechain 文件、不学 hermes 搞外键、不学 openclaw 物理 fork。**子 agent 不写独立 Turn、不持久化:整个子执行 = 父 turn 里的一次 Task 工具调用**,子结果作为 `tool_result` 进父 turn 消息流,随父 turn 一并 commit。无需为子引入 transcript 父子链字段,可观测性靠 EventBus lineage、用量靠 `<usage>` trailer。daemon 重启零额外恢复协议(子不持久化,无未完成子 turn 需恢复)。

### 10. abort 行为透明可控 — typed `AbortReason.parent-abort`
不靠 claudecode 的 `Ctrl+X Ctrl+K` 双击魔法、不靠 openclaw 的"主 agent 显式调 cascadeKill 工具"、不靠 hermes 的"父 interrupt 末尾遍历 _active_children"。本仓 [AbortReason](packages/core/src/interrupt/types.ts#L46-L50) 已有 `parent-abort` typed kind;`runChildAgent` 透传 `parentSignal`,子 loop 内 `createInterruptController({ parent })` 派生 child controller 自动注入 parent-abort kind 实现父→子级联,子 abort 不反向影响父;`factory.ts` 顶层 `runChildAgent` 永不抛、返结构化三态(completed / failed / aborted)—— **底座原语已具备,业界全没有**。同步 surface 默认级联,异步 surface(BackgroundAgent)默认解耦。LLM 看见、用户看见、行为可预测。
...
| 项 | 内容 |
|---|---|
| `AgentRoleProfile` 字段表 | `name / role / instructions / constraints / tone? / enabledTools / capabilities?` 各字段类型;主 / 子 profile 默认值与 instructions / constraints 原文;`capabilities` 何时从预留转为被消费 |
| 子执行收敛进父 turn 的边界 | 子结果作 `tool_result` 进父 turn 的精确形态;是否长期保持"子不持久化",还是后续需要子可观测落盘(权衡 resume / 审计 vs 复杂度) |
| `AgentEventMap` 顶层 `lineage` 字段 | payload 中的精确位置 + 渲染端读取约定 + 现有事件订阅者回溯影响盘点 |
| 子 `ConfirmationBroker` 选项 + `ConfirmationHub` 聚合 | `{ parentBrokerId, sourceAgentId }` 字段类型 + Hub 端如何按血缘检索;`resolveSubAgentResolver` 策略枚举与配置形态 |
| `enabledTools` 白名单收口 | 主 / 子 profile.enabledTools 终态列表;外部依赖型工具(extraTools)与 profile 声明的协同装配规则;默认值约定 |
...
| 子 profile.enabledTools 终态 | 当前 read/glob/grep,是否纳入更多只读探索类(逐个评估读/写/执行风险) |

### D. 算法 / 策略

| 项 | 内容 |
|---|---|
| dispatcher 并发错误聚合 | `runParallelBatch` 已用 `Promise.allSettled`;待论证 partial 失败下的结果聚合 / abort 时刻取证策略 |
| 现有工具的 `isParallelSafe` 真伪验证 | 不能仅凭已声明的元信息,需实际盘点 |
| 子用量 trailer 是否收敛进父 metadata | 当前 `<usage>` trailer + best-effort 解析;待论证是否进一步进父 Turn metadata 形成 canonical |
| 子 broker non-interactive resolver 策略 | `resolveSubAgentResolver` 各 policy 的解析规则 + 与共享 SecurityPipeline 的边界 |

### E. 测试拓扑

| 层 | 覆盖 |
|---|---|
| 单元 | `runChildAgent` factory / `parentSignal` 父子隔离 / `runParallelBatch` 多并发 / `profile.enabledTools` 工具过滤 / `AgentRoleProfile` 渲染 |
| 集成 | Task 工具端到端 / abort 父→子级联(typed reason)/ confirmation 父子血缘 / EventBus lineage 冒泡 |
| E2E | CLI 真实派 3 个 Task / 飞书远程 abort 子 / RPC stream-json 透传 lineage |
| 平台 | Windows 路径 / Unix 信号 / daemon 重启(子不持久化,无未完成子 turn 需恢复) |

### F. 模块边界落地

| 项 | 内容 |
|---|---|
| `@zhixing/orchestrator` 包结构 | 已落地;待 spec 收口入口 export 面 + 跨包依赖图(谁依赖 orchestrator)+ 与 core 边界回归 |
| `createAgentRuntime` 归位回归 | 已搬入 orchestrator;待 spec 盘点 CLI / server 接入点回归项与未来 BackgroundAgent 等 surface 的接入契约 |
| Task 工具的 orchestrator 委托接口 | `orchestrator/src/tools/task.ts` 调 `runChildAgent(...)` 已落地;待 spec 锁定 `input_schema` / 三态 `tool_result` 文本协议 |
