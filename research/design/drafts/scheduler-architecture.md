...
2. **missed 是无差别「补一次」（与第一部分需求冲突）**：`start()` 检测 missed + timer-loop `nextRunAt <= now` 无差别补执行。新需求要的是按 internal/external 分流——external 用户任务不补、记录事实（「关闭不管」），internal 维护补跑一次（「最终被执行」）。
...
- **missed 语义** → **重做**（无差别补一次 → 按 internal/external 分流：external 记录不补、internal 补跑一次）。
...
- **接入面 profile**（一等概念）：profile 不"砍核心"，**声明该档的完整行为画像**（启用哪组接入面 + 启动校验模式 + 生命周期，见可执行 B 的 `PROFILES` 描述符）。核心宿主按描述符**数据驱动装配**——每个接入面是带 `setup` 的单元（teardown 因 LIFO 时序约束走 shutdown-chain、不进单元接口，见可执行 B），装配 = 遍历启用集合各自 setup；新增接入面 = 注册一个单元 + 纳入某 profile 集合，**不回改装配主干**（杜绝 if-枚举式装配每加一面就改主干这一"声明面领先生效面"复发）。
  ...
  - **升格（schedule→full）本模块用「停旧起新」，热挂载留给未来**：接入面做成 `setup / teardown` 单元（解耦边界，为 unified-core 的运行时热挂载 / 位置切换留位）；但本模块**不预实现热挂载**——`zz serve` 要 full 时若已有 schedule 宿主在跑，经 RPC `server.shutdown` 令其 drain 退出（在跑任务跑完、释放端口）→ full 宿主接管（`scheduler.json` 持久、任务定义不丢），停机窗口秒级，落在 missed 容差内。运行时热挂载（让已建 scheduler late-bind 新 delivery / channel——需改 `scheduler.ts:62/78` 的构造期 readonly 依赖）属 unified-core 那层、本模块不背，避免为远期需求过度预建。
...
> **前提**：定时任务需要一个**独立于 cli 终端**的后台进程（核心宿主）来跑，而不是塞在 cli 进程里。但「要求 1：要轻」决定了它**不是 7x24 守护、更不开机自启**——核心宿主的生命周期由「接入面在场 + 待办任务」驱动、锚定 RPC 连接：cli 运行期间只要有启用的**用户定时任务**，cli 就维持一条到宿主的连接、用连接在场把宿主**保活**，宿主在场则到点必达；cli 关闭、连接断、无任何接入面在场时宿主 idle 退出。**定时语义 = 运行中必达、关闭不管**：zhixing 运行期间（用户开着 cli）到点任务必达，关闭后错过的不要求触发（用户接受「不开机就不工作」）。这条「接入面在场 → 核心在场」正是 unified-core 核心宿主生命周期模型的当前落地——cli 是第一个接入面，未来接入面与会话信号接入同一机制、不返工（保活机制详见决策 1② / 决策 7 / 可执行 C·G）。系统维护任务（internal）不要求准时、不纳入保活驱动，靠启动检查在机会跑、逾期补做一次（决策 2 / 决策 5）。
...
- **核心新建（现状为零、是本决策的主体工作量）**：
  ...
  - ② 核心宿主**按需拉起 + 运行期保活 + 关闭退出**（已拍板）：没有任何定时任务时零后台进程；一旦有启用的用户定时任务，cli 启动即 ensure 宿主并**在运行期持续维持连接**（断线重连、拉起失败重试），用连接在场保活宿主——保证 zhixing 运行期间到点必达（判据见可执行设计 C）。cli 关闭 → 连接断 → 无接入面在场 → 宿主 idle 退出（idle 判据 + ensure/idle 经端口原子交接 + client 重试，见决策 7），关闭期间错过的不补（决策 2，「关闭不管」）。系统维护逾期 / 首次未 seed 也触发 ensure（防饿死，见决策 5），但维护不要求准时、不纳入保活驱动（跑完即可让宿主 idle 退）。显式 `zz serve` 长驻。ensure 失败 → **ensure 层封装的**友好提示「定时功能当前不可用：<原因>」+ 本次操作失败（**不**复用 spawnDaemon 面向人的原始日志倾倒、**不**静默记意图补建）。
...
- 关键不是删掉到点判断（timer-loop 的 `nextRunAt <= now` 同时承担「准时触发」，删了正常触发也没了），而是**在到点判断里分流**：
  ...
  - **离线错过**（`nextRunAt` 早于「本次上线时刻 − 容差」，即应触发于 zhixing 关闭期间）→ 按 internal/external 分流（决策 6 维度，二者 SLA 不同）：
    - **external（用户任务）**：**不执行**，把这次错过记成一条**事实**（taskId、应触发时刻、检测到时刻）维护进状态，`nextRunAt` advance 到下一个未来时刻（once 错过则标记 terminal-missed）——尊重「关闭不管」：关闭期间错过的提醒不补、更不开机轰炸式补。
    - **internal（系统维护）**：**补跑一次**再 advance——维护幂等、容忍延迟、价值在「最终被执行」；纯聊天用户无 external 任务、不触发运行期保活，维护正是靠这条「启动 ensure（决策 5）→ 逾期补跑」覆盖。天级任务每次运行最多补一次、无风暴。
...
- 副作用收益：external 不补杜绝「开机一次性补一堆用户任务」的触发风暴（呼应「要求 3」杜绝 runaway）；internal 维护虽补跑但天级、数量少、每次最多一次，无风暴。
- **与「运行期保活 + 关闭退出」（决策 1②/可执行 C）同批落地、不可拆**：宿主随 zhixing 关闭而退、随下次运行而起，每次 ensure 后 `start()` 都会看到一批关闭期间 `nextRunAt` 已过期的任务；若 external 仍按旧「无差别补一次」语义，每次重新运行都触发「拉起补一堆提醒」的风暴——容差分流（external 不补、记录）正是「关闭不管」的落地（internal 维护则定向补跑一次，见上）。
...
- 强化：missed 按来源分流（external 不补防风暴、internal 维护补跑一次，见决策 2）；系统任务同样受全套保护；遇不可抗力（LLM / 网络长时间不可用）→ auto-disable + **友好反馈给用户**（要求 2），而非无限重试。
...
- 这些任务天然幂等、容忍延迟——missed 按 internal 分流「补跑一次」（决策 2）：逾期就在下次运行 zhixing 时补做一次，不要求准时、不纳入运行期保活。
- **防饿死（已拍板）**：纯聊天用户从不碰调度 → 宿主从不拉 → 系统维护永不跑（而纯聊天照样产生 transcript / journal、确实需要 gc）。死锁根因是判据方向——旧式「有逾期 system task 行才 ensure」在全新 / 纯聊天用户（json 里还没 seed 过 system task）下永远命中不了、永不拉、永不 seed（要拉才 seed、要 seed 才拉的鸡生蛋）。破法是把判据反过来：cli 启动读 scheduler.json，**「无 system task 行（首次）或有 system task 的 `nextRunAt` 已逾期」即视为该维护、ensure**（无行 = 逾期）；daemon 首次唤起即 seed（`nextRunAt` 落未来、当次不立即跑），其后任一次启动若维护逾期 → ensure → 经决策 2 internal 分流补跑一次。复用单一维护状态（json）、不另起时间戳文件。系统任务容忍延迟，偶尔靠启动唤起即可，无需为它强制常驻。
...
- **missed 语义**：external 关闭期间错过**不补**（尊重「关闭不管」、防开机补一堆）；internal 维护**补跑一次**（价值在「最终被执行」、天级无风暴）——见决策 2。
- **防饿死判据**：可执行设计 C 的 ①有启用的用户任务（运行期保活）/ ②系统维护逾期（补跑一次），正是这条分流。
...
- **ensure 与 idle 退出之间靠端口原子交接 + client 重试，不改 shutdown 契约**：idle 判据 = **无活跃 RPC client（连接）+ 无活跃会话 / 接入面**——纯锚「接入面在场」，不看 scheduler 有无近期 `nextRunAt`。运行期必达由保活坐实（有用户任务 → cli 维持连接 → connections 不为空 → 不退），无需 idle 判据再为近期任务「续命」；反过来 cli 已关闭（连接断、无接入面）时就该退，**不能因任务未来要触发而续命**——那等于「关闭还管」、违背「关闭不管」。故旧设计「scheduler 无近期 nextRunAt 才退」这条续命分支取消（连带去掉「近期窗口 ≥ 守候窗口」的软不变量）。交接方式：宿主进入 drain 即退（**保留现状 `lifecycle.ts:153-163` 的 idempotent-once 不可逆 shutdown——不为本模块重写成可中止状态机**）；drain 期间新调度操作连不上（或连上发现 shutting down）→ client 当「无宿主」**重新 ensure** → 旧的退完释放端口、新的 listen 拉起（OS 端口原子裁决保证两 listen 不并存）。代价是偶发一次「退了又拉」的秒级延迟，但不碰 shutdown 契约。**可中止 drain（drain 中途撤销退出、平滑不断正在进行的会话）是 unified-core 平滑位置切换才需要的，留给那层**——本模块 schedule profile 无会话面，退完重拉无损。
...
- cli 启动检查（`repl.ts` 的 `shouldEnsureOnStartup`）——满足任一才调 `RpcSchedulerFacade.ensureHost()`（失败给一行降级提示不静默），否则无任务时零后台：① **有启用的用户定时任务**：读 `scheduler.json`，存在任一 enabled 的 external task（**不限触发远近**——运行期必达不挑近窗）即 ensure 宿主，并在**运行期持续维持连接保活**（cli 运行期间维持连接收任务事件 + 让 connections 不为空使宿主不 idle 退；运行期内新建任务亦经写操作 ensure 后纳入保活）；② **系统维护逾期**：读 `scheduler.json`——「有 system task 的 `nextRunAt` 已逾期」或「没有 system task 行（全新 / 首次、尚未 seed）」即 ensure（**无行 = 逾期**是破死锁关键，见决策 5），ensure 后逾期维护经决策 2 internal 分流补跑一次、跑完可 idle 退（除非同时有 ① 的用户任务在保活），不纳入运行期保活驱动。两类判据复用单一 `scheduler.json`、不另起时间戳文件。**旧 `NEAR_WINDOW_MS` 近窗启发式取消**——它是「按需起落 + 到点前才拉」的遗留；运行期保活下「有用户任务即全程保活」，无需也不应按窗口挑。
...
- **missed 容差分流 + 来源分流**（`scheduler.ts` + `config.ts` + `types.ts`）：config 加 `graceWindowMs`；scheduler 记本次上线时刻 `onlineSince`（start 设、不持久化、每次拉起重置）；到点判断**以 onlineSince 为锚**分流——`nextRunAt` 早于 `onlineSince - grace`=关闭期间错过，再按 `isInternal` 二次分流（决策 2）：**external** 不执行、记 `state.lastMissed={scheduledFor,detectedAt}`、advance（once → 标 terminal-missed/disable）；**internal** 经 `executeSingleTask` 同一入口（复用同 task 并发守卫）**补跑一次**再 advance。其余=在线到点正常执行（被并发推迟则保持 due、不误判）。删 `start()` 只 log 的 missed 计数。**锚 onlineSince 而非 now**——锚 now 会让在线并发延迟随时间漂移越过容差被误判错过、once 被饿死丢失。
...
- **运行中必达、关闭不管**：只要 zhixing 开着（你在用 cli），到点的定时任务就一定触发——不分近期还是「每天 n 点」，运行期间都准时；你完全关掉 zhixing 期间错过的提醒不会触发（你接受不开机就不工作）、也不会下次开机轰炸式补。要让关掉也触发是另一条独立支线（系统级唤醒），不作默认。
