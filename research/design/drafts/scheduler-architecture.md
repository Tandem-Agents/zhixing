...
- 两实现：`LocalSchedulerFacade`（直调本进程 `Scheduler`，daemon 内 ephemeralRuntime 用）、`RpcSchedulerFacade`（经 RPC client 调 daemon，cli 用，调用前 ensure 宿主）。
...
**C. ensure 机制 + cli RPC 接入（新增 `cli/runtime/ensure-core-host.ts`）**

```
ensureCoreHost(): Promise<SchedulerFacade>
```

- 发现：`discoverServer` 读 PID/port 文件（已读实际端口）+ **就绪探测**（per-capability 显式 ready，不能只看 `/api/status` 的 scheduler 字段是否存在——它只表示「对象被赋值」≠ `start()` 完成，见决策 7）——不只查进程存活，避免连到未就绪宿主。在且就绪 → 连上返回 `RpcSchedulerFacade`；不在 → 复用 `spawnDaemon`（profile 中立，负责 detach / handshake）、由 `buildForwardedArgs` 透传 `--profile=schedule`（端口按 home 区分：传 `--port 0` 或派生）→ 等就绪 → 连上返回。**spawn 失败 / EADDRINUSE 时先重新 discover**——并发抢 ensure 的败者 child 会因 EADDRINUSE 自杀，但赢家宿主已可用，重新发现连上即可；只有重新发现也失败才判 ensure 失败（避免并发下败者 client 把「宿主已可用」误报成「定时功能不可用」）。
- `RpcSchedulerFacade` **可重连 + 重新发现 + 断线重订阅**（决策 7）：宿主 idle 消失后下次调用重新 ensure。
- 失败 → ensure 层抛**封装的友好错误**（不复用 spawnDaemon 面向人的 `printLogTail` 原始日志倾倒），cli 捕获提示 + 操作失败（决策 1②，不静默队列）。
- cli 启动轻检查（`startRepl` / 默认 action）——满足任一才调 `ensureCoreHost`，否则纯聊天零后台：① **近期用户任务**：读 `scheduler.json`，有未来近窗（如 N 分钟）内待触发的 user task；② **系统维护逾期**：仍读 `scheduler.json`——「有 system task 的 `nextRunAt` 已逾期」或「没有 system task 行（全新 / 首次、尚未 seed）」即视为逾期、ensure。**无行 = 逾期**是破死锁关键（非旧式「有逾期行才 ensure」的鸡生蛋，见决策 5）；复用单一维护状态（json）、不另起时间戳文件。
...
- **① schedule 工具**（`tools-builtin/schedule.ts`）：`createScheduleTool` 入参从「Scheduler 实例 / getter」改「SchedulerFacade」；`cli/runtime/builtin-extra-tools.ts` 注入 `RpcSchedulerFacade`（懒 ensure），`command.ts` 的 ephemeralRuntime 注入 `LocalSchedulerFacade`。（`SchedulerFacade.list()` 的 `TaskView` 须覆盖工具渲染所需字段——handleList/formatTaskBrief 用到 id / name / schedule / priority / enabled / state，别裁成读不全。）
