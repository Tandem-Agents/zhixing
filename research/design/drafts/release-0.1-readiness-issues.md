...
> **历史说明：**本文关于原生安装、私有 Node、自有更新、签名与多目标发布的方案已被[软件分发信任与产品边界](../modules/distributed-runtime/software-distribution-trust-and-target-strategy.md)取代，不得作为当前实现或发布依据；其余内容仅保留当时讨论。
...
该问题曾由 Windows 路径断言和多文件摘要断言失败暴露：ripgrep 返回原生 stdout，Node 回退却返回另一种自定义格式。失败不是单纯的测试写窄；仅改斜杠或补摘要会继续遗漏两条路径在正则、编码、行模型、候选和截断上的差异。统一结果合同与格式化器解决共同边界，当前实现仍有未闭合项，见 [grep 搜索架构](../../../docs/modules/tools/grep.md)。发布验证须覆盖真实 ripgrep 分支，只有 Node 回退通过或历史测试绿灯不能证明当前完整契约成立。
