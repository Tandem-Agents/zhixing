...
- **取证恢复代际**：每个被审 run 的 `evidenceGeneration` 在 owner 日志中严格前进，并共同进入 root、child、requestId 与 pending/result/settled。未过期同代只回放原请求；旧代先结清再换代；已经耐久的 terminal bundle 直接验真消费，不为取证重建租约。child 从签发到 dispatch 调用由 coordinator 局部持有，dispatch 接管前异常即时 finish，接管后只由 dispatch finally 终结。
