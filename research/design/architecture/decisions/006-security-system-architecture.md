...
**工作区确定**是配置驱动的，不是硬编码 `process.cwd()`。工作区是用户级偏好（知行是个人助手，workspace 跟着人走不跟着目录走），主要在全局配置 `~/.zhixing/config.json` 中设定。优先级：`CLI --workspace` > 目录级配置 > 全局配置（主路径） > `cwd` 兜底（详见 ADR-003 及 [安全系统设计方案 §3.4](../../specifications/security-system.md)）。配置中设定的工作区不会被运行位置覆盖。工作区修改是受保护操作——始终需要用户确认，因为改变工作区等于改变安全信任边界。智能体知道当前生效的工作区及其来源，修改时操作全局配置。
