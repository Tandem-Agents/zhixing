...
| 阶段 | 范围 | 状态 |
|------|------|------|
| 16.9-P1 | Outbox 基础设施（per-target FIFO、registry、事件）+ DeliveryPipeline drain 目标改为 Outbox + InboundRouter 改为 Outbox。无行为变化，铺管道 | 🔲 待实施 |
| 16.9-P2 | Tool-authored commitment：`ToolExecutionContext.commitToUser` + `ToolResult.committedToUser` + schedule 工具改造 + 系统提示抑制叙述。**修掉常规 inversion** | 🔲 待实施 |
