...
| 免打扰时段 | `heartbeat.activeHours`（仅在心跳级别过滤） | **Scheduler 层单点 Active Hours**（详见 [active-hours-execution.md](./active-hours-execution.md)） |
...
| 结果投递 | ✅ 多通道 + webhook，但耦合在心跳流程中 | **独立的 Delivery Pipeline**，可排队、可重试（忠实送达） |
| Active Hours | ✅ 心跳级别过滤，不够细粒度 | **Scheduler 层单点过滤 + urgent 穿透**（详见 [active-hours-execution.md](./active-hours-execution.md)） |
...
2. **调度层**（Scheduling）：决定何时执行——Scheduler 管理定时任务 + Active Hours 免打扰过滤 + 并发控制（免打扰详见 [active-hours-execution.md](./active-hours-execution.md)）
3. **出站层**（Outbound）：投递执行结果——Delivery Pipeline 负责排队、重试、忠实送达（Faithful Delivery 契约：不主动过滤）
...
**调度层**：
- **Scheduler**：定时任务调度器，带并发控制、优先级和 Active Hours 免打扰（详见 [active-hours-execution.md](./active-hours-execution.md)）

**出站层**：
- **Delivery Pipeline**：结果投递管线，排队 + 重试 + 忠实送达
...
Level 1 完整执行规格（概念、竞品调研、架构决策、里程碑拆解、回滚策略）见：

> **[active-hours-execution.md](./active-hours-execution.md)** —— Active Hours 权威细节规格

本节仅保留顶层语义锚点，任何实现细节以上述文档为准：

- **判定层级**：Scheduler 层单点过滤（M32 Faithful Delivery 契约否决了原设计的 Delivery 层 filter）
- **配置**：全局 `ActiveHoursConfig` { enabled, start, end, timezone, jitterWindowMs }
- **Urgent 穿透**：`priority === "urgent"` 直接执行；AI 创建任务硬编码 `normal` 防滥用
- **Deferred 语义**：非 urgent 任务在免打扰时段推迟到 `activeStart + random(0, jitterMs)`（不丢弃）
- **惊群错峰**：Jitter 默认 5 分钟窗口（借鉴 Claude Code CronJitterConfig）
- **热更新**：RPC `schedule.activeHours.update` + Scheduler rearm
- **可观察**：`serve status` / `schedule list` 显示当前时段 + deferred 标记
...
│  └──────┬───────┘                                            │
│         │                                                     │
│  ┌──────▼───────┐  注：Faithful Delivery 契约下无过滤器链     │
│  │ 投递执行      │  免打扰过滤在 Scheduler 层（见 active-hours │
│  │  ├── channel → Outbox.post() → ChannelAdapter.send()      │  -execution.md）；去重已移除               │
...
#### Known Limitations — 并发边缘（2026-04-21 记录，未修）

M32/M33 架构审查时发现以下 **pre-existing** 并发边缘情况。它们触发条件都较窄，在典型 CLI/Server 顺序调用路径下不会出现；但高并发或异常关停路径下理论可复现。单独跟踪，不阻塞已交付的 Phase 3 工作。

##### KL-1 — 并发 enqueue 时 `DeliveryQueue.save()` 有 rename race

**场景**：两个 `pipeline.enqueue(...)` 几乎同时到达（多会话并发投递）。

**机制**：两个 enqueue 都同步 push 到内存数组，都 `await queue.save()`。`save()` 里：`writeFile(tmp) → rename(tmp → path)`。并发两次：A 写 tmp，B 写 tmp（覆盖 A），A rename（把 B 的内容移到 path），B rename（tmp 已不存在，抛 ENOENT）。

**影响**：第二个 save 的 promise reject → 对应 enqueue 的 promise reject → caller 认为消息入队失败（实际已在内存队列，且内容已落盘，但错误让 caller 认为失败）。极端情况下 caller 误触发 retry 产生重复。

**触发条件**：多会话并发 + 高投递频率。生产环境罕见但理论存在。

**修复方向**：`DeliveryQueue.save()` 内加 singleflight（`activeSave: Promise<void> | null`）或串行 chain，让并发 save 共享一次实际 IO。修改范围局限在 `queue.ts`。

##### KL-2 — `start()` / `stop()` 并发导致 `flushTimer` 泄漏

**场景**：`await pipeline.start()` 正在 `await flush()`（recovery 阶段），另一代码路径此时调 `pipeline.stop()`。

**机制**：
1. start: `state = "running"`、`await this.flush()`（让出事件循环）
2. stop: `state = "stopped"`、`clearInterval(flushTimer)`（此时 timer 还未创建，no-op）、`await activeFlush`、`queue.save()`
3. start 从 flush 返回、执行 `this.flushTimer = setInterval(...)`（state 已是 stopped，但 setInterval 仍跑）
4. timer 被设上，永不被清

**影响**：Timer 不会触发有效 work（R3 的 state check 拦截 callback），但 timer 对象驻留，keep Node 进程存活（setInterval 未 unref）。

**触发条件**：并发调用 start 和 stop。典型 caller（CLI/Server 顺序 init/shutdown）不会触发。

**修复方向**：
- 方案 A：引入中间态 `starting` → `running`；stop 检测到 `starting` 可以拒绝或等待
- 方案 B：`setInterval` 前再 check 一次 `state === "running"`
- 方案 C：setInterval 返回的 timer 加 `.unref()`（兜底，不解决泄漏但阻止 keep-alive）

##### KL-3 — in-flight enqueue 期间的 stop 触发 save race

**场景**：caller A 调 `enqueue(...)` 正 `await queue.save()`；同时 caller B 调 `pipeline.stop()`，stop 里也 `await queue.save()`。

**机制**：和 KL-1 同根——两次并发 `queue.save()`，rename race。

**影响**：同 KL-1，其中一次 save reject。

**触发条件**：优雅关停时正巧有 in-flight enqueue。

**修复方向**：同 KL-1（queue 层 singleflight save）解决后 KL-3 自动消失。

#### 修复优先级

| 债 | 优先级 | 修复成本 |
|---|---|---|
| KL-1 / KL-3（同根） | P2 | queue.ts 加 20 行 + 2 条测试 |
| KL-2 | P3 | 方案 C（加 `.unref()`）最轻，3 行 |

建议作为独立工单 "Queue & Pipeline concurrency hardening" 处理，不与本次 ADR-007 工作耦合。
...
Level 1 的**权威细节规格**（概念、架构决策、PID / 状态文件 schema、里程碑拆解、回滚策略、跨平台处理）见：

> **[daemon-level-1-execution.md](./daemon-level-1-execution.md)**

本节仅保留顶层语义锚点，任何实现细节以上述文档为准：

- `zhixing serve --daemon` → 父进程 `spawn + detached + unref` → 子进程脱离终端常驻
- `zhixing serve stop` → discover PID → 优雅停机（Windows 走 RPC `server.shutdown` 降级 taskkill）
- `zhixing serve status` → 四态（running / running-unhealthy / stopped / stale）
- `zhixing serve logs` → 单文件 append 日志，`--tail` 跨平台轮询跟踪

> 本节早期版本曾给出 fork / PID 文件 / SIGTERM 优雅停机的概念流程图（基于 Unix fork 语义的粗略描述）；实际 Node.js 实现采用 `spawn + detached` 而非 `fork(2)`，完整细节以 daemon-level-1-execution.md 为准。若两处描述冲突，以后者为准。
...
type DeliveryEvents = {
  ...
  // 注：历史曾设计 'delivery:deduplicated' 事件，随 DedupFilter 一并移除（2026-04-21）。
};
```
...
| 免打扰 | 心跳级 | N/A | 无 | **Scheduler 层单点 + urgent 穿透** |
...
- ~~Active Hours 配置~~ → **独立为 Step 18**，详见 [active-hours-execution.md](./active-hours-execution.md)
- ~~去重逻辑~~ → Faithful Delivery 契约确立后移除
...
packages/core/src/delivery/
  ...
  ├── delivery-pipeline.ts   # 忠实送达 → 重试
  ├── webhook.ts             # HTTP POST 投递
  └── (active-hours 独立为 Step 18，不在 delivery 包内)
```

> 注：历史设计含 `dedup.ts`，2026-04-21 随 Faithful Delivery 契约确立而移除。
> 见本文 §4.7 "Faithful Delivery 契约" 小节。
...
// ─── Active Hours ───
// ���整定义见 active-hours-execution.md §3.2
// ActiveHoursConfig { enabled, start, end, timezone?, jitterWindowMs? }
...
interface SchedulerConfig {
  ...
  activeHours?: ActiveHoursConfig; // Step 18 加入，可选
}
...
interface ServerConfig {
  ...
  scheduler: SchedulerConfig;  // activeHours 在 SchedulerConfig 内（Step 18 加入）
...
- 心跳覆盖的真实需求，知行用更直接的方式解决：
  - 免打扰 → Active Hours（Scheduler 层单点过滤 + urgent 穿透，详见 [active-hours-execution.md](./active-hours-execution.md)）
