...
**per-role 而非 per-model**:思考强度跟使用语境走 —— light 做压缩/摘要应可关思考省钱省延迟,power 接重活应可拉满,main 用户自选。与 work-mode 的 primaryRole 正交互补(primaryRole 选角色,本字段定该角色思考深度)。`config.llm.{main,light,power}.thinking` 各自独立。
...
- **统一规则:thinking 跟随该 LLM 调用点实际使用的 role**(不硬编码、不与 work-mode 段切换归正耦合)。装配期(`create-agent-runtime.ts` 内有 config)经各调用点**函数参数**注入对应 role 的 thinking —— 不在 adapter / 无状态函数内读 config。grep 实证 **三条独立 ChatRequest 构造路径**(非两条),须全覆盖:
  ...
  - **② compaction / callText / flush strategies**:统一走 `createCompactionFlush`(L557,被 MemoryFlush / LLMSummarize / callText 共用)—— **签名须扩展为 `createCompactionFlush(roles, lightThinking?: ThinkingConfig)`**(现状只接 roles、无状态、拿不到 config),装配期传 `config.llm.light?.thinking`,内部 `roles.light.chat({...,thinking})` 填。
...
**PR 2 — schema + 传输管道**:`LLMRoleConfig` 加 `thinking?`;`ChatRequest` 加 `thinking?: ThinkingConfig`(`core/types/llm.ts`);**三条** ChatRequest 构造路径各从对应 role 的 `config.llm.<role>.thinking` 装配期注入(bindRole 链天然透传):① runAgentLoop 主对话(增 thinking 入参,active role);② createCompactionFlush —— **签名扩展 `(roles, lightThinking?: ThinkingConfig)`**,L557 传 `config.llm.light?.thinking`;③ segmentStreamFactory 段切换摘要(L751,按其实际 role);config 校验按选中 model 的 `thinkingControl` 校验形态合法性(类型不匹配拦截/warn)。验收:schema + ChatRequest 透传单测;非法组合被拦;未配时 ChatRequest.thinking 为 undefined。
