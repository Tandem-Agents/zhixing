...
- JSON 抽取 / 工具结果摘要 / 网页正文蒸馏这类"输入大、输出小、不需要长链推理"的任务和主对话 task shape 不同
- 用户可显式配 `light` 为更适合此类任务的模型（如响应快的小模型、JSON mode 友好的模型）
- **不在 light 范围内**：主对话压缩（LLMSummarize）—— 摘要质量直接关系到下一轮 LLM 的认知输入，task shape 不属于"输入大输出小"档,走 main(见 §四 Consumer 表 + ADR-SLLM-009)
...
`provider` 引用的是 `credentials.providers.<id>`（凭证唯一入口，与 config 物理隔离）；config 是决策层只记录"用哪个"。物理层不再持有任何 `defaultModel` 自荐字段——"首选哪一对 provider+model"是档位维度的产品决策，单一事实源是 `ROLE_RECOMMENDATIONS`（档位推荐表），与本能力正交。
 ...
 *    未配置默认。
 * 2. roles.main.{provider,model} 反映会话**实际使用的** effective state——
 *    直接来自 config.llm.main。consumer 读到的就是 runtime 实际跑的
 *    provider+model，不存在 ctx.llm.main 与运行时 split brain 的可能。
...
- `cli/config-editor`：`ModelRole = RoleId`；`sections/model.ts` 遍历 `ROLE_SPECS` 生成入口（标签 = `${labelZh}（${parenZh}）`）；`checks/model.ts` **只覆盖 required(main) 的阻断项**——可选角色永不进阻断清单（选填不阻断），其配置完善度由 `sections/model.ts` 就地按"该 provider 是否真有 key"派生**暗色咨询**显示
- JSONC 配置模板：`config-loader.ts` 的 `buildConfigTemplate` 对每个档位从 `ROLE_RECOMMENDATIONS` 单源派生——有推荐的档位（当前 main/light）写成生效条目，无推荐的（当前 power）输出诚实占位 + 兜底说明，模板恒为推荐表投影
...
**唯一来源（必填）**：`config.llm.main` 提供 provider 与 model，没有任何命令行覆盖入口。`config.llm?.main` 缺失 → 抛 `ProviderConfigError`（fatal，session 不应启动）。这是 `resolveLLMRoles` 的单一 fail-fast 边界——把 `ZhixingConfig.llm?` 的 optional 在此一次性 narrow。

**解析**（`resolveMainRole`）：

```
resolved   = resolveProvider(config.llm.main.provider, credentials)
finalModel = config.llm.main.model

→ main = { resolved, model: finalModel }
```

设计要点：provider/model 配置驱动单一事实源——首次推荐由档位推荐表（`ROLE_RECOMMENDATIONS`）写入配置模板，运行时只读 `config.llm.main`，不存在命令行临时覆盖路径，杜绝"命令行值 vs 配置值"二义。

### 2.2 辅助角色（light / power）解析：`resolveAuxRole`

`light` 与 `power` 共用 `resolveAuxRole(explicit, credentials, fallbackRole, roleId)`——注册表 `ROLE_SPECS` 中 `fallbackTo:"main"` 的角色都走它，`fallbackRole` 当前恒为已解析的 `main`。

**核心原则——可选角色【永不阻断流程】（`RoleSpec.required` 是唯一权威）**：`light`/`power` 是选填增强位，main fallback 恒可用，所以它们的任何配置问题都不得 fail-fast、不得卡启动/完成、UI 只暗色咨询。原"显式配错 fail-fast"被本原则取代——改为**回退 main + 结构化 `RoleDegradation` 记录**，由边缘层（`create-agent-runtime`）打印一次**可见的非致命告警**：既不阻断，又不静默掩盖用户期望的多 provider 架构（保留原关切的"可见性"，去掉"致命"）。

```
1. config.llm.<aux> 缺省（用户没显式配）
   → 用 fallbackRole（main）实例 + main.model 兜底
   → 不打印任何提示——合理的未配置默认，不是降级（隔离价值仍保留）

2. config.llm.<aux> 显式设置
   → 同 provider id 短路：aux.provider === main.resolved.id
     → 复用 main 实例，aux.model 仍独立
   → 不同 provider id：resolveProvider(aux.provider, credentials)
       成功 → 使用
       ProviderConfigError（缺凭证/baseUrl/protocol）
              → 回退 main 实例 + main.model
              → degradations += { role, configured:{provider,model}, reason }
              → 不抛、不阻断；边缘层据此可见告警
       非配置类异常 → 原样抛出（真实 bug 不掩盖）
```
...
内部流程：`loadConfig` → `loadCredentials` → `resolveLLMRoles(config, credentials)` → `createFromResolved` 实例化 main、`instanceFor` 按 provider-id 复用装配 light + power → `bindRole` 绑定 model 成 `LLMRole`。

设计要点：`roles.main.{provider,model}` 始终反映 `config.llm.main`——配置驱动单一事实源，任何 ctx.llm.main consumer 读到的就是 runtime 跑的同一对值。`resolvedRoles` 暴露配置层元信息（原本埋在 LLMProvider 实例里不可见），供消费者完成 budget 解析等 protocol-aware 工作。`bindRole` 是 `@internal`，外部 consumer 用 `createProviderRoles` 一站式构造，不应自己 bind（绕过 same-id 复用 / 缺省兜底）。

### 3.2 cli 入口装配（`packages/cli/src/run-agent.ts`）

`createProviderRoles` 从配置文件解析 main/light/power，下游所有站点统一从 `roles.main.*` 读取——provider/model 单一来源是 `config.llm.main`，无本地兜底变量。
...
function callLLMText(role: LLMRole, thinking?: ThinkingConfig): CompactLLMFn {
  return async (messages, opts) => {
    ...
    for await (const event of role.chat({
      messages, tools: [], thinking, abortSignal: opts?.abortSignal,
    })) {
...
| `ROLE_RECOMMENDATIONS` / `RoleRecommendation`（`providers/role-recommendations.ts`） | 档位推荐表（"某档位首选哪一对 provider+model"）；物理层不再有 `defaultModel` 自荐字段 |
...
| `resolveLLMRoles(config, credentials)` | **纯配置层**——实现 §二.1（main 直取配置）+ §二.2（`resolveMainRole` + `resolveAuxRole` × {light, power}），返回 `ResolvedLLMRoles { main; light; power; degradations }`（各角色 `{ resolved, model }`；`degradations` 记可选角色降级）。**不创建 LLMProvider 实例**——保持 `resolve.ts` ↔ 配置层、`create-provider.ts` ↔ 实例层单向依赖 |
| `resolveAuxRole(explicit, credentials, fallbackRole, roleId)` | light/power 共用；返回 `{ role, degradation? }`——可选角色解析失败回退 main 并记 degradation，**不抛**（永不阻断） |
...
- **名字表达"接什么活"，不锁定模型档位**：`light` = 系统侧后台辅助任务槽（I/O 边界净化），`power` = 重活槽（首个落地消费者是 work-mode 工作场景主对话，未来可纳入编程等其它高难任务）。用户即便给 `power` 配一个弱模型也**完全合法**——名字是角色用途约定，不是对模型属性的断言；同理 `light` 配强模型也合法。这与"`secondary` 描述抽象层次但用户读不懂用途"、"`cheap` 把 cost 写死进名字"两个极端都不同——`light/power` 直觉传达用途又不绑死属性。
- **无命名包袱**：知行是 internal-only 项目，无存量已发布用户，不需要为旧命名保留 BC，可一步到位选最终命名。
- **公开度优先**：角色名出现在用户配置文件（`llm.light` / `llm.power`）、config-editor 入口标签里——首次用户一眼就能判断"我要不要给后台辅助任务单独配个便宜模型"。`labelZh`/`parenZh`（如"轻量模型（可选 · 系统侧后台辅助任务）"）由 ROLE_SPECS 统一提供中文说明，进一步消除歧义。
...
- **不静默尝试任何 provider**：`try { resolveProvider("anthropic") } catch { degrade }` 式设计假设了 ANTHROPIC_API_KEY 是合理可探测的环境变量；这对国内用户错误，对所有用户都是隐式 vendor 偏好
  - 作用域澄清：本条禁的是"**未配**辅助角色时静默探测一个**硬编码 vendor**作默认"。与 §二.2 的"**显式**配了某 provider 但凭证缺失 → 回退 **已解析的 main**（非硬编码猜测）+ **可见**非致命告警"是不同决策——后者既非硬编码探测、也非静默，不与本条冲突
...
- main 解析：`roles.main.{provider,model}` 直取 `config.llm.main` ✓；缺 `llm.main` → throw（迁移提示文案）✓
- 辅助角色解析（light / power 各覆盖）：显式 + 异 id 有凭证 → 独立解析 ✓；显式 + 同 id → 复用 main 实例（model 仍独立）✓；缺省 → main 实例 + main.model 兜底 ✓；缺省时对所有 vendor 行为一致（vendor 中立回归保护）✓；**显式 + 异 id 缺凭证 → 回退 main + `degradations` 记录该角色（不 throw、永不阻断）**✓
...
| 项 | 触发条件 |
|---|---------|
| `power` 角色接入更多消费者（如编程等其它重活路由） | work-mode 之外出现新的重活类需求 |
...
| 默认值 | 平台决定（vendor lock-in） | provider 决定 | n/a | **档位推荐（可编辑/可删，删即回落 main，无 vendor lock-in）** |
