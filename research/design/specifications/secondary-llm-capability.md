...
- 辅助角色解析（light / power 各覆盖）：显式 + 异 id → 独立解析 ✓；显式 + 同 id → 复用 main 实例（model 仍独立）✓；缺省 → main 实例 + main.model 兜底 ✓；缺省时对所有 vendor 行为一致（vendor 中立回归保护）✓；显式配错 → throw（透传 resolveProvider 原错）✓
