...
| `cli` | `SkillCommandSource`(唤醒)、`/skills` 技能管理器(alt-screen)、接入交互 | `repl.ts`(`DefaultCommandRegistry`/`registerDynamicSource`)、`DynamicCommandSource`(`core/typeahead/types.ts:345`);管理器复用共享 alt-screen 屏基础 / config-editor 原语(`config-editor/ui/render.ts`、`config-editor/ui/input.ts`、`screen/screen-controller.ts`),浏览 = 面向管理的全集读(含 `disabled` + usage,§二;**非 `listAll`** —— 后者剔 disabled、补全用)、状态操作 = `setState`/`archive`,创建 / 编辑详见 skill-authoring.md |
