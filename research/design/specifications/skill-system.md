...
> ✅ **注意力窗口边界重建已落地**（见 [agent-runtime-lifecycle.md](./agent-runtime-lifecycle.md)）。实际形态比下文「v2 改一行 holder」更完整:双层 holder（实例权威 prompt + per-run 局部 prompt,后者私有以保并发 run 各自窗口内 byte-equal）+ onWindowOpen 公共订阅点;`skill-index` 段由内置订阅者经 `SkillStore.version(mode)` 门控重建（version 未变则零重算、byte-equal 不破 cache）。**装配期不再硬编码注入索引** —— 首窗 onWindowOpen 首次贡献,单一路径。下文 §3.2–§3.3 保留为设计演进记录。
...
> ✅ **已落地**:此「窗口生命周期边界注册式订阅点」即 [agent-runtime-lifecycle.md](./agent-runtime-lifecycle.md) 的 `AgentRuntimeLifecycle.onWindowOpen`,skill 索引为首个订阅者（`makeSkillIndexLifecycle`,经公共 `updateSystemPromptSegment` 贡献段）。触发面不止 compact —— 覆盖段切换 / compact / clear / resume / 首窗 / reload 全部窗口换代;「变了才换、没变 byte-equal」由 runtime 重拼后整串比对兜底。
