...
**两层物理分离**（延续 `config.jsonc` 决策层与 SecretStore 秘密层彼此独立的既有原则）：
...
- `getZhixingHome`(`core/paths.ts:19`)、`parseFrontmatter` / `stringifyFrontmatter`(`core/frontmatter.ts` —— 只支持扁平 key:value + 简单数组、**不支持嵌套对象**;读写 `name` / `description` 等扁平字段够用、`linked` 原样保存不受影响)。
...
放行 → `Store.admit` 把暂存内文件**逐个 `writeAtomic` copy** 到 `linked/<id>/`(**不靠目录 rename** —— Windows 跨卷不原子;全部成功后由 `finally` 调 `discardStaging` 清暂存),即被扫到 = 发现 = 接入来源;`index` 补该 id 状态(`mode` 接入时选定、默认 `main`)。copy 经 Store 库根 realpath 一处收口、暂存内 symlink 即被拒。Store 这步是宿主侧内部写,不经安全管线(`pipeline.evaluate` 只拦 agent loop 的 AI 工具调用),故 `linked/` 虽落在 `.zhixing/` 段内也**不触发**写确认规则 `bi-zhixing-config-write`(只对 AI 工具调用写 `.zhixing/` 生效)。
