...
③ InboundRouter 按词集匹配 → broker.resolve
     · 匹配允许词集 → allow-once
     · 匹配拒绝词集 → deny
     · 其他任意文本 → deny-with-reason（整条消息作为 reason 传给 LLM）
```
...
- 自由文本理由可直接作为 `deny-with-reason.reason` 回流给 LLM——**知行相对竞品的核心差异化价值**（按钮模型传不了自由文本）
...
| LLM 理由回流 | 需额外 UI 表单 | ❌（按钮枚举）| ❌ | ✅ **用户自由文本 → deny-with-reason.reason → LLM 看到调整方案** |
...
InboundRouter.handleMessage 入口（conversations.enqueue 之前）：
  hub.findBrokerByConversation(convId).listPending().length > 0
    ...
    └ 其他任意文本 → broker.resolve(deny-with-reason, reason = 整条消息)
...
```typescript
// packages/core/src/confirmation/types.ts（新增 ~15 行）
...
private async tryHandleAsConfirmationReply(
  ...
  } else if (decision.kind === "deny-with-reason") {
  ...
  await this.replyToInbound(msg, {
    text: formatResolutionReceipt(target.request, decision, ok),
  }).catch((e) => this.logger.error(`confirmation reply failed: ${errMsg(e)}`));
...
匹配前做标准化：`text.trim().toLowerCase()`（中文字符不受 lowercase 影响）。采用**保守完全匹配**——必须完全等于集合中的词才识别为结构化决策；其他任何文本一律作为 `deny-with-reason.reason`。
 ...
 * 不处理会被当成 deny-with-reason 导致批准命中率塌方。只 trim **末尾**，保守避免误伤
 ...
 * - 完全匹配拒绝词集（去末尾标点后）→ deny
 * - 其他任意非空文本 → deny-with-reason，reason = 原文（trim 前后空白，保留内部格式）
...
export function formatResolutionReceipt(
  ...
  switch (decision.kind) {
    ...
    case "deny":
      return `❌ 已拒绝：${request.display.title}`;
...
| `n` / `不` / `算了` / `2` / `不行！` | `deny` | `❌ 已拒绝：<title>` |
| `不要删数据库！那是生产环境` | `deny-with-reason`，reason = 原文 | `❌ 已拒绝：<title>\n理由已转给 AI：不要删数据库！那是生产环境` |
| （>2000 字符的长文本）| `deny-with-reason`，reason 截断至 2000 + `…（理由已截断）` | 同上 |
...
- **LLM 友好**：不在集合的文本一律 `deny-with-reason` 回流，语义由 LLM 解读
...
```typescript
// packages/server/src/rpc/methods/confirmation.ts（新增 ~100 行）

/** 列出当前连接可见的 pending（按 observer 过滤；显式传 conversationId 覆盖）*/
"confirmation.list": (params: { conversationId?: string }, ctx) => {
  ...
  const visible = params.conversationId
    ...
    : all.filter((e) => {
        if (!e.conversationId) return false;  // ephemeral 仅 admin 可见，这里不返回
        const observerIds = ctx.conversations.getObserverConnectionIds(e.conversationId);
        return observerIds.has(ctx.connection.id);
      });
  return { items: visible.map(toListItem) };
};

/** 解决一个 pending（Web UI 按钮点击调用）*/
...
| `confirmation.reply.matched-reason` | InboundRouter 匹配 deny-with-reason（M5 新增）| requestId / channelId / reasonLength | 自由文本比例 / 长度分布（驱动 MAX_REASON_LENGTH 调整） |
...
| A6 | 文本匹配规则 | 允许词集 + 拒绝词集 + 其他 = `deny-with-reason`；**保守完全匹配**防误判 |
...
| `packages/server/src/confirmation/__tests__/match.test.ts` | +80（新增）| APPROVE_SET 全覆盖 / DENY_SET 全覆盖 / 中文词 / 大小写无关 / 前后空白 trim / **末尾标点 trim**（"好。"/"Yes."/"好的！"/"可以～"→ allow-once；"不行！"→ deny）/ 内部标点保留（"不要删！那是生产" → deny-with-reason 且 reason 含 `！`）/ 其他文本 → deny-with-reason 保留原文 / **超长 reason 截断**（>2000 字符 → 截断 + "…（理由已截断）"）|
...
| 风险 | 概率 | 影响 | 缓解 |
|------|------|------|------|
| 用户对 pending-aware 拦截预期错位（想插入聊天反被当成拒绝理由）| 中 | 非预期 deny-with-reason | ① 确认消息文案明确"任意文本 = 拒绝理由"；② LLM 看到理由可再问一次；③ 超时兜底 |
| 词集覆盖不全，用户表达识别不到 | **低** | 本应 allow 的回复被当成 deny-with-reason | ① 末尾标点 trim 已覆盖"好。"/"Yes."/"好的！"等 IM 典型输入（§3.6）；② 词集中英文 + 数字 + 口语 + 情绪共 40+ 条；③ 可观测性事件 `confirmation.reply.matched-reason` 跟踪自由文本比例，高则补集合；④ LLM 仍能从 reason 推断意图 |
