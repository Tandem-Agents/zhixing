...
> 设计日期：2026-04-16 | 最后更新：2026-04-18
> 状态：v1.1 定稿
...
```
  ...
  移动端 ────────┤  ┌──────────────────────────────────────────┐   │
                 │  │  InboundRouter                           │   │
  钉钉 ─────────┤  │    normalize → debounce → conv-bind      │   │
  飞书 ─────────┤  │    → Agent Turn                          │   │
  企微 ─────────┤  └──────────────────────────────────────────┘   │
  Webhook ──────┤                                                  │
  API (/v1/) ───┘                                                  │
...
OpenClaw 有 `lastChannel` 追踪但切换成本高。知行的 Conversation 层已经是通道无关的（见 [conversation-model.md](./conversation-model.md)），只需在入站路由时支持"同一用户的多通道消息归入同一 Conversation"。
...
interface ChannelAdapter {
  ...
  /** 默认归组策略（可选,见 conversation-model.md §3.5） */
  bindingPolicy?: ChannelBindingPolicy;
}

/**
 * Server 注入给适配器的上下文。
 * 基础能力（消息路由、HTTP 注册）+ Connection 管理（见 conversation-model.md §6.2）。
 */
interface ChannelContext {
  ...
  /** 注册客户端连接（驱动 SessionRuntime observer 计数,见 conversation-model.md §6.3） */
  registerConnection(conn: Connection): void;

  /** 注销客户端连接 */
  unregisterConnection(connectionId: string): void;

  /** 按 connection 订阅 server 推送（消息追加、流式 delta 等） */
...
> v2.1 更新：`session.*` → `conversation.*`（术语对齐 [conversation-model.md](./conversation-model.md)）。S2.7 期间保留 `session.*` 别名（打 deprecation warning），S2.8 移除。
...
type RpcMethods = {
  ...
  // ─── 对话（v2.1 从 session.* 更名） ───
  "conversation.send":     (params: { text: string; conversationId?: string }) => { conversationId: string; runId: string; accepted: boolean };
  "conversation.list":     (params?: { includeArchived?: boolean }) => Conversation[];
  "conversation.create":   (params: { name: string; preferredModel?: string }) => Conversation;
  "conversation.history":  (params: { conversationId: string; limit?: number }) => Message[];
  "conversation.abort":    (params: { conversationId: string }) => { aborted: boolean };
  "conversation.rename":   (params: { conversationId: string; name: string }) => Conversation;
  "conversation.archive":  (params: { conversationId: string; archived: boolean }) => Conversation;
  "conversation.delete":   (params: { conversationId: string }) => { deleted: boolean };
  "conversation.subscribe": (params: { conversationId?: string }) => void;

  // ─── 背景 Agent（v2.0 新增）───
  "background.spawn":     (params: { conversationId: string; prompt: string; model?: string; tools?: string[] }) => BackgroundAgent;
  "background.list":      (params: { conversationId?: string }) => BackgroundAgent[];
...
type RpcEvents = {
  // ─── 对话（v2.1 从 session.* 更名） ───
  "conversation.delta":           { conversationId: string; delta: StreamDelta };
  "conversation.complete":        { conversationId: string; result: AgentResult };
  "conversation.message-appended": { conversationId: string; message: Message };
  "conversation.created":         { conversation: Conversation };
  "conversation.renamed":         { conversation: Conversation };
  "conversation.archived":        { conversation: Conversation };
  "conversation.deleted":         { conversationId: string };

  // ─── 背景 Agent（v2.0 新增）───
  "background.spawned":    { agentId: string; parentConversationId: string; prompt: string };
...
③ 命令识别（command detect）
    ...
    │
    ▼
④ 对话归组（conversation bind）
    │ 按 conversation-model §3.5 归组优先级匹配 Conversation
    │ 查找已有 Conversation 或创建新 Conversation
    │ 记录 lastChannel + lastRoute（用于出站路由）
    │
    ▼
⑤ 并发守卫（concurrency guard）
    │ 检查该 Conversation 的 SessionRuntime 是否 busy
    │ ├── busy → 排队（队列深度 ≤ MAX_PENDING=5，见 conversation-model §4.5）
    │ └── 空闲 �� 继续
    │
    ▼
⑥ Agent 执行（agent turn）
...
### 6.2 对话归组策略（ChannelBindingPolicy）

归组策略决定通道收到的消息进入哪个 Conversation。完整优先级规则见 [conversation-model.md §3.5](./conversation-model.md)。每个通道可声明默认策略：

```typescript
interface ChannelBindingPolicy {
  /** DM 消息：一个用户一个 Conversation */
  dm: "per-user";
  /** 群消息：一个群一个 Conversation（所有人共享） */
  group: "per-group" | "per-user-in-group";
  /** 线程消息：一个线程一个 Conversation */
  thread: "per-thread";
}
```

默认策略：DM = per-user, group = per-group, thread = per-thread。可在 config 中按通道覆写。`ChannelBindingPolicy` 作为 `ChannelAdapter.bindingPolicy` 可选属性在 §4.1 定义。

### 6.3 对话漫游（Conversation Roaming）

同一用户在多个通道的 DM 消息归入**同一 Conversation**（通过 userId 匹配）：

```
钉钉 DM (user: sunhj) ──→ Conversation: default（per-user DM 归组）
CLI 对话 (user: sunhj) ──→ Conversation: default（同一个）
...
| | /v1/chat/completions | RPC conversation.send |
|--|---------------------|----------------------|
| 协议 | HTTP REST | WebSocket JSON-RPC |
| 会话 | 无状态（每次独立） | 有状态（Conversation 持续） |
...
- `auth`、`health`、`conversation.send`、`conversation.list`、`server.status` 五个核心方法
- 推送事件：`conversation.delta`、`conversation.complete`
...
- 实现 `InboundRouter`（normalize → debounce → conversation-bind → concurrency guard → agent turn）
...
packages/server/src/
  ...
  └── conversation-roaming.ts # 对话漫游逻辑
```
...
**理由**：
- 回调模式下适配器只管"收到消息"，路由/去抖/对话归组全部由 InboundRouter 统一处理
- 适配器不需要理解 Conversation、SessionRuntime、Agent Turn 等核心概念
...
**决策**：DM 场景下同一用户的多通道消息自动归入同一 Conversation（默认为 `default`）。

**理由**：
- 个人助手的核心体验是"它认识我"——不管我从哪个通道说话，它都记得之前聊过什么
- 实现简单：`ChannelBindingPolicy.dm = "per-user"` 使跨通道 DM 天然归入同一 Conversation
- 群场景不做漫游（不同群是不同上下文）
- 用户可以 `/new <name>` 显式创建新 Conversation
