...
> **相关规格**:[skill-authoring.md](./skill-authoring.md)(**SkillSavePipeline 落盘管线** —— v2 自主产生 / 迭代复用的同一管线)。
...
- **不可动摇的边界**:技能管家**只处理它自己产生的技能**;人手写的、人在对话里让 AI 写的(对话流创建,skill-authoring.md)、外部接入的(对话流接入经 `admit_skill`,父规格 §六),一律算**用户的**、管家不碰。这是"自我进化敢放手"的前提 —— 自动化越权改用户 / 官方技能是不可接受的。

## 一、建在 v1 的哪些插座上

v2 不新造地基,接 v1 留好的插座(父规格 §九 / skill-authoring §四;Store 度量等已落地核实,**save_skill 为创建路径本轮设计、随其落地**):

- **落盘管线 `SkillSavePipeline`**(skill-authoring.md §三)—— v1/v2 共享插座:**管线无触发语义**,v1 经 `save_skill` 工具包装接「对话流触发 + 用户明确接受」,v2 经 `StewardWriter` 接「自主触发 + 来源边界」,**同一管线(脱敏 / 来源 / 索引 / 格式四不变量同一焊接点)、换包装层**。草稿产出无独立引擎:v1 是主模型的对话行为,v2 由管家自身的 LLM 编排承担(绑 `callText("main")`,产草稿后经管线落盘)。
...
**来源标记**:`SkillState` 加一个 `stewardCreated` 布尔(默认 `false`,v1 插座 = per-id 状态可扩展)。管家产生技能时标 `true`;其余一切(对话流创作经 `save_skill` 工具、对话流接入经 `admit_skill`、用户手放目录)都是 `false` = "用户的"。来源边界 = 管家只读写 `stewardCreated === true` 的技能,对 `false` 一律不碰。用户**接管**一个管家技能(在对话里改其内容)应转"用户的"——**归属是触发方语义,在包装层分流、Pipeline 不碰**(管线无触发语义,skill-authoring.md §三;若挂在管线 update 上"一体生效",管家经 StewardWriter 迭代自产技能也会被清标记、改一次就失去所有权,来源边界静默崩塌):v2 加 `stewardCreated` 字段时,**`save_skill` 工具包装**(用户路径)在 update 命中管家技能时清标记 = 用户接管;**`StewardWriter`**(管家路径)创建 / 更新恒写 `stewardCreated: true`。对 v1 纯增量;**在那之前,用户保护某个管家技能靠 `pin`**(管家淘汰跳过 pinned,§四)。注意这与 `linked` 的 fork-on-edit 是两回事 —— 后者是用户改**接入**技能时 copy 到 `own/`,那些本就 `stewardCreated=false`。
...
**产出**:管家 LLM 编排产草稿 —— 新建从复盘上下文蒸馏、迭代读已有技能改写(**该读不计 hit**,见 §五),经 `StewardWriter → SkillSavePipeline` 落盘(脱敏在管线内焊死,**不经 `save_skill` 工具包装、不带其用户确认语义**)、受限写门约束只落 `own/` 并标 `stewardCreated`。`mode` 按**产生时所处模式**自动定(main 模式产生 → `main`、工作场景产生 → `work`),不靠模型判断(父规格 §七)。

**落盘策略 —— 默认自主、产生即告知、用户事后控制**:v2 默认**自主落盘**(标 `stewardCreated`),不每次打断用户确认 —— 否则不叫"无感自动攒"(drafts §2.3)。但**完全无感会让技能凭空出现、用户困惑**,故产生时给一行**轻通知**(告知"已把『X』存成技能,可在 /skills 管理",**不要求确认、不打断**)。用户控制在**另一端**:`/skills` 管理器看到管家产物(来源可视区分)、可删 / pin / 改。skill-authoring §二/§四 的提议边界与用户确认闸门是本条的演化起点——「产生时提议用户确认」作为**可配置的温和模式**预留;默认自主 + 轻通知。这条「自主落盘」的安全底气全在 §二的来源边界 + 写隔离 + §六脱敏。
...
产生 / 迭代的每次产出都过 `scrubSecrets`(系统层,在 SkillSavePipeline 内焊死,skill-authoring §三)—— 对话里冒出的 secret 绝不固化进管家技能(技能反复加载且设计上可分享,危害放大)。导出 / 分享同理复用同一件。
