...
| 命令 | 底层 |
|---|---|
| `/work add <name> [--workdir <path>]` | `registry.add`(workdir 可选) |
| `/work list [--archived]` | `registry.list` |
| `/work remove <id>` | `session.removeWorkScene`(带 active guard;彻底删除系统目录;workdir 不动;软隐藏走 `archive`) |
| `/work rename <id> <name>` | `registry.rename` |
