...
**active 守卫归 session 层**:registry 是低层 CRUD 原语,不该知道运行态——`remove` 直接调 registry 不带任何 guard。"不能删除当前活跃场景"这条业务规则由 `RuntimeSession.removeWorkScene(id)` 实现(`IWorkModeController` 同名方法),CLI `/workscene remove` 与 LLM 工具 `workscene_change_approve action=remove` 都过此 chokepoint,guard 不可绕过(机制与策略分离 + 两入口同源)。规则:active sceneId 命中目标 id → 抛 friendly error 引导先 `/exit`。
...
现有 `createAgent(existingPermissionStore?)` 硬编码 workspace。重构按 spec 装配:
...
**confirm 触发载体是 `boundaries`,不是 `needsPermission`**:`needsPermission` 在当前实现里只是自描述文档字段(grep 全仓库无运行时消费),真正驱动 confirm 弹窗的是 `OperationClassifier` 按 `boundaries` 升级到 `external`/`critical`。enter/exit 因为只是 emit 进程内意图、不直接跨 filesystem/network/process 任何写边界,引入 `agent-context` 新 BoundaryType(`BOUNDARY_WRITE_IMPACT["agent-context"] = "external"`,见 `classifier.ts`)显式表达"切换 agent 自身运行态"语义,而不是依赖 `BoundaryImpactClassifier` 的 fail-closed critical 兜底(那条路径是忘了声明的最后保底,不应作 intended 行为)。`agent-context` 在未来"切换 persona/role/runtime 实例"这类同源工具复用。
...
| `/workscene remove <id>` | `session.removeWorkScene`(带 active guard;彻底删除系统目录;workdir 不动;软隐藏走 `archive`) |
