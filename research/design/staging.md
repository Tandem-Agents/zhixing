...
> - **摘要质量升级**（2026-05-20 完成）：当时将主对话压缩 `LLMSummarize` 从 light 升为 main，压缩与 Memory Flush 分别使用独立调用；主会话摘要改为七段结构，突出约束与偏好、关键决策和进度三态。这次取舍强调摘要是后续认知输入，质量不能只按调用成本决定。上述为当时的实现记录，`LLMSummarize` 与 Memory Flush 路径现已退出；当前调用归属及质量差异见[模型角色](../../docs/modules/providers/model-roles.md)，思考参数见[思考控制](../../docs/modules/providers/thinking-control.md)，摘要机制见[上下文架构](../../docs/modules/context/architecture.md)。
