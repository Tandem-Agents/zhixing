...
type StorageMaintenanceKind =
  ...
  | "ticket-retirement" | "stream-spool-reclaim"
  | "conversation-transfer"
  | "authority-checkpoint"
  | "managed-service-reconcile"
  | "device-lifecycle-cleanup";
type StorageMaintenanceTaskOwner =
...
single-flight 归维护义务的唯一任务所有者：AuthorityCommitLog 拥有 log migration，workscene 兼容桥迁移器拥有 workspace migration，binding reset recovery owner 拥有 workspace catalog reset，本机 workspace management host 拥有 local operation outbox，锚点 workscene owner 拥有已提交删除的系统目录收敛，DurableProjectionIndex 拥有 projection 四类任务，ArtifactLifecycleIndex 拥有 lifecycle reconcile，锚点资产维护器拥有 asset GC，workspace probe owner 拥有 probe retirement，executor data plane 共同拥有 ticket retirement 与 stream spool reclaim，conversation transfer owner 拥有 conversation transfer，authority checkpoint owner 拥有 checkpoint，managed service owner 拥有托管服务 reconciliation，device lifecycle owner 拥有设备生命周期 cleanup。`workKey = D("StorageMaintenanceWork",1,{kind,resourceId,inputIdentity})`：日志使用逻辑日志身份与源格式，workspace 迁移使用设备、migrationId 与冻结 sourceSnapshotToken，workspace catalog reset 使用设备 binding 目录身份及 previous / target generation，workscene cleanup 使用 sceneId 与删除 revision，投影使用 projectionId 与 frozen manifest/checkpoint，生命周期使用 ArtifactStore 身份与源 checkpoint 集，GC 使用 ArtifactStore 身份与候选游标/release 前沿；其余 kind 使用对应 owner 已冻结的设备/执行器/transfer/checkpoint/definition/lifecycle 身份与耐久 revision、候选或清理前沿，禁止跨 owner 或跨代合并。键覆盖“复核义务 → 申请容量 → 执行 → 步骤计量 → permit 释放”的完整生命周期；新输入必须生成新键，禁止用资源名吞并不同代工作。
...
- **日常**：`/status`："知行在 NUC 值班；这台电脑在场，任务就地执行"。目标离线→"目标电脑离线，任务已排队，开机后继续"；上线续跑→"电脑已开机，继续处理排队的任务"，完成 / 失败通知严格回到发起接入面（绑原 `turnOrigin`，同一任务幂等投递一次，禁止跨渠道广播）；值班失联→"暂时联系不上值班设备。你可以继续在这台电脑上工作（新对话）；以下功能暂不可用：…"；重连→"已恢复连接：离线期间的 2 个新对话已合并，1 个提醒待你确认后生效"。软件维护统一为“安全停机维护 → npm 安装当前明确版本或 latest → 运行新 `zz` 恢复托管”；失败只给安全重试或前向维护中的一个行动，已运行新版本不提供降级。
- **异常**：结果不明→"有一件工作结果待确认"+ 三选："我已核验，结果有效 / 放弃这次结果 / 我了解风险，重新执行"；期间被暂停任务的到期只记跳过，`/status` 可见"有 N 次定时任务因待确认暂停被跳过"，裁决后汇总呈现。设备丢失→"撤销该设备"引导流：撤销 → 列出暴露的外部账号（CredentialExposureRecord）→ 逐项"去 ×× 平台更换密钥"。显式维护失败→只给“重试”“重新安装当前明确版本”或“前向安装最新版本”中的一个行动，不提供降级。provisional 输出恒有"待确认"标注，final 后消失。
...
| 文档 | 节点 | 修改内容 |
|---|---|---|
| [对话持久化与注意力窗口架构](../../../../docs/modules/conversation/persistence.md) | S3 | TranscriptRunRecord（现 RunRecord）增 runId 唯一键；接受协议改 SealedBundle 整包；windowCompact 明确为幂等缓存指令；写路径经对话 owner 的 AuthorityCommitLog，分片文件成为可由 log 幂等重建的投影（append-only 与"原文唯一权威"性质不变，原子性单元下移一层） |
...
| 38（S10） | npm 交付、兼容与最终验收 | 落唯一 npm 全局安装和显式维护路径、schema/protocol 兼容门、诊断与本地 package 检查；执行全部不变量、故障、安全、双拓扑和四时刻产品旅程，删除自有更新、原生安装及其迁移兼容层 | 受影响类型/直接测试、一次全量构建、`package:check` 与 Windows x64 tarball smoke 通过；18 条不变量与故障/对抗矩阵逐项闭合；维护不丢工作、数据保留、应用停用/永久设备移除零混淆，单机、配对、日常、离线、uncertain、撤销/恢复旅程不回退 |
...
旧 `StableReleaseIndex`、`ReleaseManifest`、`ProgramArtifact`、ProgramStore、update receipt/controller、upgrade lifecycle、update RPC、installer/launcher/program tree、五目标 journey 与对应构建脚本没有当前生产 owner、入口或兼容责任，必须从生产、公共 exports、registry、S7/golden 和当前验收中删除。产品从未发布旧 upgrade 耐久记录，不增加迁移 reader；历史文档只保留当时事实并明确已被当前合同取代。
