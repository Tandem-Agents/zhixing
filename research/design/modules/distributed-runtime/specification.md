...
stop 的 `gate-closed` evidence 必须包含固定 `conversation/intent/final/assignment/remote/channel/scheduler/delivery/lease/permit` owner exact-set 的 canonical artifact；十类 producer gate 必须全部关闭后才开始采样，scheduler 已耐久接受的 run 在外部 activation 前即进入 active projection。delivery transport quiesce 不代表 producer admission 关闭：同 operation 必须在 `DeliveryAuthority.coordinate()` 的共享串行边界安装 artifact 中七类 exact causal source 许可，fresh/冲突 source 在父 envelope append 前拒绝；冻结 source 产生的 current/late delivery 必须耐久绑定并纳入同一 settlement，直到 source 封闭与 delivery 集合 read-back 共同成立。immediate 只等待 active attempt 并保留 queued/retry/uncertain 的耐久重放态；drain 驱动既有 delivery 到 terminal；cancel 只取消上游可取消工作，已入队 delivery 仍按不可取消义务走 drain。每项由稳定 id/revision 冻结，port 的 settle/read-back 必须消费同一 operation 的 frozen slice，之后才允许 flush、物理步骤与 `ready-to-stop`。所有 OS signal、CLI/RPC 与 executor-only 入口只进入该 coordinator。managed driver 在 RPC 得到 `ready-to-stop` 后仍须全等复验完整 endpoint lock、canonical definition digest 与 manager inspection，再调用既有 `stopCurrentExact`；lock 换代零 manager effect，旧 lock 缺失时只有 manager 独立证明旧 task 不 running 才可补旧 operation terminal。发现 successor 时等待与 marker cleanup 都以完整 lock 而非 PID 判定。
