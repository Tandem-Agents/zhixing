...
`short-pake` 字段冻结可替换 wire 边界，不构成当前生产启用授权。当前公开产品只装配 `qr-secret`：二维码与可复制文本承载同一份高熵一次性邀请。`short-pake` 的精确实现及密码学依赖闭包取得独立不可变审计证据前，相关 adapter 必须仅为开发依赖、零 export、零 build、零 production import，CLI 与第一方 surface 不得展示短码入口；该门禁不得阻断高熵配对旅程。
...
- **日常**：`/status`："知行在 NUC 值班；这台电脑在场，任务就地执行"。目标离线→"目标电脑离线，任务已排队，开机后继续"；上线续跑→"电脑已开机，继续处理排队的任务"，完成 / 失败通知严格回到发起接入面（绑原 `turnOrigin`，同一任务幂等投递一次，禁止跨渠道广播）；值班失联→"暂时联系不上值班设备。你可以继续在这台电脑上工作（新对话）；以下功能暂不可用：…"；重连→"已恢复连接：离线期间的 2 个新对话已合并，1 个提醒待你确认后生效"。稳定版更新自动检查、下载并在安全空闲点安装；正常无更新不打扰，存在进度时在当前输入区/状态区显示“正在下载更新 / 将在当前工作完成后更新 / 正在更新”，不弹模态框、不要求用户记命令。
- **异常**：结果不明→"有一件工作结果待确认"+ 三选："我已核验，结果有效 / 放弃这次结果 / 我了解风险，重新执行"；期间被暂停任务的到期只记跳过，`/status` 可见"有 N 次定时任务因待确认暂停被跳过"，裁决后汇总呈现。设备丢失→"撤销该设备"引导流：撤销 → 列出暴露的外部账号（CredentialExposureRecord）→ 逐项"去 ×× 平台更换密钥"。更新失败但旧版安全可用→"自动更新失败，仍在使用原版本"+“重试”；自动恢复成功→"更新未完成，已恢复上一版本"；无法自动恢复→只给“恢复上一个可用版本”或“联系支持（复制安全问题码）”之一。provisional 输出恒有"待确认"标注，final 后消失。
...
| anchor 永久卸载 | current device、anchor epoch、trust head 与 migration `targetDeviceId/transferId` 或 recovery-backup `target/generation` | migration：`accepted→gate-frozen→transfer-committed→cleanup-complete→terminal(retired)`；backup：`accepted→gate-frozen→checkpoint-verified→retirement-decided→gate-closed→work-settled→flushed→final-checkpoint-verified→cleanup-complete→terminal(retired)` | `transfer-committed` 或 `retirement-decided` 前可 authenticated abort；之后只前滚。无 ready 迁居目标或独立可写备份目标时零 accepted；backup final 未覆盖 actual flushed LSN 时禁止 cleanup | 恢复备份已验证／可以卸载／已永久卸载 |
...
stop 的 `gate-closed` evidence 必须包含固定 `conversation/intent/final/assignment/remote/channel/scheduler/delivery/lease/permit` owner exact-set 的 canonical artifact；十类 producer gate 必须全部关闭后才开始采样，scheduler 已耐久接受的 run 在外部 activation 前即进入 active projection。启动扫描必须在任何 advancement、scheduler、assignment、channel、mesh worker 或 conversation recovery 之前读取本机 active operation：artifact 尚未形成的前置 phase 恢复所有 producer 为 closed；artifact 已形成且未 settlement 的 phase 恢复 exact unsealed admission；`work-settled/authority-settled/transfer-committed` 及以后恢复 sealed，连续重启不得重开 fresh admission。delivery transport quiesce 不代表 producer admission 关闭：同 operation 必须在 `DeliveryAuthority.coordinate()` 的共享串行边界安装 artifact 中七类 exact causal source 许可；每个 producer 以自身 durable companion 提交 `owner/id/revision`，assignment 保留 `local:`/`relay:` 命名空间，authority 不得代填 revision。fresh、错代或冲突 source 在父 envelope append 前拒绝；冻结 source 产生的 current/late delivery 必须耐久绑定并纳入同一 settlement，直到 source 封闭与 delivery 集合 read-back 共同成立；migration 与 backup 也消费同一 accepted-work/source/delivery 闭包。delivery 的 immediate 只等待 active attempt 并保留 queued/retry/uncertain 的耐久重放态；drain 驱动既有 delivery 到 terminal；cancel 只取消上游可取消工作，已入队 delivery 仍按不可取消义务走 drain。非 delivery owner 的 immediate 不调用恢复链清空 frozen 耐久义务：local 等当前 turn/recovery pass 到安全点，remote 等已越过 inbound gate 的 admission 与当前 job/recovery promise；read-back 只允许 frozen 全等的 intent/final/assignment/job 子集，active conversation、channel connection、scheduler completion tracker、lease 与 permit 必须为零。每项由稳定 id/revision 冻结，port 的 settle/read-back 必须消费参数中的 exact operation 与 frozen slice，不得读取全局可变 operation owner；之后才允许 flush、物理步骤与 `ready-to-stop`。所有 OS signal、CLI/RPC 与 executor-only 入口只进入该 coordinator。`server.shutdown` 由 router 先做认证，再于参数解码、wiring 读取、lifecycle prepare/trigger 前拒绝非 loopback；请求只接受必填 `requestId` 和可省略的 `reason/strategy/timeoutMs` exact-set，出现的字段必须分别通过稳定文本、三策略枚举、有限正数及可表示 ETA 校验，省略才使用默认。managed driver 在 RPC 得到 `ready-to-stop` 后仍须全等复验完整 endpoint lock、canonical definition digest 与 manager inspection，再调用既有 `stopCurrentExact`；lock 换代零 manager effect，旧 lock 缺失时只有 manager 独立证明旧 task 不 running 才可补旧 operation terminal。发现 successor 时等待与 marker cleanup 都以完整 lock 而非 PID 判定。

executor removal 的 durable decision 必须携带 gate 后冻结的完整 `ownerItems(id,revision)`，target live/replay 只用该 exact-set 驱动 local/external settlement；缺失、同 id 换代或 reader 未能证明所选 transfer/destroy 终态时不得写 `authority-settled`。target durable aborted 是同 operation gate release 的唯一授权：gate 已不存在视为幂等完成，异 operation gate 零副作用，release 完成后 hydrate 或补写同一 authenticated receipt peer-effect。
...
跨进程执行 owner 不由 closed admission、PID 文件或 stop journal 代替。两根完成 active operation 扫描与 strict hydration 后，无论扫描结果是否为空，都必须在各自首个 owner/MCP/authority/workspace/data-plane/job/local 恢复效果之前，以最终同 home 非零 `host+port` 创建并监听同一种 inactive server 对象；HTTP 只稳定拒绝、WebSocket upgrade 不进入 router，PID/state/ready 均不发布。`EADDRINUSE` 失败者立即回滚且 owner/journal/manager 零效果；只有持有 exact bound handle 的 winner 可装配 closed owners、执行旧 host proof 与 `resumeActive()`。current-successor 必须同时反绑该 handle、current endpoint lock、旧 endpoint、definition 与 manager projection。terminal 与同 operation exact release 后，`runServer()`只在原对象激活既有 REST/RPC/bridge 并写发现/ready，禁止第二次 listen 或 close/rebind；异常关闭同一 handle，进程崩溃由 OS 释放，后继重读原耐久 phase 再竞争，不新增耐久 claim 或第二锁。
...
稳定安装默认自动更新且只跟随 `stable`：稳定启动器在每次真实启动时发起异步检查，foreground/按需进程在当前运行期做有界检查，托管宿主在自身生命周期内做有界周期检查；current 版本启动、首个输入和 ready 不得等待检查或网络。不得新增 updater daemon，未运行且未启用托管角色的设备无需凭空唤醒。每个 host generation 的单次与周期入口共用唯一 active-round handle，内部至多持有一个 controller 与 exact promise；tick 先过 closed/active gate，cleanup 先 close 并清 timer，再 abort/await 当前轮次，返回后旧代不得继续 network、stage、handoff 或 receipt 写入。检查、下载、解压与 stage 在旧版继续服务时进行，网络等待不持资源 permit；同一 candidate manifest digest 的并发检查、失败重试与响应丢失幂等合并。所有设备只消费同一已签 `StableReleaseIndex`、目标 `ReleaseManifest` 与最终 artifact bytes，任何签名、target、版本/序列、摘要、字节、容量或解压验证失败都在 upgrade lifecycle accept 前终止并保留 current。

稳定启动器只维护一个程序层原子 `ProgramUpdateReceipt` v1：`currentManifestDigest/target/candidateManifestDigest?/phase/operationId?/notice/code?/action?`，其中 `phase=idle|checking|downloading|staged|handed-off`，candidate digest 在 `downloading|staged|handed-off` 必填，`operationId` 只在 `handed-off` 出现且反绑 exact upgrade operation；`notice=none|updated|failed-safe|restored|action-required`，`none|updated|restored` 禁止 `code/action`，`failed-safe` 必须携稳定 code 与 `action=retry-update`，`action-required` 必须携稳定 code 与 `action=restore-previous|contact-support`。receipt 只承载检查、下载、stage 续做和一次产品通知，不复制 lifecycle phase、health、pointer 或兼容事实。更新状态由该 receipt 与 current/previous 指针、已验 program stage、active `device-lifecycle` upgrade operation 形成一个无副作用投影，CLI、server 与第一方 surface 共用；不得另建第二升级事实源。健康且无更新时不展示；`downloading/awaiting-safe-point/installing/updated/failed-safe/restored/action-required` 才可见，文案分别收敛为“正在下载更新”“将在当前工作完成后更新”“正在更新”“已更新”“自动更新失败，仍在使用原版本”“更新未完成，已恢复上一版本”和一个明确人工动作。可继续使用旧版的失败不得阻塞输入；不兼容写入仍沿既有兼容门 fail-closed。失败信息只含稳定 code、release 与 action，不含 URL、路径、设备/operation 身份或 raw error；同一失败在当前输入区/状态区保持可见直至成功、candidate 被替代或用户明确收起，`status/doctor` 离线读取同一投影。

已验 stage 自动等待下一个安全空闲点：foreground 在当前命令/交互完成后交给启动器，托管或后台形态沿同一 upgrade operation 关闭准入、收束 accepted work、刷稳并停止旧 exact host；不得按 wall-clock 强制切断。切版和健康验证失败优先自动恢复最近已验兼容版本；只有无法自动恢复时，诊断才提供“恢复上一个可用版本”的一次动作，脚本入口为 `zz update --restore-previous`，用户界面不得暴露 rollback 术语。`zz update` 仅作为立即检查/重试同一自动更新合同的显式入口，不是正常使用前提。

正式 installer 在任何程序写前同时验真 current pointer 所指 manifest 与程序根外 installation receipt。同 target 下 candidate 的 SemVer precedence 与 release sequence 必须相对两者同向严格递增；两维全等且 exact identity/bytes 全等才是 replay，任一维相等而另一维变化、交叉排序或同 precedence 异 identity 均 fail-closed。无 current 的首次安装和 pointer/receipt exact replay 可直接完成；其他已有安装只能 stage，并经现有本机认证 current-host RPC 接入同一 upgrade operation。pointer 已是 candidate 而 receipt 落后时只允许 exact active/terminal lifecycle 历史补齐，无历史不得猜测。版本 codec 与所有排序消费者共用 SemVer 2.0 parser：任意长度数字按十进制字符串比较，数字 prerelease 禁前导零且低于非数字，build metadata 不参与 precedence；原始 version 字符串仍是 exact identity。ProgramArtifact 文件、source closure 与 package graph 的 path 行必须共用 core 唯一 ordinal comparator，producer 编码前先过 production validator；构树前冻结 source/package digest，source closure 必须包含固定实际 release producer exact-set，strict tree receipt、artifact builder、target evidence 与 release:check 逐段重算全等，发布前再复验无漂移。ProgramStore 新版本目录只由已验 manifest digest 的完整 64 位小写 hex 派生，raw version 继续留在 manifest/pointer exact identity；launcher 只消费 pointer 已存目录，既有安全目录不迁移、不改写。

ProgramStore 的 rename、version、pointer、previous 和 terminal 各 phase 都先以 exact read-back 判断 achieved：stage 消失但目标 version 全验成立时不得重做 rename，pointer 已切时不得重切。当前 host 的既有 cleanup 完成且同一 bound handle 已 inactive 后，原 coordinator 才把已达 `flushed` 的 operation 推进到 pointer switch；child 随后返回，稳定 launcher 因 pointer generation 变化用 version-local runtime 接力 target 或 previous，同 generation 绝不自旋。两根只在同一个 bound server 已激活 RPC 但尚未发布 ready 的窗口，以 loopback RPC 全验 release、runtime、protocol/schema、home、endpoint、role 与 trust，成功后才写 health/terminal/installation receipt。失败恢复 previous 并由 launcher 自动接力，previous 也必须通过同一 health 门；`health-verified+` 重放不得产生第二次运行效果。

current-authority 更新状态读取成功时把该 exact connection generation 登记在 update 方法域通知表；receipt 或 lifecycle 耐久变化只发送“重新读取”的空提示，连接 close/换代原子移除旧代，通知不携带也不成为状态事实。CLI 输入区、status 与第一方 surface 始终重读同一纯投影；无法联系 current authority 时，只读一次 `loadConfig(noAutoCreate)`：无 mesh 是默认单机本机 current 的充分正事实且不得访问 SecretStore，存在 mesh 才以同一快照、existing-only trust、localDeviceId 与 anchor 角色证明本机 current，其他组合稳定 unavailable；证明成立后才从本机 ProgramStore effect-free 回退。program FileLock busy 保持既有状态且零失败 receipt；LockRecord 以版本化 strict `PID+process-birth+token`反绑 owner，heartbeat fresh 只提供快速 busy，stale 只触发进程身份复核。exact 进程仍在或身份不可证明时继续 busy，仅证明 PID 已消失或 birth 已换代时，才经既有 reclaim claim、二次 exact read-back 与 compare-token release 接替。

五目标 smoke 证据只能由目标机固定 producer 生成：先验证同 target 的 signed baseline/candidate manifest/artifact 与本机 OS/arch，基线的 precedence/sequence 必须严格更旧；全量 materialize 最终 candidate program bytes，在隔离 program root、installer state 与 `ZHIXING_HOME` 中只通过候选 runtime、stable launcher/installer 或候选 CLI/RPC 正式入口执行固定十二行“入口 + 公开参数 + 终态谓词”合同，禁止 repository/Vitest 旁路。每行绑定实际 runtime/entry bytes、解析后的 argv、公开退出/输出与耐久 read-back并形成独立 execution digest；producer 输出再绑定 target、两组 manifest/artifact、source/package 与候选执行的 canonical report。`release:check`只接受五目标 exact-set 的该报告，复验 signed baseline/candidate 和全部摘要，并在 publish 前确认输入未漂移；调用方裸 `passed:true` 不构成证据。该 producer 是固定发布编排，不扩建通用 runner、远程证明或运行时 report 存储。

公开删除语义只有两种：安装器及 `zz app remove` 显示“移除知行应用（保留全部数据）”，只停用程序层 future launch、停止当前 exact host并删除 launcher/version/stage，保留 `ZHIXING_HOME`、设备身份、信任、配置和用户工作。该流程以操作内 handle 冻结 exact definition、launch plan、原 future 状态及本操作是否改变它；安全停机或注销在 commit 前失败时只补偿本操作造成的 enabled→disabled，原 disabled 不得误启，补偿或事实不可证明时必须保留程序并给出稳定行动。默认未建立托管身份的单机不能因缺少 SecretStore binding 而失去应用移除能力：只有既有 manager 与 definition 均可证明 absent、binding 在 commit 回读仍 absent 时才按无托管事实继续，任一事实出现或不可判定即保留程序。exact unregister 提交后的固定 handoff 只建立一次性责任接纳链：父进程等待稳定 installer 的 `error/exit`且只接受 exit 0；installer 必须等待平台删除 helper 的 OS `spawn` 后才 `unref`并正常退出。任一层 spawn error、非零或 signal 都统一提示“应用未完全移除，请重试”，不泄露 raw error/path；成功只证明异步删除已接纳，不等待或宣称文件删除完成，future absent 状态仍可幂等重试，不增加 remover transaction 或耐久事实。`zz device remove --permanent` 显示“永久移除此设备及本机数据”，先以可逆 accept 展示设备名、未转移工作与不可逆后果，lost/destroy 在 continue 前必须强确认；accept 后的取消、确认拒绝、响应丢失或其他 pre-decision 退出必须以调用方已知的 exact `targetName+operationId` 重投既有 signed abort，在线收敛 cancelled，离线只报告取消已耐久登记并由既有 startup/mesh 前滚。decision dispatch 失败后只读一次既有状态：target 可读时优先，离线、空回执或读取失败时从 issuer journal 当前唯一 operation phase 纯投影；未决只给继续或取消，已决/terminal 显示现有进度，status 也不可用则只提示稍后运行 `device status`。该读回零 journal/gate/abort 写，不能把不确定响应自动改写成取消。首个公开基线前删除 `zz uninstall` 与 `zz update --rollback` 旧别名，禁止让同一词或默认参数承担两种数据后果。
