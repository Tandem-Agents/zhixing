...
稳定安装默认自动更新且只跟随 `stable`：稳定启动器在每次真实启动时发起异步检查，foreground/按需进程在当前运行期做有界检查，托管宿主在自身生命周期内做有界周期检查；current 版本启动、首个输入和 ready 不得等待检查或网络。不得新增 updater daemon，未运行且未启用托管角色的设备无需凭空唤醒。检查、下载、解压与 stage 在旧版继续服务时进行，网络等待不持资源 permit；同一 candidate manifest digest 的并发检查、失败重试与响应丢失幂等合并。所有设备只消费同一已签 `StableReleaseIndex`、目标 `ReleaseManifest` 与最终 artifact bytes，任何签名、target、版本/序列、摘要、字节、容量或解压验证失败都在 upgrade lifecycle accept 前终止并保留 current。
...
公开删除语义只有两种：安装器及 `zz app remove` 显示“移除知行应用（保留全部数据）”，只停用程序层 future launch、停止当前 exact host并删除 launcher/version/stage，保留 `ZHIXING_HOME`、设备身份、信任、配置和用户工作；`zz device remove --permanent` 显示“永久移除此设备及本机数据”，先展示设备名、未转移工作与不可逆后果，交互调用必须二次确认，非交互调用必须提供冻结设备名与显式永久确认字段，随后才进入第 37 单元既有永久 removal/uninstall 流。首个公开基线前删除 `zz uninstall` 与 `zz update --rollback` 旧别名，禁止让同一词或默认参数承担两种数据后果。
