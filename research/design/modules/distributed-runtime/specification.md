...
planned anchor 迁居启用边界：current anchor 只能选择另一台已配对、active 且启用 anchor 角色的设备；目标在本地生成 transfer 专属 issuer key，`ReadyProof` 冻结当前 trust generation、角色、配置/资产/协议/服务 revision 与秘密可解锁状态，wire 零秘密。源端先经第 33 单元接缝取得 current verified full recovery checkpoint，随后耐久关闭 fresh 准入、收束已接受执行，并从同一 `AuthorityCommitLog` source prefix 生成独立 planned export artifact、无秘密 `AuthorityCatalog` 与 `SourceFreezeProof(scope:"anchor")`；恢复 checkpoint digest 只作安全保障，不得冒充 planned export digest。目标只在 transfer 私有 staging 分块导入并核对目录/字节/digest/coverage exact-set。source 当前 issuer 在一个 envelope 原子追加唯一签名 `AnchorTransferCommit`、prepared `issuer-transition` 与 next anchor/trust epoch，该 sync 是全局唯一切点并令 source 永久 fencing；target 只接受逐字段全等 commit，在一个本地 envelope 原子安装 commit、transition、authority base、current anchor 与 next epochs，sync 后才开放服务并启用本地新 issuer key。commit 前只允许当前权威签名 abort，source 恢复原 epoch/准入、target 清 transfer 私有 staging 和未激活 key；commit 后只能前向重驱，late abort 永久拒绝。current anchor 组合根恰一 source/recovery owner，本次 anchor target 恰一有限 receiver/recovery owner，executor-only、surface、disabled 和其他非当前设备零 migration owner；公开 CLI/server 只显示“值班设备”、可行动 ready 缺口及准备/收束/传输/接管阶段。disaster recovery、恢复应用、`domain-reset`、pending-reenroll、凭据轮换与全局连续同步仍未启用。
