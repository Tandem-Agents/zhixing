...
current-authority 更新状态读取成功时把该 exact connection generation 登记在 update 方法域通知表；receipt 或 lifecycle 耐久变化只发送“重新读取”的空提示，连接 close/换代原子移除旧代，通知不携带也不成为状态事实。CLI 输入区、status 与第一方 surface 始终重读同一纯投影；无法联系 current authority 时，只有 topology/trust 能证明本机就是 current authority 才允许从本机 ProgramStore effect-free 回退，否则稳定显示 unavailable 与一个重试动作。program FileLock busy 保持既有状态且零失败 receipt；跨进程 active 必须同时满足 PID 存活和 heartbeat 新鲜，接替仍经 reclaim claim、二次 exact read-back 与 compare-token release。

五目标 smoke 证据只能由目标机固定 producer 生成：先验证 signed manifest/artifact 与本机 OS/arch，全量 materialize 最终 candidate program bytes，在隔离 program root、installer state 与 `ZHIXING_HOME` 中用候选自带 runtime 完成真实安装、launcher 启动、同版 replay 和程序移除并回读保留数据；随后以同一候选 runtime 执行固定十二行“正式入口 + 参数 + 终态谓词”直接场景合同。producer 输出绑定 target、manifest/artifact、source/package、候选执行和逐行命令/结果摘要的 canonical report；`release:check`只接受五目标 exact-set 的该报告并在 publish 前复验全部输入未漂移，调用方裸 `passed:true` 不构成证据。该 producer 是固定发布编排，不扩建通用 runner、远程证明或运行时 report 存储。
