...
公开删除语义只有两种：安装器及 `zz app remove` 显示“移除知行应用（保留全部数据）”，只停用程序层 future launch、停止当前 exact host并删除 launcher/version/stage，保留 `ZHIXING_HOME`、设备身份、信任、配置和用户工作。该流程以操作内 handle 冻结 exact definition、launch plan、原 future 状态及本操作是否改变它；安全停机或注销在 commit 前失败时只补偿本操作造成的 enabled→disabled，原 disabled 不得误启，补偿或事实不可证明时必须保留程序并给出稳定行动。默认未建立托管身份的单机不能因缺少 SecretStore binding 而失去应用移除能力：只有既有 manager 与 definition 均可证明 absent、binding 在 commit 回读仍 absent 时才按无托管事实继续，任一事实出现或不可判定即保留程序。`zz device remove --permanent` 显示“永久移除此设备及本机数据”，先以可逆 accept 展示设备名、未转移工作与不可逆后果，lost/destroy 在 continue 前必须强确认；accept 后的取消、确认拒绝、响应丢失或其他 pre-decision 退出必须以调用方已知的 exact `targetName+operationId` 重投既有 signed abort，在线收敛 cancelled，离线只报告取消已耐久登记并由既有 startup/mesh 前滚。首个公开基线前删除 `zz uninstall` 与 `zz update --rollback` 旧别名，禁止让同一词或默认参数承担两种数据后果。
