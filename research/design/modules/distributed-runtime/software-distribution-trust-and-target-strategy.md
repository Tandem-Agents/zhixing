...
- 当前实现目标为 Windows x64、macOS x64/arm64、Linux x64/arm64；宿主条件与原生产物合同见[运行时边界](node-runtime-boundaries.md)。macOS/Linux 的真实环境验收尚未完成，属于跨平台发布前置条件，不以删除启动拦截或模拟测试代替。
...
以下清单与后续执行提示、验收数字记录当时的 npm 交付收敛。其 Windows 单目标和“不建设 CI”范围已由当前跨平台目标及[发布指南](release-and-maintenance-guide.md)替代，不得继续作为实现或发布限制；历史通过记录不代表新增平台已通过。
