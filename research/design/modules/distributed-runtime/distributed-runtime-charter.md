...
- **当前启用边界**：S8 只启用 conversation 收编分支。源端导出完整会话权威前缀、SessionState 可重建读面、内容资产与 conversation-owned intent，锚点目标全验后以 `ConversationTransferCommit` 唯一切换 current owner；取证在 journal 与 workspace 读取前复验该 current owner，提交后从导入的 transcript/segment 事实补驱动既有锚点记忆蒸馏。两个生产根共用同一 transfer mesh adapter 与恢复顺序，非 anchor 零 target，公开离线入口只接受明确用户确认的本机对话。收编完成或恢复追平后，锚点只经既有 internal review 自动处理无冲突规则；排程复用现有认证确认链，`session.resume` 返回零术语摘要并重浮现当前接入面错过的待确认项，不开放 intent 接口或新增通知事实源。planned anchor 迁居、灾难恢复、全量检查点、TrustTransition 与 ReadyProof 仍保持未启用，不得由 conversation wire 或本地域入口提前承载。
