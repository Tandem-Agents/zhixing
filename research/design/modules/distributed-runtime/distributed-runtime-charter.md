...
- **自动维护是基础产品能力**：stable 安装默认自动检查、验证、下载并在安全空闲点安装稳定版更新，不要求用户记住维护命令，也不增加发布通道、后台角色或新的常驻进程。foreground/按需形态在实际使用期间检查并于当前交互结束后的安全边界切版，托管形态以同一稳定启动器有界检查；检查异步发生，当前已验版本的启动、首个输入与 ready 不等待网络。每个 host generation 只持一个可关闭的检查轮次句柄，周期 tick 不叠加轮次；停机、移除和换代必须先拒新，再 abort 并等待 exact round 后才释放依赖。所有形态只消费同一已签 stable index、程序 stage 与 `device-lifecycle` upgrade operation。检查和下载不得阻断当前工作，更新只能在完整制品已验、accepted work 已按既有 stop 闭包收束且旧 exact host 已停止后切版，禁止强杀、静默丢工作或触碰 `ZHIXING_HOME`。发布侧的 ProgramArtifact、source 与 package 路径共用唯一 ordinal 顺序；构树前冻结的 source/package closure 必须包含有限的实际 release producer，并由 tree receipt、manifest、目标证据和发布门逐段全等复验。program FileLock 只有在 PID 与 process-birth 证明原 owner 已消失或换代后才可接替，心跳过期本身不撤销所有权。健康状态默认不打扰；发现新版本、下载/安装进行中、失败仍安全使用旧版、自动恢复成功或确需用户处理时，由 CLI 输入区/状态区及第一方 surface 消费同一无副作用投影，给出固定产品文案和一个下一步，零 raw error。自动失败保留旧版并可按同一 candidate 身份重试；恢复优先自动完成，只有无法自动恢复时才由诊断引导“恢复上一个可用版本”，不要求用户理解 rollback。程序移除与设备永久退役是两个产品概念：前者明确为“移除知行应用（保留全部数据）”，若安全停机或注销失败，只回滚本操作改变的 future launch；无法证明恢复时保留程序并给出行动，exact unregister 后也只有稳定 installer 已确认平台删除 helper 被 OS 接纳才报告成功，接纳失败保留幂等重试且不伪报异步删除完成。后者只能通过“永久移除此设备及本机数据”的独立设备动作和强确认进入；accept 后的取消必须以 exact operation 写入既有 durable abort，lost/destroy 不能绕过确认；decision 响应不确定时优先读 target 状态，离线或读取失败则纯投影 issuer journal，给出唯一下一步且绝不自动 abort。公开入口不得再让同一个 `uninstall` 名称同时承载两种后果。
...
维护不成为第五套概念：稳定版自动更新默认完成，正常时不打扰；有进度或失败时在用户正在使用的界面就地显示，旧版仍安全可用则不阻塞输入，必须处理时只给一个可执行动作。用户只看到“正在更新 / 已更新 / 更新失败，仍在使用原版本 / 已恢复上一版本 / 需要你处理”，不看到 manifest、sequence、schema、protocol、stage、operation 或 rollback。移除程序与永久移除设备必须在名称、后果说明、确认和入口上完全分离。

- **当前 S10 升级与发布边界**：首次安装和 exact replay 才可由 installer 直接落地；已有安装先全验 current pointer 与程序根外 receipt 的 SemVer precedence、sequence 和 exact identity，再把更新交给当前宿主的既有 `device-lifecycle`。ProgramStore 每个 phase 以真实 stage/version/pointer read-back 判 achieved，新版本物理目录只由已验 manifest digest 派生，raw SemVer 仍是公开 exact identity；旧 host 完成既有 cleanup 且 exact bound handle 已 inactive 后，由原 coordinator 将 `flushed` operation 切到 candidate pointer，随后稳定 launcher 只在 pointer generation 变化时接力。候选或 previous 都必须在同一 bound server 激活后经 loopback health 才能 terminal。更新可见性仍由 current authority 的纯投影给出；默认无 mesh 单机可直接只读证明本机 current，配对拓扑才消费 existing-only trust，方法域通知只提示重读，lock busy 不写失败。发布报告只能由五个目标机上的固定 producer 以 signed baseline/candidate、最终候选正式入口字节、隔离程序根/home 和十二个入口/参数/终态合同生成，仓库测试不可替代；共享 SemVer parser 遵循 SemVer 2.0 precedence，program lock 的跨进程活性同时要求 PID 存活与 heartbeat 新鲜。不得建立 installer 第二 lifecycle、通用事件/轮询、租约服务、通用发布 runner 或第二事实源。
