...
- **自动维护是基础产品能力**：stable 安装默认自动检查、验证、下载并在安全空闲点安装稳定版更新，不要求用户记住维护命令，也不增加发布通道、后台角色或新的常驻进程。foreground/按需形态在实际使用期间检查并于当前交互结束后的安全边界切版，托管形态以同一稳定启动器有界检查；检查异步发生，当前已验版本的启动、首个输入与 ready 不等待网络。所有形态只消费同一已签 stable index、程序 stage 与 `device-lifecycle` upgrade operation。检查和下载不得阻断当前工作，更新只能在完整制品已验、accepted work 已按既有 stop 闭包收束且旧 exact host 已停止后切版，禁止强杀、静默丢工作或触碰 `ZHIXING_HOME`。健康状态默认不打扰；发现新版本、下载/安装进行中、失败仍安全使用旧版、自动恢复成功或确需用户处理时，由 CLI 输入区/状态区及第一方 surface 消费同一无副作用投影，给出固定产品文案和一个下一步，零 raw error。自动失败保留旧版并可按同一 candidate 身份重试；恢复优先自动完成，只有无法自动恢复时才由诊断引导“恢复上一个可用版本”，不要求用户理解 rollback。程序移除与设备永久退役是两个产品概念：前者明确为“移除知行应用（保留全部数据）”，后者只能通过“永久移除此设备及本机数据”的独立设备动作和强确认进入；公开入口不得再让同一个 `uninstall` 名称同时承载两种后果。
