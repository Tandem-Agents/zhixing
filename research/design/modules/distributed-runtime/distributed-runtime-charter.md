...
- **当前软件交付边界**：唯一正式路径是用户提供受支持 Node 后，通过 npm 全局安装同一 `@zhixing/cli` 包。安装、同版修复和前向升级均由用户显式触发；知行不后台检查、下载、切版或回滚程序，不持有私有 Node、原生安装器、stable feed、签名 manifest/index 或平台签名公证链。维护前运行 `zz stop --maintenance`，安全收束并保持 future launch disabled；npm 安装完成后运行新 `zz`，复用既有 reconcile 更新 definition 并恢复托管。`zz app remove` 只完成安全停机与 exact unregister，随后明确引导 npm 卸载并保留 `ZHIXING_HOME`。程序停用与设备永久退役继续使用不同名称、入口和数据后果。当前发布只承诺 Windows x64；macOS/Linux 保持源码与平台 adapter 可移植，但在真实环境验收前不构成交付门禁。
...
- **当前 S10 npm 交付边界**：根版本是全部公开包与 CLI 版本投影的唯一来源；公开包声明最低 Node 和 Windows x64 支持边界，生产依赖以精确版本发布，CLI 发布 shrinkwrap 绑定 registry integrity。`package:check` 只在本地构建、pack、以本轮 tarball 装配并穿过公共 CLI/subpath/helper 验收；正式 npm 发布命令在用户另行授权后才消费当次认证，按依赖拓扑使用候选 tag，所有版本与 integrity 全等后才移动 CLI `latest`。发布脚本不登录、不保存 token、不建设 CI、签名、公证、自有索引或通用发布框架。
