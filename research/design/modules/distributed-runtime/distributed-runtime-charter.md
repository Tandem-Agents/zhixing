...
> **模块定位**：分布式运行时（distributed-runtime）是知行走向伴身智能所需的一项基础能力，而非知行的产品核心；知行不是为分布式运行时而存在，分布式运行时是为知行而存在。
...
角色与权威边界：**统一不变量——每个对话任一时刻恰有一个权威 owner，对话的输入准入与串行、权威状态、RunRecord 提交必经其当前 owner**。锚点是默认 owner，同时是全局任务与调度登记、耐久投递队列等全局域状态的唯一权威（全局域永无 owner 转移、无例外），自身不执行任务。执行宿主运行完整的知行 runtime，在对应真实环境中完成整个 run，并将结果交回该对话的 owner 提交（默认即锚点），不形成第二份权威状态；任务按环境依赖选择执行宿主，目标设备离线时可靠排队，上线后按定案②的语义续跑；锚点所在设备可同时运行一个普通执行宿主，承接不依赖特定环境的任务。
...
| 类别               | 权威                   | 内容                                                                                                                                                                |
| ------------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 全局状态与期望配置 | 锚点                   | 全局任务与调度登记、耐久投递队列、信任规则、技能库、Rubric 库、渠道注册、workscene 注册、设备与锚点记录、guidance、非秘密运行配置（模型档位、能力表、策略） |
...
- **run 内权威写的事务边界**：runtime 经端口的内部权威写（task-list、schedule、技能使用记录等）落入 assignment 域的 `AuthorityMutationJournal`，以 staged overlay 生效——run 内读己之写、外界只见 provisional；写集摘要进入 RunCommitBundle，owner 在最终提交后协调会话域与锚点域幂等发布，失败 / 取消整体丢弃，uncertain 随裁决收束。锚点域 staged 决定必须在同一 AuthorityCommitLog 临界区内针对精确日志前缀规划并随单次 fsync 发布主读投影；fsync 成功即为不可回退的提交事实，此后不得执行可导致调用方收到失败的元数据 I/O。文件或 runtime refresh 等可失败派生只由权威记录与耐久 checkpoint 重驱，不得改变提交结果。文件、外部 API 等不可事务化的真实副作用不进此机制，仍走执行账本与 uncertain 语义。
...
- **托管服务**：npm 安装的同一 CLI 只为已启用的后台角色注册跨平台托管服务——anchor 崩溃 / 设备重启自恢复；executor 按用户选择在开机或登录后自动上线；纯 surface 与按需单机形态不额外常驻。definition 固定绑定当前安装包的绝对 CLI 入口与 `process.execPath`，不得调用裸 `node`、npm 或 PATH 猜测。当前 launch plan 只由 current signed trust、本机角色选择与既有 SecretStore backend binding 派生；OS definition、future-enabled、current-running 与 readiness 是可独立核验的事实，动态 enabled 不得进入 immutable definition identity，公开状态只能由同一次 plan/spec 与真实 supervisor/runtime snapshot 投影。配置、信任或用户 Node 路径换代后，由用户运行的新 CLI 复用既有 reconcile 更新 definition；产品不伪称能在旧 Node 已移动且进程尚未启动时自愈。纯 surface/空角色只经认证 mesh 把 canonical first-party exact-set 送到 current anchor，本机零宿主、零角色模块、零监听。**停机收束协议分三路径**：临时停机 / 显式维护——关闭准入、收束在途 assignment、刷稳账本与 outbox 后停机，权威保留；显式维护还须先关闭 exact future launch，失败只补偿本操作造成的 enabled→disabled，成功保持 disabled，待新 CLI reconcile 恢复；executor 移除——先收束或转移本地域权威，再停发租约、撤销身份并注销；anchor 永久卸载——先完成 AuthorityTransfer，或生成并验证加密恢复导出、经用户明确确认。协议、schema 与 epoch 不得倒退；撤销、停用、卸载、诊断和显式维护逐阶段做故障恢复验收；未启用角色零加载、零监听。
- **当前 S10 生命周期退出边界**：三条退出路径只复用同一物理 `AuthorityCommitLog` 的 `device-lifecycle` 流，并以同一 `homeId+localDeviceId` 耐久冲突键互斥；stop 的 host subject 只附加精确宿主身份，不形成可与 removal/uninstall 并存的第二本机 owner。临时停机先同时关闭固定 owner exact-set 的全部 producer gate，再冻结 accepted-work artifact；已经耐久接受的 scheduler run 必须在交给外部 activation 前进入冻结投影。任一非终态 operation 重启时，必须先由 durable operation、artifact 与 phase 恢复 exact admission/sealed 投影，再启动 producer 恢复或公开准入。delivery 的 producer admission 与 transport quiesce 是两个事实：同 operation 把 artifact 中的 exact causal source 安装到 `DeliveryAuthority` 共享串行边界；source 调用方提交自己的稳定 `kind/id/revision`，local/relay 命名空间不得折叠，authority 只全等核验而不代填。fresh 或冲突 source 在父 envelope append 前拒绝，冻结 source 的当前与迟到 delivery 持久反绑该 operation，直到 source 与 delivery 集合共同封闭；immediate 只要求可耐久重放，drain/cancel 对已入队 delivery 均收束到既有 terminal。非 delivery owner 的 immediate 先等已越过准入的 turn、recovery、job 与 inbound 物理步骤到既有耐久安全点，只允许与 frozen `id/revision` 全等的 intent/final/assignment/job 子集留存，active conversation、channel、scheduler tracker、lease 与 permit 必须归零；drain/cancel 的既有终态不放宽。各 port 只按自身收到的同一 operation 与 frozen `id/revision` 逐项收束、回读并 flush，不得读取进程级可变 operation owner；OS signal、CLI/RPC 与 executor-only 都只能经同一 coordinator 到达 `ready-to-stop`。托管实例的物理停止还须全等匹配 endpoint lock、immutable definition 与 manager projection，PID 或 service id 单独不授权。executor removal 的 target winner 与 issuer peer-effect 分别留在现有两根日志：用户决定只消费副作用前展示、gate 后冻结的同一 owner snapshot artifact，`ownerItems` 必须原样进入 local/external settlement；durable authenticated abort 授权同 operation 的幂等 gate release 后 exact replay 原 receipt，ready、cleanup-ready 与 issuer terminal仍恰一前滚。普通 revoked admission 不放宽，只允许历史身份进入固定 terminal-only 分支。backup uninstall 的 service generation 与两次 checkpoint identity 分离；retirement decision 在同一 transaction 反绑十 owner artifact，之后以 immediate 安全语义逐项 settle/read-back，再 flush 权威日志、outbox、lease/permit 与物理步骤，`flushed` envelope 的实际 LSN 才是最终 checkpoint 水位。首个 checkpoint 与覆盖该水位的最终 checkpoint 都必须由同一 recovery root 走现有 service 真解封验证，final 通过后才允许 cleanup。cleanup 只遍历冻结 current-home roots，以固定批次经 storage governor 前滚，并严格按 unregister、非 device-key secret、cleanup-ready、issuer terminal、exact device-key compare-delete、本机 terminal 排序。`server.shutdown` 在 router 认证后仍须于解码和首个效果前拒绝非 loopback 连接；全部公开 facade 在任何 lifecycle/gate/journal 效果前执行方法级 strict exact-set；不新增通用 manifest、历史 mesh、storage/secret lifecycle 或第二事实源。

- **当前软件交付边界**：唯一正式路径是用户提供受支持 Node 后，通过 npm 全局安装同一 `@zhixing/cli` 包。安装、同版修复和前向升级均由用户显式触发；知行不后台检查、下载、切版或回滚程序，不持有私有 Node、原生安装器、stable feed、签名 manifest/index 或平台签名公证链。维护前运行 `zz stop --maintenance`，安全收束并保持 future launch disabled；npm 安装完成后运行新 `zz`，复用既有 reconcile 更新 definition 并恢复托管。`zz app remove` 只完成安全停机与 exact unregister，随后明确引导 npm 卸载并保留 `ZHIXING_HOME`。程序停用与设备永久退役继续使用不同名称、入口和数据后果。跨平台交付目标、宿主条件及待验收状态见[运行时边界](node-runtime-boundaries.md)；真实环境验收是对应平台发布前提，不能用源码可移植性替代。

托管服务的失败重驱保持有限：加入既有 canonical-home worker 的调用方若共享到首轮失败，只能以自己的当前输入竞争一次同 home successor，successor 再失败即终止。纯 surface 的 live poll 只复用既有连接期退避，在当前 owner 与 surface generation 内对 transport transient 和 current-owner busy 重试；连接关闭由该代唯一 controller 持有，poll attempt 只借用 signal，成功循环不得累积连接级 callback 或 promise reaction；协议、认证等稳定错误终止该代 controller，已提交 final/status 的响应丢失仍由既有历史补读。三平台 start 与 inspect/install 共用同一有限 manager 分类，命令成功后仍须独立 inspect，任何失败都不得反向授权 absent、改写 definition 或公开状态。
...
维护不成为第五套概念：用户只需“安全停机维护 → npm 安装 → 运行新 `zz`”。成功时不制造额外状态，失败时只给重试、重新安装当前明确版本或前向安装最新版本中的一个行动，不暴露内部 definition、protocol、operation 或路径。已运行新版本后不承诺降级。停用程序与永久移除设备必须在名称、后果说明、确认和入口上完全分离。

- **当前 S10 npm 交付边界**：根版本是全部公开包与 CLI 版本投影的唯一来源；公开包声明最低 Node 与原生产物目标边界，生产依赖以精确版本发布，CLI 发布 shrinkwrap 绑定 registry integrity。`package:check` 在本地构建、pack、以本轮 tarball 装配并穿过当前平台公共 CLI/subpath/helper 验收；平台 CI 负责多目标构建与检查，不发布。正式 npm 发布命令在用户另行授权后才消费当次认证，要求全目标产物闭包，按依赖拓扑使用候选 tag，所有版本与 integrity 全等后才移动 CLI `latest`。发布脚本不登录、不保存 token，不建设签名、公证、自有索引或通用发布框架。
...
**安全对抗矩阵**（绑定所属实施节点，全部以 fail-closed 自动化测试验收）：当前高熵配对邀请的过期 / 重放 / MITM / single-use 竞争与秘密外泄拒绝，以及短码 PAKE 生产装配审计门（S2）；证书与票据的签发者、受众、scope、expiry 校验与重放拒绝（S2 / S6）；权限快照版本回滚拒绝、过期 fail-closed、断线语义（S4）；吊销传播时限与连接终止（S2）；盲中继保密性（S2 / S6）；确认应答的原始接入面绑定、旁观面不可代答（S6）；权威端口越权——跨会话访问、越权全局写、AuthorityCapability 重放与吊销（S4 / S5）；ResourceLease 伪造 / 跨任务复用 / 父额度超卖 / 过期与崩溃回收 / 重复扣账（S4）。
...
Windows supervisor 的唯一系统读面只接受 numeric state、当前用户有限身份集合与完整 typed trigger/action exact-set，不读取本地化展示。配置换代的安全点进一步固定为 explicit drain 已接受工作；非 managed 配置先关闭 future launch，再等待旧 endpoint 精确终结并按新配置连接，旧代 turnover 不得授权停止后继实例。
...
- **当前 S10 stop successor 与公开错误补充**：anchor/executor 两根先扫描并 strict hydrate 原 operation 的 closed 投影，再在任何 owner 恢复或 listener 公开前，以最终同 home 非零端点绑定同一种 inactive server 对象；OS `listen` 成功是进程执行单飞，失败者在 owner/journal/manager 零效果时退出。只有持有该 exact handle 的 winner 可证明旧 host 已停止并续推；随后 `accepted/gate-closed/work-settled/flushed/ready-to-stop` 只补各自未完成步骤，旧 host terminal 与 exact release 后在同一对象激活既有 ingress、恢复剩余耐久义务，禁止 close/rebind 或第二次 listen。`server.shutdown` prepare 的既有 `RpcAppError` 原样投影，其他异常仅返回固定内部错误与重试同请求行动，零 raw error、零 trigger；不建立耐久 successor claim、独立恢复计划或通用 server lifecycle/错误框架。

### §15 验收纲

体验：S1 后单机全量测试与行为快照零差异；"配对 → 恢复包 → ready → 指定值班"是一条零术语引导流，零认知走查通过；离线排队提示、完成通知、uncertain 呈现、离线继续工作与收编复核、设备撤销与外部轮换旅程零认知走查通过；软件维护统一为“安全停机维护 → npm 安装当前明确版本或 latest → 运行新 `zz` 恢复托管”，失败只给安全重试或前向维护中的一个行动，已运行新版本不提供降级；“移除应用（保留数据）”与“永久移除此设备及本机数据”零误解。结构：不变量 1–18 全部自动化验收。故障与安全：§12 故障矩阵与安全对抗矩阵逐行有测试。产品：全程只见"值班 / 干活"语言，不暴露拓扑术语。性能观测只用于按需诊断，不得替代功能验收，也不得成为开发单元的同步阻断门禁。
