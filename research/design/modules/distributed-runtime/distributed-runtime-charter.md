...
- **当前 S10 生命周期退出边界**：三条退出路径只复用同一物理 `AuthorityCommitLog` 的 `device-lifecycle` 流。临时停机先同时关闭固定 owner exact-set 的全部 producer gate，再冻结 accepted-work artifact；已经耐久接受的 scheduler run 必须在交给外部 activation 前进入冻结投影。各 port 只按同一 operation 的 frozen `id/revision` 逐项收束、回读并 flush，OS signal、CLI/RPC 与 executor-only 都只能经同一 coordinator 到达 `ready-to-stop`；托管实例的物理停止还须全等匹配 endpoint lock、immutable definition 与 manager projection，PID 或 service id 单独不授权。executor removal 的 target winner 与 issuer peer-effect 分别留在现有两根日志：用户决定只消费副作用前展示、gate 后冻结的同一 owner snapshot artifact，`ownerItems` 必须原样进入 local/external settlement；durable authenticated abort 授权同 operation 的幂等 gate release 后 exact replay 原 receipt，ready、cleanup-ready 与 issuer terminal仍恰一前滚。普通 revoked admission 不放宽，只允许历史身份进入固定 terminal-only 分支。backup uninstall 的 service generation 与两次 checkpoint identity 分离，retirement 水位只取其 lifecycle record 所在 commit envelope 的实际 LSN；首个和覆盖该 LSN 的最终 checkpoint 都必须由同一 recovery root 走现有 service 真解封验证。cleanup 只遍历冻结 current-home roots，以固定批次经 storage governor 前滚，并严格按 unregister、非 device-key secret、cleanup-ready、issuer terminal、exact device-key compare-delete、本机 terminal 排序。全部公开 facade 在任何 lifecycle/gate/journal 效果前执行方法级 strict exact-set；不新增通用 manifest、历史 mesh、storage/secret lifecycle 或第二事实源。
