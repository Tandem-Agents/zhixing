...
- **当前 S10 stop successor 与公开错误补充**：anchor/executor 两根在任何 owner 恢复或 listener 前以 closed producer 装配原 operation，并先证明旧 host 已停止；随后 `accepted/gate-closed/work-settled/flushed/ready-to-stop` 只补各自未完成步骤，旧 host terminal 后才 exact release、恢复剩余耐久义务并开放 successor。`server.shutdown` prepare 的既有 `RpcAppError` 原样投影，其他异常仅返回固定内部错误与重试同请求行动，零 raw error、零 trigger；不建立独立恢复计划或通用错误框架。
