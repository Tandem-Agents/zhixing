...
- **当前 S9 备份边界**：trusted-home 的 current anchor 或显式启用备份的单机 current anchor 恰一持有全量 checkpoint owner；它从同一 `AuthorityCommitLog` 冻结完整前缀及仍受保留的权威资产闭包，继续使用唯一 `CheckpointEnvelope.v1` 分块加密到一个物理独立目录或一台 active paired device。每日义务与迁居前强制接缝复用同一耐久 created→replicated→verified 生命周期；恢复包只含用户持有的恢复主秘密，验证必须从真实目标回读并完整解封。`fullBackupReady` 与旧 S2 mesh-ready 分开投影；新代真验证后旧代才 superseded，27 天后经现有生命周期与 storage governor 回收且不误删共享业务资产。首次配对先在受限认证 onboarding link 上写入、回读并验证首份全量 checkpoint，再激活恢复根和开放业务 mesh。planned anchor 迁居、灾难恢复、`TrustTransition`、`ReadyProof`、检查点导入/恢复和全局同步仍未启用，不得由备份管理面或本地域入口提前承载。
