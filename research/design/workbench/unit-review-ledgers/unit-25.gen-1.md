...
- **当前状态**:U25-16～U25-19 已在同一冻结指纹上完成实现、受影响包直接验证、最终交付账本、专项功能审查与四路冷启动对抗复核；四项均已验证，本任务未进入全单元终审或最终验证
- **连续无新增问题轮数**:0 / 2
- **交付物是否冻结**:是（仅用于 U25-16～U25-19 专项功能审查与四路对抗复核；任何生产交付物修改均立即失效）
- **交付物文件集**:以同目录 `unit-25.delivery-manifest.json` 为唯一账本；当前 `pathCount=169`、`deletedCount=0`，完整路径、分组、路径集哈希均只从该文件读取，不再人工抄写
- **当前交付物指纹**:`git-delivery-manifest-v1:df892a12002cf5b7fe4e46f318f7f9b5f7581607aaedf9903c9e8f3bf6b82360`；由 `node scripts/unit-delivery-manifest.mjs --base 994f05c --check research/design/workbench/unit-review-ledgers/unit-25.delivery-manifest.json` 复算通过，`pathCount=169`、分组和为 169、`deletedCount=0`
...
| V25-08 | U25-17 的真实双拓扑组合根与八族逐 case 耐久见证 | CLI ledger/scenario 专项集；descriptor 分离结构检查；验收真实性冷启动复核 | conformance、生产包 test-support、ledger/scenario、生产 descriptor、真实 host/outbox/recovery owner 及对应测试 | 专项直接验证 / 中 / 40.84 秒 | 当前源码八族逐 case 12/12；中央只合并 package-owned scenario，exact-set、typed reason、零写拒绝、真实 reopen、owner/resource 见证、旧映射结构门禁及四路对抗复核均通过 | `git-delivery-manifest-v1:df892a12002cf5b7fe4e46f318f7f9b5f7581607aaedf9903c9e8f3bf6b82360` | 有效 |
...
| V25-10 | 四项受影响包证据、派生资产与最终交付账本 | 复用输入未变的既有包测；运行四项当前直接测试；最终运行 manifest `--write` 后 `--check` | core、owner-kernel、server、CLI 既有包测输入；当前四项直接输入；169 路径最终交付账本 | 专项合并验证 / 中 | 既有包级结果对应生产输入自 07c4794 后未变；本轮当前直接测试全部通过；manifest 生成器 1/1、最终 `--check` 通过，零删除、分组和为 169；专项审查、四路对抗和差异审计无未处置发现 | `git-delivery-manifest-v1:df892a12002cf5b7fe4e46f318f7f9b5f7581607aaedf9903c9e8f3bf6b82360` | 有效 |
