...
- **当前状态**:问题盘点（U25-16/U25-17 专项已验证，待恢复全单元审查流程）
- **连续无新增问题轮数**:0 / 2
- **交付物是否冻结**:否
- **交付物文件集**:第 25 单元起点 `994f05c` 至当前工作区共 159 个非工作台交付路径（CLI 70、core 45、executor 4、orchestrator 6、owner-kernel 7、rpc 2、runtime-host 5、server 11、脚本 6、根配置 1、架构与规格 2）；无删除项；路径集 SHA-256 `4a4e4a5300a25a3456c97ded614f8d2ac6dd03c701daef84d1e0a32362ab3a7c`
- **当前交付物指纹**:`git-delivery-manifest-v1:381b5b9fff9b185bb39e296639d4fa35c6080e309b7e3d3d88b74518e3e61f85`（159 个路径排序后逐行计算 `status<TAB>path<TAB>blob`；现存文件的 blob 取 `git hash-object --path=<path> <path>`，删除项取单元起点 blob；UTF-8、LF、无末尾换行；工作台、单元登记文件和暂存状态不计入）
...
| ------------------------ | ------------------------- | ---------------- | ---- |
| 151 个当前交付路径 | core 导出与 schema、server canonical registry golden、可执行 S7 耐久合同账本、隔离 S1/current 终端性能资产、包级类型、本机管理 owner/outbox 与生产组合根 | 独立审查与冻结准备时逐组落账 | 待核查 |
...
>
> **当前实现核账**：U25-01～U25-15 的既有实现证据绑定旧 133 文件基线，保持“待验证”。U25-16/U25-17 已在当前 159 路径交付物上完成集中修复、受影响包测试、专项功能审查及所有权拓扑、崩溃耐久协议、验收证据真实性三路冷启动对抗复核，状态更新为“已验证”；该结论不代表全单元终审或最终验证完成。
>
> **本轮实施顺序**：`U25-16 → U25-17`。先闭合真实本机管理组合根及 acknowledge 身份合同，再让 conformance 和逐 case 见证复用该生产链；禁止在旧的可选 lease 或隐式 outbox 身份上继续搭验收适配层。
...
| U25-16 | **P0，工作量：中；来源本轮独立审查，补充开发（D25-F08 的显式身份闭包未完成），并吸收同轮 P2 身份加固项；反证后重开。** 默认 `anchor+executor` 在 `topology-command.ts` 已取得 canonical workspace owner lease，但 `command.ts` 把它误传给不接收该字段的 workscene cleanup，构造 `AssemblyContext` 时反而漏传；executor access-surface 因而以 `undefined` 启动 host 并确定失败。另一方面，acknowledge 的请求联合、调用端、exact-key parser 与 outbox API 均未携带 `outboxId`，同水位幂等返回甚至先于任何根身份核对。 | 本机管理组合根的两级身份没有被建模为跨边界的显式必填合同：进程排他身份 `owner lease` 以可遗漏的可选字段在 topology→assembly 间手工搬运，耐久世代身份 `outboxId` 则被留在当前 host/checkpoint 的环境上下文中。两处都把“当前 home 下恰好只有当前根”当作隐含前提，因此属于同一根因。 | 默认单机拓扑在 host ready 前失败，本机 workspace 管理与 catalog recovery 不可用；旧 client/旧 pending page 的确认可能被当前 endpoint 上另一 outbox 解释，确认回执也不能自证明所属根。影响常驻与按需 bootstrap、AssemblyContext/access-surface、host/client/outbox、workspace 命令、角色门控和默认拓扑测试；受影响 IR25-01～02、04、12～13、16、24、26～29。 | 建立单一、角色判别的本机管理装配身份：`executor` 分支必须携带 lease，非 executor 分支必须无 lease；由唯一装配函数从 `ServeBootstrapContext` 生成并贯穿默认 `anchor+executor`、executor-only 与 one-shot host，禁止再把 lease 作为可遗漏字段散传，lease 仍只由最外层在 host 关闭后释放，并删除 workscene cleanup 上的误传。定义同一 exact-key 协议族：ack 请求携带 `outboxId + throughSeq + prefixDigest + entries`，回执携带 `outboxId + throughSeq + prefixDigest`；client 从 pending delivery 原样带入，host/outbox 在幂等回放或水位变更前先反绑 checkpoint outboxId，client 再反绑回执。验收覆盖真实默认拓扑至 host ready、三类角色/入口所有权、旧 outbox 对新 host 的同水位确认、错 outboxId、越洞/回退/错摘要及确认请求/响应丢失，证明零第二 owner、零误确认和零重复副作用。 | 已验证 |
| U25-17 | **P1，工作量：大；来源本轮独立审查，补充开发（D25-F09 的真实逐分支验收闭包未完成）；反证后重开。** 八个生产 descriptor 虽已 exact-set 聚合，但 S7 双拓扑 conformance 原先绕过生产 owner-lock/bootstrap、受信 transport、host/client/outbox；首轮修复已接通这条前半链并移除缓存整包测试，却仍由测试侧 `ScenarioRuntimeBinding` 手写返回 family、reasonCode、recoveryOwner 与 resourceIdentity，case 也直接取自 descriptor 枚举行。执行分支虽有真实断言，见证分类并非从生产终态、typed error 或实际恢复执行中读取；尤其 outbox 场景只实例化低层 outbox，未由声明的 `LocalWorkspaceManagementHost` 恢复 owner 重开。 | 当前验收仍把“分支执行成功”和“该分支属于哪个耐久 case、由谁恢复、绑定什么资源”分成两套来源：生产调用产生证据文本，测试元数据补齐用于过账的身份。只要断言片段仍成立，错误 owner、错误资源分类或 reason 漂移仍可能由测试元数据遮蔽；这仍是见证面与被验面未真正分离的同一根因。 | 任一记录族的生产分支缺失或错绑、错误码漂移、validator/reducer 绕过、日志未重放、recovery owner 未装配，乃至双拓扑本机管理前半链完全缺失时，门禁仍可能通过；影响八个记录族、S7 in-process/mesh conformance、耐久账本、scenario adapters、结构与变异验收；受影响 IR25-22～23、26、32。 | 两种拓扑继续复用已接通的生产 owner/host/client/outbox 前半链，并只在锚点业务事实耐久后确认 delivery。逐 case 见证改为由执行函数返回生产观察：variant 从真实耐久记录/终态反解 family、kind、case 与 reason，rejection 从真实 typed error/diagnostic 读取稳定 code 并证明零耐久写，corruption 只在存储边界定向变异后由生产恢复入口读取 fail-closed code；producer 取实际调用对象/函数，recoveryOwner 只有实际重开调用完成才可登记，resourceIdentity 从本轮真实 stream/log/path/key 规范化取得，outbox 全分支必须经 `LocalWorkspaceManagementHost` 恢复闭包。descriptor 只在 harness 侧提供期望并做 exact-set/字段对账；删除 `ScenarioRuntimeBinding`、reason 前缀公式和所有测试侧 owner/resource 预填。结构与逐点变异必须证明改 descriptor、跳过 producer/recovery、换错资源或伪造证据均失败。 | 已验证 |
...
| V25-07 | U25-16 的角色门禁、唯一 workspace host/outbox、稳定消费凭据、下游幂等交付与 reset 同身份确认 | CLI 六文件合并专项集；专项功能审查与所有权/崩溃协议冷启动复核 | bootstrap、host/outbox、workspace command、daemon/access/executor 装配、默认拓扑及对应测试 | 专项直接验证 / 中 / 36.60 秒 | 六文件 30/30 通过；全仓生产调用图只有默认组合根、executor-only 与 one-shot 三类合法入口，executor 身份必携 lease 且 host 关闭后外层才释放；ack 请求、回执和 checkpoint 在任何同水位短路前均反绑 outboxId，错根、越洞、回退、错摘要与丢响应闭合 | `git-delivery-manifest-v1:381b5b9fff9b185bb39e296639d4fa35c6080e309b7e3d3d88b74518e3e61f85` | 有效 |
| V25-08 | U25-17 的真实双拓扑组合根与八族逐 case 耐久见证 | CLI 六文件合并专项集；descriptor 分离结构检查；验收真实性冷启动复核 | conformance、ledger/scenario、生产 descriptor、真实 host/outbox/recovery owner 及对应测试 | 专项直接验证 / 中 / 36.60 秒 | 合并专项集 30/30、其中耐久账本 12/12 通过；in-process/mesh 均经真实 owner/host/client/outbox 后进入业务链；scenario adapter 零 descriptor 导入、零 reason 前缀公式、零字符串 owner 预填，八族 independent executable set 与 descriptor exact-set 全等，真实 producer/recovery/resource 及伪造变异均受检 | `git-delivery-manifest-v1:381b5b9fff9b185bb39e296639d4fa35c6080e309b7e3d3d88b74518e3e61f85` | 有效 |
