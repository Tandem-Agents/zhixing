...
- **当前状态**:U37-01、U37-07 已在专项冻结指纹上完成实现、最小必要验证、F37-29～F37-36 功能审查与四路冷启动对抗，均更新为已验证；U37-02～U37-06 的既有结论继续复用。修复影响的独立审查节点已映射为 27 项 `[~]`，其余 13 项 `[x]` 保持不变；未进入全单元终审或提交验证
- **连续无新增问题轮数**:0 / 2
- **交付物是否冻结**:是（仅冻结 U37-01/U37-07 本批专项交付物；未进入全单元冻结准备）
- **交付物文件集**:本批 20 个生产、公共合同、直接测试及分布式运行时架构/规格文件；全单元既有闭包继续复用，过程账本与独立审查清单不计入交付指纹
- **当前交付物指纹**:`sha256:e31735041aacd970a80acc92b7105849cd012709d272cf244e1a7989a9a97f76`（U37-01/U37-07 修复后专项冻结）
...
| ---- | ---------- | -------- | ---------- | ---------------------- | ---- |
| U37-01 | **P0/大，同根再次重开。** `AuthorityDeliveryPipeline.closeAdmissionForLifecycle()`只停止 transport attempt；`DeliveryAuthority.prepareEnqueues()`仍可在共享 `coordinate()` 内接受七类 canonical enqueue，现有直接测试也证明 quiesce 后可追加第二项。stop/removal把前者误作 producer gate，且 delivery read-back允许非终态通过。**价值裁决记录：**U37-01 已闭合 frozen identity、executor-only与exact-host；C37-C07满足重开条件。C37-C09证明原描述漏掉 staged/status/control等七类source，C37-C10证明delivery没有通用取消事实，故把“cancel写delivery取消”收窄为“取消上游可取消producer，既有delivery按不可取消义务收束”。backup十owner顺序仍是独立U37-07。保持现状或改弱停机/移除合同都会丢已接受结果；复用现有 lifecycle operation/artifact、共享delivery串行边界与严格source companion谓词即可，P0/大与方案比例成立。仅当生产事实证明canonical authority在lifecycle barrier后已拒fresh source且所有causal delivery在ready前共同封闭时关闭；不得恢复第二journal或通用lifecycle。最终结论：改写并保留；用户体验达标、架构达标。 | transport attempt quiesce与canonical producer admission没有独立事实边界；lifecycle artifact也未冻结可继续产生delivery的exact causal source，并据现有source companion把迟到delivery并入同operation直到producer与delivery集合共同封闭。 | **生产端**：`job-result`、`staged`、conversation final/status、job status、scheduler notice、conversation control response七类source transaction，`OwnerDeliveryParticipant`/`DeliveryAuthority`、pipeline、stop ports与两类可达target removal/migration。**类型组合**：三策略×三形态，fresh/frozen source，enqueue前后、attempt前后，queued/retry/attempting/uncertain/terminal，两target生产根、迁居、效果/响应丢失和连续重启；lost-device诚实unknown主链不在本项。**消费者**：`ready-to-stop`、`authority-settled/revocation-ready`、revoke/cleanup及公开结果。**异常终态**：fresh delivery越过冻结或causal delivery未终结即停止/移除；cancel伪造不存在的delivery终态；重启重放错误phase。**审查/测试**：IR37-01、IR37-08～IR37-10、IR37-12、IR37-15～IR37-18、IR37-20、IR37-28、IR37-31～IR37-32、IR37-34、IR37-36、IR37-38、IR37-40；真实七类source envelope、authority log、三策略与两类可达target根。 | 在现有 `DeliveryAuthority.coordinate()` 串行边界增加窄 operation-scoped admission：上游gate关闭后，把lifecycle artifact中的exact causal source `kind+id+revision`作为唯一许可；同source同intent的既有重放允许，fresh/冲突source在父envelope append前拒绝。七类source继续用现有 companion exact谓词反绑，允许source产生的delivery自动进入同operation集合；逐项回读causal producer不再产出后，在同一串行边界封闭许可并捕获当前及迟到delivery。pipeline仍只控制transport：immediate等待active attempt并回读queued/retry/uncertain等耐久可重放态；drain驱动至现有terminal；cancel只取消上游可取消工作，已enqueue delivery按不可取消义务走drain，uncertain用既有用户决议阻断；可达removal/migration要求全量terminal且无新项才ready，lost主链仍如实unknown。gate只由现有operation+artifact恢复，重启在公开producer admission前重装。验收穿过真实七类source、共享log与pipeline，覆盖三策略、三形态、两类可达target、迁居、每个sync/响应窗和连续重启，证明fresh零append、causal delivery恰一归集、后继generation零误处理；不得新增journal、manifest、source registry或通用lifecycle。 **修复后证据：** `DeliveryAuthority` 已在共享串行边界安装 operation-scoped source permits、持久化 item lifecycle binding、捕获冻结时及迟到 pending delivery，并在 causal owner 终结后 seal；七类 `OwnerDeliveryParticipant`/scheduler producer 均进入该边界，pipeline 保持纯 transport 且按 immediate/drain/cancel 收束。stop 与两类可达 removal/migration 只消费同 operation read-back；core 84/84、owner-kernel 15/15、CLI stop/removal/uninstall 17/17、S7 20/20 与全工作区构建通过，F37-29～F37-32 在冻结指纹复核通过。 | 已验证 |
...
> **最新证据有效性说明：**C37-C07 已证伪 F37-21/F37-22 对 delivery producer gate 与 stop/removal 交界的“已闭合”结论；C37-C08 已证伪 backup checkpoint/cleanup 历史四路中“安全停机后清退”的结论。U37-02～U37-06 及其未被两项触发的证据继续有效，不得用历史专项通过关闭 U37-01/U37-07。
...
### U37-01/U37-07 四路冷启动对抗复审（修复后同一指纹）
