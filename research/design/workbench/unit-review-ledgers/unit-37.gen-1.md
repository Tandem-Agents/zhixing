...
| ---- | ---------- | -------- | ---------- | ---------------------- | ---- |
| U37-01 | **P0/中，同根再次重开（P37-20转入）。** 当前生产调用图证明：anchor/executor两根虽在启动时扫描active stop并以closed参数装配，但local owner在`recoverAcceptedWork=false`时仍无条件执行`recoverReadinessProjections()`，mesh也在该门禁前续推既有恢复链；`isHostStopped()`的current-successor分支只验证本进程自构造endpoint/PID。项目唯一真实进程owner是更晚发生的`runServer()` OS端口listen，PID文件明确不是锁，故两个新PID可在端口仲裁前同时消费同一operation/phase并产生owner效果。**价值裁决记录（P37-20，完整转入）：**原独立审查结论保留P0/中并提出向`device-lifecycle`流新增耐久successor claim；反向核对证明operation、accepted-work artifact与phase已完整保存恢复游标，新增claim会复制运行期所有权、引入claim死亡/换代判定且仍需处理Windows PID复用，成本高于问题价值。保持现状或后置会保留真实双successor窗口，修正规则会牺牲逐阶段安全恢复，单靠既有journal/owner幂等不能约束外部效果前的跨进程执行；但现有OS端口listen已提供内核级恰一owner与崩溃自动释放。新决定改写方案、保留P0/中：前移并复用该单飞，不新增耐久claim。只有新生产事实证明同home实际端口不能在inactive ingress下前置占有，或权威范围要求无本地监听的恢复执行者，才重开耐久claim/新抽象方案。最终结论：改写并保留；用户体验达标、架构达标。**历史记录（保留，不覆盖本轮裁决）：** **P0/中，同根再次重开（P37-18转入）。** `HostStopCoordinator.resumeActive()`只续推与当前host全等的operation，异PID只会在旧operation已达`ready-to-stop`且旧host已停止时补terminal；anchor虽在producer恢复前读取active operation，却把它折叠成`artifactReady+delivery.sealed`并在装配后丢弃resume结果，executor-only则先启动local/job owner才调用同样被丢弃的resume。两根都没有把`strategy+phase+already-settled`恢复成可执行续跑，local settled标记又只存在内存。**最新价值裁决记录：**原P37-18评为P0/大并提出独立“恢复计划”；反向核对证明现有operation、accepted-work artifact、phase、coordinator与ports足以唯一表达恢复，不需要新抽象或大范围lifecycle改造，故改写保留P0、工作量降为中。保持现状会让三形态逐阶段崩溃恢复失败；改规则或仅补测试不能消除真实启动故障。只有新事实证明现有phase/artifact无法表达受支持终态或同根扩展到其他lifecycle，才重开“大工作量/新抽象”主张。最终结论：改写并保留；用户体验达标、架构达标。**本轮根因收敛记录：**原影响面漏列artifact尚未形成的`accepted`，且把旧host停止证明只放在补terminal处；源码证明`accepted`同样会在新PID永久悬置，而successor在旧host仍可能运行时不得先结算其owner。新决定把五个非终态phase全部纳入，并要求异host在任何owner效果前先以完整endpoint/definition/manager投影证明旧代已消失或已由当前exact successor取代；`work-settled+`只恢复already-settled，terminal后才把剩余耐久义务作为successor正常恢复输入。评级/工作量不变；只有出现无法用现有phase和根内release原语表达的生产终态，才重开独立恢复计划。**历史已验证子问题（不得恢复）：**delivery admission/sealed、七类source exact revision、local/relay namespace与migration共同闭包已由F37-37～F37-42、C37-C16及专项复审证明；原P0/大的scheduler准备泛化主张已被价值裁决否定。 | closed producer admission只约束fresh work，不构成跨进程执行owner；真实OS端口单飞建立在首个effectful owner恢复之后，导致同一耐久phase缺少先于全部owner/journal/manager效果的exact运行期线性化点。 | **生产端：**anchor `command.ts`、executor-only runtime、`HostStopCoordinator.resumeActive()`、local conversation readiness/lifecycle projection、mesh/worker恢复、server `runServer()/listen`与exact-host read-back。**类型组合：**三策略×managed/on-demand/foreground，`accepted/gate-closed/work-settled/flushed/ready-to-stop`，十owner空/非空、两个并发successor、扫描有/无active operation、terminal→activate窗口、owner取得前后崩溃、generic close、各sync/响应窗口与连续重启。**消费者：**phase settlement/flush/terminal、accepted-work恢复与producer reopen、manager current-successor判断及后续stop/removal/uninstall。**异常终态：**端口失败者已重复执行对话/job或lifecycle projection、交错推进phase、误release，或winner刚terminal但未activate时后到进程扫描无active并产生第二组装配效果；`work-settled+`也可能发生二次composition-root效果。**测试与受影响审查项：**须穿过anchor/executor真实composition root与真实OS端口竞争；IR37-07～IR37-12、IR37-32、IR37-34～IR37-36、IR37-38、IR37-40。**历史边界：**U37-01已验证的delivery admission/sealed、七类source exact revision、local/relay namespace、migration共同闭包及U37-02～U37-11结论不重做。 | 复用现有server端口owner并做窄两段启动：两根先扫描active stop（存在时strict hydrate并形成closed投影），随后所有production启动无论扫描有无active operation，都在MCP/authority/workspace/data-plane/job/local/pre-server装配前以最终同home非零endpoint建立inactive-ingress OS owner；竞争失败者在任何owner/journal/manager效果前退出。winner仅以`recoverAcceptedWork=false`的真正无效果closed assembly装配，local readiness/lifecycle projection与mesh/worker恢复须延后到旧host proof成功后的`resumeActive()`；current-successor反绑同一bound handle、current endpoint lock、旧endpoint、definition与manager投影。terminal与同operation exact release后才在同一server对象激活既有REST/RPC/bridge并写发现/ready投影，禁止第二次listen或close/rebind；失败关闭同一handle，崩溃由OS释放，后继重读当前耐久事实再竞争。验收覆盖两根、三策略/三形态、五phase、十owner、双successor、有/无active扫描、terminal→activate、owner后崩溃接替、generic close、效果/响应丢失与连续重启，证明owner前零效果、任一时刻恰一执行者、inactive ingress零分派、`work-settled+`零二次效果且失主后唯一前滚；不得新增第二journal/锁、耐久claim、通用server lifecycle/coordinator或改动已验证delivery/source边界。**修复后证据：**anchor/executor均在首个owner效果前取得同一最终非零home端点的inactive server handle，loser以`EADDRINUSE`零效果退出；closed local/mesh恢复延迟到旧host proof成功后的原phase续推，terminal/exact release后在同一对象激活。共享exact endpoint谓词修复C37-C26，server/CLI直接闭包36/36、S7 20/20、最终workspace build 17/17通过，专项指纹为`sha256:b3cf4b54249035744432e2dbd2f1a9260687c102f9114fac9200de0cffd2dee4`；F37-64～F37-71与四路对抗同指纹核销。 | 已验证 |
...
| U37-06 | **P2/小，同根再次重开并完成收敛。** 既有server `device.*`/`server.uninstall.*`及removal mesh request的exact-key修复继续有效；`server.shutdown`仍把原始params强转DTO，unknown字段被忽略，错类型/空reason及非正timeout被静默改成默认值，`NaN/Infinity`等数值还可能在prepare后生成ETA时失败。**历史价值裁决记录：**原P1/中曾把strict输入、S7和全部证据充分性合并；因无越权/错终态且泛化S7会重复维护，已收窄为facade strict输入P2/小。**本轮价值裁决记录：**原P37-16把loopback授权与strict参数共同评为P1；反方复核确认malformed输入不构成独立权限问题，授权拆入U37-10，本项维持P2/小。C37-C20只补齐同根的“效果后格式化失败”窗口，不恢复旧评级：入口已由U37-10限于认证本机连接，当前合法停机不受影响，最小修复只复用现有helper。若畸形字段可绕过身份/策略或造成不可恢复效果，再重开评级。最终结论：改写、合并并保留；用户体验达标、架构达标。 | Unit37公开request的plain-object/exact-key谓词没有覆盖`server.shutdown`，且默认化、范围校验与响应投影没有在首个生命周期效果前形成一个完整strict decode边界。 | **生产端：**`buildServerShutdownMethod()`、`asRecord/assertExactRecord/stableText`及现有strategy decoder；既有device/uninstall/mesh修复不重做。**类型组合：**合法省略、unknown、缺失requestId、reason/strategy/timeoutMs错类型、空值、非有限值、非正值、ETA不可表示值与合法重放。**消费者：**loopback shutdown lifecycle、trigger及稳定invalid-params行动。**异常终态：**调用者误以为畸形字段生效而实际按默认值停机，或prepare已耐久后响应投影失败；合法路径、安全和既有终态不变。**受影响审查项：**IR37-35、IR37-36、IR37-40。 | 在U37-10 loopback guard之后、读取lifecycle/trigger之前，以`asRecord`取得plain object；以必填`requestId`加“仅当实际出现时才加入”的`reason/strategy/timeoutMs`构造exact key-set并调用`assertExactRecord`。`requestId`继续用`isProtocolIdentifier`；出现的reason必须经`stableText`，strategy必须为既有三枚举，timeout必须为有限正数且`Date.now()+timeoutMs`可生成合法ETA；省略时才使用现有默认。先完成全部decode并预计算ETA，再调用prepare与trigger。验收覆盖unknown、缺失、错类型、空值、`NaN/Infinity`、非正/不可表示timeout及合法省略/60s重放，证明非法输入零prepare/零trigger且合法请求、默认值与三策略不变；不得重做已验证facade、扩S7或新增codec框架。**修复后证据：**`buildServerShutdownMethod()`已在wiring/prepare前依次完成loopback guard、plain-object/exact-set解码、字段类型/范围校验与可表示ETA预计算；参数化直接测试覆盖unknown、缺失、错类型、空值、`NaN/Infinity`、非正及不可表示timeout，全部证明零prepare/零trigger，合法默认与三策略保持原语义。F37-52～F37-55、C37-C20及四路专项复审在当前专项冻结指纹上通过。 | 已验证 |
...
| U37-09 | **P1/中，新增并完成收敛。** OS signal固定以`immediate/30s`进入stop coordinator；但local conversation对三策略都把pending final/assignment/intent与active work清空，remote job owner也无条件归零，只有delivery已正确区分`immediate`。进一步正向核对十owner后，C37-C19证明“所有非delivery义务均可留存”同样错误：channel连接、scheduler completion tracker、lease、permit及当前conversation/job/inbound物理步骤必须在ready前归零，只有conversation identity与冻结的intent/final/assignment/remote耐久义务可作为exact frozen子集留存。**价值裁决记录：**原P37-15把delivery和CLI stop列入影响；delivery已正确、CLI显式stop使用`cancel`，二者排除。原U37-09方案把scheduler也概括为可耐久留存；scheduler当前事实是进程内活跃completion tracker，不能作为重启安全点，故本轮收窄。保持现状会使OS安全停机持续blocked；把signal改成cancel或允许物理owner留存都会牺牲体验/安全，按owner事实复用现有ports是最小完整方案，P1/中与工作量中成立。修复后仅在可留存的耐久义务仍被强制归零、活动物理/资源事实未归零却ready，或drain/cancel被弱化时重开。最终结论：改写并保留；用户体验达标、架构达标。 | 非delivery owner settlement/read-back把`immediate`的“停止活动效果并将已接受义务留在exact可重放事实”与`drain`的“义务全部终结”合并；同时缺少按owner事实区分“可耐久留存”与“必须物理归零”的唯一判据。 | **生产端：**OS signal与显式immediate、anchor/executor `HostStopCoordinator`、同一frozen artifact/ports、`LocalConversationOwner`、inbound router、executor job owner、channel、scheduler、lease/permit；delivery与CLI cancel不重做。**组合：**anchor/executor，managed/on-demand/foreground，十owner空/非空，conversation active/idle，intent/final/assignment/remote exact frozen子集，channel/scheduler/lease/permit及job/inbound活动，gate→freeze→settle→read-back→ready各窗、timeout、效果/响应丢失和连续重启。**消费者：**`ready-to-stop`、exact host stop与supervisor重驱。**异常终态：**耐久义务被要求归零导致SIGTERM/SIGINT超时blocked，或活动效果/资源仍存在却过早ready。**受影响审查项：**IR37-09、IR37-34～IR37-36、IR37-40。 | 不改coordinator、artifact或port签名，所有port继续消费同一`operationId+strategy+frozen id/revision`。`immediate`先关准入并等待当前inbound、conversation turn、job/recovery attempt到既有耐久安全点；local read-back只允许conversation identity及intent/final/assignment中仍与frozen id/revision全等的子集留存，remote只允许同样全等的耐久job/relay子集留存；channel连接、scheduler completion tracker、lease与permit必须为零。`drain`仍要求全部owner终态，`cancel`仍只取消可取消上游并等待不可取消/已enqueue义务。验收穿过anchor/executor真实port、OS signal与显式immediate，覆盖十owner空/单/多、active→safe、冻结子集、successor、timeout、响应丢失和连续重启，证明immediate可ready且义务可恢复、活动事实归零、drain/cancel不回退；不得改delivery、CLI stop或新增lifecycle框架。**修复后证据：**local owner的`immediate`只停止recovery loop并等待active conversation/lease归零，read-back按strategy只允许frozen全等intent/final/assignment子集；command与executor远端投影同样只允许frozen exact子集，channel/scheduler/lease/permit仍要求零。专项测试穿过真实deferred intent与受控active turn，证明安全点前阻塞、安全点后保留耐久子集；实现期C37-C21还证伪并修正了generic close二次drain。F37-47～F37-50、C37-C19/C37-C21及四路专项复审在当前专项冻结指纹上通过。 | 已验证 |
| U37-10 | **P1/小，新增并完成收敛。** `server.shutdown`只声明`requiresAuth`，而连接已有可信`loopback`投影且`ServerConfig/ServeOptions.host`允许非loopback监听；handler在解析参数、取得lifecycle与prepare前没有方法级本机授权。**价值裁决记录：**原P37-16把loopback与strict参数共同评为P1；默认host虽为loopback且调用者须认证，但非loopback是受支持配置，home成员身份不能授权停止当前物理host，故权限根因独立保留P1/小；strict输入并入U37-06。保持现状或仅修文档会保留可达远程停机，新增通用授权框架又超过问题价值；复用同文件既有local uninstall loopback guard为最小完整修复。修复后仅在认证非loopback连接仍能到达参数细节、lifecycle prepare或trigger时重开。最终结论：改写并保留；用户体验达标、架构达标。 | 破坏性的本机生命周期方法漏接连接层既有loopback guard；router认证只证明home成员，不能授权远程停止该连接所达的物理host。 | **生产端：**RPC router `requiresAuth`、`buildServerShutdownMethod()`、socket连接loopback投影、默认/非默认监听与shutdown lifecycle adapter。**组合：**unauthenticated、authenticated loopback/nonloopback，合法/畸形params，prepare前/后、trigger前/后与同request重放。**消费者：**当前host lifecycle、exact stop和公开RPC稳定行动。**异常终态：**认证远端成员可耐久接受并停止本机host，或权限拒绝晚于参数/生命周期效果。**受影响审查项：**IR37-34～IR37-36、IR37-40。 | 保留router的`requiresAuth`作为第一层；在handler入口、参数解码和任何server/lifecycle读取或效果之前复用`ctx.connection.loopback`，非loopback统一返回既有local-only/invalid-action错误且零prepare/零trigger；通过后再执行U37-06 strict decode与原coordinator。直接测试覆盖未认证由router拒绝、认证非loopback对合法/畸形输入同样零效果、loopback合法三策略与request重放、默认/非默认监听。不得新增授权框架、改变mesh/lifecycle主链或远程管理范围。**修复后证据：**handler首句已复用`ctx.connection.loopback`，认证非loopback在params、server wiring、prepare与trigger之前统一拒绝；真实WebSocket集成证明未认证仍由router先拒绝，handler直接测试证明认证非loopback对合法与畸形输入均零prepare/trigger，loopback合法重放与三策略不变。F37-51/F37-54/F37-55及四路专项复审在当前专项冻结指纹上通过。 | 已验证 |

| U37-11 | **P2/小，新增（P37-19转入）。** `server.shutdown`的blocker/owner/flush路径可抛普通`Error`，共享dispatcher把原始message写入公开`data.message`；现有直接测试只断言handler内部抛错，没有穿过真实wire投影。**价值裁决记录：**原P37-19评级为P2/小，但方案同时修改共享`toJsonRpcError()`；反向核对未发现当前消息必含秘密，也无事实支持扩展到全部RPC。新决定为改写保留P2/小，方案只落在shutdown facade。保持现状会保留明确的失败体验违约，但不破坏停机耐久性，故不得升为P0/P1；只有真实证据证明其他公开方法同样可达raw泄漏或现有消息包含敏感数据，才重开共享dispatcher或更高评级。最终结论：改写并保留；用户体验达标、架构达标。**本轮根因收敛记录：**生产`requestShutdown`是ready后排入微任务的`void`幂等清理触发器，不存在可由本次wire响应承诺的异步失败；当前也没有可稳定区分blocker/owner/flush的typed domain error，按原message分类会重新耦合内部文本。新决定只投影`lifecycle.prepare()`同步返回的失败：既有`RpcAppError`原样保留，其他异常统一包装为固定`INTERNAL_ERROR`、固定中文message与固定非敏感重试行动，原message永不进入wire；trigger成功语义与共享dispatcher均不改。评级/工作量不变；只有新增稳定typed failure合同或真实其他RPC raw泄漏证据，才重开分类/共享修改。 | `buildServerShutdownMethod()`没有在shutdown输出边界收束`lifecycle.prepare()`异常，普通错误因此落入共享dispatcher的raw-message后备路径；这不是U37-06的输入strict decode根因，也不是异步cleanup失败协议。 | **生产端：**`buildServerShutdownMethod()`、shutdown lifecycle `prepare()`、`RpcAppError`、真实RPC registry/dispatcher/error projector。**类型组合：**验证/未装配的既有`RpcAppError`，blocker、owner settlement、flush及未知普通异常，prepare耐久写前/后/响应丢失、合法loopback重放与ready后void trigger。**消费者：**CLI/本机RPC调用者及公开稳定行动。**异常终态：**停机保持原耐久phase但调用者收到内部英文实现文本、错误形态不稳定；合法ready/trigger与耐久性不变。**受影响审查项：**IR37-33、IR37-34、IR37-36、IR37-40。 | 保持共享dispatcher、其他RPC和trigger不变。只在`buildServerShutdownMethod()`包住`lifecycle.prepare()`：`RpcAppError`原样抛出；任何其他值统一转成现有`RpcAppError(INTERNAL_ERROR)`，公开message固定为`安全停机未完成`，data固定为`{ action: "retry-same-request" }`，绝不复制原message。映射失败时不得排队trigger；prepare返回ready后仍按原顺序排队一次void trigger，合法同request重放继续由既有lifecycle/cleanup幂等吸收。验收穿过真实`HandlerRegistry→RpcDispatcher→connection.sendError`，分别注入blocker/owner/flush/unknown及各耐久窗口，断言exact code/message/data、零raw、失败零trigger；验证既有invalid/unwired `RpcAppError`与合法三策略重放字节语义不变。不得修改共享dispatcher、其他RPC或增加错误/codec框架。**修复后证据：**`buildServerShutdownMethod()`只包住`lifecycle.prepare()`：既有`RpcAppError`原样抛出，其他值统一固定`INTERNAL_ERROR/安全停机未完成/{action:retry-same-request}`，失败零trigger，ready后原void trigger不变。真实registry/dispatcher wire与方法合同57/57、server类型检查、S7结构门禁及workspace构建通过；F37-61～F37-63逐格复核通过。 | 已验证 |

> **U37-01本轮根因收敛记录（并入该问题）：**P37-20命中的不是closed admission或phase恢复本身，而是跨进程执行owner建立得太晚：anchor的pre-server装配与executor的MCP/authority/workspace/data-plane/job/local owner启动均早于唯一OS端口`listen`。F37-64～F37-71据此把所有production启动（scan有/无active stop均同样）收紧为同一非零home端点、同一bound server handle的`scan(+active时strict hydrate)→inactive bind→assembly→active时exact proof/resume→原对象activate`；否则一个winner刚写terminal、尚未activate时，后到且扫描到“无active”的进程仍会走旧晚bind路径并提前产生效果。active stop分支的closed assembly中，`recoverAcceptedWork=false`必须覆盖readiness/lifecycle projection与mesh/worker恢复，首个此类效果只能由`resumeActive()`在旧host proof成功后触发。端口失败者在全部owner/journal/manager效果前退出，异常关闭同一handle，崩溃由OS释放。该记录只细化U37-01现有P0/中方案，不改变其价值裁决、评级、工作量或已验证子边界。

> **最新证据有效性说明：**P37-20对F37-56～F37-60的证伪记录继续保留；当前U37-01结论只绑定修复后专项指纹`sha256:b3cf4b54249035744432e2dbd2f1a9260687c102f9114fac9200de0cffd2dee4`及下方F37-64～F37-71修复后核销。C37-C23的根内proof顺序与C37-C24的advancement补恢复继续复用，U37-11的F37-61～F37-63及U37-02～U37-10其余结论未被触发。原12个`[!]`不得由修复验证直接改为`[x]`，已统一改为`[~]`等待独立重审。

### U37-01 successor进程owner根因收敛固定矩阵（当前）

> 本矩阵只冻结U37-01当前根因、最优方案与直接验收，不代表实现或测试通过；复审对象为上方同一份未修改的U37-01正式记录。

| 编号 | 直接变体 | 稳定身份、唯一事实与线性化点 | 零副作用边界、消费终态与直接验收 |
| ---- | -------- | ---------------------------- | ---------------------------------- |
| F37-64 | anchor/executor-only × managed/on-demand/foreground；`accepted/gate-closed/work-settled/flushed/ready-to-stop`；十owner空/非空 | `device-lifecycle operationId+strategy+phase+accepted-work artifact`是唯一耐久恢复游标；`resolved non-zero home host+port`的同一实际HTTP server handle是唯一进程执行owner，OS `listen`成功是跨进程线性化点，PID/state文件只作发现 | 两根只可在仲裁前扫描active operation、strict hydrate原artifact并形成closed投影；任何owner恢复、journal advance、manager效果与公开ingress均晚于成功bind。逐phase生产根测试须证明端口失败者效果计数恒零 |
| F37-65 | 单/双successor，同/异启动入口，同home竞争与异home并行，有/无active operation，端口取得成功/`EADDRINUSE` | anchor在`setupAssemblyUnits`前、executor在`mcpHub.connectAll/setupAuthorityRuntime/localWorkspaceHost/dataPlane/job/local owner start`前争用同一home端口；无active operation也走同一早期bind，防止另一winner刚写terminal但尚未activate时产生第二执行者；异home使用各自端点独立仲裁 | 同home只有winner继续装配，loser立即走原启动回滚且零owner/journal/manager效果；异home互不阻塞。真实双进程须覆盖terminal→activate窗口并分别穿过两条production composition root，不能用进程内布尔量或mock owner代替 |
| F37-66 | inactive ingress期间HTTP/WS、bind后恢复失败、terminal/release后激活、激活前后generic close | server包只把现有`createServer+listen`窄拆为inactive bind与同对象activate；bind handle同时是endpoint lock来源，禁止第二次listen、close/rebind或另设端口锁 | inactive HTTP稳定拒绝且WebSocket upgrade不进入router/auth/handler，零业务效果；恢复失败关闭同一handle，成功则在terminal与exact release后原子挂接既有REST/RPC/bridge并写PID/ready投影。测试须证明激活前零请求分派、激活后既有字节语义不变 |
| F37-67 | 三策略、五phase、十owner、freeze/settle/flush/physical/terminal/release各效果与响应丢失 | 只有持有exact bound handle的winner可调用`isHostStopped()`与`resumeActive()`；current-successor证明同时反绑该handle、当前endpoint lock、旧endpoint、definition与manager投影，phase仍决定唯一未完成步骤 | `accepted`只补gate/freeze，`gate-closed`只按原artifact与strategy settle，`work-settled`只flush，`flushed`只physical，`ready-to-stop`只terminal；`work-settled+`不重做owner效果，错operation/stale completion零release，三策略安全点与exact-host stop不回退 |
| F37-68 | old PID仍活/已死/PID复用，managed manager running/stopped/不可判定，foreground/on-demand，current successor | 旧host停止证明与当前执行owner是两项独立事实：旧endpoint必须完整不等于current handle且旧代已停止；managed再验canonical definition与manager projection，current-successor只可指向当前bound handle | 旧PID仍活、PID复用、definition漂移或manager不可判定均保持closed并关闭当前inactive handle，零owner/terminal/manager stop；旧operation不得以service id或自身PID停止winner |
| F37-69 | winner bind后、owner效果前/后、journal sync前/后崩溃；一个或多个后继；连续重启 | 进程死亡由OS释放同一socket；后继重新扫描原operation/artifact/phase并重新竞争，不新增耐久claim。既有phase幂等与exact read-back吸收效果成功但响应丢失 | 失主期间端口不可被两个进程同时持有；后继只补原phase未完成步骤，terminal后的根内release/普通耐久恢复沿用已验证合同。真实child-process故障注入须证明最终唯一前滚且无二次`work-settled+`效果 |
| F37-70 | scan/strict hydration失败、多active operation、端口0测试接缝、非默认内部host/port | production两根只以各自已解析的同一非零home endpoint参与owner证明；端口0的一次性server单测不提供lifecycle owner证据，受控覆盖必须给竞争进程同一非零endpoint | scan/hydration/identity失败发生在bind前并零效果；配置端点不全等不得共享operation。验收固定同home同endpoint竞争和异home并行，禁止用两个`port=0`实例伪造单飞通过 |
| F37-71 | U37-01与U37-02～U37-11、第30～36、第38边界；保持现状/弱化规则/现有机制/新增claim比较 | 方案只前移既有OS端口owner并让`runServer()`消费同一handle；operation/artifact/phase、十owner ports、delivery/source、migration、三策略与supervisor事实均不复制 | 保持现状会让受支持恢复产生并发owner效果或永久闭门，P0/中成立；耐久claim、第二journal、独立恢复计划和通用server lifecycle成本更高且未解决PID复用。直接验收仅覆盖U37-01影响闭包，不引入S7 registry、新runner或Unit38能力 |

### U37-01 四路冷启动对抗复审（同一未修改问题列表）

| 复审角色 | 主动反例与直接交界 | 结论 |
| -------- | ------------------ | ---- |
| OS端口owner与首个owner效果排序 | 从零并发启动同home anchor/executor successor，分别把local readiness、mesh、MCP、authority setup、journal advance放在listen前，尝试以PID文件、closed admission或当前进程存活代替owner | 通过：F37-64～F37-66明确只有同一非零home端口的OS bind能在线性化后授权效果；closed投影只约束fresh admission，PID只作发现，失败者在全部owner/journal/manager效果前退出 |
| 五phase/三策略/十owner双successor隔离 | 对每个phase注入十owner空/非空、immediate/drain/cancel、old/current endpoint、效果成功响应丢失及两个winner候选，尝试让`work-settled+`再次恢复owner或用旧operation停止current successor | 通过：F37-67～F37-68把执行资格反绑exact handle，phase只补未完成步骤；三策略安全点、delivery/source与exact-host边界直接复用，未产生逐phase或逐owner补丁 |
| owner崩溃接替与inactive-ingress边界 | 在bind后、proof中、owner效果后、terminal/release前后杀死winner，连续启动后继；在terminal后另起扫描为无active的进程，同时向inactive HTTP/WS发请求并尝试通过close/rebind制造无owner窗口 | 通过：F37-65/F37-66/F37-69要求所有production启动先争用同一handle且从bind持有到activate/close，terminal不释放socket，inactive请求零分派，只有进程崩溃/失败才由OS释放；后继重读当前耐久事实再竞争，既无第二listen窗口也无需耐久successor claim |
| 生产体验、范围价值及历史裁决边界 | 重新比较保持现状、取消逐phase恢复、文件/PID锁、复用OS端口与新增耐久claim，并核查其余U37、第30～36和第38交界 | 通过：现方案同时给用户唯一恢复执行者与恢复期间稳定不可用边界，并以既有内核owner消除结构债；U37-02～U37-11及已验证delivery/source/三策略/exact-stop均未被重开，无Unit38或通用框架扩面 |

> **当前收敛结论：**同一份未修改的U37-01正式记录已通过四路冷启动对抗复审；根因、P0/中评级、工作量、最优方案和直接验收闭合。下一步只能按该执行合同修复，当前不得恢复旧专项“已验证”结论。

### U37-01 修复后专项核销（同一冻结指纹）

> 冻结指纹：`sha256:b3cf4b54249035744432e2dbd2f1a9260687c102f9114fac9200de0cffd2dee4`。本节只核销U37-01，不替代独立审查或全单元提交验证。

| 编号 | 修复后事实链与直接证据 | 核销结论 |
| ---- | ---------------------- | -------- |
| F37-64 | anchor在`setupAssemblyUnits(pre-server)`前、executor在MCP/authority/workspace/data-plane/job/local owner前调用production `bindServer()`；active operation仍只由既有journal/artifact/phase strict hydrate | 通过：OS `listen`是唯一跨进程执行owner，PID/state仅作发现；端口失败者在owner/journal/manager效果前退出 |
| F37-65 | 两根无论有无active operation均使用最终同home非零endpoint；production bind原语的真实child-process竞争得到恰一winner，`EADDRINUSE` loser零继续执行；源码结构合同反向锁定两根bind早于首个owner效果 | 通过：同home单飞、异home隔离成立；terminal→activate期间socket仍由同一winner持有，不产生第二装配者 |
| F37-66 | `BoundZhixingServer`在同一已listen对象上提供inactive/activate/close；inactive HTTP稳定503，WS upgrade在router/auth前拒绝，`runServer()`消费原handle且不二次listen | 通过：激活前零业务分派，terminal/exact release后原对象激活；失败与generic close均关闭同一handle |
| F37-67 | `recoverAcceptedWork=false`现在覆盖local readiness/lifecycle projection与mesh/worker恢复；`resumeActive()`仍按五phase、原strategy和frozen artifact补未完成步骤 | 通过：`accepted/gate-closed`只在proof后产生对应效果，`work-settled+`零二次owner settlement，错operation零release |
| F37-68 | anchor/executor共用`ownsCurrentSuccessorEndpoint()`，同时校验当前bound handle、old/current PID及端口；managed链继续复验definition与manager projection | 通过：旧PID仍活、PID复用、端口换代、definition漂移和manager不可判定均fail-closed；当前winner不会被旧operation误停 |
| F37-69 | 真实child-process覆盖bind竞争、winner进程退出未close与后继重新取得同一端口；phase与响应丢失继续由现有journal/exact read-back前滚 | 通过：OS自动释放失主socket，后继重读原phase再竞争，无耐久successor claim且无并发owner窗口 |
| F37-70 | production只接受解析后的非零host/port作为生命周期owner；端口0仅保留普通server单测，不进入U37-01单飞证据 | 通过：同home竞争证据使用同一固定非零端点；scan/hydration失败仍发生在bind及owner效果前 |
| F37-71 | 差异仅覆盖server两段启动、两条CLI composition root、closed recovery、exact successor谓词、直接测试、既有S7有限检查和三份权威文档 | 通过：未新增claim、journal、锁、lifecycle/coordinator、runner或Unit38能力；U37-02～U37-11与已验证delivery/source/三策略边界未重开 |

### U37-01 修复后四路冷启动对抗（同一冻结指纹）

| 复审角色 | 冷启动主动反例 | 结论 |
| -------- | -------------- | ---- |
| OS端口owner与首个owner效果排序 | 从无active/五phase分别让同home双successor竞争，并尝试把readiness、mesh、MCP、authority、journal或manager效果移到bind前；同时攻击inactive HTTP/WS | 通过：两根源码顺序与真实OS竞争共同证明只有exact handle winner能越过装配边界，loser和inactive ingress均零owner效果 |
| 五phase/三策略/十owner隔离 | 重造空/非空owner、响应丢失、`work-settled/flushed/ready-to-stop`重放、旧endpoint与端口换代，尝试二次settle或用旧operation停止winner | 通过：phase只补未完成步骤，closed recovery延后，exact handle+endpoint谓词阻断换代误认；原三策略安全点保持不变 |
| owner崩溃接替与inactive边界 | 在bind后、proof/owner效果/terminal/release窗口杀死winner，再启动一个或多个后继；反复close并尝试第二次listen/rebind | 通过：同一对象从bind持有到activate/close，崩溃只由OS释放；后继重读耐久phase再竞争，generic close幂等且无无主激活窗口 |
| 生产体验、范围价值与历史边界 | 重新比较维持晚bind、PID/文件锁、耐久claim与现有OS owner，并核对其余U37、Unit30～36及Unit38交界 | 通过：现方案以最小现有内核原语提供唯一恢复者和稳定503边界，既无用户可见并发恢复也无新生命周期债务，范围未扩张 |

### U37-01/U37-11 根因收敛固定矩阵（历史；U37-01部分已被P37-20证伪，U37-11部分保留）

> 本矩阵只冻结问题根因、最优方案与直接验收，不代表实现或测试通过；复审对象为上方同一份未修改的 U37-01/U37-11 正式记录。

| 编号 | 问题与直接变体 | 稳定身份、唯一事实与线性化点 | 零副作用边界、消费终态与直接验收 |
| ---- | -------------- | ---------------------------- | ---------------------------------- |
| F37-56 | U37-01：anchor/executor-only×managed/on-demand/foreground，old/current host，`accepted/gate-closed/work-settled/flushed/ready-to-stop`启动扫描 | `operationId+homeId+localDeviceId+strategy+exact host generation`与同operation phase是唯一恢复游标；两根均须在local/job/mesh/scheduler/channel/delivery producer恢复及listener前完成扫描并形成closed投影 | 多operation、错home/device、strict hydration失败或旧host状态不可判定时保持全部gate关闭并终止启动，零owner/terminal/manager效果；真实两根测试逐phase证明无早恢复/早监听 |
| F37-57 | U37-01：`accepted`无artifact、`gate-closed`有artifact，十owner空/非空、三策略及freeze/settle各sync与响应窗 | 异host先由完整旧endpoint lock、definition与manager/current-successor投影证明旧代已消失或被exact successor取代；`accepted`只补close+一次freeze，`gate-closed`只hydrate原artifact并按原strategy逐owner settle/read-back | 旧host仍可能运行或证明不足时不得freeze/settle；artifact缺失/损坏/错operation、owner id/revision漂移均在新效果前fail-closed；重启不得重采样或并发结算旧host |
| F37-58 | U37-01：`work-settled/flushed`、三策略、generic close、flush/物理安全点效果或响应丢失 | startup投影从phase确定`alreadySettled`；`work-settled+`给local及直接owner恢复同operation settled事实并禁止accepted-work recovery/settlement，随后只按phase补flush或物理步骤 | 任一settled owner不得二次drain/cancel；flush/物理失败停在原phase且gate保持关闭；真实`immediate`非空耐久子集证明generic close前后字节全等，`drain/cancel`既有终态不放宽 |
| F37-59 | U37-01：`ready-to-stop`、old host停止/未停/current successor、terminal/release各响应窗与连续两次新PID | old operation的exact stopped/replaced read-back是terminal唯一授权；terminal sync后，同operation root-local release依次清stop gate/settled投影并复用delivery、job、mesh、inbound、channel、scheduler恢复原语，清除startup投影是successor开放点 | 未terminal、错operation或stale completion零release；旧operation绝不调用manager/cleanup停止successor；terminal后剩余耐久义务只作为successor普通恢复恰一续跑，随后才listener/producer开放；terminal响应丢失由下一启动幂等收敛 |
| F37-60 | U37-01：三策略×五phase×十owner、same-host live replay、old/new PID、后续stop/removal/uninstall直接交界 | live同host仍由现有prepare/resume到ready并由原trigger停止；successor恢复只消费原strategy/phase/artifact，不改delivery/source、ownerItems、checkpoint或local-device conflict事实 | 三策略settled/read-back与U37-09保持全等，`work-settled+`零二次效果；后继lifecycle只在旧terminal且所有gate exact release后准入，不增加恢复计划、第二事实源或通用lifecycle |
| F37-61 | U37-11：validation/unwired既有`RpcAppError`，blocker/owner/flush/unknown普通异常，prepare写前/各phase写后与响应丢失 | `requestId+strategy`及lifecycle durable phase仍是业务事实；handler异常出口是公开投影唯一边界，已有`RpcAppError`原样保留，其他异常统一为`INTERNAL_ERROR`、`安全停机未完成`与`{ action: "retry-same-request" }` | 任何prepare失败均零trigger且wire不含原message、stack、operation/device/path；用同request重试只重驱原phase；不得按易变message分类或修改lifecycle事实 |
| F37-62 | U37-11：真实registry/dispatcher、合法三策略重放、prepare→ready→void trigger、trigger前后窗口 | router认证、loopback与strict decode结论沿用U37-06/U37-10；prepare返回ready是成功响应线性化点，之后production `requestShutdown`仅排入幂等void cleanup，不是本次wire可报告失败 | 失败必须在真实`connection.sendError`呈现exact固定code/message/data且零raw；成功仍先ready后排trigger，同request合法重放不增lifecycle winner；不捕获异步cleanup、不改共享dispatcher或其他RPC |
| F37-63 | U37-01↔U37-11及U37-02～U37-10、第30～36、第38边界与价值比例 | 两项分别只补stop现有phase执行消费/root-local exact release，以及shutdown方法级输出包装；互不共享新状态或错误分类 | U37-01不重开delivery admission/source revision/namespace/migration；U37-11不重开strict输入/loopback或扩到其他RPC；P0/中与P2/小分别由核心启动不可用和有限失败体验事实支撑，无独立恢复计划、错误框架、新runner或第38能力 |

### U37-01/U37-11 四路冷启动对抗复审（历史；U37-01相关结论已失效）

> 四路均从当前权威合同与生产源码重新推导，不复用既有专项通过结论；复审期间未修改上方两条正式问题记录。

| 复审角色 | 主动反例与直接交界 | 结论 |
| -------- | ------------------ | ---- |
| stop successor phase执行恢复与两根启动顺序 | 清空全部内存态，逐一重造`accepted`无artifact、`gate-closed`非空artifact、`work-settled/flushed/ready-to-stop`，anchor先建startup投影、executor先启owner、old/current PID、manager不可判定、各sync/响应丢失与连续两次新PID | 通过：F37-56～F37-59把旧host证明前置到任何successor owner效果，五phase各只补未完成步骤；两根必须消费resume结果并在terminal后exact release，故无永久闭门、早接单、并发结算或误停successor缺口 |
| 三策略settled/close与exact-host边界 | 对十owner空/非空重造`immediate/drain/cancel`，从`work-settled+`启动并触发owner recovery、generic close、flush/physical失败，交叉old endpoint消失、manager仍显示current successor running及后继lifecycle竞争 | 通过：already-settled由durable phase恢复，terminal前零二次settlement/generic drain；old terminal只由完整旧代消失/替换证明授权，release仅匹配同operation，U37-09三策略与U37-08本机互斥均未回退 |
| shutdown失败的wire错误投影 | 经真实`HandlerRegistry→RpcDispatcher→connection`重造invalid/unwired、blocker、owner、flush、unknown、prepare部分耐久后抛错、ready成功及合法同request重放，并尝试把void trigger异常冒充响应失败 | 通过：方法边界保留既有`RpcAppError`，普通异常只产生固定`INTERNAL_ERROR/message/action`且失败零trigger；ready后void trigger不属于可回传失败，合法重放保持原语义，U37-06/U37-10及共享dispatcher均无需修改 |
| 生产体验、范围价值及历史裁决边界 | 反向比较保持现状、改弱启动合同、复用现有机制、最小完整修复与新增恢复/错误框架，并核查U37-01↔U37-11、其余U37、第30～36及第38 | 通过：不修U37-01会在受支持崩溃恢复中永久不可用或错误接单，P0/中成立；U37-11只影响失败可行动性且无当前敏感泄漏证据，P2/小成立。两项方案均达到当前范围内最优体验与架构，未恢复被否定主张或扩面 |

### U37-01/U37-11 修复后专项核销（历史冻结指纹；U37-01不再有效）

> 以下结论绑定排除过程账本与独立清单后的22文件交付指纹 `sha256:17e74faf319780e8a1b43939bc8313c7c5b2c4db42b3fd1c685eb75e4a0e9109`；生产、测试、架构、规格与S7描述符自冻结后未修改。

| 编号 | 当前指纹生产证据 | 结论 |
| ---- | ---------------- | ---- |
| F37-56 | anchor在access surface前、executor在MCP/authority owner前扫描唯一active stop；两根均以closed projection启动local/job/mesh/channel/scheduler/delivery，listener只在resume terminal后创建 | 通过：三形态、old/current host与五phase只有原operation一个启动owner；错home/device、多operation、坏artifact或旧host不可判定均fail-closed |
| F37-57 | `resumeActive()`对异host先调用exact `isHostStopped`，成功后才进入`#resume()`；`accepted`只关gate/freeze，`gate-closed`只hydrate原artifact并逐owner settle/read-back | 通过：proof-before-recovery成立；实现期C37-C23反例已修正，旧host仍活、错artifact/id/revision均零owner/terminal/manager效果 |
| F37-58 | durable phase投影`alreadySettled`，local start恢复同operation settled标记；coordinator对`work-settled`只flush、`flushed`只physical，generic close看到settled不再重做drain | 通过：三策略均不二次settle；direct test在work-settled/flushed故障窗证明settlement计数不增，immediate耐久子集保持exact |
| F37-59 | 两根消费`resumeActive()`返回的同operation terminal；先exact release local/delivery，再恢复frozen或普通耐久义务，随后恢复scheduler/job/mesh/inbound/channel/conversation并清startup投影 | 通过：未terminal、错operation与stale completion零release；旧operation不调用stop successor；advancement经C37-C24补入terminal后恢复，连续重启唯一前滚 |
| F37-60 | HostStop 15/15覆盖五phase、proof不足、same/old host与响应窗；closed-startup消费链16/16、local exact settlement/release 3/3、assignment reopen 1/1覆盖owner交界 | 通过：三策略/十owner与U37-02～U37-10既有合同未回退，无独立恢复计划、第二事实源或Unit38能力 |
| F37-61 | shutdown handler只包住`lifecycle.prepare()`；既有`RpcAppError`原样抛出，普通throwable固定映射，不读取raw message/stack | 通过：blocker/owner/flush/unknown及prepare各耐久窗均零raw、零trigger；同request只重驱原phase |
| F37-62 | 真实`HandlerRegistry→RpcDispatcher→connection.sendError`断言exact code/message/data；prepare ready后仍只排一次既有void trigger | 通过：server方法合同57/57，合法三策略与ready后cleanup语义不变，未修改共享dispatcher或其他RPC |
| F37-63 | server类型检查通过；CLI类型检查仅复现8个既有credentials错误且无U37新增；S7 20/20+registry golden、workspace 17包构建与diff check通过 | 通过：P0/中与P2/小均由当前损失支撑，方案范围有限且未恢复已否定主张 |

### U37-01/U37-11 修复后四路冷启动对抗（历史冻结指纹；U37-01不再有效）

| 复审角色 | 冷启动主动反例 | 结论 |
| -------- | -------------- | ---- |
| stop successor phase与两根启动顺序 | 从零重建anchor/executor、五phase、old/current PID、manager不可判定、无/坏artifact、各sync/响应丢失和连续两次successor；专门尝试在old-host proof前触发frozen recovery | 通过：所有producer先closed，proof是异host任何owner效果的前置门；五phase只补未完成步骤，terminal前零早开/并发settle，C37-C23已耐久处置 |
| 三策略settled/close与exact-host | 独立注入十owner空/非空、immediate耐久子集、drain/cancel终态、generic close、flush/physical失败、current successor running与stale completion | 通过：phase恢复settled、旧endpoint完整证明、root-local exact release共同隔离旧/新代；U37-09、U37-08、Unit36 exact-stop均未回退 |
| shutdown wire错误投影 | 不复用方法测试结论，经真实dispatcher重造既有RpcAppError、普通Error/非Error、prepare部分耐久、同request重放、ready后void trigger异常 | 通过：wire只见既有公开错误或固定`INTERNAL_ERROR/message/action`，失败零trigger；异步cleanup不伪装成响应失败，U37-06/U37-10边界不变 |
| 产品体验、范围价值与历史裁决 | 重新比较保持现状、改弱要求、现有机制、最小修复、独立恢复计划/通用错误框架，并差异审计U37-02～U37-10、第30～36与第38 | 通过：保持现状会造成核心恢复不可用和不稳定错误体验；当前方案以现有journal/artifact/coordinator/ports和RpcAppError闭合，体验与架构均达标且无扩面 |

### U37-06/U37-09/U37-10 根因收敛固定矩阵（当前）

| 编号 | 问题与直接变体 | 稳定身份、唯一事实与线性化点 | 零副作用边界、消费终态与直接验收 |
| ---- | -------------- | ---------------------------- | ---------------------------------- |
| F37-47 | U37-09：anchor/executor，managed/on-demand/foreground，OS signal与显式`immediate`，conversation/intent/final/assignment空或非空 | 同一`operationId+strategy+frozen owner id/revision` artifact；gate关闭后，当前conversation turn/recovery pass结束并回读无active是物理安全线性点；conversation identity与intent/final/assignment仅可保留frozen全等子集 | fresh/successor item、active turn或错revision不得ready；真实local port证明非空耐久子集可恢复而不是被强制清空，响应丢失/重启只消费原artifact |
| F37-48 | U37-09：remote durable job/relay、inbound admission，单/多accepted item与当前attempt | remote frozen id/revision与现有job ledger/relay事实是唯一耐久输入；先drain已越过inbound gate的当前admission并等待当前job/recovery promise到耐久点，再允许frozen全等子集留存 | current attempt、未完成admission或artifact外remote item阻断ready；anchor/executor真实port覆盖active→safe、同id换代、效果/响应丢失与连续重启 |
| F37-49 | U37-09：channel、scheduler、lease、permit及delivery交界 | channel连接、scheduler completion tracker、lease/permit是活动物理/资源事实，安全点唯一为零；delivery继续由既有pipeline以attempt结束与耐久非终态判定，不纳入本项 | 任一channel连接、completion tracker、lease/permit非零均阻断ready；直接证据证明不把它们伪装成可重放义务，也不改变delivery已验证终态 |
| F37-50 | U37-09：`immediate/drain/cancel`、timeout、各settle/read-back窗口与连续重启 | 三策略复用同一frozen artifact与port签名；immediate按F37-47～F37-49，drain全终结，cancel仅取消可取消上游并等待不可取消/已enqueue义务 | immediate不得要求耐久子集归零或容许活动事实；drain/cancel不得退化为immediate；OS signal、显式RPC、anchor/executor三形态逐窗验收 |
| F37-51 | U37-10：unauthenticated/authenticated、loopback/nonloopback、默认/非默认监听、合法/畸形请求与重放 | router认证事实先于handler；handler唯一方法级授权是连接创建时的`ctx.connection.loopback`，其检查点早于参数解码、server wiring、prepare与trigger | 未认证由router拒绝；认证非loopback不因参数形态泄露后续分支且零prepare/trigger；loopback合法请求继续原三策略与request replay |
| F37-52 | U37-06：plain object、合法省略、unknown/缺失字段及reason/strategy类型与空值 | `asRecord`与动态exact key-set是唯一对象边界；requestId必填，reason只在出现时经`stableText`，strategy只接受既有三枚举；省略才用既有默认 | null/array/unknown/缺requestId/错类型/空或过长reason/非法strategy全部在读取lifecycle前拒绝；合法省略字段与既有默认字节不变 |
| F37-53 | U37-06：timeout缺省、正数、零/负数、`NaN/Infinity`、ETA不可表示及prepare/trigger窗口 | timeout出现时必须有限且为正，`Date.now()+timeoutMs`必须先形成合法ETA；完整decoded request与ETA是首个prepare效果的唯一输入 | 任何数值/ETA失败零prepare/trigger；合法30s默认与60s请求保持原结果；直接测试证明不存在prepare成功后response格式化失败 |
| F37-54 | U37-10↔U37-06：权限、解码、wiring、prepare、trigger的完整效果顺序 | 固定顺序为router auth→loopback guard→strict decode+ETA→wiring→durable prepare→trigger；同request replay继续由原lifecycle负责 | 前四步任一失败均零生命周期效果；prepare响应丢失后合法loopback重放仍返回原operation，非loopback与畸形输入永不借重放越界 |
| F37-55 | 三项与其余U37、第30～36及第38边界 | 只消费既有artifact/ports、connection loopback与strict helpers；U37-01～U37-05/U37-07～U37-08、delivery immediate、CLI cancel及认证/lifecycle主链继续作为冻结合同 | 不新增授权/lifecycle/codec框架、S7或runner，不改变第38能力；直接差异审计证明改动对象、停止条件与验收证据均有限且无同根残留 |

### U37-06/U37-09/U37-10 四路冷启动对抗复审（同一未修改问题列表）

| 复审 | 主动反例与直接交界 | 结论 |
| ---- | ------------------ | ---- |
| 非delivery `immediate`安全点与三策略隔离 | 不复用前轮结论，从十owner空/非空重造conversation/job/inbound活动、耐久intent/final/assignment/remote子集、channel/scheduler/lease/permit及timeout/重启；反查delivery与CLI cancel | 通过：F37-47～F37-50唯一地区分可留存耐久子集和必须归零的活动事实，drain/cancel终态未弱化，C37-C19已同根处置 |
| 三进程形态、信号/RPC与连续恢复 | 独立重造anchor/executor、managed/on-demand/foreground、OS signal/显式immediate，各settle/read-back/ready响应窗、同id successor与连续重启 | 通过：同一operation/artifact/port输入贯穿三形态，旧operation只处理frozen generation；timeout只报告真实blocker，不伪造ready或重复accepted work |
| shutdown loopback授权与strict输入效果边界 | 独立重造未认证、认证远端、认证本机、默认/非默认监听、合法省略、unknown/错类型/空值/非有限timeout及prepare响应丢失 | 通过：F37-51～F37-54固定auth→loopback→decode→effect顺序；所有拒绝均零prepare/trigger，合法本机重放保留原终态，C37-C20已同根处置 |
| 生产体验、范围价值与历史裁决边界 | 依次比较保持现状、修规则、复用机制、最小修复与新增框架；核对U37-01～U37-05/U37-07～U37-08、第30～36及第38边界 | 通过：U37-09的安全停机卡死与U37-10的远程停机授权均为当前P1，U37-06维持P2；三项方案同时满足最优用户体验和最优架构，未恢复delivery/CLI/既有facade或扩建通用框架 |

> **收敛结论：**同一份未修改的U37-06/U37-09/U37-10问题记录已通过四路冷启动对抗复审；根因、评级、工作量、最优方案、直接验收及历史边界均闭合，下一步只能按三项执行合同实施，不得继续扩面。

### U37-06/U37-09/U37-10 修复后固定矩阵核销（专项冻结指纹）

| 矩阵 | 修复后生产事实与直接证据 | 结论 |
| --- | --- | --- |
| F37-47 | local `immediate`复用同一operation/artifact/frozen id/revision，只停止recovery loop并等待active conversation与lease归零；真实deferred intent场景证明冻结耐久子集不会被重算或清空 | 通过：active物理步骤阻断ready，frozen exact intent/final/assignment可留存；generic close不再二次drain同一已settled operation |
| F37-48 | command与executor read-back均消费strategy；`immediate`先调用现有job owner `drain()`等待recovery/running/interaction promise，再只允许ledger中的frozen exact remote子集；该`drain()`不删除耐久accepted-work | 通过：错revision/successor与当前attempt继续阻断；job-assignment-worker recovery直接测试4/4通过，未新增remote事实源或绕开现有port |
| F37-49 | channel、scheduler、lease、permit继续要求零；delivery pipeline及其`immediate/drain/cancel`投影未改 | 通过：活动连接、completion tracker与资源事实不能伪装成耐久子集，delivery既有终态不回退 |
| F37-50 | OS signal与显式`immediate`仍进入原coordinator；`drain`继续全终结，`cancel`继续取消可取消上游并等待不可取消/已enqueue义务 | 通过：三策略共享identity而不共享错误终态；受控active turn证明安全点前等待、安全点后唯一前滚 |
| F37-51 | RPC router仍以`requiresAuth`先拒未认证连接；handler首个分支检查可信`ctx.connection.loopback` | 通过：真实WebSocket未认证请求由router拒绝；认证非loopback对合法/畸形params均在decode、wiring、prepare、trigger前零效果拒绝 |
| F37-52 | loopback通过后以`asRecord`取得plain object，按实际出现字段形成exact-set；requestId/reason/strategy逐项复用现有strict helper | 通过：null、array、unknown、缺失、空值与错类型全部零生命周期效果；合法省略才采用原默认 |
| F37-53 | timeout出现时必须有限且为正，`Date.now()+timeoutMs`先预计算为可表示ETA，随后才读取wiring并prepare | 通过：零、负数、`NaN/Infinity`及不可表示值均零prepare/trigger；不存在效果后ETA格式化失败窗 |
| F37-54 | 固定效果序为router auth→loopback guard→strict decode+ETA→wiring→durable prepare→trigger | 通过：前三层任一失败均零效果；合法loopback requestId重放继续由原shutdown lifecycle收敛 |
| F37-55 | 变更仅落在既有stop owner projection、shutdown handler、直接测试和三份架构/规格文本 | 通过：U37-01～U37-05/U37-07～U37-08、delivery immediate、CLI cancel、既有device/uninstall/removal facade与第38单元边界均未改 |

### U37-06/U37-09/U37-10 修复后四路冷启动对抗复审（同一专项冻结指纹）

| 对抗角色 | 冷启动反例重造 | 结论 |
| --- | --- | --- |
| 非delivery `immediate`安全点与三策略隔离 | 从空/非空十owner、active conversation/recovery/job/inbound、frozen intent/final/assignment/remote、channel/scheduler/lease/permit、timeout与successor重新推导，不复用实现期结论 | 通过：只有frozen exact耐久子集可留存，所有活动物理/资源事实到零；`drain/cancel`与delivery语义未弱化，C37-C19/C37-C21/C37-C22均有耐久处置 |
| 三进程形态、信号/RPC与连续恢复 | 反查anchor/executor、managed/on-demand/foreground、OS signal/显式immediate、效果/响应丢失及连续重启，核对exact operation/host边界 | 通过：所有入口仍汇入原coordinator并消费同一artifact/strategy；当前效果只到既有耐久安全点，后继host与错revision零误作用 |
| shutdown授权与strict输入效果边界 | 重造未认证、认证远端、认证本机、默认/非默认监听、plain-object边界、字段全集、非有限/不可表示timeout与合法重放 | 通过：auth→loopback→decode→effect顺序唯一；非法输入和远端连接均零prepare/trigger，合法本机三策略与重放不变，C37-C20已关闭 |
| 生产体验、范围价值与历史裁决边界 | 重新比较保持现状、修正规则、复用机制、最小完整修复与新增框架；双向核对其余U37、第30～36和第38边界 | 通过：P1/P1/P2及中/小/小工作量仍有当前事实依据；方案同时达到当前范围内最优体验与最优架构，未恢复已否定主张或新增通用框架 |

> **修复后专项结论：**同一专项冻结指纹上的F37-47～F37-55全部通过，C37-C19～C37-C22均已核销，四路冷启动对抗未产生独立新根因；U37-06、U37-09、U37-10均为已验证。该结论不替代受影响独立审查节点的重新审查。
...
>
> 历史执行状态：`F37-09～F37-20` 中 U37-01/U37-02/U37-03/U37-05 对应旧格已被 C37-C02～C37-C05 证伪；`F37-21～F37-28` 曾替代这些旧格，但F37-22的非delivery `immediate`终态已被U37-09证伪。U37-04旧格未受影响；U37-06旧格仅覆盖既有device/uninstall/mesh输入，未覆盖本轮新发现的`server.shutdown` strict输入。
...
| C37-C19 | 若把U37-09概括为“所有非delivery frozen owner在immediate均可留存”，channel连接、scheduler completion tracker、lease、permit及当前conversation/job/inbound活动会被错误当成耐久重放事实并越过ready | 同根收窄U37-09；只有conversation identity与intent/final/assignment/remote中仍与frozen id/revision全等的耐久子集可留存，所有活动物理/资源事实必须归零；drain/cancel原终态不变 | 修复后复核通过：F37-47～F37-50逐owner绑定唯一安全点；真实deferred intent/active turn与生产port投影证明耐久子集留存、活动事实归零且三策略不回退 |
| C37-C20 | `server.shutdown`的`timeoutMs=Infinity`可通过当前正数判断，lifecycle prepare先耐久，随后ETA格式化抛错；只拒unknown或错类型仍留下效果后失败窗 | 同根扩写U37-06；loopback guard后一次性strict decode全部字段并预计算可表示ETA，任何失败先于lifecycle/trigger | 修复后复核通过：F37-52～F37-54与参数化直接测试覆盖非有限/不可表示值，证明全部拒绝先于wiring/prepare/trigger且合法重放不变 |
| C37-C21 | 实现期真实deferred intent反例证明：即使`settle/read-back`已正确允许`immediate`保留frozen耐久子集，后续generic `LocalConversationOwner.close()`仍会再次执行无strategy的drain，清空或拒绝该子集 | 同根并入U37-09；同operation已完成host-stop settlement后，generic close只能停止recovery loop并释放owner资源，不得另起第二次owner settlement；其他普通close继续原行为 | 修复后复核通过：受控测试证明frozen intent在ready/close后保持exact，active turn先到安全点且未被abort；drain场景仍拒绝非终态子集，无新问题编号 |
| C37-C22 | 冷启动复核质疑anchor/executor remote settlement调用`jobOwner.drain()`会把允许留存的耐久job子集一并清空，从而使strategy-aware read-back成为伪边界 | 当前源码证伪：`JobAssignmentWorker.drain()`只等待recovery pump、running与interaction recovery promise，不修改ledger；`acceptedWorkItems()`随后从ledger重读耐久义务，command/executor read-back再以frozen exact subset约束 | 当前源码证伪并直接复核通过：job-assignment-worker recovery测试4/4通过；active attempt先结束、ledger耐久事实仍由原owner投影，F37-48结论保持成立 |
| C37-C23 | 实现期首版把`gate-closed`投影为startup立即`recoverAcceptedWork`，会在异host exact stopped/replaced证明前执行frozen owner效果，与仍活旧host并发恢复 | 同根并入U37-01；stop startup无论phase均以closed且不恢复owner装配，只有`resumeActive()`先完成异host证明并进入原phase时，`onAcceptedWorkFrozen`才恢复原artifact；`work-settled+`则terminal后按普通剩余义务恢复 | 修复后复核通过：proof不足场景保持gate-closed且effect order字节不变；gate-closed successor只在proof成功后settle并terminal，HostStop 15/15与四路冷启动共同覆盖 |
| C37-C24 | terminal后恢复集合最初只覆盖scheduler/job/mesh/inbound/channel/conversation/delivery，遗漏启动时因active lifecycle明确跳过的advancement recovery | 同根并入U37-01；抽取原有`recoverAdvancementAcceptedWork()`窄helper，无active lifecycle保持原启动调用，有stop successor则在旧terminal与exact release后调用一次，不新增producer或状态 | 修复后复核通过：production调用图只有正常启动与stop terminal后两个互斥调用点；proof不足/失败不调用，S7、workspace build与冻结四路确认其余lifecycle不受影响 |
| C37-C25 | P37-20穿透两条production composition root后证明：anchor的`setupAssemblyUnits()`与executor的MCP/authority/workspace/data-plane/job/local owner装配均早于`runServer()`真实端口仲裁；两个新PID可同时完成closed投影并执行owner/恢复效果，`isHostStopped()`还会把自构造PID/endpoint误作current exact successor | 同根并入U37-01；F37-64～F37-71把既有同home非零server端口的OS bind前移为首个effectful owner前唯一进程线性化点，同一bound server handle贯穿inactive ingress、current-successor proof、terminal/release与最终激活，不新增耐久claim或第二锁 | 修复后复核通过：两根均在首个owner效果前取得同一非零home端点，真实OS竞争恰一winner，inactive ingress零分派；winner崩溃后OS释放，后继重读原phase再竞争。V37-20～V37-22及修复后四路绑定当前专项指纹 |
| C37-C26 | 冷启动对抗发现首版修复的current-successor只校验“旧PID等于当前PID且当前handle拥有当前端口”，未要求旧endpoint端口与当前endpoint全等；受控非默认端口或PID复用时，进程可把无关旧endpoint误判为自己替代 | 同根并入U37-01；anchor/executor共用窄`ownsCurrentSuccessorEndpoint()`，同时要求old/current PID全等、old/current端口全等且当前bound handle实际拥有该端口，managed路径继续复验definition与manager projection | 修复后复核通过：共享谓词4/4覆盖exact、错旧端口、错PID与handle非owner；两根均只调用该谓词，替换后workspace build、S7与当前专项指纹重新取得，旧指纹和首轮对抗结论已作废 |

### U37-01/U37-08 根因收敛固定矩阵（历史；U37-01后续根因已重开，U37-08结论保留）
...
| F37-22 | coordinator 把 frozen slice 原样传入每个 settle/read-back；local owner、scheduler、delivery、remote/channel均先校验 subset，生命周期日志与 local owner checkpoint 后才 `ready-to-stop` | 部分失效：exact frozen identity、phase/gate与后继host隔离仍有效；U37-09证明非delivery owner未按strategy区分`immediate`安全点与`drain`全终结，旧结论不得证明三策略均可ready |
...
| 复审 | 主动反例与直接交界 | 结论 |
| ---- | ------------------ | ---- |
| stop accepted-work / exact-host | 在冻结指纹上重造F37-04非空、三策略/三形态、blocker/permit、manager响应丢失、PID/lock successor与连续重启；核查U37-01↔U37-03及Unit36 exact-stop | 失效：U37-09证明该轮把非delivery `immediate`安全点误判为owner全终结；仅frozen identity、endpoint lock与exact manager stop子结论保留 |
...
| facade / 产品体验 / 范围价值 | 重造全部server/mesh方法unknown/缺失/错类型/旧peer、公开内部id、伪checkpoint owner及Unit38扩面；反向核定六项评级、工作量与价值裁决边界 | 失效：U37-06/U37-10证明该轮遗漏`server.shutdown`的strict decoder与loopback方法级授权；既有device/uninstall/mesh facade子结论保留 |
...
| L37-08 | U37-09；stop accepted-work专项与修复后四路曾通过 | 只验证port调用、冻结identity与效果顺序，把统一的“owner已终结”判据当成三策略都可接受的安全点；未用非空真实owner逐一对账`immediate/drain/cancel`终态 | 对每个非delivery生产owner注入非空frozen work，沿freeze→settle→read-back逐策略核对：`immediate`须等待活跃物理步骤到耐久可重放点但允许非终态义务，`drain`须全终结，`cancel`只取消可取消上游并保留不可取消义务。适用于OS signal、anchor/executor stop与三进程形态 | 本轮独立审查：执行并发现P37-15，新增U37-09 |
| L37-09 | U37-06/U37-10；公开facade专项与价值复审曾通过 | 以device/uninstall/removal mesh样本替代公开lifecycle方法全集，未把`server.shutdown`逐方法核对连接授权与strict request decode | 从方法registry枚举全部公开lifecycle入口；每个方法在首个效果前分别核对认证、loopback/方法级授权与exact-key/type/range decode，任一只依赖默认bind或DTO强转即判缺口。适用于shutdown/removal/uninstall公开入口 | 本轮独立审查：执行并发现P37-16/P37-17，新增U37-10并同根重开U37-06 |
| L37-10 | U37-01；F37-56～F37-60、专项四路与修复验证曾通过 | 只验证单个successor按phase恢复及旧host证明，没有把跨进程真实执行owner的线性化点与首个owner外部效果排序，也未并发启动两个新PID穿过两条production composition root | 对所有pre-runtime lifecycle恢复逐项画出“耐久scan→进程owner→首个外部效果→公开ingress”顺序；真实并发启动两个同home successor，要求失败者在任何owner/journal/manager效果前退出，winner崩溃后后继只前滚原phase。适用于anchor/executor stop successor及其他声明跨进程单飞的启动恢复 | 本轮独立审查：执行并发现P37-20，同根重开U37-01；确认现有OS端口owner建立过晚 |
...
| V37-16 | U37-01五phase、两根closed startup与root-local release直接证据 | CLI HostStop 15/15；executor/local surface/lifecycle 16/16；local owner定向3/3；assignment reopen 1/1 | 本批host-stop coordinator、anchor/executor startup、local/job/mesh owner与直接测试 | 修复直接验证 / 中 / 约69s | 35/35通过；proof不足零effect、work-settled/flushed零重复settlement、exact release/reopen与active safe point均成立 | `sha256:17e74faf319780e8a1b43939bc8313c7c5b2c4db42b3fd1c685eb75e4a0e9109` | 有效 |
| V37-17 | U37-11方法级错误包装与真实wire | server shutdown方法/registry/dispatcher定向57/57；server `tsc --noEmit` | server shutdown handler、真实dispatcher/connection与直接测试 | 修复直接验证 / 低 / 约8s | 57/57与类型检查通过；既有RpcAppError保持、普通异常固定code/message/data、零raw/零trigger | 同上 | 有效 |
| V37-18 | CLI类型边界与派生结构 | CLI `tsc --noEmit`归因；`pnpm s7:lint` | CLI受影响源码、S7有限descriptor/test/registry golden | 修复直接验证 / 低 / 约56s | CLI仅复现8个既有credentials类型错误，零U37新增；S7结构20/20及registry golden通过 | 同上 | 有效 |
| V37-19 | 最终可消费构建、差异卫生与冻结 | `pnpm build`唯一同输入；`git diff --check HEAD`；排除过程账本/独立清单后22文件SHA-256聚合 | 17个workspace包及22文件生产/测试/架构/S7交付闭包 | 修复直接验证+冻结 / 中 / 138.6s | 17/17构建成功，diff check通过，22文件指纹稳定；未重复同输入构建 | 同上 | 有效 |
