...
- **当前状态**:U37-01/U37-11已完成实现、最小必要验证、F37-56～F37-63专项功能审查及同一冻结指纹四路冷启动对抗，均为已验证；独立审查当前23项`[x]`、0项`[!]`、17项`[~]`，受影响节点旧证据已作废并待独立重审。U37-02～U37-10未被新生产事实触发的既有已验证结论继续复用；尚未进入全单元终审或单元提交验证
- **连续无新增问题轮数**:0 / 2
- **交付物是否冻结**:是（仅U37-01/U37-11本批22文件专项交付物冻结；不代表全单元终审、提交验证或封版完成）
- **交付物文件集**:41个Unit37完整生产、公共合同、直接测试、S7与分布式运行时架构/规格文件；本轮专项实际变更11个生产、公共合同、直接测试与架构/规格文件，过程账本与独立审查清单不计入专项指纹
- **当前交付物指纹**:`sha256:17e74faf319780e8a1b43939bc8313c7c5b2c4db42b3fd1c685eb75e4a0e9109`（U37-01/U37-11本批22文件专项指纹；过程账本与独立审查清单排除）
...
| ---- | ---------- | -------- | ---------- | ---------------------- | ---- |
| U37-01 | **P0/中，同根再次重开（P37-18转入）。** `HostStopCoordinator.resumeActive()`只续推与当前host全等的operation，异PID只会在旧operation已达`ready-to-stop`且旧host已停止时补terminal；anchor虽在producer恢复前读取active operation，却把它折叠成`artifactReady+delivery.sealed`并在装配后丢弃resume结果，executor-only则先启动local/job owner才调用同样被丢弃的resume。两根都没有把`strategy+phase+already-settled`恢复成可执行续跑，local settled标记又只存在内存。**最新价值裁决记录：**原P37-18评为P0/大并提出独立“恢复计划”；反向核对证明现有operation、accepted-work artifact、phase、coordinator与ports足以唯一表达恢复，不需要新抽象或大范围lifecycle改造，故改写保留P0、工作量降为中。保持现状会让三形态逐阶段崩溃恢复失败；改规则或仅补测试不能消除真实启动故障。只有新事实证明现有phase/artifact无法表达受支持终态或同根扩展到其他lifecycle，才重开“大工作量/新抽象”主张。最终结论：改写并保留；用户体验达标、架构达标。**本轮根因收敛记录：**原影响面漏列artifact尚未形成的`accepted`，且把旧host停止证明只放在补terminal处；源码证明`accepted`同样会在新PID永久悬置，而successor在旧host仍可能运行时不得先结算其owner。新决定把五个非终态phase全部纳入，并要求异host在任何owner效果前先以完整endpoint/definition/manager投影证明旧代已消失或已由当前exact successor取代；`work-settled+`只恢复already-settled，terminal后才把剩余耐久义务作为successor正常恢复输入。评级/工作量不变；只有出现无法用现有phase和根内release原语表达的生产终态，才重开独立恢复计划。**历史已验证子问题（不得恢复）：**delivery admission/sealed、七类source exact revision、local/relay namespace与migration共同闭包已由F37-37～F37-42、C37-C16及专项复审证明；原P0/大的scheduler准备泛化主张已被价值裁决否定。 | startup只把耐久stop phase投影为准入状态，没有把同一operation的strategy、phase、settled与exact release恢复为successor可消费的执行游标；两根调用方又丢弃resume结果，使durable winner与运行时owner、terminal及重新开放动作断开。 | **生产端：**anchor `command.ts`、executor-only runtime、`HostStopCoordinator.resumeActive()`、startup lifecycle投影、local/job/mesh/scheduler/channel/delivery owner start/close/release及exact-host read-back。**类型组合：**三策略×managed/on-demand/foreground，`accepted/gate-closed/work-settled/flushed/ready-to-stop`，十owner空/非空、旧/新PID、generic close、各sync/响应窗口与连续重启。**消费者：**producer closed/recovery/reopening、flush/ready/terminal、exact successor与后续stop/removal/uninstall。**异常终态：**anchor永久闭门、executor successor先恢复接单、旧operation永久冲突、与仍活旧host并发结算，或`work-settled+`二次恢复/close drain改变`immediate`耐久子集。**受影响审查项：**IR37-07、IR37-09、IR37-10、IR37-34～IR37-36、IR37-40。**历史边界：**既有delivery/source/removal/migration/backup已验证子结论不重做。 | 保持现有journal、artifact、coordinator与十owner ports。两根都须在任何producer恢复/监听前读取同一stop operation，以closed producer装配ports，并投影完整`operationId+strategy+phase+artifactReady+alreadySettled`；异host先全等证明旧endpoint已停止或被当前exact successor取代，证明不足则保持全gate关闭并终止启动，零owner/terminal/manager效果。随后按原phase执行：`accepted`只补关gate与一次freeze，`gate-closed`只hydrate原artifact并按原strategy做exact settlement，`work-settled`恢复local already-settled且只补flush，`flushed`只补物理安全点，`ready-to-stop`不重做前相；达到ready后仅给旧host写terminal，绝不调用其operation停止successor。两根必须消费resume结果，并以同operation窄release复用现有delivery、job、mesh、inbound、channel、scheduler恢复原语，给local owner补exact release/settled清理；terminal后清除启动投影、恢复剩余耐久义务并开放successor，错operation或stale completion零释放。验收穿过anchor/executor真实装配，覆盖三策略、三形态、五phase、十owner空/非空、旧host未停/已停/当前successor、generic close、各sync/响应丢失及连续两次新PID；证明旧operation唯一前滚、successor零误停/零早开、`work-settled+`零二次settlement、`immediate`子集仅在terminal后按正常恢复恰一续跑。不得新增恢复计划、第二事实源或通用lifecycle。**修复后证据：**anchor/executor两根均在producer/listener前读取active stop并以closed projection装配；异host先经exact endpoint/definition/manager证明，再由现有coordinator按五phase补步。`work-settled+`恢复already-settled；旧terminal后按同operation释放并恢复剩余义务，stale operation零释放。HostStop 15/15、closed-startup消费链16/16、local owner 3/3、assignment reopen 1/1、S7 20/20与workspace 17包构建通过；CLI类型检查无U37新增错误。F37-56～F37-60逐格复核通过。 | 已验证 |
...
> **最新证据有效性说明：**U37-01/U37-11已在专项冻结指纹 `sha256:17e74faf319780e8a1b43939bc8313c7c5b2c4db42b3fd1c685eb75e4a0e9109` 上完成实现、最小必要验证、F37-56～F37-63逐格复核、C37-C23～C37-C24核销与修复后四路冷启动对抗；两项正式问题为已验证。U37-02～U37-10未触发重开的既有结论继续有效。受本批生产、公共合同与直接测试变更影响的17个独立审查节点已置为`[~]`并作废旧证据；专项验证不得直接产生`[x]`。

### U37-01/U37-11 根因收敛固定矩阵（当前）
...
### U37-01/U37-11 四路冷启动对抗复审（同一未修改问题列表）
...
### U37-01/U37-11 修复后四路冷启动对抗（同一冻结指纹）
