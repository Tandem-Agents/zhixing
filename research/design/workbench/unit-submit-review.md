...
```
目标：只收敛第 33 单元正式问题列表中同根重开的 U33-04、U33-10 两个 P1 和 U33-05 一个 P2，使三项真正命中已有配对设备首次恢复根建立、目标离线时的耐久 readiness 投影及 paired client 容量准入旁路的根因，并具备可由执行者一次实施的最优方案与完整验收条件。不修改实现，不运行构建或测试，不审查其他问题；其余 U33 问题与 EX33-01 的既有结论直接复用，U33-04 的 cleanup 启动门禁、U33-05 的 O(total) 驻留和 U33-10 的内部状态泄露等已否定或已修主张不得恢复，第 34～38 单元能力不得提前并入本单元。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 33 单元正式文件中的 U33-04、U33-05、U33-10、EX33-01，只依据三项问题最新的事实、价值裁决、方案、验收条件和状态继续。
...
1. 按 `U33-04 → U33-10 → U33-05` 从权威架构、规格和当前生产调用图重建事实链，核准恢复包读取、recipient/root identity、paired receiver 注册、target availability、耐久 created/replicated/verified 投影及两处 paired client/governor 注入的唯一事实源、稳定身份、线性化点、消费者、异常终态、当前损失、受影响审查项、评级和工作量；判断当前描述命中根因还是表象。同根内容必须合并，独立根因不得互相遮蔽；既有历史修复与价值裁决未被新生产事实触发时不得恢复原主张或扩面方案。
2. 穷尽直接变体：U33-04 覆盖已有配对设备且无恢复根的 v1/v2、正确/错误 recipient、receiver 缺失/建立/退役、断连、响应丢失、并发与 terminal replay，并核对目录 setup 与新设备 onboarding；U33-10 覆盖 verified/unverified、target 在线/离线、paired runtime 缺失、进程重启/重连、root/chain 变化及 CLI/server 同源；U33-05 覆盖 backup command 与首次 pairing 两构造点、governor 容量拒绝/取消/成功、网络等待、固定 range buffer 与并发消费。每格必须指出稳定身份、耐久事实、零副作用边界、消费终态和直接验收；无法解释即继续修正根因。
3. 以锁定范围内的最优架构审查方案：U33-04 只调整现有 setup 次序并复用 paired receiver/staging、strict command 与 `RecoveryActivationCoordinator` 的无根 current-issuer 模式；U33-10 只抽取并复用现有 service 的只读 readiness projector，使 availability 与 durable readiness 分层；U33-05 只把两入口已有 `storageMaintenance` 注入现有 paired target，保持网络不持 permit。方案必须用最少文字说清改什么、怎么改、关键边界及完成判据；不得新增 receiver/状态/容量框架、通用扫描/备份/路由/存储/事务/outbox/事件总线/registry、第二事实源、多目标或云能力、新 lint/test runner、监控、诊断、benchmark 和信息采集。发现缺口时直接修正对应原问题，使执行者无需实现猜测即可一次完成。
4. 三项看似闭合后，对同一份未修改问题列表执行四路冷启动对抗复审：paired 首次 root establishment 与 v1/v2 coordinator 边界、target availability 与 durable readiness 投影、paired capacity admission 与 permit 生命周期、产品体验/范围价值及历史裁决边界。各路必须抛开前轮结论，从当前合同和源码主动构造第 2 步反例，并核查 `U33-04↔U33-10`、`U33-04↔U33-05`、`U33-10↔U33-05` 及三项与 EX33-01、第 34～38 单元边界的直接交界；发现真实反证则修正原记录并重新执行四路复审。

只有现有架构无法唯一推出方案，且选择会显著改变产品需求、用户体验、成本或单元边界时才暂停；其余架构选择按整体最优且不留债务自主收敛。

完成条件：同一份未修改问题列表通过四路冷启动对抗复审；U33-04、U33-05、U33-10 的全部受支持首次 paired root、v1/v2 replay、离线 readiness 与 bounded capacity 变体均被根因完整解释，影响面无遗漏，评级与工作量有事实依据，最优方案和验收条件可直接执行，不会因同根残留继续局部返工。满足后明确回复“U33-04、U33-05、U33-10 的根因与最优方案已闭合”并立即停止。
...
```text
目标：彻底解决第 33 单元同根重开的 U33-04、U33-10 两个 P1 和 U33-05 一个 P2，闭合已有配对设备首次恢复根建立、目标离线时的耐久 readiness 投影及 paired client 容量准入旁路的全部同根直接变体；不得扩展到其他问题或全单元流程。其余 U33 问题与 EX33-01 的既有结论直接复用，U33-04 的 cleanup 启动门禁、U33-05 的 O(total) 驻留和 U33-10 的内部状态泄露等已否定或已修主张不得恢复，第 34～38 单元能力不得提前实施。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 33 单元正式文件中的 U33-04、U33-05、U33-10、EX33-01，只依据三项问题最新的根因、价值裁决、F33-30～F33-40 固定矩阵、方案、验收条件、反证账和状态继续。
...
1. 修复前从权威架构、规格与当前生产调用图重建 F33-30～F33-40 固定矩阵。U33-04 覆盖已有 active paired device 且无恢复根的 v1/v2、包内真实 recipient/checkpoint、source 无根 bootstrap、target receiver 缺失/建立/退役、错 issuer/recipient、断连、并发、响应丢失、连续重启和 terminal replay，并核对目录 setup 与新设备 onboarding；U33-10 覆盖 verified/unverified、target 在线/离线、paired runtime 缺失、slot 重启/重连、root/chain/target generation 变化、坏配置及 CLI/server/public mapper；U33-05 覆盖三个 paired client 构造点、两处 governor 遗漏、容量拒绝/取消/成功、网络挂起、固定 range buffer 与并发消费。逐格绑定 root/recipient/checkpoint、current generation 与 request/part identity，唯一耐久事实、线性化点、零副作用边界和直接证据，并持续核对历史裁决、EX33-01 及第 34～38 单元边界。
2. 按 `U33-05 → U33-10 → U33-04` 一次完成。先把 backup command 已有 `context.capacity.storage` 与首次 pairing 已有 `input.storageMaintenance` 注入各自 `PairedRecoveryCheckpointTarget`，runtime owner 的正确构造保持不变；网络请求仍先取得有界 range part，permit 只覆盖现有 decode/消费 step并在交付后释放，不跨网络或 authority/store/lifecycle 锁，不改 wire/chunk/source-sink。

   再从 `AuthorityCheckpointService.status()` 抽取不依赖 target I/O 的唯一只读 readiness projector，只读取 current trust/anchor/target generation、`AuthorityCommitLog` 与本地 full envelope；service 与 `ConfiguredCheckpointOwnerSlot` 共用。配置和身份可解析但 target/runtime unavailable 时，slot 保持 `state:unavailable` 并保留 projector 的 `fullBackupReady`；`projectRecoveryBackupStatus` 不再把 unavailable 强制映射为 false。无配置、坏 binding、generation 不匹配或缺 verified/full envelope 时为 false，trust/root/member 矛盾继续 fail-stop，公开 DTO exact keys 与稳定错误隔离不变。

   最后拆开 paired 初始 root 的 package prepare/readback 与 target connection：先用既有无回显 reader 严格解码 v1/v2，以包内 legacy checkpoint/root 或 v2 root/recipient 冻结真实 identity，再连接 target。在既有 `ProductionMeshControlPlane`、service registry、paired receiver/staging 上增加仅供“已配对、无 root、current issuer”的 root-establishment 模式，只注册现有 strict checkpoint service，不开放普通 mesh/business service且正常 ready gate不放宽。target 首个 begin 在现有私有 staging 耐久绑定 `{homeId,source=currentIssuer,targetId,checkpointId,recipientKeyId}`，同载荷 exact replay、冲突稳定拒绝、断连/重启续驱；真实回读后仍由现有 `RecoveryActivationCoordinator` 原子激活，激活可见后有限模式自动退役。同步现有 S7 有限 descriptor/validator 与直接测试，目录 setup、新设备 onboarding 及 cleanup 后台语义不变。同根残留并入原问题，禁止新增 receiver/readiness/capacity 框架、通用扫描/备份/路由/存储/事务/outbox/事件总线/registry、第二事实源、多目标/云能力、新 lint/test runner、监控、诊断、benchmark 或信息采集。每个实质阶段完成后立即更新正式状态与证据。
3. 按验证手册运行受影响闭包的最小必要类型检查、paired target/governor、backup setup/root activation、mesh bootstrap/receiver、checkpoint readiness/slot/public projection 直接合同与场景测试，以及必要的现有 S7/派生资产检查；源码有变时按项目常驻规则取得一次同输入构建证据。真实反例必须穿过两生产根和真实 paired transport/staging、`RecoveryActivationCoordinator`、`AuthorityCommitLog`、ArtifactStore 与 storage governor，直接注入无根 v1/v2、错 issuer/recipient、receiver 缺失、断连/响应丢失、离线 target/runtime、root/chain 变化及容量拒绝/取消/网络挂起；不得以 mock 自报 activation/readiness/permit 或只验证返回值，不得运行包全测、模块回归、配置×故障笛卡尔积或与三项验收无关的验证。失败先归因，实现问题直接修复并回到第 2 步。
4. 验证通过后冻结当前交付物指纹，整轮只读地逐格重建 U33-04、U33-05、U33-10 事实链；测试通过不得代替功能判断，矩阵全部完成后才统一归并。随后对同一指纹执行四个相互隔离的冷启动对抗角色：paired 首次 root establishment 与 v1/v2 coordinator 边界、target availability 与 durable readiness 投影、paired capacity admission 与 permit 生命周期、产品体验/范围价值及历史裁决边界。各角色须抛开既有结论，主动重造第 1 步全部适用反例，并核查 `U33-04↔U33-10`、`U33-04↔U33-05`、`U33-10↔U33-05` 及三项与 EX33-01、第 34～38 单元边界的直接交界。
...
结束条件：同一冻结指纹上的 U33-04、U33-05、U33-10 方案全部落地，受影响闭包的最小必要验证通过，专项功能审查与四路冷启动对抗均留下完整矩阵；累计同根反证全部有耐久处置，证明已有配对设备在无 recovery root 时只经 current-issuer 的有限 strict checkpoint transport 建立首份副本，v1/v2 包内 identity 全程唯一，错误/并发/丢响应/重启可重驱且激活前普通业务仍关闭；target/runtime 离线只降低 availability，不改写 current generation 的耐久 readiness，CLI/server 同源；三个 paired client 构造 exact-set 全部使用同一设备 governor，网络等待零 permit且固定 decode buffer 受准入。历史裁决重开条件仍不成立，EX33-01与第34～38单元边界未改变，三项均已更新为“已验证”。满足后明确报告“U33-04、U33-05、U33-10 三项问题已彻底解决”并立即停止；不得进入全单元终审或单元提交验证。
...
- **交付基线**：U33-04、U33-05、U33-10 专项修复当前指纹 `92953a6e3f7d5cd179f9c5c5a933f844487036541d5710b2d4effaab7a4e488e`，覆盖 22 个非工作台生产实现、直接测试、S7 与架构/规格路径；`research/design/workbench/` 流程账本不参与功能指纹。此前 `16a896e8…`、`1004b849…`、`8156307a…`、`c66cd68d…` 仅作历史输入，受本轮装配、协议、状态投影与门禁变化影响的旧证据已失效
...
- **当前任务进度**：U33-04、U33-05、U33-10 已在同一专项指纹上完成实现、最小验证与四路对抗，正式状态均为“已验证”；当前为 0 项 [ ]、14 项 [x]、0 项 [!]、22 项 [~]。P0/P1 当前问题已清零，但受变更影响项须重新独立审查，尚未判定单元可提交
- **状态约定**：[ ] 未审；[x] 已完成且无 P0/P1；[!] 存在 P0/P1 阻断问题；[~] 输入变化，须重审，旧证据不代表当前结论

> **清单状态**：本轮变化直接触及首次无根 paired topology、strict receiver/staging、target client governor、current-generation readiness、公开映射、S7 与架构规格，故其完整生产/消费链及跨项组合共 22 项改为 `[~]`；其余 14 项输入未变继续复用 `[x]`。下表第五列对 `[~]` 仅为历史证据，后续独立审查不得把专项验证当作清单通过。
...
| 当前完整交付物 `dd50eec8`→当前 `HEAD 43a1de0b` 的 59 个变化路径 | 事实来源 | 53 个功能/架构/测试路径逐一归入 IR33-01～IR33-36；6 个 workbench 流程路径明确排除，不参与功能通过判定。 |
...
| ---- | ---- | -------- | ---------------------- | -------- |
| IR33-01 | [~] | 单元身份、边界与完整交付物 | 冻结 HEAD `dd50eec8` 到当前工作区的 42 个变化路径并二元归属；38 个功能路径必须全部反绑 D33-01～D33-08，4 个 workbench 路径不参与功能判定；不得混入第 34～38 单元、通用备份/同步框架或未配置单机的新恢复概念。 | 当前冻结 HEAD `43a1de0b…`；`git diff --name-only dd50eec8..HEAD` 为 59 路径，其中 53 个功能/架构/测试路径逐一反绑 D33-01～D33-08，6 个 workbench 路径仅维护流程。原 42/38/4 是专项修复前计数，已由当前 exact-set 取代；交付物未出现第 34～38 单元、通用备份/同步框架或未配置单机恢复概念。 |
...
| IR33-03 | [~] | 恢复包版本与旧 S2 兼容 | 新编码只含版本化高熵恢复主秘密与根身份，不内嵌全量密文；旧 secret+trust-only package 仍可严格解码和完成既有 mesh-ready 重放，但不能由新编码器生成、不能被识别为 full payload 或令 `fullBackupReady` 为真；未知版本/字段/非规范编码拒绝。 | P1：codec 本身严格，v2 新编码与 v1 trust-only replay 分层也成立；但 `backup setup --device` 在无恢复根时先生成新 root/recipient 并连接 active paired receiver，随后才读取恢复包。目标侧 active receiver 又只在 trust 已有 `recoveryBackupPublicKey` 时注册；既有无根 home 因而无法进入首次 paired root activation，v1 包也无法先决定其冻结 recipient。该受支持兼容入口在任何 checkpoint I/O 前稳定失败，影响 IR33-16/23/34；其余版本边界成立。 |
| IR33-04 | [~] | checkpoint/verification strict codec 与关联 | envelope、full payload、stream record、verification、paired command/result 必须是严格判别联合并逐字段反绑 originating checkpoint/purpose/issuer/recipient/target/ref/digest/nonce/seq；异载荷 exact replay、错关联与未知字段在任何耐久推进或 I/O 前拒绝。 | 当前唯一 paired-result decoder 被常规 mesh 与 onboarding socket 共用，paired client 在分类前逐命令反绑 kind/checkpoint/seq/offset/ref/commit；checkpoint/full/verification codec 均 exact keys、严格联合并在副作用前拒绝错关联、未知字段和异载荷 replay。未发现 P0/P1。 |
...
| IR33-08 | [~] | 捕获与本地资产资源治理 | 日志页、资产页、明文块、临时空间和 ArtifactStore I/O 必须逐步使用现有 `authority-checkpoint` storage governor；permit 不跨 authority/store/lifecycle 锁，等待可取消，容量不足/磁盘满/stop 保持零伪成功和可重驱事实。 | 两遍 capture、`readRange` 大资产、1 MiB chunk source/sink、local CAS 与目标步骤均逐块释放并受 owner signal/governor 约束，完成值不再携整包 chunks；permit 不跨网络或 authority/store/lifecycle 锁。P2：backup/首次 pairing 客户端构造 `PairedRecoveryCheckpointTarget` 时未注入现有 storage governor，故其 256 KiB range 解码 buffer 未纳入设备统一准入；固定上界避免当前核心失败，不改变本项通过。 |
...
| IR33-11 | [~] | 单目标配置与稳定 target identity | `zz backup setup` 一次只允许独立目录或一台 active paired device；版本化 targetId→binding 严格解析、稳定持久化，targetId 不泄漏/反推路径；切换配置不丢仍有 created/replicated/superseded 未清理代际的旧映射，异身份/异载荷重放拒绝。 | 版本化 binding map 保留 current 与历史 target；setup 只接受目录或 active paired 二选一，verification candidate 先从 checkpoint stream 选 `{checkpointId,targetId}` 再按 binding 打开，verify/recover/cleanup 全程使用 created 冻结 target。异身份/异载荷冲突稳定拒绝；首次无根 paired 入口的独立兼容缺口已归 IR33-03，不改变 target identity 结论。 |
...
| IR33-14 | [~] | paired 有限协议与认证边界 | paired service 只暴露 begin/progress/append/commit/get/range/retire/activate-root 有限命令，严格绑定 home、source/target active device、recipient、checkpoint/ref/range/seq、签名 root event/record 与结果类型；无根阶段仅 current issuer 可用 strict checkpoint service，active receiver 的 activate-root 只允许本地 trust exact terminal replay；未知命令、错成员/身份或越界在业务 I/O 前拒绝。 | 历史证据只覆盖 activate-root 加入前的有限协议；当前输入增加双端 root fact、root-activated 结果与 active exact replay，须按新协议 exact-set 重审。 |
| IR33-15 | [~] | paired 断点续传、staging 与真实回读 | 传输按有界 part 续做，receiver 只写 target 私有 staging，exact-set 完整验真后才原子提升目标；断连、乱序、重复、部分 append/commit、响应丢失和连续重启复用同 checkpoint 身份且不重复/串包；get/range 必须从已发布目标逐字节回读，retire 不误删共享内容。 | part/seq/offset/progress 与稳定 staging identity 支持断点续传和效果后 replay；receiver 以 `ChunkSource/ChunkSink` 逐块验真后才发布，range 从已发布目标真回读，retire 只命中指定 checkpoint。父目录全部走 handle bridge。P2：客户端 range 解码未注入统一 governor（见 IR33-08），但固定 256 KiB 上界不构成当前 P0/P1。 |
| IR33-16 | [~] | 首次配对 onboarding 顺序 | 新设备在业务成员/mesh 能力开放前只运行受限认证 onboarding receiver；issuer 以同 checkpointId 写入、真回读、解封验证并原子激活恢复根后才发布业务信任，恢复包读回失败、链变化、断连或响应丢失不得出现 mesh-ready 但无已验证 full backup 的窗口。 | P1：首次加入新设备的 onboarding socket 路径已按受限 receiver→写入→真回读/解封→同日志激活→enroll 排序；但“已有 active paired device、home 尚无恢复根”的 `backup setup --device` 也是本单元首次 paired root activation。该入口先连仅在已有 backup key 时注册的 active receiver，后读恢复包，因而稳定不可达，且 v1 legacy recipient 无法先绑定；同根见 IR33-03/23/34。 |
...
| IR33-19 | [~] | root/chain 变化与 activation 原子性 | 首次单机目录 setup、首次配对 onboarding 及已有配对设备无根 setup 均复用现有 coordinator；后者必须在 source 原子激活后把与已存 checkpoint plan 全等的签名 root event/record 耐久提交 target，双端效果后重放全等；target 根可见前普通业务关闭，提交后有限 topology 退役并进入正常 topology。 | 历史证据未覆盖已有配对设备的双端 root fact、响应丢失 active replay 与 target 重启；当前输入须沿真实双设备链重审。 |
| IR33-20 | [~] | 真实目标回读、真解封与 verified 事务 | verify 只能按 created 绑定的真实 target/checkpoint 回读，完整打开 envelope/payload 并核对 root、chain、scope、source、target、recipient、nonce/digests；随后同一 `AuthorityCommitLog` 事务幂等写全等 verification，错包/旧根/旧链/效果后丢响应零错误推进。 | verification candidate 与 `verify()` 全程使用 created.targetId/recipient/generation，resolver 打开相应历史或当前真实目标；逐块 open 复核 envelope/payload/root/chain/source/scope/nonce/digest，随后同日志事务幂等写同 target verification 与 supersede。错包、旧根/链和效果后 replay 不错误推进；未发现 P0/P1。 |
| IR33-21 | [~] | 恢复主秘密输入与零持久化 | 第一方 verify/setup 只经保密交互临时读取/生成恢复包；主秘密不得进入 argv、env、配置、SecretStore、日志、status、RPC 或错误文本，使用后清零；创建/复制/日常 owner 不接触主秘密，只有真解封路径能取得 verificationNonce。 | verify/setup/onboarding 共用有界 `readRecoveryPackageFromTty`，独占 TTY raw input、零字符回显、支持取消并在 finally 恢复终端和清零 buffer；恢复主秘密不入 argv/env/config/SecretStore/log/status/RPC/错误，日常 owner 不持秘密，verificationNonce 仅真解封产生。未发现 P0/P1。 |
| IR33-22 | [~] | 双 readiness 投影 | 既有 S2 `ready` 继续只表示 mesh/root activation；`fullBackupReady` 必须由当前 root、独立 target、全量 scope 的 created+replicated+真解封 verified 全等派生，并给出有限 checkpoint/target/time/lsn 内部投影；旧 trust-only、仅复制、验证失败、旧根/链或每日过期不得冒充或改变冻结 ready 谓词。 | P1：available service 的 status 正确按 current root/chain 与 created+replicated+verified 投影，旧 trust-only/失败代不冒充；但 runtime slot 在当前 target 暂时离线或 paired runtime 缺失时直接返回 `unavailable, fullBackupReady:false`，不读取同一 AuthorityCommitLog 的既有 verified 事实。一个已验证、仍耐久存在的独立备份会因目标暂时离线或进程重启被错误降为未就绪，违反冻结 readiness 谓词。 |
| IR33-23 | [~] | backup CLI、/status 与产品语言 | `zz backup setup/verify/status`、server.info 与 `/status` 必须消费同一 readiness/status 合同，稳定显示未配置/待验证/可恢复和下一动作；错包、目标不可达、容量/暂态失败可行动且不把 pending/失败说成完成，不暴露 root、LSN、digest、CAS、target 内部路径等实现术语。 | P1：DTO 四态、有限 nextAction 与零内部 id/raw error 已成立；但两处生产分叉破坏同一产品合同：①无根 home 的 `backup setup --device` 在读取恢复包前连接一个目标侧尚未注册的 active receiver，无法完成首份副本（同根 IR33-03/16）；②服务端 target 暂时离线时固定报告 `fullBackupReady:false`，而独立 `backup status` 可从耐久 lifecycle 投影既有 verified 代，产生同一事实两种结论（同根 IR33-22）。 |
| IR33-24 | [x] | verified 后替换与最少一代可恢复 | 只有同 current root 的新 full checkpoint 从独立目标真解封并 verified，才能在同一 checkpoint 事务把旧代标记 superseded；较新候选失败、仅 replicated、旧根或非 full 不替换，首次 ready 后始终保留至少一个维持 `fullBackupReady` 的代际。 | 只有真实 target 全包解封验证成功后，verified 与同 current recovery key 的旧代 superseded 才在同一 CommitEnvelope 落定；replicated/失败/旧根/非 full 不替换，响应丢失 exact replay 收敛。IR33-22 是状态投影分叉，不改变耐久替换事实；未发现 P0/P1。 |
...
| IR33-26 | [x] | 跨根、跨目标与代际恢复 | root rotate 后旧根包立即不计 ready且不 rewrap；新根 full verified 前不得清唯一旧代物理副本，目标切换仍能按耐久 targetId 找回/清理旧代；并发 verify/supersede/cleanup、响应丢失和连续重启必须收敛到单调代际，无 ready 回滚或跨 target 误删。 | root/chain generation 使旧代不计 current ready且不 rewrap；verification candidate、recoverPending、verify 与 cleanup 都按 created.targetId 解析历史 binding，新根 verified 前旧独立 target 不被清。并发/丢响应靠同日志 exact facts 单调收敛；IR33-22 只误投影 target 离线时的 ready，不导致跨代事实或误删。 |
| IR33-27 | [~] | 两生产根、角色与 receiver exact-set | 显式启用单机 current anchor、anchor+executor、anchor-only/远端 executor 中只有 current anchor 恰一 service/capture/target client；receiver 分 onboarding、无根 current-issuer finite、active 三阶段各恰一，finite 只含 endpoint+strict checkpoint，激活应答后退役并重载 active，executor-only、surface、非 current anchor 和未启用单机零 owner/权威读取。 | 历史证据没有无根 finite→active 的完整装配生命周期；当前输入须反绑三阶段 receiver/service exact-set。 |
| IR33-28 | [~] | 启动恢复、公开准入与关闭 | 启动在 backup 管理入口和第 34 单元接缝前恢复 created/replicated/verified/superseded/cleanup 与 target binding/readiness；关闭先拒绝新创建，在安全页/块边界取消并等待 active work、停 timer/receiver、释放 permit，未终态耐久事实留给重启重驱，不遗留双 owner/loop 或伪终态。 | P1：slot start 已在 serve 监听前 await 配置 reload、target 打开、`recoverPending` 与 owner start；关闭也拒新建、取消并等待 active、释放 target/permit，未终态事实可重驱。但 target 暂时离线时 startup 直接发布 `unavailable, fullBackupReady:false`，未从耐久 log 恢复既有 verified readiness，公开准入后的状态因进程重启/目标在线性而漂移；同根 IR33-22/23。cleanup 仍按既定非阻塞边界后台执行。 |
| IR33-29 | [x] | 迁居前 forced 窄接缝 | 下游只能查询当前 root 的 verified full checkpoint或以稳定 request 请求当前 owner 创建/复制同一候选；无 recovery package 时不得假装 verified，失败保留可重试状态；接缝不得读取/导入 catalog、冻结 source、提交 transfer、切换 current anchor 或执行 restore。 | forced 与 daily 共用 keyed single-flight 和 created lifecycle，稳定 request 只重放自身 candidate；无恢复主秘密只能达到 replicated/pending，不能伪造 verified。公开窄接缝未读取/import catalog、提交 transfer、切 current anchor 或执行 restore；IR33-22 的 unavailable 投影不扩展该接缝。 |
| IR33-30 | [~] | S7 入口、装配与角色门禁 | 现有单一 S7 gate/golden 必须逐项反绑规格 recovery-backup 行的 setup/verify/status、daily/forced trigger、paired begin/progress/append/commit/get/range/retire/activate-root，及生产 owner create/start/stop、current-anchor guard、onboarding/finite/active receiver 顺序、有限 service set、target trust commit、active terminal replay 与三 client governor exact-set；真实变异 fail-closed且合法两根零误杀，不建新 lint/发现框架。 | 历史证据早于 activate-root、有限 topology 退役和双端 trust 提交；当前 S7 17/17+golden 仅作专项证据，须在独立审查中按新输入判定。 |
| IR33-31 | [~] | 安全、最小权限与 wire/path 隔离 | 有限核对 full payload、envelope、recovery package、directory manifest、paired command/result、status/RPC：只含冻结身份和内容引用；秘密、环境事实、绝对路径、raw store/log 与通用读取/删除能力不逸出，签名/受众/role/root/target/ref/range/path guard 均在该链首次副作用前验证。 | strict payload/envelope/package/paired result、签名/受众/role/root/target/ref/range guard 均在副作用前；公开 status 与 failure code 不泄露内部 id/path/raw error，秘密经无回显边界。directory/paired child I/O 已改为 frozen handle-relative bridge，raw store/log 与通用路径能力未逸出；未发现 P0/P1。 |
| IR33-32 | [~] | 资源、并发与故障终态 | 空/大资产、并发 capture/verify/cleanup、容量饱和、磁盘满、取消、stop、网络等待和锁序必须有界；网络不持 permit，permit 不跨 authority/store/lifecycle 锁，single-flight/分页/chunk 有上限；每个失败要么零耐久副作用，要么留下可恢复的唯一 checkpoint 事实且不静默成功。 | 两遍 capture、1 MiB source/sink、分页、keyed single-flight、owner abort、handle-relative target 与 startup recover 使总量有界并保留唯一耐久 progress；网络等待不持 permit，permit 不跨 authority/store/lifecycle 锁。P2：两处 paired client 未把其固定 256 KiB range decode buffer 接入 storage governor，统一容量准入存在有限旁路；无 O(total) 驻留或伪终态，故不改变本项通过。 |
| IR33-33 | [~] | 成比例的直接验收证据 | 证据计划必须覆盖 strict codec/恢复包、真实 AuthorityCommitLog/FileArtifactStore/ArtifactLifecycleIndex、双端无根 v1/v2 setup、target trust 提交与 active terminal replay、容量/governor、离线 readiness、两生产根与 S7 真实变异；配置只做角色 exact-set，不做配置×故障笛卡尔积。 | 专项已有真实双设备 v1/v2 2/2、CLI 支撑 9/9、mesh 过滤 2/2、S7 17/17+golden及同输入构建；因交付输入变化，本项保持 `[~]`，不得以专项通过替代独立功能判断。 |
| IR33-34 | [~] | 上游 S2 与共同装配点兼容 | 有限核对既有 trust-only package、source coordinator、target bootstrap trust store、mesh-ready、首次配对、已有配对无根 serve 的 finite→active 切换、canonical registry 与 server.info；不得改旧签名语义、放宽普通业务或把后继 transfer/restore/sync 并入 S9。 | 历史证据未覆盖 target 根事实与同进程正常 topology 重载；当前输入须与目录 setup、新设备 onboarding、普通 serve 隔离和 trust/root fail-stop 双向对账。 |
| IR33-35 | [~] | 架构/规格同步与后继能力守界 | 总纲 S9、规格 full payload/§7/§8/§15、core exports、mesh/CLI/server 合同和生产调用图必须全等；第 34/35 单元只见冻结窄接缝/载荷，`SourceFreezeProof(anchor)`、AuthorityCatalog import、TrustTransition、ReadyProof、AnchorTransferCommit、restore/domain-reset/reenroll/credential rotation 未进入当前 wire、CLI 或装配。 | 总纲当前 S9、规格 full payload/§7/§8/§15 与 core/mesh/CLI/server 名义构件同步，当前偏差已落入 IR33-03/22/23 等有限实现审查项；交付物未启用 SourceFreezeProof、catalog import、TrustTransition、ReadyProof、AnchorTransferCommit、restore/domain-reset/reenroll/credential rotation。未发现范围外能力提前实施。 |
| IR33-36 | [~] | 来源、D33 义务与路径反向闭包 | D33-01～D33-08、全部适用来源条款及当前功能路径 exact-set 必须按 core 合同/retention/governor、mesh full capture/crypto/service/target/owner、paired/onboarding/root activation、CLI config/commands/assembly、server/status、架构规格、S7 与直接测试八组逐一归入 IR33-01～IR33-35；新增 helper、协议字段、生产入口和测试均须有完整链路落点。 | 本轮新增 target root activation helper、finite control 生命周期、normal topology reload 与真实双设备测试，已归入受影响项并统一标 `[~]`；后续独立审查须重算当前 exact-set，不得复用旧 53 路径口径。 |

> 两个 P1 与一个 P2 已转入正式问题列表并在专项指纹上完成修复；当前清单为 14 项 `[x]`、22 项 `[~]`、零 `[!]`/`[ ]`。两类问题表保持空，专项验证不替代后续对 `[~]` 的正式独立审查。
