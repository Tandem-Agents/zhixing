...
```
目标：只收敛第 33 单元正式问题列表中同根重开的 U33-04～U33-06 三个 P1 和新增的 U33-13 一个 P2，使四项真正命中 target setup/verify/startup lifecycle、checkpoint 总量驻留、directory/paired 父目录物理身份及 source-local release 事实语义的根因，并具备可由执行者一次实施的最优方案与完整验收条件。不修改实现，不运行构建或测试，不审查其他问题；U33-01～U33-03、U33-07～U33-12 与 EX33-01 的既有结论直接复用，价值裁决已经删除、降级或收窄的主张不得恢复，第 34～38 单元能力不得提前并入本单元。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 33 单元正式文件中的 U33-04～U33-06、U33-13、EX33-01，只依据四项问题最新的事实、价值裁决、方案、验收条件和状态继续。
...
1. 按 `U33-04 → U33-05 → U33-06 → U33-13` 从权威架构、规格和当前生产调用图重建事实链，核准 current/历史 target binding、setup/verify/startup owner、capture/open/target buffer ownership、configured/owned parent 与 child 系统调用、source-local/target retention 的唯一事实源、稳定身份、线性化点、消费者、异常终态、当前损失、受影响审查项、评级和工作量；判断当前描述命中根因还是表象。同根内容必须合并，独立根因不得互相遮蔽；U33-04 的 cleanup 启动门禁收窄、U33-06 的额外文件排除及 U33-13 的 P2 边界，未满足正式重开条件时不得恢复原主张或扩面方案。
2. 穷尽直接变体：U33-04 覆盖 directory/paired setup、current/历史 target、created/replicated、效果前后失败、离线恢复、公开准入和连续重启；U33-05 覆盖零/大资产、record/artifact/chunk 总驻留、capture/create/open、directory/paired commit/read、容量等待、取消和 stop；U33-06 覆盖 configured/owned parent 在 assert→open/rename/rm 间的 symlink/junction/reparse/rename 替换、最终文件替换、发布、回读、retire、越根和重启；U33-13 覆盖 verified→superseded、24 小时 local GC、27 天 target retire、共享业务 ref、响应丢失和连续恢复。每格必须指出稳定身份、耐久事实、零副作用边界、消费终态和直接验收；无法解释即继续修正根因。
3. 以锁定范围内的最优架构审查方案：U33-04 只复用 durable binding map、现有 service/owner slot 与 paired transport，按 created.targetId 验证、setup 同步首份复制、公开准入前只恢复未终态 created/replicated；U33-05 保持 wire/chunk 不变，以现有 governor 驱动有限 producer/consumer 和增量 verification；U33-06 复用 frozen root binding 与 no-follow handle，增加两 adapter 共用的有限 handle-relative child I/O 原语；U33-13 修正规格边界并让 superseded/即时 local-released 与 source-local GC 共用唯一事实，27 天只约束独立 target。方案必须用最少文字说清改什么、怎么改、关键边界及完成判据；不得新增扫描/备份/路由/存储/事务/outbox/事件总线/registry、第二事实源、多目标或云能力、通用文件系统/streaming框架、新 lint/test runner、监控、诊断、benchmark 和信息采集。发现缺口时直接修正对应原问题，使执行者无需实现猜测即可一次完成。
4. 四项看似闭合后，对同一份未修改问题列表执行四路冷启动对抗复审：target setup/verify/startup lifecycle、checkpoint 总量资源上界、directory/paired parent-child 物理身份、local/target retention 事实与产品范围价值。各路必须抛开前轮结论，从当前合同和源码主动构造第 2 步反例，并核查 `U33-04↔U33-13`、`U33-04↔U33-05`、`U33-05↔U33-06` 及四项与 EX33-01、第 34～38 单元边界的直接交界；发现真实反证则修正原记录并重新执行四路复审。

只有现有架构无法唯一推出方案，且选择会显著改变产品需求、用户体验、成本或单元边界时才暂停；其余架构选择按整体最优且不留债务自主收敛。

完成条件：同一份未修改问题列表通过四路冷启动对抗复审；U33-04～U33-06、U33-13 的全部受支持 target 代际、setup/verify/startup 终态、总量 buffer 上界、parent/child 物理身份与 local/target retention 事实均被根因完整解释，影响面无遗漏，评级与工作量有事实依据，最优方案和验收条件可直接执行，不会因同根残留继续局部返工。满足后明确回复“U33-04～U33-06、U33-13 的根因与最优方案已闭合”并立即停止。
...
```text
目标：彻底解决第 33 单元同根重开的 U33-04～U33-06 三个 P1 和新增的 U33-13 一个 P2，闭合 target setup/verify/startup lifecycle、checkpoint 总量驻留、directory/paired 父目录物理身份及 source-local release 事实语义的全部同根直接变体；不得扩展到其他问题或全单元流程。U33-01～U33-03、U33-07～U33-12 与 EX33-01 的既有结论直接复用，价值裁决已经删除、降级或收窄的主张不得恢复，第 34～38 单元能力不得提前实施。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 33 单元正式文件中的 U33-04～U33-06、U33-13、EX33-01，只依据四项问题最新的根因、价值裁决、F33-17～F33-29 固定矩阵、方案、验收条件、反证账和状态继续。
...
1. 修复前从权威架构、规格与当前生产调用图重建 F33-17～F33-29 固定矩阵。U33-04 覆盖 directory/paired setup、current/历史 target、created/replicated、效果前后失败、离线恢复、公开准入和连续重启；U33-05 覆盖零/大 payload、单个超大 artifact、海量小 ref、record/artifact/chunk 总驻留、daily/root activation、local CAS/bootstrap/owner、directory/paired、容量/磁盘、取消和 stop；U33-06 覆盖 configured root 建立及 configured/owned parent 在每个 child 系统调用前的 symlink/junction/reparse/rename 替换、最终文件替换、发布、回读、retire、越根和重启；U33-13 覆盖 verified→superseded、24 小时 local GC、27 天 target retire、共享业务 ref、legacy local-released、响应丢失和连续恢复。逐格绑定 source-head、checkpoint/target/binding、configured/owned parent/child handle 与 retention fact 身份，唯一耐久事实、线性化点、零副作用边界和直接证据，并持续核对价值裁决重开条件、EX33-01 及第 34～38 单元边界。
2. 按 `U33-13 → U33-04 → U33-05 → U33-06` 一次完成。先同步总纲、规格与 reducer，唯一规定 `checkpoint-superseded` 为 source-local release fact并继续驱动 `ArtifactLifecycleIndex`；停止生产 `local-released`，仅把历史记录兼容解码为不参与 retention/status/完成的 no-op；27 天后只按 created.targetId 幂等 retire 独立 target并写 `target-retired`。

   再让 candidate 先从 checkpoint stream 按当前 root/chain/recipient 选出稳定 `{checkpointId,targetId}`，CLI 随后从 durable binding map 打开精确 adapter；verify 的 replicated、verification、verified/failed 全程只使用 created 冻结的 targetId/recipient/generation。directory/paired setup 都先耐久 select 并打开所选 adapter：无 recovery root 时复用现有 root establish 真写入、回读、验证和激活，已有 root 时用稳定 requestId 同步 create+replicate，只有对应 durable 副本成立才成功。把 owner slot start 改为可等待阶段，serve 在公开监听前完成配置 reload 与 created/replicated 恢复；成功发布 available，可选目标离线发布 unavailable并重试，trust/root/member 矛盾 fail-stop，cleanup 保持后台。

   保持 envelope、wire 与 1 MiB chunk 不变，建立 checkpoint 内部窄 `ChunkSource/ChunkSink`。capture 在同一冻结 source-head 上两遍执行：首遍逐页/逐 typed root 形成目录和 retention exact-set，每次追加 descriptor/ref 前核对既有 1 MiB header 预算并释放正文；复核 snapshot current 后，第二遍重读同一日志前缀并用现有 `ArtifactStore.readRange` 固定块读取大资产，逐块加密写入现有 local CAS，最后按已落 chunk descriptors 签 envelope并写 created。service、owner、bootstrap authority/store、directory、paired 与 activation 只传轻量 `{envelope,envelopeRef}` 和按 seq 的 source/sink；target/staging、verify/activation 均逐块验真、fsync、释放/清零，完成值不得携带 materialized chunks。每个活 buffer 持同一 governor permit，网络等待和外层 authority/store/lifecycle 锁不得持 permit。

   最后保留 frozen root binding，在 `@zhixing/mesh` 内建立唯一 checkpoint-child 跨平台句柄桥：POSIX 从冻结 dirfd 以逐组件 no-follow/beneath 的 *at/openat2 操作完成创建、打开、rename、unlink和fsync；Windows 从 filesystem/drive root handle 以 RootDirectory 相对打开、handle rename/disposition、FileId/volume/reparse 复核与 flush 完成同一语义。configured root 不存在时也只能从已打开 filesystem root逐组件创建；桥只接受已验证单组件名并暴露 checkpoint 固定 mkdir/open/read/write/rename/exact-set remove/sync，两 adapter 禁止再对 root/child 使用路径版副作用。同步直接相关架构、规格和测试。同根残留并入原问题，禁止恢复 cleanup 启动门禁、额外文件主张或 source-local 27 天保留，禁止新增通用 streaming/文件系统框架、扫描/备份/路由/存储/事务/outbox/事件总线/registry、第二事实源、多目标/云能力、新 lint/test runner、监控、诊断、benchmark 或信息采集。每个实质阶段完成后立即更新正式状态与证据。
3. 按验证手册运行受影响闭包的最小必要类型检查、setup/verify/startup、full capture/crypto/bootstrap/activation、directory/paired source-sink 与跨平台句柄 bridge、retention/cleanup 直接合同和场景测试，以及必要的现有 S7/派生资产检查；源码有变时按项目常驻规则取得一次同输入构建证据。真实反例必须穿过真实 `AuthorityCommitLog`、`ArtifactStore`、`ArtifactLifecycleIndex`、storage governor、directory/paired target与两生产根，直接注入历史 target、离线/响应丢失、超大 artifact/海量小 ref、容量/磁盘、每个 parent-child 系统调用前替换、共享 ref 和连续重启；不得以 mock 自报 lifecycle/上界/物理身份或只验证返回值，不得运行包全测、模块回归、配置×故障笛卡尔积或与四项验收无关的验证。失败先归因，实现问题直接修复并回到第 2 步。
4. 验证通过后冻结当前交付物指纹，整轮只读地逐格重建 U33-04～U33-06、U33-13 事实链；测试通过不得代替功能判断，矩阵全部完成后才统一归并。随后对同一指纹执行四个相互隔离的冷启动对抗角色：target setup/verify/startup lifecycle、checkpoint 总量资源上界、directory/paired parent-child 物理身份、local/target retention 事实及产品范围价值。各角色须抛开既有结论，主动重造第 1 步全部适用反例，并核查 `U33-04↔U33-13`、`U33-04↔U33-05`、`U33-05↔U33-06` 及四项与 EX33-01、第 34～38 单元边界的直接交界。
...
结束条件：同一冻结指纹上的 U33-04～U33-06、U33-13 方案全部落地，受影响闭包的最小必要验证通过，专项功能审查与四路冷启动对抗均留下完整矩阵；累计同根反证全部有耐久处置，证明 setup 成功、历史 verify 与 startup available 均绑定同一 created target代际且可选目标故障不阻普通业务；daily/root activation 从 capture 到验证的总驻留只随固定并发×chunk/header 上界增长；configured root建立及全部 child I/O在 POSIX/Windows 都锚定冻结物理身份并保持零根外副作用；source-local 只由 superseded释放，独立 target只由27天 target-retired终结，legacy事实不分叉。价值裁决重开条件仍不成立，EX33-01与第34～38单元边界未改变，四项均已更新为“已验证”。满足后明确报告“U33-04～U33-06、U33-13 四项问题已彻底解决”并立即停止；不得进入全单元终审或单元提交验证。
...
- **交付基线**：U33-04～U33-06、U33-13 专项修复当前指纹 `8156307a701608189b5e97d0badba4c773900bd1edef92cce793f17f3b14e23b`，覆盖 27 个非工作台生产实现、直接测试、跨平台 bridge 与架构/规格路径；`research/design/workbench/` 流程账本不参与功能指纹。原十二项专项指纹 `c66cd68d…` 及更早冻结提交只作历史审查输入，受本轮共享合同与生产装配变化影响的旧证据已失效
...
- **当前任务进度**：U33-04～U33-06、U33-13 已完成专项修复并在同一指纹上验证；36 项清单因 core durable log/identity、checkpoint source/sink、两生产根、物理 bridge、架构规格与直接证据输入变化，现为 0 项 [ ]、0 项 [x]、0 项 [!]、36 项 [~]。正式问题均已验证，临时问题列表保持为空；尚未执行修复后的正式独立审查
- **状态约定**：[ ] 未审；[x] 已完成且无 P0/P1；[!] 存在 P0/P1 阻断问题；[~] 输入变化，须重审，旧证据不代表当前结论

> **清单状态**：U33-04～U33-06、U33-13 修复已清除原 15 项 `[!]`；本轮变化贯穿共享 authority log/identity、full checkpoint 生产链、两生产根、target adapter、跨平台 bridge、架构规格与必要证据，使 36 项登记输入及跨项关系全部变化，故统一改为 `[~]`。下表第五列均只保留为历史证据，不能代表当前结论；后续独立审查只审这些 `[~]`，不得把专项验证当作清单通过。
...
| CLI 管理、配置、配对、装配与生命周期 | `packages/cli/src/commands/__tests__/info-commands.test.ts`、`packages/cli/src/commands/info-commands.ts`、`packages/cli/src/index.ts`、`packages/cli/src/runtime/rpc-management-facade.ts`、`packages/cli/src/serve/access-surface.ts`、`packages/cli/src/serve/backup-command.ts`、`packages/cli/src/serve/backup-runtime-owner.test.ts`、`packages/cli/src/serve/backup-runtime-owner.ts`、`packages/cli/src/serve/backup-target-config.ts`、`packages/cli/src/serve/command.ts`、`packages/cli/src/serve/mesh-bootstrap-store.ts`、`packages/cli/src/serve/mesh-pair-command.test.ts`、`packages/cli/src/serve/mesh-pair-command.ts`、`packages/cli/src/serve/mesh-runtime-assembly.ts`、`packages/cli/src/serve/mesh-runtime-bootstrap.test.ts`、`packages/cli/src/serve/recovery-package-input.test.ts`、`packages/cli/src/serve/recovery-package-input.ts`、`packages/cli/src/serve/surface-asset-authority.ts`、`packages/cli/src/setup-delivery.ts` | IR33-03～IR33-04、IR33-11～IR33-23、IR33-27～IR33-35 |
...
> **修复后失效说明**：当前指纹 `8156307a…` 改变了全部审查项登记的共享输入或跨项组合关系；以下 36 行旧证据统一失效，仅作历史事实保留。状态 `[~]` 表示须在当前完整交付物上重新独立判定，不表示仍有 P0/P1。

| 编号 | 状态 | 审查对象 | 有限审查范围与通过条件 | 证据记录 |
| ---- | ---- | -------- | ---------------------- | -------- |
| IR33-01 | [~] | 单元身份、边界与完整交付物 | 冻结 HEAD `dd50eec8` 到当前工作区的 42 个变化路径并二元归属；38 个功能路径必须全部反绑 D33-01～D33-08，4 个 workbench 路径不参与功能判定；不得混入第 34～38 单元、通用备份/同步框架或未配置单机的新恢复概念。 | `git diff --name-only dd50eec8..2ea2e546` 与定稿路径 exact-set 一致：38 个功能路径均可反绑 D33-01～D33-08，4 个 workbench 路径仅承载流程；未发现第 34～38 单元能力、通用备份/同步框架或未配置单机恢复概念进入交付物。 |
...
| IR33-03 | [~] | 恢复包版本与旧 S2 兼容 | 新编码只含版本化高熵恢复主秘密与根身份，不内嵌全量密文；旧 secret+trust-only package 仍可严格解码和完成既有 mesh-ready 重放，但不能由新编码器生成、不能被识别为 full payload 或令 `fullBackupReady` 为真；未知版本/字段/非规范编码拒绝。 | `recovery-package.ts` 对 v1/v2 分支与 unknown fields 做严格解码；`backup-command.ts`、`mesh-pair-command.ts` 均优先把 v1 `legacyCheckpoint` 送入现有 `RecoveryActivationCoordinator`，v2 编码器不生成旧载荷。full readiness 只接受 full generation/verification，trust-only replay 不会产生 `fullBackupReady`。 |
| IR33-04 | [~] | checkpoint/verification strict codec 与关联 | envelope、full payload、stream record、verification、paired command/result 必须是严格判别联合并逐字段反绑 originating checkpoint/purpose/issuer/recipient/target/ref/digest/nonce/seq；异载荷 exact replay、错关联与未知字段在任何耐久推进或 I/O 前拒绝。 | P2：常规 mesh transport 会严格解析 paired result，首次 pairing socket 只做外层 exact keys 后断言内部类型；但 `PairedRecoveryCheckpointTarget` 对每个命令的 result kind、checkpointId、seq/offset/progress、ref bytes/digest 都在推进前逐项手工复核，无法构造错关联成功终态。缺口仅是未知内部字段未 fail-closed，降为非阻断 strictness 债务；其余 codec/关联成立。 |
| IR33-05 | [~] | 唯一 authority 前缀冻结与复验 | capture 只能从 current anchor 唯一 `AuthorityCommitLog` 固定一个 logId/lsn/frameEndOffset/prefixDigest，按规范页读取恰至该前缀并在返回前复验；并发追加进入下一代，坏尾、错 log/prefix、越 LSN、读中漂移在 `checkpoint-created` 前零事实、零目标写。 | `full-checkpoint.ts` 先取得 `log.checkpoint`，以 origin checkpoint 分页读取并逐 envelope 约束至目标 LSN，返回前再次 `readEnvelopeAt` 校验 logId/lsn/frameEndOffset/prefixDigest；捕获完成前不写 checkpoint 生命周期或目标，后续并发追加不混入冻结代。 |
| IR33-06 | [~] | 六类权威覆盖与保留资产闭包 | 逐行核对 §7 六类：四类应入内容完整、环境/秘密与非权威缓存零进入；ArtifactLifecycleIndex 必须追平并绑定 IR33-05 的同一 log checkpoint 后再生成当前保留记录及传递 `ArtifactRef` 闭包。共享/嵌套/外置/大资产去重且完整，缺 ref、错 bytes/digest、索引落后/损坏或覆盖缺口均 fail-closed，不从投影文件或缓存补事实。 | `captureFullAuthorityCheckpoint` 先冻结 `checkpointRetentionSnapshot` 并以其中同 log head 读取 authority prefix，随后只经注册型 record/root classifier 形成候选并用 `retainedAtCheckpoint` 复核同一 source-head；index 变化整轮重冻。真实 log/store/index 用例覆盖大资产、共享 ref 与会话删除，缺失或损坏 ref 在 created 前拒绝。 |
| IR33-07 | [~] | 禁止类别与 checkpoint 防递归 | 环境事实、SecretStore/设备秘密、workspace 原始路径、设备/执行缓存、非权威缓存和旧 checkpoint envelope/chunks 不可表示；checkpoint lifecycle 的旧引用不得递归进入新载荷，但当前候选本地 envelope/chunks 仍由生命周期索引正确保留。 | 捕获只读取 authority log 的规范提交记录，资产闭包只接受六个注册 schema 与各自严格 validator；环境、SecretStore、原始路径及缓存没有注册 root。`checkpoint` stream 明确不作为新载荷候选递归，当前候选 envelope/chunks 则由生命周期 owner 单独持有；未发现投影或缓存补事实路径。 |
| IR33-08 | [~] | 捕获与本地资产资源治理 | 日志页、资产页、明文块、临时空间和 ArtifactStore I/O 必须逐步使用现有 `authority-checkpoint` storage governor；permit 不跨 authority/store/lifecycle 锁，等待可取消，容量不足/磁盘满/stop 保持零伪成功和可重驱事实。 | P1：各 I/O 叶步骤已接入 `authority-checkpoint` governor 与 owner signal，但 `captureFullAuthorityCheckpoint` 仍同时保留全部 record pages，并以 `Promise.all` 读入全部 retained artifacts；create/open、directory/paired read 与 paired commit 又分别物化整包 chunks 或全部声明内容。固定 16 MiB permit 只包住单次调用，不能约束跨步骤累计驻留；受支持的大 authority 可在已获 permit 时以 O(备份总量) 内存增长直至失败。 |
| IR33-09 | [~] | periodic/root-activation 共用密码内核 | 两种 purpose 必须复用唯一 X25519-HKDF-SHA256、AES-256-GCM create/open 内核及 ephemeral enc、wrapped DEK、nonce/AAD、verificationNonce、自摘要和 issuer 签名；periodic 不接受 plan，root-activation 必须全等绑定完整 plan，跨 purpose replay 拒绝。 | `checkpoint.ts` 的 create/open 是两种 purpose 的唯一实现，统一 X25519-HKDF-SHA256、AES-256-GCM、AAD、wrapped DEK、自摘要与 issuer 签名；purpose-specific payload 校验强制 periodic 无 plan、root-activation 全等 plan，跨 purpose 或错受众在解封/推进前拒绝。 |
...
| IR33-11 | [~] | 单目标配置与稳定 target identity | `zz backup setup` 一次只允许独立目录或一台 active paired device；版本化 targetId→binding 严格解析、稳定持久化，targetId 不泄漏/反推路径；切换配置不丢仍有 created/replicated/superseded 未清理代际的旧映射，异身份/异载荷重放拒绝。 | P1：配置文件与 `resolveTarget` 已保留历史 binding，`recoverPending`/cleanup 可按 created.targetId 找回目标；但 `verify` 先以 `#assertCreatedBinding` 强制 created 等于当前 target/generation，后续 verification/replicated 关联也写死当前 target，CLI `verificationCandidate` 只列当前代。另 paired `backup setup` 仍只选配置并返回，不执行该 binding 的首次创建/复制；历史验证和新 binding 首份副本均没有完整生产入口。 |
| IR33-12 | [~] | 目录目标物理独立与路径安全 | configured root 必须冻结 lexical/canonical path 与 filesystem identity，拒绝与 authority root 同物理域、root 自身 link/reparse、非目录、越根及绑定/遍历/打开/读后替换；最终文件 no-follow、handle identity 与 frozen containment 全程一致，非法路径零根外读写。 | P1：configured/owned root 的 lexical/canonical/dev/ino 冻结、root link 拒绝与最终文件 no-follow/handle 前后 identity 已实现；但所有 child `open/rename/rm` 仍以 canonical 字符串执行。另一进程可在 `assertCheckpointDirectoryIdentity` 之后、系统调用之前替换父目录为链接或 junction，使首次写入/删除先落到根外，事后 path/handle 复核只能报错而不能满足“零根外 I/O”。directory 与 paired staging 共用此竞态。 |
| IR33-13 | [~] | 目录目标原子耐久发布与真回读 | 每个 checkpoint 使用私有临时目录，逐文件写入/fsync、manifest/envelope/chunk exact-set 校验、原子 rename 与目录 fsync 后才可见；同载荷幂等、异载荷冲突、部分/崩溃发布不可见，read 必须逐字节回读同包，retire 只删除指定 superseded 代际。 | P1（与 IR33-12 同根）：稳定 staging、逐文件 handle fsync、manifest/envelope/chunk 关联、rename+目录 fsync、同载荷重放和真回读均成立；但发布、回读与 retire 的父目录仍只在路径系统调用前后分离核验。父身份在核验与 `open/rename/rm` 间被替换时可先产生根外副作用，故原子发布/退役的物理边界未闭合。额外文件仅属已收窄的空间健壮性主张，本轮不据此扩级。 |
| IR33-14 | [~] | paired 有限协议与认证边界 | paired service 只暴露 begin/progress/append/commit/get/range/retire 有限命令，严格绑定 home、source/target active device、recipient、checkpoint/ref/range/seq 与结果类型；未知命令、已撤销或失去 active target 角色、错成员/身份、任意 path/ArtifactStore 或越界 range 在业务 I/O 前拒绝。 | receiver 只注册 begin/progress/append/commit/get/range/retire，命令 codec 严格绑定 home、source/target、recipient、checkpoint/ref/range/seq；mesh assembly 仅向当前 issuer anchor 授权 active paired receiver，未知命令/身份/角色/越界 range 在 staging/target I/O 前拒绝。onboarding 客户端结果解码缺口已归 IR33-04。 |
| IR33-15 | [~] | paired 断点续传、staging 与真实回读 | 传输按有界 part 续做，receiver 只写 target 私有 staging，exact-set 完整验真后才原子提升目标；断连、乱序、重复、部分 append/commit、响应丢失和连续重启复用同 checkpoint 身份且不重复/串包；get/range 必须从已发布目标逐字节回读，retire 不误删共享内容。 | P1：part/seq/offset、envelope 与 partial handle fsync、稳定 staging identity、乱序/重放拒绝、发布后 range 真回读已闭合；但 staging 与 durable target 仍受 IR33-12 的父目录替换竞态，且 `commit` 先把全部已上传 chunks 读入数组再调用 target，source `read` 也累计整包 chunk。大备份会绕过“有界 part”的进程内存上界，路径竞态可先发生根外 I/O。 |
| IR33-16 | [~] | 首次配对 onboarding 顺序 | 新设备在业务成员/mesh 能力开放前只运行受限认证 onboarding receiver；issuer 以同 checkpointId 写入、真回读、解封验证并原子激活恢复根后才发布业务信任，恢复包读回失败、链变化、断连或响应丢失不得出现 mesh-ready 但无已验证 full backup 的窗口。 | P1：受限 receiver、严格 paired result、受治理 capture/target、无回显输入、真回读/解封及同日志 trust+verified 后才 enroll 的顺序均成立；但首次 full capture/open 与 paired commit 仍按整包驻留内存（IR33-08/15）。合法大 authority 可在信任开放前耗尽内存并使首次配对无法完成，故该受支持 onboarding 仍有可达失败。 |
| IR33-17 | [~] | 周期 owner exact-set 与 stable due identity | trusted-home current anchor 或显式启用备份的单机 current anchor 恰一 owner；UTC 日历日/forced request 与当前 root+target 共同确定唯一候选，同根同目标的同日并发与 daily/forced single-flight，墙钟回拨及 root/target 切换不得另造或误重放候选；无根、无目标、非 current anchor、executor-only、surface、target 已撤销和未启用单机零周期事实并返回稳定可行动状态。 | `candidateKey` 由 generation（root/recipient/chain/target）与 daily/forced request 派生，checkpointId 继续绑定冻结 source prefix；同键加入同 promise，异键稳定返回 `checkpoint-candidate-busy`，不建队列。current-anchor/显式配置 owner 与角色零装配由生产 slot、descriptor 及 S7 exact-set 共同约束。 |
| IR33-18 | [~] | created→replicated 耐久生命周期与重放 | 创建前捕获可重算；`checkpoint-created` 后 checkpoint/source/recipient/target/envelopeRef 全冻结，复制只重驱同一代，目标 durable 后才写 `checkpoint-replicated`；效果前后失败、取消、响应丢失、配置切换、停机与连续重启不另造候选或伪造复制。 | P1：service 内 created→target durable→replicated、同 generation/request/source exact replay 及历史 binding 的未复制重驱已实现；但 serve 仅 `start()` 后以零延迟 timer 异步触发 daily，公开管理/status 可先服务，`status()` 自身只 reload 不执行 `recoverPending`。因此崩溃遗留 created 可在公开准入窗口保持未重驱，startup 恢复顺序未满足冻结生命周期。 |
| IR33-19 | [~] | root/chain 变化与 activation 原子性 | 首次单机目录 setup、首次配对 onboarding 及后续 establish/rotate 均复用现有 coordinator；未激活候选在 root/chain/target 变化时失效并安全重建，已提交激活 exact replay；created/replicated/verification/trust event 的原子边界保持既有 S2 合同，不得出现当前根有效但相应独立备份未验证的窗口。 | `RecoveryActivationCoordinator` 对 package/created/replicated/目标 readback/open 与 trust+verified+superseded 采用同 checkpointId 和同 `AuthorityCommitLog` 原子事务；production onboarding 在该事务完成后才 enroll。错误 root/chain/target 不能提交，已提交 activation 可 exact replay。周期候选身份缺口已归 IR33-17，不改变 activation 原子边界。 |
| IR33-20 | [~] | 真实目标回读、真解封与 verified 事务 | verify 只能按 created 绑定的真实 target/checkpoint 回读，完整打开 envelope/payload 并核对 root、chain、scope、source、target、recipient、nonce/digests；随后同一 `AuthorityCommitLog` 事务幂等写全等 verification，错包/旧根/旧链/效果后丢响应零错误推进。 | P1（与 IR33-11/18 同根）：真回读、full binding、签名/nonce 与 verified+superseded 同日志事务对当前 target 成立；但 `verify()` 在调用历史 resolver 前执行 `#assertCreatedBinding`，强制 created.targetId/generation 等于当前值，并在 replicated/verification 中继续使用当前 targetId。切换后已耐久复制的旧 binding 无合法 verify 入口，不能按其冻结身份终态化。 |
| IR33-21 | [~] | 恢复主秘密输入与零持久化 | 第一方 verify/setup 只经保密交互临时读取/生成恢复包；主秘密不得进入 argv、env、配置、SecretStore、日志、status、RPC 或错误文本，使用后清零；创建/复制/日常 owner 不接触主秘密，只有真解封路径能取得 verificationNonce。 | 两处生产入口共用 `readRecoveryPackageFromTty`：复用 stdin ownership/key events、只在 TTY raw mode 接受有限输入，字符/退格不回显，Ctrl-C/非 TTY 稳定拒绝，finally 恢复终端并清零输入 buffer。恢复包未进入 argv/env/config/SecretStore/status/RPC，日常 owner 不持主秘密。 |
| IR33-22 | [~] | 双 readiness 投影 | 既有 S2 `ready` 继续只表示 mesh/root activation；`fullBackupReady` 必须由当前 root、独立 target、全量 scope 的 created+replicated+真解封 verified 全等派生，并给出有限 checkpoint/target/time/lsn 内部投影；旧 trust-only、仅复制、验证失败、旧根/链或每日过期不得冒充或改变冻结 ready 谓词。 | created 耐久记录冻结 rootKeyId/recipient/trustChainHead/target 与 source，status/readiness 只接受与当前 generation 全等且 verification 签名有效的 full 代；chain 变化和同日 root rotate 定向用例均先变为 not-ready 并产生新 checkpoint，trust-only/replicated/失败代不冒充 full ready。 |
| IR33-23 | [~] | backup CLI、/status 与产品语言 | `zz backup setup/verify/status`、server.info 与 `/status` 必须消费同一 readiness/status 合同，稳定显示未配置/待验证/可恢复和下一动作；错包、目标不可达、容量/暂态失败可行动且不把 pending/失败说成完成，不暴露 root、LSN、digest、CAS、target 内部路径等实现术语。 | P1：公开 DTO 已收为四态、ready 与 nextAction，server/CLI 不再泄漏 id/LSN/raw error；但 `backup setup --device` 只持久化 binding 后即返回“设备在线时由当前主设备完成”，不在本命令完成首次创建/复制。若 serve 已运行，slot 最迟到下一小时 turn 才 reload；setup 的冻结完成语义与可立即行动体验仍未交付。 |
| IR33-24 | [~] | verified 后替换与最少一代可恢复 | 只有同 current root 的新 full checkpoint 从独立目标真解封并 verified，才能在同一 checkpoint 事务把旧代标记 superseded；较新候选失败、仅 replicated、旧根或非 full 不替换，首次 ready 后始终保留至少一个维持 `fullBackupReady` 的代际。 | verify 在目标真回读和完整 open 成功后，才在同一 `AuthorityCommitLog` 事务写 verified 并标记同 current recovery key 的旧 full verified 为 superseded；replicated/失败/旧根/非 full 均不能替换，事务效果后重放按全等事实收敛。readiness 的 chain freshness 缺口单列 IR33-22。 |
| IR33-25 | [~] | 27 天 retention 与本地/远端清理 | superseded 满 27 天后才以 storage-maintenance single-flight 回收本地 envelope/chunks 与原 target 副本；远端不可达/删除失败保留重试义务且不影响新代 ready，未发布临时目录可清，created/replicated 未替换候选可续做；ArtifactLifecycleIndex 复核保证共享业务 ref 零误删。 | P2：独立 target 副本仍由 service 保留至 27 天并可重试，`checkpoint-superseded` 只令 source-local CAS 缓存提前进入通用 GC；这不会删除用户唯一独立恢复副本，原 P1 的“失去恢复回退材料”损失不成立。真实问题仅是 `local-released` 仍在 27 天后记录、却不再代表本地缓存释放决定。应把 27 天冻结义务限定为独立 target，并让 source-local 缓存释放事实与 `checkpoint-superseded` 全等；该 P2 不改变本项通过状态。 |
| IR33-26 | [~] | 跨根、跨目标与代际恢复 | root rotate 后旧根包立即不计 ready且不 rewrap；新根 full verified 前不得清唯一旧代物理副本，目标切换仍能按耐久 targetId 找回/清理旧代；并发 verify/supersede/cleanup、响应丢失和连续重启必须收敛到单调代际，无 ready 回滚或跨 target 误删。 | P1：generation/readiness 已使旧 root/chain 不计 current ready，historical resolver 可续做未复制和清理；但旧 target 的 replicated 代无法通过当前 `verify`（IR33-11/20），目标切换后的合法独立副本因此不能完成真回读验证。source-local CAS 在 superseded 后提前释放仅属 IR33-25 的 P2 合同一致性问题，不再作为本项阻断依据。 |
| IR33-27 | [~] | 两生产根、角色与 receiver exact-set | 显式启用单机 current anchor、anchor+executor、anchor-only/远端 executor 中只有 current anchor 恰一 service/capture/target client；首次 pairing onboarding receiver 与 active paired backup receiver 按阶段各恰一，executor-only、surface、非 current anchor 和未启用单机零 owner/权威读取。 | `createConfiguredCheckpointOwner` 受 current trust issuer/显式配置约束，serve 组合根只在 current anchor 装配一个 owner/service；executor-only、surface、非 current anchor 与未配置单机不装配。onboarding receiver 只存在于入网前 socket，active receiver 只在已入网非 issuer target 且仅授权当前 issuer。S7 对该事实的防漂移不足另归 IR33-30。 |
| IR33-28 | [~] | 启动恢复、公开准入与关闭 | 启动在 backup 管理入口和第 34 单元接缝前恢复 created/replicated/verified/superseded/cleanup 与 target binding/readiness；关闭先拒绝新创建，在安全页/块边界取消并等待 active work、停 timer/receiver、释放 permit，未终态耐久事实留给重启重驱，不遗留双 owner/loop 或伪终态。 | P1：owner stop 会拒绝新候选、abort 并等待 active，signal 已贯穿 capture/target/paired/governor 安全叶步骤；但 `ConfiguredCheckpointOwnerSlot.start()` 仅安排零延迟 timer，serve 不等待其 reload/recoverPending/cleanup 后再公开。`status()` 也不驱动恢复，故启动可先暴露管理与下游接缝，再异步处理 created/cleanup 欠账，不满足“恢复先于公开准入”。 |
| IR33-29 | [~] | 迁居前 forced 窄接缝 | 下游只能查询当前 root 的 verified full checkpoint或以稳定 request 请求当前 owner 创建/复制同一候选；无 recovery package 时不得假装 verified，失败保留可重试状态；接缝不得读取/导入 catalog、冻结 source、提交 transfer、切换 current anchor 或执行 restore。 | forced request 进入与 daily 共用的 candidate-key single-flight：同键重放同候选，异键稳定 busy，不返回另一 request 的结果；status 只认当前 generation 的 full verification，无恢复主秘密不会伪造 verified。交付物未出现 catalog/import/transfer/current-anchor switch/restore 等后继能力。 |
| IR33-30 | [~] | S7 入口、装配与角色门禁 | 现有单一 S7 gate/golden 必须逐项反绑规格 recovery-backup 行的 setup/verify/status、daily/forced trigger、paired put/get/retire，及生产 owner create/start/stop、current-anchor guard、onboarding/active receiver 顺序和角色 exact-set；删除、重复、错角色、错顺序、绕过或新增入口的真实变异 fail-closed，合法两根和未启用配置零误杀，不建新 lint/发现框架。 | canonical row 继续覆盖 CLI setup/verify/status；冻结 `RECOVERY_CHECKPOINT_OWNER_DESCRIPTOR` 与 `PAIRED_CHECKPOINT_RECEIVER_DESCRIPTOR` 分别声明 daily/forced 和 begin/progress/append/commit/get/range/retire，生产分支以 descriptor 驱动。现有 S7 同时核对 owner create/start/stop、current-anchor guards、active/onboarding receiver 数量与顺序，直接变异用例覆盖删除/错 owner/phase/顺序。 |
| IR33-31 | [~] | 安全、最小权限与 wire/path 隔离 | 有限核对 full payload、envelope、recovery package、directory manifest、paired command/result、status/RPC：只含冻结身份和内容引用；秘密、环境事实、绝对路径、raw store/log 与通用读取/删除能力不逸出，签名/受众/role/root/target/ref/range/path guard 均在该链首次副作用前验证。 | P1：strict wire/result、受众与 role/range guard、无回显秘密输入、公开 status DTO 及有限 verification failure code 已闭合；但 directory/paired child I/O 仍以路径打开父目录，父 identity 核验与首次副作用非原子（IR33-12/15）。本地可替换目录者可令数据先写入或删除根外目标，事后拒绝不能恢复零副作用安全边界。 |
| IR33-32 | [~] | 资源、并发与故障终态 | 空/大资产、并发 capture/verify/cleanup、容量饱和、磁盘满、取消、stop、网络等待和锁序必须有界；网络不持 permit，permit 不跨 authority/store/lifecycle 锁，single-flight/分页/chunk 有上限；每个失败要么零耐久副作用，要么留下可恢复的唯一 checkpoint 事实且不静默成功。 | P1：single-flight、分页、1 MiB crypto/transfer part、网络外 permit、abort 与磁盘错误留存耐久事实主体成立；但 capture/open/paired commit/read 的整包驻留使实际内存不受单步 permit 约束，父路径 TOCTOU 可先产生根外副作用，startup 恢复仍存在公开准入前未重驱窗口。source-local 缓存提前释放已降为 IR33-25 的 P2，不作为本项阻断依据；其余大资产、替换与崩溃矩阵仍有可达失败。 |
| IR33-33 | [~] | 成比例的直接验收证据 | 证据计划必须覆盖 strict codec/恢复包、真实 AuthorityCommitLog/FileArtifactStore/ArtifactLifecycleIndex、六类覆盖与防递归、共用 crypto/篡改、目录/paired 目标、created→verified/替换/清理、容量/停机、根/链变化、双 readiness、两生产根与 S7 真实变异；配置只做角色 exact-set，不做配置×故障笛卡尔积。 | P1：新增证据已覆盖真实 log/store/index 六类 retention、generation、single-flight、旧 target 未复制重驱、27 天 target cleanup、root 预替换/hardlink、governor/abort、v1/TTY/status/optional slot 与 S7 descriptor 变异；但未覆盖父目录在 assert→open 间替换、总量大于 permit 的内存峰值、历史 target verify、paired setup 首份副本及恢复先于公开准入。supersede 后 source-local ref 立即释放仅需按 IR33-25 的 P2 新规则同步事实含义，不再构成必要 P1 证据缺口。 |
| IR33-34 | [~] | 上游 S2 与共同装配点兼容 | 有限核对既有 trust-only root activation package、恢复包读取、mesh-ready、首次配对、单机/anchor serve 启动、canonical registry 与 server.info 共同装配点；full checkpoint 只扩展同一信封/日志/状态，不改旧签名语义、不要求已部署用户立刻配置备份、不把 backup 失败变成普通业务不可用。`@zhixing/mesh` manifest/tsup 新出口不得改变受管安全依赖精确版本/owner或暴露 PAKE 适配器；不把全部 session/confirmation/transfer 协议回归扩成 S9 门禁。 | v1 trust-only checkpoint 已接回 S2 coordinator，v2/full 未改旧签名与 root activation 判别；可选 backup 采用 disabled/available/unavailable slot，坏 config、离线 target 或缺 paired runtime 不阻断普通 serve，trust/root 身份矛盾仍 fail-stop。server.info 仅投影公开 DTO，mesh 受管依赖锁版与 PAKE 生产出口无漂移。 |
| IR33-35 | [~] | 架构/规格同步与后继能力守界 | 总纲 S9、规格 full payload/§7/§8/§15、core exports、mesh/CLI/server 合同和生产调用图必须全等；第 34/35 单元只见冻结窄接缝/载荷，`SourceFreezeProof(anchor)`、AuthorityCatalog import、TrustTransition、ReadyProof、AnchorTransferCommit、restore/domain-reset/reenroll/credential rotation 未进入当前 wire、CLI 或装配。 | 当前总纲 S9、规格 full payload/§7/§8/§15 与 core/mesh/CLI/server 名义合同已同步；交付物未出现 SourceFreezeProof、AuthorityCatalog import、TrustTransition、ReadyProof、AnchorTransferCommit、restore/domain-reset/reenroll/credential rotation 的 wire、CLI 或生产装配。实现偏差已落入对应功能审查项，不存在后继范围提前实施。 |
| IR33-36 | [~] | 来源、D33 义务与路径反向闭包 | D33-01～D33-08、全部适用来源条款及 38 个功能路径必须按 core 合同/retention/governor、mesh full capture/crypto/service/target/owner、paired/onboarding、CLI config/commands/assembly、server/status、架构规格、S7 与直接测试八组逐一归入 IR33-01～IR33-35；每组具生产端、消费端、共享原语、装配、正常/异常/恢复和证据落点，不得有未判定、重复、越界或以测试存在代替功能判断。 | P1：D33-01～D33-08 与全部规范章节及当前交付路径均已在本轮直接核查；资源总量、target lifecycle 与物理身份三条生产链仍有断点，不能以专项测试绿色代替功能判断。反向路径表的旧计数属于工作台维护输入，不产生独立产品失败，已按 P33-17 价值裁决删除；问题修复改变交付闭包后仍须按常规维护原则同步最终 exact-set。 |

> 第 33 单元 U33-04～U33-06、U33-13 专项修复已完成：0 项 `[ ]`、0 项 `[x]`、0 项 `[!]`、36 项 `[~]`。正式问题四项均为“已验证”，EX33-01 仍排除；修复后的正式独立审查尚未执行，本表不得提前判定通过。
