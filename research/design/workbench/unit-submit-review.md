...
```
目标：只收敛第 34 单元正式问题列表中的 U34-01～U34-02 两个 P0、U34-03～U34-07 五个 P1 和 U34-08 一个 P2，使八项真正命中 source 权威准入与 accepted-work 闭包、target 私有完整导入和原子发布、current-owner/trust 全入口接管、双端 phase 重驱、设备资源与 stop、commit 前 readiness 复验、必要生产证据及设备名选择体验的根因，并具备可由执行者一次实施的最优方案与完整验收条件。不修改实现，不运行构建或测试，不审查其他问题；价值裁决已经降级或拆分的主张不得恢复，第 35～38 单元 source-less/disaster recovery、恢复应用、全局同步与生命周期能力不得提前并入本单元。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 34 单元正式文件中的 U34-01～U34-08，只依据八项问题最新的事实、价值裁决、方案、验收条件和状态继续。
...
1. 按 `U34-01 → U34-02 → U34-03 → U34-04 → U34-05 → U34-06 → U34-07 → U34-08` 从权威架构、规格和当前生产调用图重建事实链，核准 source fence/accepted identity、target transfer journal/base、signed current owner/trust record、双端 phase decision、chunk/permit/lifecycle、ReadyProof/current snapshot、两生产根证据和用户 target projection 的唯一事实源、稳定身份、线性化点、消费者、异常终态、当前损失、受影响审查项、评级和工作量；判断现有描述命中根因还是表象。同根内容必须合并，独立根因不得互相遮蔽；U34-03 的 P0→P1 降级、U34-06 与 U34-08 的安全/体验拆分未满足正式重开条件时不得恢复原评级或合并方案。
2. 穷尽直接变体：U34-01 覆盖全部 writer/trigger、fresh/accepted、active/queued/pending/final/outbox、drain 超时、abort 和连续重启；U34-02 覆盖空/非空 retained exact-set、共享 ref、私有导入、两次 sync、`onInstalled` 效果前后失败和 terminal replay；U34-03～U34-04 覆盖旧 source、在线/离线 peer、全部第一方入口、prepare/freeze/import/commit/abort 双端切点、远端离线、cleanup 与连续恢复；U34-05～U34-06 覆盖大 authority、容量/磁盘、网络挂起、并发 stop、各 readiness revision 漂移及 commit/abort 边界；U34-07～U34-08 覆盖两生产根、真实 log/store/staging、S7 变异、ready/缺口、唯一名称/重名与 raw ID 隔离。每格必须指出稳定 identity、耐久事实、唯一线性化点、零副作用边界、消费终态和直接验收；无法解释即继续修正根因。
3. 以锁定范围内的最优架构审查方案：U34-01 复用 assembly lifecycle、AuthorityCommitLog guard 与 owner 耐久投影建立同一 fence/closure；U34-02 复用 per-transfer private store/journal、immutable base、现有 CAS 与单 envelope 原子 install；U34-03 复用 signed `HomeTrustRecord`、current-authority resolver、mesh `reconcileTrust` 和第一方 composition；U34-04 让 live/recovery 共用现有 phase driver；U34-05 只建立 planned 窄 chunk source/sink并复用 storage governor/lifecycle abort；U34-06 只在 existing-transfer ready replay 与双端 commit 重验同一 durable proof；U34-07 只扩展现有直接测试和有限 S7 gate；U34-08 只复用 target readiness 形成用户目标投影。方案必须用最少文字说清改什么、怎么改、关键边界及完成判据；不得新增第二事实源、通用迁移/同步/路由/存储/事务/outbox/事件总线/registry、新 lint/test runner、监控、诊断、benchmark 或信息采集。发现缺口时直接修正对应原问题，使执行者无需实现猜测即可一次完成。
4. 八项看似闭合后，对同一份未修改问题列表执行四路冷启动对抗复审：source fence/accepted-work 与 target 私有原子发布、current-owner/trust 接管与双端 phase 连续恢复、资源/stop/readiness 提交安全、生产证据/产品体验/范围价值。各路必须抛开前轮结论，从当前合同和源码主动构造第 2 步反例，并核查 `U34-01↔U34-02`、`U34-02↔U34-04`、`U34-03↔U34-04`、`U34-05↔U34-06`、`U34-01～U34-06↔U34-07`、`U34-06↔U34-08` 及八项与第 35～38 单元边界的直接交界；发现真实反证则修正原记录并重新执行四路复审。

只有现有架构无法唯一推出方案，且选择会显著改变产品需求、用户体验、成本或单元边界时才暂停；其余架构选择按整体最优且不留债务自主收敛。

完成条件：同一份未修改问题列表通过四路冷启动对抗复审；U34-01～U34-08 的全部受支持 writer/accepted obligation、retained authority、双端 commit/abort、current-owner/trust 接管、资源/stop、readiness 漂移、两生产根证据和设备选择体验均被根因完整解释，影响面无遗漏，评级与工作量有事实依据，最优方案和验收条件可直接执行，不会因同根残留继续局部返工。满足后明确回复“U34-01～U34-08 的根因与最优方案已闭合”并立即停止。
...
```text
目标：彻底解决第 34 单元 U34-01～U34-02 两个 P0、U34-03～U34-07 五个 P1 和 U34-08 一个 P2，闭合 source 权威准入与 accepted-work 闭包、target 私有完整导入和原子 authority base 发布、current-owner/trust 全入口接管、双端 phase 重驱、设备资源与 stop、commit 前 late-ready reservation、必要生产证据及设备名选择体验的全部同根直接变体；不得扩展到其他问题或全单元流程。价值裁决已经降级或拆分的主张直接复用，第 35～38 单元 source-less/disaster recovery、恢复应用、全局同步与生命周期能力不得提前实施。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 34 单元正式文件中的 U34-01～U34-08，只依据八项问题最新的根因、价值裁决、F34-01～F34-10 固定矩阵、C34-C01～C34-C09 反证账、方案、验收条件和状态继续。
...
1. 修复前从权威架构、规格与当前生产调用图重建 F34-01～F34-10 固定矩阵。U34-01 覆盖两生产根的全部 source writer/trigger、fresh/exact、active/queued/pending/final/outbox、drain timeout、abort 与连续重启；U34-02 覆盖空/非空 retained exact-set、共享 ref、per-transfer private import、immutable base、两次 sync、publish/post-install 效果前后失败；U34-03～U34-04 覆盖旧 source、在线/离线 peer、全部 first-party/planned 管理入口及 prepare/fence/freeze/import/commit/abort 双端切点；U34-05～U34-06 覆盖大 authority、海量 ref、容量/磁盘、网络挂起、stop、全部 readiness revision、ready 响应窗口、expiry 与重启；U34-07～U34-08 覆盖两生产根、真实 log/store/index/staging、S7 变异、side-effect-free readiness、TTY/非TTY、唯一名/重名及 raw ID 隔离。逐格绑定 accepted/transfer/ref/base/current-owner/phase/part/proof/target identity、唯一耐久事实、线性化点、零副作用边界和直接证据，并持续核对既有价值裁决及第 35～38 单元边界。
2. 按 `U34-05 → U34-06 → U34-01 → U34-02 → U34-04 → U34-03 → U34-08 → U34-07` 一次完成。先复用 `AuthorityCommitLog.readTail`、同 source-head 的 `ArtifactLifecycleIndex`、现有 governor 与 assembly lifecycle建立 planned 窄 `ChunkSource/ChunkSink`、closing promise和全端口 gate：网络先取固定 range，permit只覆盖本地 read/decode/write/fsync，stop拒新 ready/apply/range/command后取消并等待在途步骤。existing-transfer `ready` 随后在 target lifecycle 锁内 exact revalidate并在现有 journal写 `{proofDigest,snapshot,expiry}` late-ready reservation；相关本地 revision 改变须等待或在证明 source 未 commit时 durable abort，source commit前重取同 reservation，post-commit只 forward。

   再在两生产根以唯一 `PlannedSourceFence` 统一全部 source 入口：副作用前登记稳定 accepted token，同一 gate拒 fresh并冻结 token exact-set，log guard只允许本 transfer和 accepted有限收束；每个 token必须成为源端终态或 AuthorityCatalog canonical pending obligation，closure cursor与log head全等后才冻结 prefix。target为每个 transfer使用独立 root/journal/ArtifactStore，imported前全验 export/catalog/retained exact-set；按原 envelope bytes/LSN/digest形成不可见 immutable base，CAS提升后由target同一 AuthorityCommitLog envelope原子发布 committed-base pointer、signed trust/current/epochs/install，全部 read/projection只在pointer可见后按source base→target tail组合，启动/replay重驱key、trust和service admission。

   live与startup共用planned phase driver，逐 phase查询/幂等投递同一identity；source commit永久fence并持续投递，source durable abort先恢复同一admission/accepted owner/recovery loop再异步清target，target terminal replay继续post-install或private cleanup。复用signed `HomeTrustRecord`、current-authority/current-conversation resolver、mesh `reconcileTrust`和canonical first-party surface，让session/global/task/channel/confirmation/notification及planned管理exact-set在副作用前只命中new current owner，旧issuer/epoch永久拒绝且不建设continuous sync。最后增加无key/staging副作用的target readiness summary；TTY按设备名/序号、非TTY按唯一名称选择，重名稳定拒绝，内部ID只交strict prepare。U34-07只扩展现有直接测试和S7有限gate，不新建runner。同步直接相关架构、规格和测试。同根残留并入原问题，禁止新增第二事实源、通用迁移/同步/路由/存储/事务/outbox/事件总线/registry、监控、诊断、benchmark或信息采集。每个实质阶段完成后立即更新正式状态与证据。
3. 按验证手册运行受影响闭包的最小必要类型检查、planned source fence/closure、private import/composite base、phase recovery、current-owner/first-party、governor/stop/late-ready、DTO/CLI与两生产根直接合同/场景测试，现有 S7 lint及必要派生资产检查；源码有变时按项目常驻规则取得一次同输入构建证据。真实反例必须穿过两生产根与真实 `AuthorityCommitLog`、`FileArtifactStore`、`ArtifactLifecycleIndex`、private staging、governor及认证 mesh，直接注入 fresh/accepted、non-empty retained、共享 ref、publish/post-install失败、双端响应丢失、旧source/peer、网络挂起/stop、ready响应窗口漂移和连续重启；不得以mock自报closure/private/atomic/current-owner/reservation或只验证返回值，不得运行包全测、模块回归、配置×故障笛卡尔积或与八项验收无关的验证。失败先归因，实现问题直接修复并回到第 2 步。
4. 验证通过后冻结当前交付物指纹，整轮只读地逐格重建 U34-01～U34-08 事实链；测试通过不得代替功能判断，矩阵全部完成后才统一归并。随后对同一指纹执行四个相互隔离的冷启动对抗角色：source fence/accepted-work与target private atomic publication、current-owner/trust全入口接管与双端phase连续恢复、资源/stop/late-ready commit安全、生产证据/设备选择体验/范围价值。各角色须抛开既有结论，主动重造第1步全部适用反例，并核查 `U34-01↔U34-02`、`U34-02↔U34-04`、`U34-03↔U34-04`、`U34-05↔U34-06`、`U34-01～U34-06↔U34-07`、`U34-06↔U34-08` 及八项与第35～38单元边界的直接交界。
...
结束条件：同一冻结指纹上的 U34-01～U34-08 方案全部落地，受影响闭包的最小必要验证通过，专项功能审查与四路冷启动对抗均留下完整矩阵；C34-C01～C34-C09及新增同根反证全部有耐久处置，证明source fence后fresh写零追加、accepted work恰一终态或pending落点；target retained exact-set与immutable base在单envelope前零公开、全consumer只读同一composite authority；旧source/peer/全部first-party只解析new current owner，双端phase在commit/abort切点和连续重启下唯一收敛；网络等待零permit、stop后零新I/O，pre-commit readiness响应窗口由durable reservation封闭；两生产根与现有S7拒绝装配漂移，用户按设备名取得side-effect-free可行动选择。价值裁决重开条件仍不成立，第35～38单元能力未提前实施，八项均已更新为“已验证”。满足后明确报告“U34-01～U34-08 八项问题已彻底解决”并立即停止；不得进入全单元终审或单元提交验证。
...
- **交付基线**：当前 `HEAD 735a3dcd…` 加工作区第 34 单元修复交付，共 28 个非工作台功能/架构/测试路径，内容指纹 `6e7b3fadd1358c6b3af463814682f79c111d30fb1c01415f0da968f5cbd1f6d3`；工作台文件不参与功能指纹。
...
- **当前任务进度**：正式账本中的 U34-01～U34-08 已完成修复、最小必要验证与专项四路对抗复审；当前为 0 项 `[ ]`、1 项 `[x]`、0 项 `[!]`、35 项 `[~]`。IR34-07 的 Unit 33 checkpoint 接缝输入未变，直接复用；其余项目因合同、生产装配、功能链或交付路径变化而失效，须在后续独立审查中按当前冻结指纹重新判定。
- **状态约定**：[ ] 未审；[x] 已完成且无 P0/P1；[!] 存在 P0/P1 阻断问题；[~] 输入变化，须重审，旧证据不代表当前结论。

> **清单状态**：正式问题已全部修复并在问题修复范围内验证，当前不存在 `[!]`；35 项因输入变化标为 `[~]`，旧证据仅作历史记录、不代表当前结论。下一轮独立审查只审 `[~]`，IR34-07 直接复用，禁止重复审查。
...
| ---- | ---- | -------- | ---------------------- | -------- |
| IR34-01 | [~] | 单元身份、边界与完整交付物 | 冻结当前 28 个非工作台路径并逐一反绑 D34-01～D34-08；只交付 planned current-anchor 迁居，不得混入 Unit 35 disaster/source-less recovery、Unit 36～38 生命周期或通用迁移/同步框架。 | 上轮 47 路径证据已因交付物变化失效；待按当前 28 路径、冻结指纹与生产装配重新审查。 |
| IR34-02 | [~] | strict 合同、身份与摘要关联 | `ReadyProof`、`AuthorityCatalog`、planned `AnchorTransferCommit/Abort/Command/Result/TransferRecord` 与 `SourceFreezeProof` 必须是 exact-key 判别联合；request/transfer/source/target/epoch/ref/offset/commit/abort 逐字段反绑 originating command，JCS/digest/signature/schema/version 全等，planned/disaster/conversation 混型在副作用前拒绝。 | 已核对 core/mesh strict decoder、JCS 摘要与签名入口、命令/结果关联和 mode 判别：生产入口在 I/O 前校验 exact keys、schema/version、身份、引用/范围及 commit/abort；持久记录由已校验生产者写入并受 AuthorityCommitLog envelope 完整性保护，未发现受支持路径可注入混型或错误关联，未见 P0/P1。 |
| IR34-03 | [~] | 目标资格与 readiness | 候选/list/prepare 只接受另一台已配对、active、启用 anchor 角色的设备；ReadyProof 同时冻结 current home/trust generation、角色、配置能力、protocol/asset/service revision、SecretStore unlocked、有效期与目标身份，任一漂移/过期/离线给出稳定拒绝且零 prepared。 | 已核对候选过滤、目标 ReadyProof 生成/验证及 prepare 前校验：仅另一 active anchor-role 成员可进入，home/trust generation、角色、revision、SecretStore、期限和目标身份均反绑；漂移、过期、离线在 target prepared 写入前稳定拒绝。用户可行动 readiness 呈现另由 IR34-31 判定；本项未见 P0/P1。 |
| IR34-04 | [~] | transfer issuer key 生命周期 | 目标 issuer key 必须在目标本机按 transfer 单飞生成、SecretStore 回读验真并以 possession proof 绑定 ReadyProof；prepare replay 同 key，pre-commit abort 只删该未激活 key，commit 原子激活后按 issuerKeyId 重载，私钥零 wire/log/status/error，设备 identity 与 issuer identity 不混同。 | 已确认生成、回读、possession proof、同 transfer 复用、激活及私钥隔离成立；但 target 先耐久写 `aborted` 再删除 issuer key，重启或删除暂态失败后的 terminal replay 直接返回，不再重驱 key 清理，导致 pre-commit abort 可永久遗留未激活私钥，违反必要秘密生命周期合同，存在 P1。 |
| IR34-05 | [~] | 有限 mesh transport 与认证 | ready/transfer/source-range 只经现有认证 mesh 与 negotiated service；连接 peer、current source、prepared target、签名 keyId、phase exact-set 全等，未知/错方向/错设备/错命令在 target/source I/O 前拒绝，传输断连只重放同一 durable identity且不产生第二权威。 | 已核对 ready/target/source-range 三类 negotiated service、peer authorization、current source/target/keyId/phase 反绑与 range allow-list：未知服务、错方向、错设备、错签名或非法阶段均在业务 I/O 前拒绝；传输层不持权威状态，未见第二事实源或 P0/P1。 |
| IR34-06 | [~] | prepare、竞争与双端 durable identity | source/target `prepared` 必须共享稳定 requestId/transferId/source/target/sourceEpoch/nextEpoch/ready/transition；同载荷 exact replay，异载荷冲突；同一 home 同时至多一项非终态迁居，竞争在关闭准入或创建第二 issuer/staging 前拒绝。 | 双端字段和单端 exact replay 校验成立，但 source 先写 durable `prepared` 再调用 target：target prepare 效果前失败后，source 同载荷 replay 直接返回本地 prepared 而不重驱 target，后续 freeze/commit 永久收到 target 未 prepared；此时 cancel 又要求 target 已有状态才成功，单次暂态失败可把迁居卡死，存在 P1。另 target 所有 transfer 共用一份 staging/journal 根，terminal abort 后清理与新 transfer 可并发，未形成 transfer-private durable identity，存在同根 P1。 |
...
| IR34-08 | [~] | source fresh admission 全写面栅栏 | 从 fence 线性化点起，第一方 RPC、channel input、session/global/task/intent、新 confirmation/interaction 生产、scheduler timer/manual trigger、新 delivery 生产及所有 `AuthorityCommitLog` 旁路均不得产生 fresh 权威事实；IR34-09 已耐久接受的 interaction/final/delivery 等只可走原身份有限收束，另只允许该 transfer 的 recovery/commit/abort 记录；abort 前无 durable fence 时可安全恢复。 | `stopAccepting` 只拒绝 inboundRouter 并暂停 scheduler；更关键的是 AuthorityCommitLog guard 在 `draining` 模式无条件放行全部 entries，而非只放行 fence 前已接受身份的有限收束。第一方 RPC、global/task/intent 或直接 log append 可在 durable fenced 后继续产生 fresh 权威事实，存在 P0。 |
| IR34-09 | [~] | accepted work drain 与终态诚实性 | fence 前已接受的 conversation/user job/system job、queued/active run、interaction/confirmation、staged publish、final/delivery/outbox/uncertain 必须逐类收束到可证明终态或作为明确 pending obligation 冻结；timeout/abortAll/flush/recovery-loop 失败不得伪造 drain 成功或丢弃义务。 | planned lifecycle 只顺序调用 `abortAllAndWait`、job drain、delivery flush 后停止 recovery loop；它未像现有 local-owner close 那样复核 active work、pending finals/assignments、recovery backlog 与 leases，且 `abortAllAndWait` 超时可返回而结果被忽略。未证明静止即推进 frozen，存在 P0；未收束义务又未进入 catalog。 |
| IR34-10 | [~] | 唯一 source prefix 与 SourceFreezeProof | admission durable closed 且 IR34-09 完成后，source 只从同一 `AuthorityCommitLog` 取得一个 `{logId,lsn,frameEndOffset,prefixDigest}`；snapshot、export、catalog 与 `SourceFreezeProof(scope:"anchor")` 必须全等绑定同一 prefix，追加/坏尾/缺 ref/错 epoch 时 created/frozen/target import 零错误推进。 | 就本项绑定机制核对：drain 返回后切至 frozen guard，再取唯一 checkpoint；`readSnapshot` 强制同 LSN，export、catalog.source 与签名 proof 共同反绑 logId/lsn/frameEndOffset/prefixDigest/source epoch，target 再做 canonical/binding 校验。IR34-08～09 的前置终态缺陷不改变本项 prefix 绑定事实；未见独立 P0/P1。 |
| IR34-11 | [~] | AuthorityCatalog 六类覆盖 | 逐行核对规格 §7：global、conversation authority/content、execution assets、trust/current anchor 与全部 pending obligations 完整；环境事实、SecretStore、workspace raw path、设备/非权威缓存不可表示。streams、coverage、record/ref/count/digest canonical exact-set，少列、多列、重复、乱序、空置真实义务均 fail-closed。 | catalog 固定声明六类 coverage 并计算 streams/count/digest，但生产 builder 无条件写 `pendingObligations: []`，未从 assignment/interaction/confirmation/final/delivery/intent 等耐久投影枚举真实未终态义务；因此真实 pending 会被错误宣称为空并随迁丢失，存在 P0。 |
| IR34-12 | [~] | planned export 与 retained artifact 闭包 | 独立 planned export 只含 IR34-10 prefix 的 canonical commit envelopes，catalog 的 authorityRecords/retainedArtifacts 与真实引用闭包全等；artifact 先耐久后引用，共享/嵌套/大资产去重且 bytes/digest 全验，恢复 checkpoint 内容不得冒充 export，缺件不得 freeze/import/commit。 | source 的 export/prefix 与 retained ref 枚举、缺件检查成立；但 target freeze 只拉取 export 与 catalog，import 仅写状态，未拉取 `catalog.retainedArtifacts`。任一目标共享 CAS 尚无的真实 retained artifact 都会到 commit promotion 才因 private staging 缺件失败，正常完整数据面不可迁居，存在 P0。 |
| IR34-13 | [~] | 容量治理与有界传输 | export、source range、target pull/private staging、promotion/install 每个物理步骤必须使用同一设备 `storageMaintenance` governor 与 lifecycle abort；buffer/part 固定上界，网络等待零 permit，permit 不跨 authority/store/lifecycle 锁；容量拒绝、磁盘满、取消/stop 要么零事实，要么保留唯一可重驱事实。 | target `putVerifiedStream` 的 governor step 包住整个 async chunk 消费循环，chunk producer 在 permit 内执行远端 range 请求，网络等待实际持有 permit；source `readRange` 未在 planned transfer 层取得容量准入，source export 又整体 materialize 为 Buffer。容量饱和/挂起可阻塞设备维护并突破规定的步骤上界，存在 P1。 |
| IR34-14 | [~] | target 私有 staging 与冻结验真 | target 只在该 transfer 私有 authority root/journal/ArtifactStore 接受 export/catalog/retained refs；range 连续且关联原 ref/offset/length，完整 bytes/digest/catalog/source-prefix/coverage exact-set 验真后才写 frozen/imported；部分、重复、重排、响应丢失和重启不串 transfer、不使共享 CAS/当前 authority 可见。 | target constructor 只建立共享的 `<stagingRoot>/artifacts` 与 `<stagingRoot>/journal`，未按 transferId 建私有 root；且 imported 在 retained refs 尚未拉取/验真时即写入。terminal abort 后清理可与下一 transfer 共用 digest/staging 并发，部分数据会串代或被误删，存在 P1；retained 缺件问题同时命中 IR34-12。 |
| IR34-15 | [~] | import、共享 CAS 提升与清理边界 | imported 必须反绑 frozen export/catalog，promotion 对已有/共享 digest 幂等且不覆盖异 bytes；pre-commit abort 仅清私有 staging和未激活 key，绝不删共享业务 CAS；commit 后 retained refs 仍有唯一 owner，部分 base import/progress 可重驱且不形成可服务的半 authority。 | frozen ref 反绑与 CAS digest 幂等检查成立；但 imported 未证明 retained refs 完整，abort 先写 terminal 再清共享 staging/key且 terminal replay 不重驱清理。commit 还先逐条导入 live authority base、再安装 trust/current、再写私有 committed，崩溃可留下未终态的半 authority；清理/安装缺少可恢复进度，存在 P1。 |
| IR34-16 | [~] | commit 前最终复验 | source 签 commit 前重新验证 current issuer/home/trust chain、ReadyProof 未过期且 capability/revision 未漂移、target/import/catalog/freeze/transition 全等、next epochs 单调；任一改变保持 source fenced 或可安全 abort，零 signed commit。 | source 在签 commit 前复验 current trust、proof 期限、catalog/freeze/transition 和 epochs；但 proof 中 target capability/protocol/asset/service revision 只在 prepare 生成，source/target commit 均未重新读取 target readiness snapshot 比较。目标配置或能力在冻结后漂移仍可触发不可逆 source commit，存在 P1。 |
| IR34-17 | [~] | source 唯一原子 commit 切点 | 当前 issuer 只能在 source 同一 `AuthorityCommitLog` envelope/一次 sync 原子追加唯一 signed planned commit，并使该 commit 所绑定的 prepared issuer-transition、next anchor/trust epochs 与 current-anchor 投影共同生效；不得另造第二切换事实。重复同决定零追加，异决定冲突；该 sync 后 append fence 永久 committed，旧 source 不因 target 未应答恢复。 | 已核对 owner commit：signed planned commit、同一 prepared transition 与 `planned-anchor-source-committed` current-anchor 投影由 journal 的同一 AuthorityCommitLog transaction/envelope 写入；reducer 拒绝异决定，terminal replay 零追加，sync 后 fence 切为 committed且 target 未应答不恢复。未发现第二切换事实或独立 P0/P1。 |
| IR34-18 | [~] | authority base 导入与不可见性 | target 导入原 source commits 时必须保持 source LSN/envelope identity、stream exact-set和引用在场，可幂等断点续做；在最终安装 envelope sync 前，导入进度、records、投影和旧 current-owner 均不可被公开服务当作已接管 authority，冲突 progress fail-closed。 | `installPlannedAuthorityBase` 将每个 source envelope 的 entries 逐批直接追加到 target 的 live AuthorityCommitLog，并另写 sourceLsn/digest progress；原 envelope/LSN identity 未被保留，records 与投影在最终安装前已进入生产日志而非不可见 base。崩溃会留下可观察的半导入 authority，存在 P0。 |
| IR34-19 | [~] | target 原子安装与服务开放 | target 逐字段验证同一 commit/transition/catalog/export/issuer key 后，在一个本地 `AuthorityCommitLog` envelope 原子发布 trust event/record、planned install、authority base/current anchor/next epochs；sync 后才激活 issuer、reconcile trust和开放服务，sync 后无决定性外部 I/O，效果后响应丢失只 exact replay。 | target 实际顺序为：多 envelope 导入 live base→激活 issuer key→单 envelope 写 trust/install→私有 journal 写 committed→调用 `onInstalled` reconcile。base/current/epochs 非同一 sync，且 committed 后仍有决定性外部 reconcile；若该调用失败或进程退出，replay 在 committed 分支直接返回而永不重驱 reconcile，source 已永久交权但 target 不开放服务，存在 P0。 |
| IR34-20 | [~] | pre-commit abort | prepared/fenced/frozen/imported 均只接受 current source 对同 request/transfer/sourceEpoch 的签名 abort；source 的 durable abort 是清 fence、以原 epoch 恢复准入/recovery/scheduler 的唯一线性化点，不等待跨设备物理清理。target 收到同一 abort 后先耐久隔离该 transfer 并拒绝任何后继 commit，再幂等清私有 staging、删除未激活 key；target 离线、清理失败、断连/丢响应/重启均保留同一投递/清理义务，异 abort 冲突。 | 签名/identity/late-commit 拒绝成立，但 live abort 在 source 已 durable aborted 后仍等待 target RPC 成功才清 fence/resume，target 离线会卡住恢复；startup 却吞掉投递失败后无条件清 fence/resume且不保留重投。target terminal replay 又不重驱 staging/key 清理。线性化与恢复语义分叉，存在 P1。 |
| IR34-21 | [~] | post-commit forward-only 与 tombstone | source commit 后 late abort 永久拒绝；target 未提交、效果后丢响应、离线和连续重启只重发同 commit直至安装，source始终拒写；target已提交只 exact replay。旧 source cleanup/tombstone不得改变 authority或恢复 issuer，后续迁回只能新 transfer/更高 epoch。 | late abort 与 source committed fence 成立；但 `recoverBeforeAdmission` 对 committed 只尝试一次 target send 并吞掉失败，没有 durable retry owner/loop。更严重的是 target 私有 committed 后若 `onInstalled` 失败，后续 exact replay直接返回且不完成服务接管。响应丢失/离线/连续重启不能保证 forward-only 最终收敛，存在 P0。 |
| IR34-22 | [~] | 旧 source 全能力永久 fencing | commit 后逐项核对 control/session/global/job/delivery/confirmation/intent/assignment/signing/mesh receiver与直接 log append：旧 anchor、旧 issuer、旧 anchorEpoch/trustEpoch 全部 fail-closed，允许的重定向只基于已认证 current-anchor 事实且不得代理旧写。 | AuthorityCommitLog committed guard 能拒绝非 transfer/trust entries，但 source 的 bootstrap/control trust、issuer 与公开 surface 未在 source commit 后切换或降级；它仍以旧 current 身份列举/接收管理与会话请求，只在下游 append 时失败，且 guard 仍放行任意 anchor-transfer stream entry。能力并未在入口按新 current-owner 事实永久收窄，存在 P0。 |
| IR34-23 | [~] | current anchor 路由与第一方接管 | target 安装后 CLI/server/channel/scheduler/global state、会话列表/创建、确认/通知、任务及所有第一方管理入口只解析唯一 current anchor；source/target/其他设备对相同请求不会双写或显示两套值班事实，单机与分布式 surface 语义等价。 | 当前交付只为迁居命令新增四个管理 RPC；未抽取/切换覆盖 CLI/server/channel/scheduler/global/session/confirmation/task 的 current-anchor 第一方路由。target 仅 reconcile 本机 trust，旧 source 与其他设备入口未基于同一 durable current-anchor 转发或拒绝，迁居后会出现旧入口报错/陈旧值班事实而非无感接管，存在 P0。 |
| IR34-24 | [~] | pending obligations 迁移与恢复 | AuthorityCatalog 必须从耐久事实枚举 assignment、interaction、final、delivery、intent、confirmation 等实际非终态 exact-set；target 接管后以原稳定 identity、水位和重试语义恢复，terminal replay不追加，源端无遗留 producer/loop，遗漏/多列/错 owner 在开放服务前失败。 | production catalog 无条件输出空 pendingObligations，target 安装链也没有按原 identity/watermark 恢复 assignment、interaction、final、delivery、intent、confirmation 的 owner/loop；source 只停止部分 loop。任何真实非终态义务都会被漏报且 target 仍可开放，存在 P0，与 IR34-09/11 同根。 |
| IR34-25 | [~] | trust/peer 收敛与旧 issuer 拒绝 | issuer-transition 必须由旧 current issuer签名、指向 active anchor target的 transfer issuer public key并产生 next trust epoch；target `HomeTrustRecord` 可验签，在线/离线 peer重连后只接受新链头/issuer，旧签名和旧 epoch永久拒绝，合法既有 device-key transition兼容不漂移。 | transition 的旧 issuer 签名、active target/key 与 next epoch 校验以及 target record 验签成立；但新 HomeTrustRecord 只在 target `onInstalled` reconcile，本次交付没有把它耐久收敛到旧 source 和其他 paired peer。它们的 trust watcher仍只读各自旧 bootstrap record，重连仍可能信任旧 issuer/链头，存在 P0。 |
| IR34-26 | [~] | 启动恢复顺序 | source 重启必须在任何公开 producer 前重装 durable fence并重驱 commit/abort；target重启从私有 journal/base progress恢复并在原子安装前保持服务关闭；双端事实不对称、坏尾、缺 artifact/key、冲突/歧义 fail-closed，连续重启最终唯一收敛且无双 owner。 | source fence 的恢复顺序早于 control.start，但只对 committed/aborted 各做一次 best-effort send；prepared 双端不对称不重驱，target committed 后未完成 onInstalled 也不重驱，abort 清理不重驱。连续重启可永久停在 source committed/target未服务、source prepared/target空或 target残留清理状态，存在 P0。 |
| IR34-27 | [~] | 并发、重放与严格结果终态 | prepare/fence/freeze/import/commit/abort/status/read-range 的并发、重复、异载荷、错 request/transfer/ref/offset/state和效果前后失败都二元落定；result先与 originating command关联再分类，stable conflict与retryable不混淆，任何拒绝在对应权威/存储副作用前发生。 | strict command/result/ref/offset 关联和多数 reducer 冲突检查成立；但 prepare 的 source-effect/target-before-effect 失败、abort 的 source-terminal/target离线、target commit-effect/onInstalled-before-response 等切点均没有统一 terminal replay，重复命令会返回错误阶段或跳过未完成效果。状态机并发/响应丢失未二元闭合，存在 P0。 |
| IR34-28 | [~] | 两生产根与角色 exact-set | anchor+executor 与 anchor-only/current-anchor 各恰一 source/recovery owner，source range receiver 仅 current source；合格 non-current active anchor 可各有一个有限 readiness/strict receiver，但在首条 `prepared` 前零 transfer record/staging/recovery owner，prepared 后只有命中 targetId 的设备可持该非终态迁居。executor-only、surface、disabled、非 anchor 与未命中 target 的候选零迁居 owner/耐久状态；trust 换代时旧实例退役、新实例单次安装，零重复 listener/service。 | 已核对两处 `MeshRuntimeAssembly` 生产根、动态 role key、唯一 owner/target 构造和 source/target service disposer；S7 反绑 owner/receiver phase、两根及 enabled active anchor exact-set。executor-only/surface/disabled 无 owner，trust role 变化先 dispose 再单次安装。共享 staging 缺陷归 IR34-14，不改变本项装配数；未见独立 P0/P1。 |
| IR34-29 | [~] | 生命周期、关闭与资源收束 | start 须先恢复 fence/transfer 再接公开流量；stop 先拒绝 fresh 管理/业务写，安全取消或等待当前物理 step，停止 owner/receiver/retry loop 并释放 permit/key handle/disposer；未终态事实留给重启。cleanup 失败不得伪造 abort/commit，也不得阻断已原子安装 target 或无迁居角色设备的普通业务；source 一旦 committed 仍永久 fenced，不得以“普通业务”名义复权。 | start 顺序能先装 source fence；但 assembly.stop 只复用 conversation transfer abort、停止 control/worker 并 dispose planned services，planned owner/target 无 closing gate、无进行中物理 step abort/drain，也无 committed/abort 重试 owner可停止。并发 stop 可让 I/O 在 disposer 后继续或留下无法判定效果，存在 P1。 |
| IR34-30 | [~] | 安全、最小权限与数据隔离 | wire/log/catalog/export/RPC/错误只含必要稳定身份与ArtifactRef；秘密、恢复主秘密、SecretStore内容、环境、workspace绝对路径、设备缓存、raw store/log/通用读取删除能力零逸出；签名、peer、role、epoch、ref与path/容量guard均在首次对应副作用前验证。 | 已核对 wire/records/catalog/RPC/public error 字段与 owner/receiver ports：只携稳定 identity、签名对象和 ArtifactRef；issuer 私钥仅经窄 SecretStore helpers，本地 raw log/store/path/环境未出 public surface，server 将内部异常归一为稳定中文错误。IR34-13 的容量时序缺陷单列，不构成本项信息逸出；未见独立 P0/P1。 |
| IR34-31 | [~] | CLI/server 产品旅程 | `zz duty targets/migrate/continue/cancel` 与四个 authenticated `dutyMigration.*` 必须同源：列表稳定且 CLI/用户渲染只展示目标 `displayName`（opaque `targetDeviceId` 仅作内部选择关联，不直接展示），ready 缺口可行动，prepare 后明确仍可取消，commit 展示收束/传输/接管且结果不明可同编号继续，commit 后取消诚实拒绝；公开 exact keys/错误零 anchor/epoch/issuer/catalog/CAS/stream/secret/path/raw error 术语。 | 四个 authenticated RPC 与 CLI 阶段/继续/取消文案同源，内部错误已映射；但 `zz duty targets` 明确输出并要求用户使用 raw deviceId，且列表不提供逐目标 readiness/可行动缺口。迁居仍可完成，当前损失是违反已锁定的设备名选择与提前纠错体验，不造成权威错误或数据丢失；价值裁决为 P2，故本项按规则保持 `[x]`。 |
| IR34-32 | [~] | 分层、导出与上游兼容 | core strict合同、mesh trust/key/ready、owner-kernel drain、server投影、CLI组合根依赖方向无环且无复制事实源；Unit33 checkpoint/S2 trust/SecretStore/mesh/现有conversation transfer兼容，package export/build与供应链隔离不漂移。 | 已核对依赖方向与 package exports：strict 状态合同留在 core，trust/key/ready 留在 mesh，生产 owner/transport/management 装配留在 CLI，server 只作 RPC 投影；复用 Unit33 checkpoint、既有 SecretStore/mesh/conversation 原语且未反向依赖或复制其事实源。功能缺陷由对应条目承载，本项未见独立 P0/P1。 |
| IR34-33 | [~] | S7、registry 与 golden exact-set | 现有单一 S7 gate必须反绑两生产根issuer key注入、owner/target构造、service phase/role/order、recovery-before-admission、四CLI RPC与canonical server registry；新增/删除/重复/换序/绕过/错误root和动态未注册方法均fail-closed，合法拓扑/golden零误杀，不建新lint/发现框架。 | 已逐格核对 `inspectPlannedAnchorTransferAssembly` 与 canonical registry/golden：两生产根 issuer key、唯一 owner/target、service phase/role/order、recovery-before-control.start 及四个 canonical RPC 均有有限 exact-set/count 反绑，合法角色动态装配不被误杀，未新增 lint/发现框架。其规则不能替代功能证据的问题归 IR34-34。 |
| IR34-34 | [~] | 成比例的直接验收证据 | 必须有strict codec/reducer、ReadyProof/issuer transition、真实双端AuthorityCommitLog/FileArtifactStore/private staging、checkpoint接缝、source drain/fence、catalog/pending exact-set、双端commit/abort/response-loss/restart、旧epoch拒绝、两生产根与S7真实变异的直接证据；测试通过不得替代逐格功能判断，不做配置×故障笛卡尔积。 | 现有 direct tests 覆盖 codec、ready/key、单次 happy-path prepare/freeze/commit/abort 与一次 lost response；未用真实反例覆盖 fresh writer 穿透 drain、非空 pending/retained refs、shared staging 竞争、source/target 各切点崩溃、onInstalled 效果失败、peer/第一方接管、stop 与两根真实故障。测试标题宣称 private/atomic 但未观测相应边界，已实际漏过多个 P0/P1，证据闭包为 P1。 |
| IR34-35 | [~] | 后继能力与非目标边界 | 逐路径确认未实现source-less/disaster recovery、restore/domain-reset/pending-reenroll/credential rotation、自动failover/quorum、continuous sync、服务托管/卸载/升级，也未改变Unit33 checkpoint语义或新增通用迁移/路由/事务/outbox/registry/监控诊断框架。 | 已反向搜索交付路径和装配：仅 planned mode、现有 current source/target mesh、Unit33 verified checkpoint 接缝与有限 RPC；未新增 disaster/source-less、restore/reset/reenroll/rotation、failover/quorum/sync 或 Unit36～38 服务生命周期能力，也未改写 Unit33 checkpoint 语义或建设通用框架，未见 P0/P1。 |
| IR34-36 | [~] | 来源、D34义务与路径反向闭包 | D34-01～D34-08、全部适用来源条款与28个非工作台路径必须按core合同、CLI source-target/assembly/management、server RPC、架构规格、S7和直接测试逐一归入IR34-01～IR34-35；零未判定来源/条款/枚举行/功能链/交付路径，零重复或无法独立判定条目。 | 上轮 47 路径反向对账已因交付闭包变化失效；待对当前 28 路径、适用来源与 IR34-01～IR34-35 双向复审。 |

> 正式问题 U34-01～U34-08 均已修复并标为“已验证”；当前独立审查清单为 1 项可复用、35 项输入变化待复审，不得把修复自审或专项对抗复审计作独立审查通过。
