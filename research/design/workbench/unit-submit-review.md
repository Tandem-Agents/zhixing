...
```
目标：只收敛第 34 单元正式问题列表中同根再次重开的 U34-09、U34-04、U34-03，使 U34-04 一个 P0、U34-09/U34-03 两个 P1 真正命中 candidate terminal/首次 prepared 耐久判定、runtime consumer 所有权交接与 channel ingress current-owner exact-set 的根因，并具备可由执行者一次实施的最优方案与完整验收条件。不修改实现，不运行构建或测试，不审查其他问题；U34-01～U34-02、U34-05～U34-08 与 EX34-01 的既有结论直接复用，旧轮已经闭合的跨 transfer claim、pre-bootstrap key/private completion、canonical RPC exact-set及价值裁决收窄不得恢复或扩面，第 35～38 单元 source-less/disaster recovery、恢复应用、全局同步与通用生命周期能力不得提前并入本单元。
...
1. 按 `U34-09 → U34-04 → U34-03` 从权威架构、规格和当前生产调用图重建事实链，核准 source candidate terminal/transfer state transaction、target-wide/per-transfer journal与cleanup、installed authority generation、conversation recovery cache、delivery timer/active flush、channel registry/adapter连接、inbound admission与current-authority resolver的唯一事实源、稳定身份、线性化点、消费者、异常终态、当前损失、受影响审查项、评级和工作量；判断现有描述命中根因还是表象。同根内容必须合并，独立根因不得互相遮蔽；旧轮跨 transfer claim、pre-bootstrap key/private completion、canonical RPC exact-set仍直接复用，未出现新生产反证时不得恢复旧方案或扩面。
2. 穷尽直接变体：U34-09 覆盖 ready前后cancel双序、same/异载荷、released↔prepared竞争、target prepared后重启、迟到签名release、abort/commit、响应丢失与连续恢复；U34-04 覆盖live target已discover/未discover、authority install前后、六种非空pending、source delivery flush/timer/active外调每一切点、pre-commit abort、commit、target开放、响应丢失与连续重启；U34-03 覆盖current source/new target/第三anchor的启动与物理channel连接、迁前stop、abort恢复、post-install激活、provider断连重投、router切换竞态、target离线及连续重启。每格必须指出稳定identity、耐久事实、唯一线性化点、零副作用边界、消费终态和直接验收；无法解释即继续修正根因。
3. 以锁定范围内的最优架构审查方案：U34-09 只复用现有source/target journals，在各自唯一事务中联合判定candidate terminal与首次prepared，并让target cleanup强制读取稳定per-transfer phase；U34-04 只给现有conversation recovery增加planned-install authority-generation重建，给现有delivery pipeline增加可逆quiesce/resume，并在公开开放前read-back catalog obligation owner；U34-03 只把现有channel配置与registry连接分离，按signed current owner及planned lifecycle切换物理接收owner，并保留router副作用前最终守卫。方案必须用最少文字说清改什么、怎么改、关键边界及完成判据；不得新增第二事实源、通用锁/registry/lifecycle、跨机channel relay、迁移/同步/路由/存储/事务/outbox/事件总线、新lint/test runner、监控、诊断、benchmark或信息采集。发现缺口时直接修正对应原问题，使执行者无需实现猜测即可一次完成。
4. 三项看似闭合后，对同一份未修改问题列表执行四路冷启动对抗复审：candidate terminal/prepared与物理cleanup耐久排序、source/target runtime consumer恰一所有权、channel ingress物理owner与切换竞态、生产证据/产品体验/范围价值及EX34-01边界。各路必须抛开前轮结论，从当前合同和源码主动构造第2步反例，并核查 `U34-09↔U34-04`、`U34-04↔U34-03`、`U34-09↔U34-03` 以及三项与 U34-01～U34-02、U34-05～U34-08、EX34-01、第35～38单元边界的直接交界；发现真实反证则修正原记录并重新执行四路复审。

只有现有架构无法唯一推出方案，且选择会显著改变产品需求、用户体验、成本或单元边界时才暂停；其余架构选择按整体最优且不留债务自主收敛。

完成条件：同一份未修改问题列表通过四路冷启动对抗复审；U34-09、U34-04、U34-03 的全部受支持cancel/prepared竞争、磁盘phase cleanup、live authority consumer重建、source delivery停机、channel物理owner切换、故障/响应丢失/连续恢复终态均被根因完整解释，影响面无遗漏，评级与工作量有事实依据，最优方案和验收条件可直接执行，不会因同根残留继续局部返工。满足后明确回复“U34-03、U34-04、U34-09 的根因与最优方案已闭合”并立即停止。
...
- **交付基线**：当前 `HEAD a24df557068012e20a5d5c025571bedc28a1df06`；U34-03/U34-04/U34-09 最新修复影响 17 个非工作台生产/测试/S7 路径，按排序后的 `path<TAB>file-sha256` 清单冻结内容指纹 `183ec30ed6238f9e9919cbd166cbec1fd8638f1908219753013987d230fb92b4`。工作台文件不参与功能指纹；未受影响路径继续复用原结论。
...
- **当前任务进度**：三项专项实现与直接验证已完成；25 个未受影响 `[x]` 继续复用，11 个受修改影响的审查项已失效为 `[~]`，等待下一轮独立审查按新指纹重新判定。本次任务不执行该独立审查。
- **状态约定**：[ ] 未审；[x] 已完成且无 P0/P1；[!] 存在 P0/P1 阻断问题；[~] 输入变化，须重审，旧证据不代表当前结论。

> **清单状态**：修复后输入已更新；0 项 `[ ]`、25 项 `[x]`、0 项 `[!]`、11 项 `[~]`。旧问题证据仅说明失效原因，不代表新指纹结论；EX34-01 继续排除。
...
| IR34-04 | [~] | transfer issuer key 生命周期 | 目标 issuer key 必须在目标本机按 transfer 单飞生成、SecretStore 回读验真并以 possession proof 绑定 ReadyProof；prepare replay 同 key，pre-commit abort 只删该未激活 key，commit 原子激活后按 issuerKeyId 重载，私钥零 wire/log/status/error，设备 identity 与 issuer identity 不混同。 | 当前冻结指纹下发现 P1：target `releaseCandidate()` 只在内存 `#contexts` 命中时检查 per-transfer durable phase；重启后 context 为空，迟到的签名 release 可把已 prepared claim 标为 released，并删除 transfer issuer key、journal 与 staging，破坏 prepared/commit 的耐久重放。拒绝条件未反绑磁盘上的 per-transfer journal，完整 key lifecycle 未闭合。 |
...
| IR34-06 | [~] | prepare、竞争与双端 durable identity | source/target `prepared` 必须共享稳定 requestId/transferId/source/target/sourceEpoch/nextEpoch/ready/transition；同载荷 exact replay，异载荷冲突；同一 home 同时至多一项非终态迁居，竞争在关闭准入或创建第二 issuer/staging 前拒绝。 | 当前冻结指纹下发现与 IR34-04 同根 P1：source `#prepareCandidate()` 在远端 ready 返回后追加 prepared 时，只归约 transfer stream，未在同一 `AuthorityCommitLog` 事务重读 candidate terminal；并发 claim-only cancel 可先写 released，随后 prepare 再写 prepared，形成 terminal claim 与非终态 transfer 并存。双端 candidate terminal 与首次 prepared 没有唯一耐久线性化点。 |
...
| IR34-19 | [~] | target 原子安装与服务开放 | target 逐字段验证同一 commit/transition/catalog/export/issuer key 后，在一个本地 `AuthorityCommitLog` envelope 原子发布 trust event/record、planned install、authority base/current anchor/next epochs；sync 后才激活 issuer、reconcile trust和开放服务，sync 后无决定性外部 I/O，效果后响应丢失只 exact replay。 | 当前冻结指纹下发现 P0：live target 的既有 `ConversationProtocolRuntime` 已令 `#recoveryDiscovered=true`；安装新 authority base 后，post-install callback 再调 `recoverReadinessProjections()` 会直接返回 0，未重建迁入 conversation/interaction/confirmation/final 投影，却继续删除 private progress、清 readiness gate并开放服务。原子安装后的“可服务”终态未被真实 consumer completion证明。 |
...
| IR34-21 | [~] | post-commit forward-only 与 tombstone | source commit 后 late abort 永久拒绝；target 未提交、效果后丢响应、离线和连续重启只重发同 commit直至安装，source始终拒写；target已提交只 exact replay。旧 source cleanup/tombstone不得改变 authority或恢复 issuer，后续迁回只能新 transfer/更高 epoch。 | 当前冻结指纹下发现与 IR34-19/22/24 同根 P0：source fence 只 `flush()` delivery pipeline，没有暂停或转交其定时 consumer；commit 后旧 source 仍可对 frozen pending delivery 执行外部 send，随后 outcome append 才被 log guard拒绝，target 接管后会按同一 pending 再发送。forward-only 只约束日志，未约束外部消费 owner。 |
| IR34-22 | [~] | 旧 source 全能力永久 fencing | commit 后逐项核对 control/session/global/job/delivery/confirmation/intent/assignment/signing/mesh receiver与直接 log append：旧 anchor、旧 issuer、旧 anchorEpoch/trustEpoch 全部 fail-closed，允许的重定向只基于已认证 current-anchor 事实且不得代理旧写。 | 当前冻结指纹下发现与 IR34-19/21/24 同根 P0：旧 source 的 `AuthorityDeliveryPipeline` timer 在 fence/commit 后仍处于 running；append guard只能拒绝发送后的 outcome记录，不能撤销已经发生的渠道副作用。旧 source 不是全能力永久 fenced，存在重复外发。 |
| IR34-23 | [~] | current anchor 路由与第一方接管 | target 安装后 CLI/server/channel/scheduler/global state、会话列表/创建、确认/通知、任务及所有第一方管理入口只解析唯一 current anchor；source/target/其他设备对相同请求不会双写或显示两套值班事实，单机与分布式 surface 语义等价。 | 当前冻结指纹下发现 P1：canonical registry relay只覆盖 server RPC；anchor-role 设备仍无条件装配并连接 `setupChannels`，`InboundRouter.handleMessage()` 未消费 `currentAnchorDeviceId` 或 planned readiness gate。非当前 target/第三 anchor 收到渠道消息时会直接写本地 authority，随后 planned base install可覆盖该事实；路由 exact-set 未覆盖权威矩阵明确列出的 channel ingress。 |
| IR34-24 | [~] | pending obligations 迁移与恢复 | AuthorityCatalog 必须从耐久事实枚举 assignment、interaction、final、delivery、intent、confirmation 等实际非终态 exact-set；target 接管后以原稳定 identity、水位和重试语义恢复，terminal replay不追加，源端无遗留 producer/loop，遗漏/多列/错 owner 在开放服务前失败。 | 当前冻结指纹下发现与 IR34-19/21/22 同根 P0：六类 catalog 只被正确分组，callback并不证明实际 owner已重建。live conversation callback因旧 `#recoveryDiscovered` no-op；source delivery consumer未停。结果是 target可漏掉 interaction/confirmation/final 重驱，同时 source/target可并行消费 delivery，违反“目标恢复、源端零遗留 loop”。 |
...
| IR34-26 | [~] | 启动恢复顺序 | source 重启必须在任何公开 producer 前重装 durable fence并重驱 commit/abort；target重启从私有 journal/base progress恢复并在原子安装前保持服务关闭；双端事实不对称、坏尾、缺 artifact/key、冲突/歧义 fail-closed，连续重启最终唯一收敛且无双 owner。 | 当前冻结指纹下发现与 IR34-04/06 同根 P1：target 重启后 `#contexts` 为空，迟到 release 不读取磁盘 per-transfer journal即可终结 target-wide claim并删除已 prepared key/journal/staging；连续恢复因此可能丢失唯一可重驱事实。正常 committed bootstrap顺序本身可重放，但 candidate release 的重启分支不满足 fail-closed。 |
| IR34-27 | [~] | 并发、重放与严格结果终态 | prepare/fence/freeze/import/commit/abort/status/read-range 的并发、重复、异载荷、错 request/transfer/ref/offset/state和效果前后失败都二元落定；result先与 originating command关联再分类，stable conflict与retryable不混淆，任何拒绝在对应权威/存储副作用前发生。 | 当前冻结指纹下发现与 IR34-04/06 同根 P1：source ready await后的 prepared append与 claim-only cancel 的 released append分属两个事务且不互相重读；并发可形成 `candidate.terminal=released` 与 transfer `phase=prepared` 并存。target delayed release还可在重启后删除 prepared物理状态，拒绝与副作用没有唯一排序。 |
...
| IR34-33 | [~] | S7、registry 与 golden exact-set | 现有单一 S7 gate必须反绑两生产根issuer key注入、owner/target构造、service phase/role/order、recovery-before-admission、四CLI RPC与canonical server registry；新增/删除/重复/换序/绕过/错误root和动态未注册方法均fail-closed，合法拓扑/golden零误杀，不建新lint/发现框架。 | 当前 S7 只检查 claim-before-context/key 的文本顺序、completion函数存在、六类分组与 canonical RPC registry；它不会拒绝 release绕过磁盘phase、cancel/prepared跨事务、conversation旧缓存no-op、source delivery loop存活或channel ingress缺current-owner。现有变异仍可在这些根因下全绿，必要有限门禁不完整，属三项实现缺陷的同根证据缺口。 |
| IR34-34 | [~] | 成比例的直接验收证据 | 必须有strict codec/reducer、ReadyProof/issuer transition、真实双端AuthorityCommitLog/FileArtifactStore/private staging、checkpoint接缝、source drain/fence、catalog/pending exact-set、双端commit/abort/response-loss/restart、旧epoch拒绝、两生产根与S7真实变异的直接证据；测试通过不得替代逐格功能判断，不做配置×故障笛卡尔积。 | 当前新增测试只覆盖顺序执行的 lost-ready release、双 transfer竞争、pre-bootstrap key失败、六类值分组和 RPC relay exact-set；没有并发 cancel↔ready/prepared、target重启后迟到release、live protocol旧缓存、source delivery timer、non-current channel ingress或真实六类consumer终态。当前证据无法证伪三项可达失败，需随对应根因补最小直接场景。 |
...
> 正式 U34-03、U34-04、U34-09 已在专项冻结指纹完成修复与验证；EX34-01不重开。临时问题列表保持为空，11个受影响项已从`[!]`改为`[~]`，其旧证据只作为重审输入；其他25项继续复用。本次不开始独立审查。
