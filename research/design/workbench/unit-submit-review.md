...
```
目标：只收敛第 38 单元正式问题列表中的 U38-01～U38-08，使 U38-01 一个 P0/大、U38-02～U38-03 两个 P0/中、U38-04～U38-05 两个 P1/中和 U38-06～U38-08 三个 P2/小真正命中五目标安装发布与安装级反降级、ProgramStore/lifecycle 幂等切换恢复、升级 frozen source/delivery 闭包、current-authority 更新投影、两根 candidate health、事实化工件资源上限、program lock 单飞及 release:check 固定门禁的根因，并具备可由执行者一次实施的最优方案与完整验收条件。不修改实现，不运行构建或测试，不审查其他问题；第 30～37 单元既有 owner/transfer/checkpoint/trust/exposure/supervisor/stop/removal 合同直接复用，EX38-01～EX38-02 的既有排除直接复用，价值裁决否定的当前 v1 schema activation、任意 2 GiB 流式框架、独立 updater daemon、第二事实源、通用 updater/lifecycle/manifest/storage/secret/诊断框架、report 运行时耐久化及范围外发布能力不得恢复。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 38 单元正式文件中的 U38-01～U38-08、EX38-01～EX38-02，只依据八项问题最新的事实、价值裁决、方案、验收条件和状态继续。
...
1. 按 `U38-01 → U38-02 → U38-03 → U38-04 → U38-05 → U38-06 → U38-07 → U38-08` 从权威架构、规格和当前生产调用图重建事实链，核准 release/index/manifest/artifact/installer receipt，ProgramStore pointer 与 lifecycle phase，accepted-work/source/delivery operation，topology/current authority/receipt projection，anchor/executor health，artifact capacity，file-lock owner及 release report/ledger 的唯一事实源、稳定 identity、线性化点、消费者、异常终态、当前损失、受影响审查项、评级和工作量；判断当前描述命中根因还是局部表象。同根内容必须合并，独立根因不得互相遮蔽；价值裁决与 EX38-01～EX38-02 未被新生产事实触发时不得恢复旧评级、旧方案或扩面。
2. 穷尽直接变体：U38-01 覆盖五 target、首次/重放/更新/移除、无 home、Unicode/空格、旧 sequence 与同 identity 异 bytes；U38-02 覆盖 stage/version/pointer/previous、各 phase 效果/响应窗口与连续重启；U38-03 覆盖十 owner、七 source、enqueue 前后、late/fresh、seal/read-back；U38-04 覆盖三形态、surface/empty、离线/失败/换代、sticky action 与 success 消费；U38-05 覆盖两根、三形态、ready 窗口与 identity 漂移；U38-06 覆盖五目标真实包上限、容量不足与续传；U38-07 覆盖慢下载、双进程、owner 崩溃/PID 复用；U38-08 覆盖 workspace gate、固定 50 id 与可重建报告。每格必须指出稳定 identity、唯一事实、效果排序、零副作用边界、消费终态和直接验收；无法解释即继续修正根因。
3. 以锁定范围内的最优架构审查方案：U38-01 只复用现有 release identity/ProgramArtifact 生成并安装五目标程序树、launcher/remover和保留高水位的program receipt；U38-02 只用现有 ProgramStore/DeviceLifecycleJournal 做逐 phase achieved read-back；U38-03 只接通既有 DeliveryAuthority lifecycle admission/sealed；U38-04 只复用 topology/current-authority 与纯 projection builder；U38-05 只复用 bound inactive server 与同一 health snapshot；U38-06 只让实际产物上限和容量预检共用事实；U38-07 只复用现有 FileLock；U38-08 只删除 passed bypass 并 strict 校验固定 50 id。方案必须用最少文字说清改什么、怎么改、关键边界及完成判据；不得恢复当前 v1 schema activation、任意 2 GiB 流式处理、第二事实源或通用更新/生命周期/存储/诊断/发布框架。发现缺口时直接修正对应原问题，使执行者无需实现猜测即可一次完成。
4. 八项看似闭合后，对同一份未修改问题列表执行四路冷启动对抗复审：五目标安装发布/反降级与资源边界、ProgramStore 切版恢复与 candidate health、accepted-work delivery/current-authority surface 体验、发布证据/范围价值及历史裁决边界。各路必须抛开前轮结论，从当前合同和源码主动构造第2步反例，并核查 `U38-01↔U38-06↔U38-08`、`U38-02↔U38-05`、`U38-03↔U38-04`、`U38-04↔U38-05`、`U38-06↔U38-07` 以及八项与 EX38-01～EX38-02、第 30～37 单元既有合同和范围外发布能力的直接交界；发现真实反证则修正原记录并重新执行四路复审。

只有现有架构无法唯一推出方案，且选择会显著改变产品需求、用户体验、成本或单元边界时才暂停；其余架构选择按整体最优且不留债务自主收敛。

完成条件：同一份未修改问题列表通过四路冷启动对抗复审；U38-01～U38-08 的全部受支持安装/更新/恢复/移除、迟到 delivery、current-authority 投影、两根 health、容量/锁与发布门禁变体均被根因完整解释，影响面无遗漏，评级与工作量有事实依据，最优方案和验收条件可直接执行，不会恢复已删除的 schema activation、过强资源/耐久主张或扩建通用框架，也不会因逐 target、逐 phase、逐入口或逐证据行补丁继续返工。满足后明确回复“U38-01～U38-08 的根因与最优方案已闭合”并立即停止。
...
```text
目标：彻底解决第 38 单元正式问题列表中的 U38-01～U38-08，使 U38-01 一个 P0/大、U38-02～U38-03 两个 P0/中、U38-04～U38-05 两个 P1/中和 U38-06～U38-08 三个 P2/小闭合五目标安装发布与安装级反降级、ProgramStore/lifecycle 幂等切换恢复、升级 frozen source/delivery 闭包、current-authority 更新投影、两根 candidate health、事实化工件资源上限、program lock 单飞及 release:check 固定门禁的全部同根直接变体；不得扩展到其他问题或全单元流程。第30～37单元既有 owner/transfer/checkpoint/trust/exposure/supervisor/stop/removal 合同和 EX38-01～EX38-02 直接复用；价值裁决否定的当前v1 schema activation、任意2GiB流式框架、独立updater daemon、第二事实源、通用updater/lifecycle/manifest/storage/secret/诊断框架、report运行时耐久化及范围外发布能力不得实施。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第38单元正式文件中的U38-01～U38-08、EX38-01～EX38-02，只依据八项问题最新的根因、价值裁决、F38-01～F38-08固定矩阵、最优方案执行合同、验收条件、反证账和状态继续。
...
1. 修复前从权威架构、规格与当前生产调用图重建F38-01～F38-08固定矩阵。U38-01覆盖五target、首次/exact replay/更新/app remove、无home、Unicode/空格、旧sequence、同identity异bytes与各响应窗口；U38-02覆盖stage/version/pointer/previous、全部phase效果窗口、自动/显式恢复、terminal/receipt与连续重启；U38-03覆盖十owner、七source、enqueue前后、late/fresh、seal/read-back；U38-04覆盖三形态、surface/empty、离线/失败/换代、sticky action与success消费；U38-05覆盖anchor/executor、三形态、inactive/ready窗口与identity漂移；U38-06覆盖五目标实际包上限、容量不足与续传；U38-07覆盖慢下载、双进程、owner崩溃/PID复用；U38-08覆盖workspace gate与固定50 id。逐格绑定release/installer receipt、operation/phase/pointer、source/delivery、current authority/notice、bound server/health、artifact capacity、file-lock token及release report/ledger identity，唯一事实、线性化点、资源上界、零副作用终态和直接证据，并持续核对EX38-01～EX38-02、第30～37与范围外发布边界。
2. 按`U38-01 → U38-06 → U38-08 → U38-07 → U38-02 → U38-03 → U38-05 → U38-04`一次完成。先从workspace production allowlist生成五目标自包含Node 22程序树、固定launcher和installer/remover，平台签名/公证后冻结最终ProgramArtifact bytes；installer在程序根外保存唯一已验`target+keyId+releaseVersion+sequence+manifestDigest` receipt，完整树与launcher全等回读后才提交，exact replay返回原效果，旧sequence或同identity异bytes零写，app remove只删程序层并保留receipt与home。以五目标实测最大archive/expanded bytes加固定headroom形成一个版本化有限策略，release:check、decoder与updater共用；下载前按同卷partial+artifact+最坏展开+durable temp预检，解码后按文件总量复验，保留4MiB续传。删除能写passed的workspace gate旁路，固定50 id exact-set拒绝缺失、额外、重复和错分类。把`withProgramLock`改为现有core FileLock窄适配，复用token/heartbeat/live-owner/compare-release。

   随后只用现有ProgramStore与DeviceLifecycleJournal为每phase建立exact achieved read-back：stage/version全验，pointer按冻结source→执行一次、同operation目标已实现→只补journal、其他→fail-closed；terminal后receipt/cleanup响应丢失从同operation历史补齐。自动与显式previous恢复都进入同一upgrade lifecycle，恢复只回到冻结已验source，不把失败candidate登记为previous。ProgramUpgradeCoordinator在freeze回调把artifact中的七类exact source和当前delivery安装进现有DeliveryAuthority同operation admission；source逐项终结，delivery最后seal/read-back，全terminal后才`work-settled`，startup在producer前恢复同一admission/sealed。

   最后复用U37同home bound inactive server，在anchor/executor装配完成但激活router/ingress和发布ready前，用同一纯snapshot全等验证target manifest、protocol、静态v1 schema、signed trust generation/digest、home、role plan与exact endpoint；health read-back后才激活原server，失败保持inactive并走同operation previous恢复。topology确定后仅真实本机host启动updater；surface/empty本机零updater/home/listener，只经既有current-authority认证有限入口消费由receipt+pointer+stage+lifecycle+health构成的同一纯投影。action-required只由candidate换代、成功或exact dismiss结束；updated/restored以同receipt token compare-and-consume，旧token不清新通知。同步直接相关架构、规格、发布指南、50行ledger、必要派生资产与直接测试；禁止当前v1 schema activation、任意2GiB流式框架、第二事实源、独立daemon及通用更新/生命周期/存储/诊断/发布框架。
3. 按验证手册运行受影响闭包的最小必要类型检查、五目标真实installer/launcher/反降级与ProgramArtifact上限、release:check固定门禁、FileLock双进程、ProgramStore逐phase切换/恢复、十owner七source delivery闭包、两根inactive health及current-authority投影直接合同和场景测试，核对必要派生资产；源码有变时按项目常驻规则取得一次同输入构建证据。真实反例必须穿过production release builder/installer、ProgramStore/DeviceLifecycleJournal、DeliveryAuthority、anchor/executor两根bound server、current-authority relay、core FileLock与release:check，直接注入错target/旧sequence/同identity异bytes、各sync/响应丢失、迟到delivery、identity漂移、低容量、锁失主和ledger缺格；不得以mock自报installed/healthy/terminal/current-authority/passed或只验证返回值，不得运行包全测、模块回归、target×故障笛卡尔积或无关验证。失败先归因，实现问题直接修复并回到第2步。
4. 验证通过后冻结当前交付物指纹，整轮只读逐格重建U38-01～U38-08事实链；测试通过不得代替功能判断，F38-01～F38-08全部完成后才统一归并。随后对同一指纹执行四个相互隔离的冷启动对抗角色：五目标安装/反降级与资源边界、ProgramStore切版恢复与candidate health、accepted-work delivery/current-authority surface体验、发布证据/产品价值及历史裁决边界。各角色须抛开既有结论，主动重造第1步全部适用反例，并核查`U38-01↔U38-06↔U38-08`、`U38-02↔U38-05`、`U38-03↔U38-04`、`U38-04↔U38-05`、`U38-06↔U38-07`以及八项与EX38-01～EX38-02、第30～37单元和范围外发布能力的直接交界。
5. 新发现首次出现即以C38-C01起的稳定编号写入正式问题证据与反证账；收口前对价值裁决、专项审查和四路记录做差异审计，每项只能以“同根合并”“当前源码证伪”或“修复后复核通过”关闭。发现真实反证时先修正对应问题的根因、方案、验收和F38矩阵，再回到第2步；任何交付物修改都会使冻结指纹与对抗结论失效。

结束条件：同一冻结指纹上的U38-01～U38-08方案全部落地，受影响闭包的最小必要验证通过，专项功能审查与四路冷启动对抗均留下完整矩阵；全部C38反证有耐久处置，证明五目标最终bytes、安装receipt和反降级在无home/replay/remove下唯一收敛；ProgramStore每phase效果丢失与previous恢复只前滚一次；十owner七source及迟到delivery在ready前共同封闭；surface只消费current authority同一投影且sticky/success终态正确；anchor/executor坏candidate零ready；实际正式包上限、容量预检、FileLock单飞与固定50 id门禁无旁路。EX38-01～EX38-02和第30～37单元结论不变，范围外发布能力未实施，八项均更新为“已验证”。满足后明确报告“U38-01～U38-08 八项问题已彻底解决”并立即停止；不得进入全单元终审或单元提交验证。
...
- 清单代次：gen-1
- 当前状态：U38-01～U38-08 已完成实现、直接验证和专项对抗收口；按修复影响并集将 26 个节点置为 `[~]`，等待后续独立重审；2 个已排除记录继续有效
...
> 修复后状态更新：下表所有 `[~]` 行的最后一列均为修复前历史证据，现已统一作废，不得代表当前交付物结论；后续独立审查须基于当前生产实现、公共合同、直接测试与派生资产重新记录证据。未受影响的 `[x]` 行继续有效。

| 状态 | 编号 | 审查对象与独立通过条件 | 权威依据 | 必须记录的直接证据 |
|---|---|---|---|---|
| [x] | IR38-01 | 单元身份、范围和来源完整：S10 的交付物、五目标、稳定自动更新、发布与最终验收全部归项；明确排除未变成门禁，所有规范来源、章节和枚举行均有适用性结论。 | charter、specification §15、D38-01～D38-10 | 已逐项对账 charter、requirements、specification、S2 review、release guide、D38-01～D38-10、历史封版账与 50 行 ledger；31 项覆盖全部当前交付组，未发现未判定来源或范围外门禁。 |
| [~] | IR38-02 | 发布身份唯一：根 package.json 的 stable 0.1.0 是 workspace、CLI、构建和运行投影唯一版本源；release sequence、五个 target、index/manifest/artifact identity 全等。反降级高水位只来自已验安装 manifest 与同 home upgrade 记录；无 home、应用移除后重装和手动兼容恢复均不得降低，错误 target、旧序列及同 identity 异 bytes fail-closed。 | charter、specification §1/§5.3/§15、D38-01 | 版本与五 target codec 严格，但仓库没有实际 installer/program receipt：`assertReleaseAdvance`只比较 current manifest，应用移除/重装后既无程序安装闭包，也无由该闭包持有的耐久高水位；价值裁决已将原独立问题合并为正式 U38-01。 |
| [x] | IR38-03 | StableReleaseIndex v1 与 ReleaseManifest v1 严格规范：各自 exact key-set、JCS UTF-8 规范字节、签名域、HTTPS URL、摘要/字节和 index→manifest→artifact 交叉绑定闭合；unknown、缺失、错类型、非规范编码、签名或摘要错误在任何持久效果前拒绝。 | specification §1、Unit38 补充、D38-01 | `release.ts`、`canonical.ts`、`release-verifier.ts` 的 production decode→canonical bytes→Ed25519→binding 链在下载和 stage 写前完成；exact key-set、HTTPS、digest/bytes/target 反例均 fail-closed。 |
| [~] | IR38-04 | ProgramArtifact v1 合同完整：exact key-set、规范 UTF-8、文件 canonical 排序、路径、权限、单文件/总大小、base64url 与展开上界受限，禁止路径穿越、链接逃逸和额外根；下载字节、manifest 摘要、stage read-back 与执行字节全等。 | specification §4/§10.1/§15、release guide、D38-02 | decoder 的身份、路径、权限、摘要与展开边界成立；2 GiB 是实现自设上限，当前又没有可发布的正式工件，不能据此证明当前 P0/P1 用户损失。资源合同需按实际五目标产物收窄并做容量预检，已转正式 U38-06（P2）。 |
| [x] | IR38-05 | 兼容与耐久 schema：上述十三项 exact-set 由实际 codec/record descriptor 机械闭合并被 manifest、运行投影和 reader/writer policy 共用。bridge 初始写旧格式；每个真实 AuthorityCommitLog 首条新格式记录与该 schema activation 同 envelope，之后兼容恢复也只写已激活版本；无新写不虚构迁移，format/protocol/schema activation/epoch 均单调。current/candidate/previous 不得靠 reader fallback 隐式降级或写入不兼容状态。 | specification §1.4/§4/§5.3/§9/§15、D38-01/D38-06 | 当前首个公开基线的十三项 `readMin/readMax/writeVersion` 全为 1，candidate 与 previous 互读门禁会拒绝不同 writer；没有当前可达的 schema 提升或 bridge。activation 要求超前于当前产品事实，已转正式 EX38-01 并登记精确重开条件。 |
| [~] | IR38-06 | 供应链与凭据边界：三项批准生产依赖精确锁定，PAKE 仅开发依赖；四项只由 mesh 声明，门禁覆盖实际 workspace、锁文件完整性、运行闭包、许可证、安装脚本、官方审计事实和固定向量。生产导出、构建、包和运行时无 PAKE/短码路径，配对只用高熵邀请；工件不含开发依赖、source map、测试资产、源码秘密、私钥、SecretStore 或 home 数据。 | s2-security-supply-chain-review、specification §2、D38-02 | `pnpm security:supply-chain` 通过（4 direct/25 closure），PAKE 为 dev-only 且无生产 export；但 release artifact builder 接受外部任意 program root，仓库没有五目标程序树/包内容生产器，因而无法证明最终包排除源码、开发资产和秘密。 |
| [~] | IR38-07 | 五目标安装与 launcher：五个 target 的用户级程序根、PATH 中稳定 zz/zhixing、不可变 versions 与原子 current 同一合同；首次/重复安装验证平台签名、内嵌 manifest 和最终 bytes，只写程序层与内置 stable URL。空格/Unicode 路径成立；无 home 时只复用重复安装路径，零 host、零配对/恢复概念、零 lifecycle 或 ZHIXING_HOME 写入。 | charter、release guide、specification §8/§15、D38-03 | 仓库无五目标 installer/launcher 构建或安装实现；`build-release-artifact.mjs` 只封装调用方提供的目录，`app-remove.ts` 反而假定未生成的 `installer/remove-zhixing(.exe)` 已存在。首次/重放/无 home/Unicode 安装链不存在。 |
| [~] | IR38-08 | 自动维护触发正确：真实 foreground、on-demand、managed 三种本机 host 启动均异步检查，managed 按既定周期；current 启动、ready、首个输入和工作不等待网络。离线/禁用无破坏效果，无 updater daemon；pure surface 只消费 current-authority 更新投影，本机零 updater、home、module、listener。 | charter §11/§14～§15、requirements VII、release guide、D38-03 | REPL 与 hidden serve 在角色解析前无条件创建本地 ProgramStore 并启动检查；surface/empty 仍运行本机 updater、读取本机 receipt。正式生成 channel/trust 当前为 `undefined`，且 release build/check 未强制嵌入，正式交付不能保证自动更新可用。 |
| [~] | IR38-09 | 获取与续传安全：检查、下载、Range 续传、超时、断流、重试和容量门禁只接受同一候选 identity；网络 permit、响应头、长度、摘要和最终字节均验证，失败可重放且不污染 current。 | specification §10.1/§15、release guide、D38-02/D38-03 | Range、Content-Range、长度、digest 与 current 不污染成立；全量内存/容量上限和 mtime 抢锁可导致更新失败，但未证明会破坏 current 或核心工作，分别转为正式 U38-06/U38-07（P2），不再阻断本项。 |
| [~] | IR38-10 | ProgramStore 与指针耐久：stage、current、previous、active stage 的 identity、原子写、目录同步、read-back 和崩溃窗口闭合；任何响应丢失通过耐久事实前滚，不产生第二事实源。 | specification §4/§15、D38-01/D38-05 | stage→version rename 后、pointer 写前崩溃会丢失 active stage；pointer 写后、journal advance 前重启又因 generation 不同永久拒绝。已存在 version 目录不复验 expanded bytes；restore pointer 效果丢响应后重放会再次交换回坏版本。 |
| [~] | IR38-11 | ProgramUpdateReceipt 是唯一程序层更新事实：phase、notice、action 及条件字段必须逐项符合上表 exact-set，receipt 不复制 lifecycle、health、pointer 或兼容事实。它与 current/previous、已验 stage 和 active upgrade 形成唯一无副作用投影；CLI、server、status、doctor、输入区和第一方 surface 消费同一结果。固定文案可行动、零 raw，失败在成功、candidate 换代或用户明确收起前保持可见。 | charter §11、requirements VII、specification Unit38 补充、D38-01/D38-07 | receipt decoder/固定文案严格，但每次 check 先覆盖为 checking；已有 action-required 可被随后网络失败降成 retry-update。CLI/status/doctor 只读本机 receipt，surface 未走 current authority；updated/restored 无消费确认，离线时会在每次启动重复显示。 |
| [~] | IR38-12 | 升级接纳与安全点：只有完整已验 stage 且当前工作到达安全空闲点才 accept；随后关闭十类 producer gate，冻结同 operation accepted-work/source/delivery artifact，并固定使用 drain 逐 owner settle/read-back、flush 和物理收束。anchor/executor/foreground 不丢已接纳工作、不早停、不并行第二 operation；stop/removal/uninstall 只在耐久 conflict 边界保留各自既有策略，不扩成升级三策略。 | charter §10～§13、specification §3/§6/§15、第37单元冻结合同、D38-04 | upgrade 复用十 owner ports 与 drain，但 `ProgramUpgradeCoordinator` 冻结后没有调用 stop/uninstall 已有的 `installLifecycleAdmission`/`onFrozen`；冻结 causal source 的迟到 delivery 未反绑 upgrade operation，gate→freeze 竞态可漏出 settlement 集合。 |
| [~] | IR38-13 | 生命周期冲突与恢复：升级、stop、target removal、migration、backup uninstall 对同一设备的耐久 conflict identity、phase、重放和 terminal 唯一；任一阶段重启只前滚原 operation，不能绕过已封闭准入。 | specification §6/§15、第37单元冻结合同、D38-04 | DeviceLifecycleJournal 的 local-device conflict 与单 operation phase 成立；但 `old-host-stopped` 阶段的 stage rename/pointer write、`pointer-switched` 后 receipt/terminal 及 restore swap 均无效果 read-back，连续重启不能唯一前滚原 operation。 |
| [~] | IR38-14 | 旧 host 与切换顺序：只有 exact host/endpoint/definition/manager 事实授权停止旧实例；old-host-stopped 后才切 current pointer，切换效果丢响应可由 read-back 判定，绝不误停 successor。 | charter §10/§12、specification Unit38 补充、第36～37单元冻结合同、D38-04/D38-05 | exact endpoint/definition/manager proof 复用第36～37单元边界，pointer 也只在 old-host-stopped 后切换；但切换成功、journal advance 响应丢失后，generation 不再等于 frozen generation，恢复直接报错而非识别已切换效果。 |
| [~] | IR38-15 | candidate 启动健康门禁：anchor 与 executor、managed/on-demand/foreground 都以同一 candidate/version/schema/trust/role/endpoint identity 完成 preflight、listener、ready 和 health；不健康或身份漂移不得发布 ready。 | specification §2/§5.3/§8/§15、D38-05/D38-06 | anchor 在 listener 后才通过 loopback 核对 release/protocol/static schema/home/endpoint/role；executor 又只核对 pointer 即 `completeHealthy`，未验证同一 RPC health、trust/role/endpoint。两根与 ready 顺序仍不等价；当前 v1 schema 无需另造 activation 门禁。 |
| [~] | IR38-16 | 恢复语义有限而完整：健康失败自动恢复 previous；用户可在支持窗口执行 guided restore；恢复仍走同一安全生命周期和健康验证，current/previous 交换可重放，产品不暴露 rollback 术语或无限历史版本。 | charter §11～§12、release guide、specification Unit38 补充、D38-05 | 自动恢复在交换 pointer 后直接写 `health-verified/rolled-back`，未启动并验证 previous；效果丢失会二次交换。`zz update --restore-previous` 仅检查进程未运行后直接换 pointer，完全绕过 DeviceLifecycle、accepted-work、supervisor 与健康门禁。 |
| [~] | IR38-17 | 清理和资源上界：仅清理非 current、非 previous、非 active stage 的程序版本，固定批次且 read-back；失败可重入，不触碰 home、SecretStore、checkpoint 或 lifecycle evidence，磁盘和内存增长有界。 | specification §4.5/§10.1/§15、D38-05 | versions/stage 只删非保留目录、每批 128，未触及用户数据；超宽工件上限和目录全枚举缺少当前 P0/P1 损失，已按实际正式产物收窄为正式 U38-06（P2）。 |
| [x] | IR38-18 | 协议不兼容与 mesh 行为：远端 peer 版本/能力/schema 不兼容时仍提供规定的只读诊断，写入和 activation fail-closed；兼容 peer 正常工作，旧 peer 不触发本地降级或第二同步路径。 | specification §2.5/§5.3/§9/§15、D38-06 | 冻结 mesh protocol/capability admission 继续 fail-closed；当前十三项 writer 均为 v1，升级互读检查拒绝版本漂移，不存在本单元可达的 schema activation。相关未来版本跃迁主张已转正式 EX38-01。 |
| [~] | IR38-19 | 离线诊断和 surface：doctor、status 与 pure surface 只读组合 release manifest、current/previous pointer、stage/receipt、active upgrade、protocol/schema、真实 managed snapshot、checkpoint configuration、SecretStore inspect 和 lifecycle blocker；host absent、离线、无候选、下载、待安全点、恢复及失败均收敛为一个 code/message/action。不得 activation、SecretStore 写、网络探测或 lifecycle 效果，不泄漏秘密、URL、路径、内部 identity、环境变量或 raw error。 | charter §11、release guide、specification Unit38 补充、D38-07 | doctor 为 effect-free 并组合 pointer/receipt/stage/managed/checkpoint，但未读取 active lifecycle；status/REPL/pure surface 只消费本机 receipt，非 current-authority 投影，且 sticky action-required 可被下一次 check 覆盖。当前 v1 schema 不要求额外 activation 投影。 |
| [~] | IR38-20 | 显式 update 与 server RPC：认证和 loopback、strict 输入、request identity、并发/重放、公开错误及生命周期 handoff 全部在首个效果前验证；CLI 与 RPC 只调用同一 controller/lifecycle，不产生旁路。 | specification §1.5/§8/§15、release guide、D38-03/D38-04 | `server.update.prepare/health` 的 auth、loopback、strict decoder 与稳定 requestId 成立；普通 CLI update 复用 controller/RPC handoff。`--restore-previous` 则直接调用 ProgramStore，构成同一公开 update 命令中的 lifecycle 旁路。 |
| [~] | IR38-21 | 应用移除：先复用现有安全 stop 收束 current/future launch，再只移除程序工件；用户 home、配置、信任、SecretStore、checkpoint 和可恢复数据保持，效果丢失可重放。 | charter §10～§12、requirements VII、release guide、D38-03 | CLI 先调用现有 safe stop，随后仅 handoff 到假定存在的 program-root installer；仓库未生成/交付任何五目标 remove-zhixing 实现，故 future launch、程序删除、数据保留 read-back 与重放均没有生产路径。 |
| [~] | IR38-22 | 永久设备移除与误操作边界：与应用移除保持独立入口、强确认和不可逆语义；只将 zz uninstall 与 zz update --rollback 作为旧别名拒绝，不能误伤当前 zz device remove --permanent。文案、help、交互和非交互参数清楚区分“移除应用并保留数据”与“永久移除设备及本机数据”。 | requirements VII、release guide、specification §8/§15、D38-03/D38-08 | `zz app remove` 明示保留全部数据；`zz device remove --permanent`/`--current` 保持独立强确认与不可逆语义；旧 `zz uninstall`、`zz update --rollback` 无路由且当前永久移除入口未被误伤。 |
| [x] | IR38-23 | 七组旧路径按上表 exact-set 完整退役：每组的 reader、writer、组合根、公开 export、迁移状态、别名/注入参数、golden 和文档引用均反向清点；只保留 durable schema inventory 明示且被 minimumRollbackVersion 实际消费、无隐藏开关的正式 reader。默认安装不扫描旧位置，生产图无双写、双读、旧 capability 或 shadow owner。 | specification §8/§13/§15、D38-08 | 七组 production reader/writer/route 扫描未发现双写、旧 capability 或 shadow owner；`StopDeps.killFn/taskkillFn`未调用字段和CLI README迁移文字没有生产消费或当前用户损失，价值裁决后已转正式 EX38-02。 |
| [~] | IR38-24 | 五目标发布工件构建：每个 target 从同一 source/package closure 形成自包含 Node 22 程序树，消解 workspace:*，只含运行资产；消费平台签名/公证后才冻结最终归档 bytes、manifest 和逐文件摘要。版本、target、权限、原生件、launcher read-back 全等，开发占位 channel、开发依赖、install script、source map、测试资产和秘密不得进入正式包。 | release guide、specification Unit38 补充、D38-02/D38-09 | `build-release-artifact.mjs` 仅校验并序列化外部 `--program-root`；没有构建 Node 22 自包含树、消解 workspace、生成 launcher/installer、运行五目标平台签名或执行 launcher read-back 的生产链，仓库亦无五目标最终工件。 |
| [~] | IR38-25 | release:check 是确定、受控输出且无外部发布副作用的发布门：先运行 lint/test/build，再验证同一 source/package 输入、五目标、上述 12 项 smoke、签名/公证、manifest/artifact、50 行 ledger 和文档版本；结束前复验输入未漂移，才原子写 release-report 与可选 candidate index。不依赖 Git 暂存/干净状态，不保管私钥、不调用外部发布；失败不得留下伪成功或半写正式输出。 | release guide、specification §15、D38-09 | 当前根本阻断是 release:check 只能验证调用方伪造/外置的目标证据，不能消费仓库生成的五目标正式产物（U38-01）；`--skip-workspace-gates`与ledger exact-set仅属发布门硬化，已转正式 U38-08（P2），可重建report不追加运行时耐久要求。 |
| [~] | IR38-26 | invariant-01～invariant-18 逐行闭合：每行分别核对 source、真实 producer、consumer、composition root、terminal 与现存直接测试，且升级前后/恢复后仍成立；只有 18 行各自有结论和证据时本项通过。 | charter §13、specification §12、final acceptance ledger、D38-10 | ledger 含 18 个固定 id 且引用文件存在，但升级冻结 causal source 的迟到 delivery 未进入同 operation，已直接破坏 accepted-work/delivery 不变量；其余 schema activation 与 2 GiB 资源主张经价值裁决不再构成当前阻断。 |
| [~] | IR38-27 | 上表 10 个 fault id 逐行闭合：每行分别覆盖其规范终态及适用的效果前失败、效果后响应丢失、重启和恢复；fault-update 还必须证明旧版安全可用或兼容恢复并给唯一用户行动。只有 10 行各自有结论和生产证据时通过。 | charter §12/§12.1、final acceptance ledger、D38-10 | 10 个固定 id 齐全；前九项沿用已封版证据。`fault-update` 仅有 happy-path coordinator/temp store 测试，未注入 rename→pointer、pointer→journal、restore swap、terminal→receipt 的效果后响应丢失，旧版安全可用/唯一行动不成立。 |
| [~] | IR38-28 | 上表 8 个 security id 逐行闭合，并在第 38 单元交界核对发布签名、规范字节、target/path、秘密和供应链；每个拒绝均在首个副作用前，公开结果无敏感原文。只有 8 行及本单元安全交界各自有结论时通过。 | charter §4/§12/§13、s2 review、final acceptance ledger、D38-10 | 8 个固定安全 id 与供应链门禁齐全，release decode/signature/target/path fail-closed；但最终 program root 由外部调用方提供且无内容 exact-set，正式五目标包不存在，无法证明源码秘密/SecretStore/home/dev asset 零包含；反降级高水位亦缺失。 |
| [~] | IR38-29 | topology-single-machine 与 topology-paired-devices 分别闭合：anchor/executor 和 managed/on-demand/foreground 复用同一更新、lifecycle、health、兼容与诊断原语；pure surface 只走 current authority 的有限投影，本机零 updater/lifecycle/host。未启用角色零模块/监听，无漏装、双装或角色旁路。 | charter §2～§3/§14、specification §8、final acceptance ledger、D38-10 | 两个 topology id 只引用既有 role/mesh 测试，未覆盖更新装配。REPL/serve 在角色计划前无条件启本地 updater；pure surface 非零本机 ProgramStore，executor 又绕过 anchor 使用的 loopback health gate，拓扑不等价。 |
| [~] | IR38-30 | 上表 12 个 journey id 必须逐行给出入口、用户语言、终态和直接证据；另将 12 项 release smoke exact-set 逐项映射到相关旅程。开箱、配对、日常、离线、uncertain、迁居/恢复、停机/永久移除与四个更新旅程均保持零拓扑术语、正常无噪声、失败一个下一步；只有 12 行和 12 项 smoke 各自有结论时通过。 | charter §11/§15、requirements VII、release guide、final acceptance ledger、D38-10 | 12 个 journey id 与 12 smoke 名称齐全；但 first-run 无真实 launcher/installer，auto-update 正式 channel 未嵌入，update-recovery 绕过健康/不可重放，app-remove 只用测试伪造 installer，release-matrix 无五目标产物，四条更新旅程均未穿过生产交付链。 |
| [~] | IR38-31 | 最终交付装配与证据充分：当前变更集中的 root/scripts、core、cli、mesh、owner-kernel、providers、server、tools-builtin、模块文档和 workbench 每组均反向归入 IR38-02～IR38-30；所有 production 文件有真实 producer、consumer、composition root 和公开出口，直接测试走生产路径。生成 version/channel、S7、golden、导出、ledger、guide、report 与代码一致，无孤立实现、测试专用生产旁路或范围外能力。 | charter §15、specification §13/§15、release guide、D38-01～D38-10 | 文件分组均可归项，但 app-remove 测试现场伪造 installer，release builder/check 只消费外部 program tree/evidence，generated channel/trust 为空；升级的 pointer/health/delivery 与 current-authority 投影也未闭合真实 composition root。静态 schema activation 经价值裁决不再列为当前阻断。 |
...
- 修复后状态映射完成：5 项未受影响并保持 `[x]`，26 项受生产实现、公共合同或直接测试变化影响而改为 `[~]`；无 `[ ]`、`[!]`。
- U38-01～U38-08 已在正式文件更新为“已验证”，但修复验证、专项功能审查和多角色对抗不直接产生 `[x]`；当前独立审查尚未重新执行，不能据此判定清单通过。
