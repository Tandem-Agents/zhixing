...
- **交付基线**：第 24 单元起点 `010dfd1` 至当前 `HEAD` 的 112 个功能文件（Feishu 6、CLI 56、core 23、executor 8、orchestrator 1、owner-kernel 10、rpc 1、runtime-host 1、server 5、规格 1）；工作台及单元登记文件不计入功能交付闭包
- **交付指纹**：待交付物冻结后登记；清单定稿不以当次指纹作为功能审查项
- **目标提交边界**：第 24 单元（S6）中继、渠道确认与最终性整合的生产实现、直接相关测试与规格补充
- **当前任务进度**：67%（16 / 24 项通过，8 项存在 P1）
...
| 当前完整交付物（112 个功能文件）                                       | 适用     | 6 个 Feishu、56 个 CLI、23 个 core、8 个 executor、1 个 orchestrator、10 个 owner-kernel、1 个 rpc、1 个 runtime-host、5 个 server 和 1 个规格文件逐一归入 IR24-01～IR24-24；工作台及单元登记文件属于流程维护，不计入本单元功能审查。              |

### 审查项

| 编号    | 状态 | 审查点                                    | 完整范围与通过条件                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 证据与结论                                                                                                          |
| ------- | ---- | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| IR24-01 | [x]  | 单元边界与交付闭包                        | 将 112 个功能文件逐一归入后续审查项；核对新增导出、构造器、生产者、消费者和组合根均有唯一落点。只包含 S6 中继、渠道确认、最终性及直接测试/规格，不漏实现，也不夹带第 25 单元及以后能力。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 112 个功能文件已按包与生产链归入 IR24-01～IR24-24；新增 v2 assignment 记录、job runtime 与恢复闭包均有审查落点，未夹带后续单元产品能力；未发现 P0/P1。 |
...
| IR24-03 | [!]  | 权威、幂等与唯一事实源                    | pending/finished 只由 executor assignment 流权威决定；relay/outbox/status 仅作投影。conversation allow-once、job grant、callback、control 和重复请求均按耐久键先完整比对再回放原结果，禁止在回放前重新验当前时钟、资格、随机数或产生外部副作用。对短期能力接管固定核对 fresh、已耐久接管未完成、已完成三态及首段 fsync 后跨 TTL/吊销重试，接管后不得退回 fresh 资格。                                                                                                                                                                                                                                                                                                                                                                                                                                           | audit-only settlement 仅以 owner 调用成功的瞬时响应结束本轮，没有写 executor assignment 的耐久完成事实；恢复枚举仍只认 `cancel-proof-owner-accepted`，形成完成声明与本地义务事实分叉；存在 P1。 |
...
| IR24-14 | [!]  | 生产装配、角色门控与生命周期              | 在唯一 CLI 组合根逐一核对 executor stream、ticket、owner relay/challenge、surface direct/relay、channel callback、status/final 与既有资产服务的生产实例、服务注册和依赖顺序。S6 只在闭环后原子启用；每个长期资源取得后立即进入统一启动回滚，正常停机逆序释放；所有后续失败点可回滚，未启用角色零加载、零监听。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | executor-role 已稳定创建 job worker，但新 `AgentRuntime`→job adapter 未闭合异常/提前退出时的底层 promise 生命周期；同时没有测试驱动该真实 adapter、`createJobRuntime` 与 executor-role 启停装配，存在 P1。 |
...
| IR24-24 | [!]  | 来源、条款、交付物与教训反向对账          | 逐行核对本表每个来源判定均有 IR 落点，每个 IR 均能反向指向规范条款和有限文件/生产链；112 个功能文件无未归项。执行 L23-01、03、05～09、11、13～14、23～27、32、35、41、44～50、52～56、58～59 的检测动作，并对 X23-03、X23-11 给出维持排除或重开的事实；发现缺项时只能补入本单元边界内的有限条目。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 112 个功能文件均有归属；反向对账确认两步兼容桥、audit-only 退休和真实 job runtime/装配验收三处缺口，分别关联 IR24-03、14、17～20、22；存在 P1。 |
