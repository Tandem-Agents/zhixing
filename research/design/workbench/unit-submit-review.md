...
```
目标：只收敛第 38 单元正式问题列表中的 U38-19～U38-20，使两个 P2/小真正命中应用移除 commit 后固定 handoff 的子进程接纳反馈，以及永久设备移除 decision 发出后既有耐久 operation 的稳定失败投影，并具备可由执行者一次实施的最优方案与完整验收条件。不修改实现，不运行构建或测试，不审查其他问题；U38-01～U38-18 与 EX38-01～EX38-03 的既有结论直接复用，U38-17 已验证的 pre-commit future 补偿、managed current stop 和 exact unregister，U38-18 已验证的强确认、explicit cancel、exact abort 与跨重启恢复不得恢复或扩面；价值裁决否定的 remover 两阶段事务、第二 lifecycle/journal、decision 失败自动 abort、通用 updater/lifecycle/runner 及范围外发布能力不得并入本单元。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 38 单元正式文件中的 U38-19～U38-20、EX38-01～EX38-03，只依据两项最新事实、价值裁决、方案、验收条件和状态继续。
...
1. 按 `U38-19 → U38-20` 从权威架构、规格和当前生产调用图重建事实链，核准 exact unregister、stable installer/remover handoff、child spawn 接纳，以及 device-removal operation、decision dispatch、status/continue/cancel 投影的唯一事实源、稳定 identity、线性化点、消费者、异常终态、当前损失、受影响审查项、评级和工作量；同根内容必须合并，历史修复与价值裁决未被新生产事实触发时不得恢复旧评级、旧方案或扩面。
2. 穷尽直接变体：U38-19 覆盖五目标、首段 handoff 与删除 helper 的 spawn 成功/error、权限竞态、进程资源失败、future absent 后重放；U38-20 覆盖 transfer/destroy/lost、decision 首效果前拒绝与效果后响应丢失、在线转离线、未决/已决/terminal、显式 cancel 交界及连续重启。每格必须指出稳定 identity、唯一事实、效果排序、零副作用边界、消费终态和直接验收；无法解释即继续修正根因。
3. 以锁定范围内最优架构审查方案：U38-19 只在现有固定 handoff 等待 child `spawn/error` 后再 unref，并让稳定 installer 把 helper 启动失败投影为唯一重试行动；U38-20 只在 continue 失败后读取既有 `device status`，按未决或已决投影一个可执行下一步，显式 cancel 继续复用 exact abort且不得自动 abort。方案必须用最少文字说清改什么、怎么改、关键边界及完成判据；不得新增 remover transaction、第二 operation/journal、自动取消、通用 lifecycle/错误框架或范围外发布能力。发现缺口时直接修正对应原问题，使执行者无需实现猜测即可一次完成。
4. 两项看似闭合后，对同一份未修改问题列表执行四路冷启动对抗复审：handoff 接纳与幂等重试、decision 不确定态与唯一用户行动、`U38-19↔U38-17`及`U38-20↔U38-18`直接交界、生产体验/范围价值及历史裁决边界。各路必须抛开前轮结论主动构造第2步反例，并核查两项与U38-01～U38-18、EX38-01～EX38-03、第30～37单元合同及范围外发布能力的直接交界；发现真实反证则修正原记录并重新执行四路复审。

只有现有架构无法唯一推出方案，且选择会显著改变产品需求、用户体验、成本或单元边界时才暂停；其余架构选择按整体最优且不留债务自主收敛。

完成条件：同一份未修改问题列表通过四路冷启动对抗复审；U38-19 的五目标 handoff 接纳、失败反馈与 future absent 重试，以及 U38-20 的 decision 前后失败、在线/离线、status/continue/cancel 与连续重启变体均被根因完整解释，影响面无遗漏，评级与工作量有事实依据，方案和验收可直接执行，不恢复已验证边界或扩建通用框架。满足后明确回复“U38-19、U38-20 的根因与最优方案已闭合”并立即停止。
...
```text
目标：彻底解决第 38 单元正式问题列表中的 U38-19～U38-20，使两个 P2/小闭合应用移除 commit 后两层固定 handoff 的子进程接纳反馈，以及永久设备移除 decision 发出后既有耐久 operation 的在线/离线稳定失败投影全部同根直接变体；不得扩展到其他问题或全单元流程。U38-01～U38-18、EX38-01～EX38-03与第30～37单元既有结论直接复用，U38-17已验证的pre-commit future补偿、managed current stop和exact unregister，U38-18已验证的强确认、explicit cancel、exact abort与跨重启恢复不得恢复或扩面；价值裁决否定的remover两阶段事务、第二lifecycle/journal、decision失败自动abort、通用updater/lifecycle/runner及范围外发布能力不得实施。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第38单元正式文件中的U38-19～U38-20、EX38-01～EX38-03，只依据两项问题最新的根因、价值裁决、F38-24～F38-25固定矩阵、C38-C16～C38-C17反证、最优方案执行合同、验收条件和状态继续。
...
1. 修复前从权威架构、规格与当前生产调用图重建F38-24～F38-25固定矩阵。U38-19覆盖五目标、runtime/installer stat后权限竞态、第一层installer的spawn/error/非零/signal、第二层helper的spawn/error、资源失败、正常异步删除及future absent重放；U38-20覆盖transfer/destroy/lost、首效果前拒绝与效果后响应丢失、online→offline、target状态失败、issuer未决/已决/terminal/aborted、status自身不可用、explicit cancel与连续重启。逐格绑定program root/runtime/installer/helper责任身份、唯一target active operation与issuer/target journal phase，线性化点、零副作用终态和直接证据，并持续核对U38-01～U38-18、EX38-01～EX38-03、第30～37与范围外发布边界。
2. 按`U38-19 → U38-20`一次完成。先把`program-installer-entry`的删除helper调度改为一次性异步接纳：`scheduleProgramRootRemoval()`返回Promise，helper只在`spawn`后unref并resolve，`error`reject；installer顶层await并以exit 0表示helper已被OS接纳。`defaultHandoff()`不得在第一层spawn后提前unref，须等待installer的`error/exit`，只接受exit 0；spawn error、非零或signal统一投影固定“应用未完全移除，请重试”，不得泄露raw error/path。该回执只证明删除责任已接纳，不等待或伪报延迟文件删除完成；commit后失败保留future absent幂等重试，不新增operation、journal或两阶段remover事务。

   再补齐现有`deviceRemovalStatus()`的纯投影：target status可读时优先消费；target离线或读取失败时只从issuer journal当前operation phase投影既有`DeviceRemovalState`，active和abort待target确认不得返回空，terminal保持历史结果，零新journal/gate效果。`removeDeviceWithManagement()`和`continueDeviceRemoval()`的decision dispatch共用窄失败helper：continue抛错后只读一次status；terminal或已决进度直接呈现，未决固定提示“移除已登记，可继续或取消”，status也不可用则固定提示稍后运行`device status`，不得自动abort或透出raw错误。explicit cancel继续走U38-18既有exact abort。同步直接相关架构、规格、发布指南、ledger与直接测试；禁止修改已验证pre-commit补偿/stop/unregister、强确认/cancel/abort主链，禁止新增第二事实源、通用lifecycle/错误框架或范围外发布能力。
3. 按验证手册运行受影响闭包的最小必要CLI类型检查、两层remover接纳和device decision失败投影直接合同与场景测试，核对必要派生资产；源码有变时按项目常驻规则取得一次同输入构建证据。真实反例必须穿过production app-remove→version-local runtime→stable installer→平台helper，以及device CLI→RPC→target/issuer journal→mesh/status，直接注入两层child各终态、权限/资源失败、future absent重放、三decision模式、效果/响应丢失、online/offline、各journal phase、status再失败与连续重启；不得以mock自报accepted/phase或末端返回值代替真实child事件和journal read-back，不得运行包全测、模块回归、target×故障笛卡尔积或无关验证。失败先归因，实现问题直接修复并回到第2步。
4. 验证通过后冻结当前交付物指纹，整轮只读逐格重建两项事实链；测试通过不得代替功能判断，F38-24～F38-25全部完成后才统一归并。随后对同一指纹执行四个相互隔离的冷启动对抗角色：handoff接纳与幂等重试、decision不确定态与唯一行动、U38-17/U38-18直接交界、生产体验/范围价值及历史裁决边界。各角色须抛开既有结论主动重造第1步全部适用反例，并核查两项与U38-01～U38-18、EX38-01～EX38-03、第30～37单元和范围外发布能力的直接交界。
5. 新发现首次出现即以C38-C18起的稳定编号写入正式问题证据与反证账；收口前对C38-C16～C38-C17、历轮反证、专项审查和四路记录做差异审计，每项只能以“同根合并”“当前源码证伪”或“修复后复核通过”关闭。发现真实反证时先修正对应问题的根因、方案、验收和F38矩阵，再回到第2步；任何交付物修改都会使冻结指纹与对抗结论失效。

结束条件：同一冻结指纹上的U38-19～U38-20方案全部落地，受影响闭包的最小必要验证通过，专项功能审查与四路冷启动对抗均留下完整矩阵；C38-C16～C38-C17及新增同根反证均有耐久处置，证明五目标第一层installer与第二层helper只有在完整接纳后才向用户成功，任一启动失败给固定重试行动且program/数据安全，future absent重放可继续；transfer/destroy/lost在decision效果前后、在线/离线、status失败与连续重启下只投影既有operation，未决/已决/terminal各有唯一行动且零自动abort。U38-01～U38-18、EX38-01～EX38-03与第30～37单元结论不变，范围外发布能力未实施，两项均更新为“已验证”。满足后明确报告“U38-19～U38-20 两项问题已彻底解决”并立即停止；不得进入全单元终审或单元提交验证。
...
| IR38-04 | [x] | ProgramArtifact v1 合同完整：exact key-set、规范 UTF-8、文件 canonical 排序、路径、权限、单文件/总大小、base64url 与展开上界受限，禁止路径穿越、链接逃逸和额外根；下载字节、manifest 摘要、stage read-back 与执行字节全等。 | 本轮独立证据：从 production `validateProgramArtifact()` 反向核对到 `compareProgramArtifactPaths()`、`collectProgramFiles()`、source/package 行排序和 artifact 编码；四处共用 ordinal comparator，编码前再次经过 production validator，混合大小写、下划线、隐藏目录、Unicode 与 locale 不再产生 producer/decoder 分叉，非法路径仍在写 tree/report/publish 前拒绝。 |
...
| IR38-08 | [x] | 自动维护触发正确：真实 foreground、on-demand、managed 三种本机 host 启动均异步检查，managed 按既定周期；current 启动、ready、首个输入和工作不等待网络。离线/禁用无破坏效果，无 updater daemon；pure surface 只消费 current-authority 更新投影，本机零 updater、home、module、listener。 | 本轮独立证据：单次与 managed 入口共用同一 active-round owner；tick 先过 closed/active gate，已有轮次只复用 exact promise。两根三形态启动不等待网络，teardown 均按 close→clear timer→abort→await 执行，CleanupRegistry 也等待异步 disposer；pure surface 装配未引入 updater、timer 或 listener。 |
| IR38-09 | [x] | 获取与续传安全：检查、下载、Range 续传、超时、断流、重试和容量门禁只接受同一候选 identity；网络 permit、响应头、长度、摘要和最终字节均验证，失败可重放且不污染 current。 | 本轮独立证据：更新下载仍由 candidate identity、permit、Range/长度/摘要和最终 bytes 门禁；跨进程锁升级为 strict `pid+birth+token`。heartbeat 过期只触发身份复核，exact 活进程或身份不可证明均保持 busy，仅消失/换代后在 reclaim claim 内全等重读并删除，旧 token release 不能释放后继锁。 |
