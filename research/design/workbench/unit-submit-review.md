...
```
目标：只收敛第 38 单元正式问题列表中的 U38-13～U38-18，使 U38-13 一个 P0/小、U38-14/U38-16/U38-17 三个 P1/中和 U38-15/U38-18 两个 P1/小真正命中 ProgramArtifact canonical order、发布 source/package provenance、自动检查轮次所有权、FileLock 进程身份活性、应用移除失败补偿及永久设备移除 durable cancel 的根因，并具备可由执行者一次实施的最优方案与完整验收条件。不修改实现，不运行构建或测试，不审查其他问题；U38-01～U38-12 与 EX38-01～EX38-03 的既有结论直接复用，已验证的五目标程序树、existing-install lifecycle、SemVer/storage identity、current-authority 纯投影、candidate-only 十二 journey 与 release gate 不得恢复或扩面；价值裁决否定的远程证明、通用 updater/lifecycle/runner、租约服务、前置重复确认、流式 cleanup 框架、第二事实源及范围外发布能力不得并入本单元。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第 38 单元正式文件中的 U38-13～U38-18、EX38-03，只依据六项最新事实、价值裁决、方案、验收条件和状态继续。
...
1. 按 `U38-13 → U38-14 → U38-15 → U38-16 → U38-17 → U38-18` 从权威架构、规格和当前生产调用图重建事实链，核准程序树路径排序、source/package fingerprint 与 tree receipt、自动检查 active round、FileLock process identity、managed future/current 双事实及 device-removal operation/gate 的唯一事实源、稳定 identity、线性化点、消费者、异常终态、当前损失、受影响审查项、评级和工作量；同根内容必须合并，历史修复与价值裁决未被新生产事实触发时不得恢复旧评级、旧方案或扩面。
2. 穷尽直接变体：U38-13 覆盖五目标混合大小写、下划线、隐藏目录与 Unicode 路径；U38-14 覆盖 source/package/producer 任一变化、旧 tree/report 重放与输入漂移；U38-15 覆盖两根三形态、轮次各效果窗口、stop/remove/换代；U38-16 覆盖 live/crashed、sleep/pause、heartbeat、PID reuse 与 stale release；U38-17 覆盖 future-disable、current-stop、definition 不可判定及响应丢失；U38-18 覆盖 transfer/destroy/lost、accepted 后取消/拒绝/异常及连续重启。每格必须指出稳定 identity、唯一事实、效果排序、零副作用边界、消费终态和直接验收；无法解释即继续修正根因。
3. 以锁定范围内最优架构审查方案：U38-13 只让 producer/decoder 共用 ordinal comparator；U38-14 只把现有 source/package closure 与全部实际 producer 写入 exact receipt/fingerprint；U38-15 只让现有自动检查入口返回可 abort/await 的唯一 active-round handle；U38-16 只给现有 FileLock record 增加 process-birth identity 并收紧接替谓词；U38-17 只复用 managed adapter/read-back 对 stop 失败补偿 re-enable、不可判定 fail-closed；U38-18 只把 accepted 后所有退出映射到既有 durable abort，并保留 lost 强确认。方案必须用最少文字说清改什么、怎么改、关键边界及完成判据；不得新增远程证明、通用 updater/lifecycle/runner、租约服务、第二事实源、前置重复确认或流式 cleanup 框架。发现缺口时直接修正对应原问题，使执行者无需实现猜测即可一次完成。
4. 六项看似闭合后，对同一份未修改问题列表执行四路冷启动对抗复审：五目标工件规范与发布 provenance、自动检查与 FileLock 的单飞/清理生命周期、应用移除与永久设备移除的失败/取消可信性、生产体验/范围价值及历史裁决边界。各路必须抛开前轮结论主动构造第2步反例，并核查`U38-13↔U38-14`、`U38-15↔U38-16`、`U38-17↔U38-18`、六项与U38-01～U38-12、EX38-01～EX38-03、第30～37单元合同及范围外发布能力的直接交界；发现真实反证则修正原记录并重新执行四路复审。

只有现有架构无法唯一推出方案，且选择会显著改变产品需求、用户体验、成本或单元边界时才暂停；其余架构选择按整体最优且不留债务自主收敛。

完成条件：同一份未修改问题列表通过四路冷启动对抗复审；U38-13～U38-18 的全部受支持路径排序/来源证明、自动轮次/锁接替、应用移除补偿和永久移除取消变体均被根因完整解释，影响面无遗漏，评级与工作量有事实依据，方案和验收可直接执行，不恢复已验证边界或扩建通用框架。满足后明确回复“U38-13～U38-18 的根因与最优方案已闭合”并立即停止。
...
```text
目标：彻底解决第 38 单元正式问题列表中的 U38-13～U38-18，使 U38-13 一个 P0/小、U38-14/U38-16/U38-17 三个 P1/中和 U38-15/U38-18 两个 P1/小闭合 ProgramArtifact canonical order、发布 source/package provenance、自动检查轮次所有权、FileLock 进程身份活性、应用移除失败补偿及永久设备移除 durable cancel 的全部同根直接变体；不得扩展到其他问题或全单元流程。U38-01～U38-12、EX38-01～EX38-03与第30～37单元既有结论直接复用，已验证的五目标程序树、existing-install lifecycle、SemVer/storage identity、current-authority纯投影、candidate-only十二journey与release gate不得恢复或扩面；价值裁决否定的远程证明、通用updater/lifecycle/runner、租约服务、前置重复确认、流式cleanup框架、第二事实源及范围外发布能力不得实施。

首个动作及每次续跑或历史压缩后的首个动作：读取《单元审查与修复工作台》及第38单元正式文件中的U38-13～U38-18、EX38-03，只依据六项问题最新的根因、价值裁决、F38-18～F38-23固定矩阵、最优方案执行合同、验收条件、反证账和状态继续。
...
1. 修复前从权威架构、规格与当前生产调用图重建F38-18～F38-23固定矩阵。U38-13覆盖五目标大小写、下划线、隐藏目录、Unicode、不同locale及artifact/source/package重放；U38-14覆盖source/package/任一release producer变化、旧tree/report、构树后漂移；U38-15覆盖两根三形态、连续tick与fetch/download/stage/handoff/receipt各窗口、stop/remove/换代；U38-16覆盖sleep/pause/crash、heartbeat、PID reuse、birth读取失败及主锁/reclaim/claim；U38-17覆盖future enabled/disabled/absent/unknown、definition drift及disable/stop/reenable/unregister窗口；U38-18覆盖transfer/destroy/lost/cancel、accept响应丢失、同target旧/新operation、在线/离线、abort响应丢失与重启。逐格绑定path/closure/receipt、round handle、lock token/PID/birth、managed definition/current host、device operation/abort identity，唯一事实、线性化点、零副作用终态和直接证据，并持续核对U38-01～U38-12、EX38-01～EX38-03、第30～37与范围外发布边界。
2. 按`U38-13 → U38-14 → U38-15 → U38-16 → U38-17 → U38-18`一次完成。先在core release protocol导出唯一ordinal path comparator，production decoder、`collectProgramFiles`及source/package row排序全部复用；artifact编码前调用production validator，非规范producer输出零tree/report/publish。随后定义有限release-producer path exact-set并纳入现有source fingerprint；构树前计算source/package digest并写strict tree receipt，artifact builder、target evidence和release:check逐段重算全等，publish前复验输入未漂移，旧tree/report不得跨closure重放。

   再让现有单次与managed自动检查入口返回同一种active-round handle，内部最多一个`controller+promise`；tick统一经过closed/active gate，`stop()`先close并清timer，再abort/await exact promise，finally只清同代引用，两根teardown都必须await后才释放依赖。FileLock record升级为版本化strict`PID+process-birth+token`身份，Linux `/proc`、macOS `ps`、Windows固定只读创建时间查询收束为core内窄resolver；heartbeat stale只触发身份复核，exact进程仍活或无法证明时保持busy，身份消失/换代后才在既有reclaim claim内全等重读并删除，release继续compare-token。

   最后把program removal prepare收窄成冻结exact spec/definition/before state/changed flag的操作内handle，给现有adapter增加只启用同一已存在定义且绝不install/start/rewrite的`enableFutureExact`；stop或commit失败只补偿本操作造成的enabled→disabled并read-back，原disabled零误启，managed事实或补偿不可证明时保留程序并给稳定行动，只有current安全停止且future exact absent才handoff remover。永久设备移除保留preflight accept；accept前可判纯输入错先拒绝，在既有`device.continue(cancel)`增加仅cancel可带的`targetName+operationId` strict变体并让remove入口强制使用，authority全等核对后才重投现有signed abort，原后续continue按唯一target恢复形态不变；accept响应丢失、cancel、确认拒绝及pre-decision异常都走该exact变体，错/旧operation零取消，在线收敛cancelled，离线读回issuer-abort后显示pending并由现有startup/mesh重放，lost/destroy先强确认且cancel不再二次确认。同步直接相关架构、规格、发布指南、ledger、必要派生资产和直接测试；禁止新增远程证明、通用updater/lifecycle/runner、租约服务、第二锁/事实源、前置重复确认、流式cleanup框架或范围外发布能力。
3. 按验证手册运行受影响闭包的最小必要类型检查、canonical artifact/provenance、自动轮次清理、FileLock进程身份、app removal补偿和device durable cancel直接合同与场景测试，核对必要派生资产；源码有变时按项目常驻规则取得一次同输入构建证据。真实反例必须穿过production release producer/decoder/release:check、两根automatic controller与ProgramStore、FileLock主锁/reclaim/claim、managed adapter/stop/remover、device CLI/RPC/journal/mesh，直接注入混合路径与locale、producer漂移、各round效果窗口、真实child pause/crash/PID reuse、future/current双事实、accept/abort效果与响应丢失及连续重启；不得以mock自报canonical/settled/alive/restored/cancelled或末端返回值代替真实read-back，不得运行包全测、模块回归、target×故障笛卡尔积或无关验证。失败先归因，实现问题直接修复并回到第2步。
4. 验证通过后冻结当前交付物指纹，整轮只读逐格重建六项事实链；测试通过不得代替功能判断，F38-18～F38-23全部完成后才统一归并。随后对同一指纹执行四个相互隔离的冷启动对抗角色：五目标工件规范与发布provenance、自动检查与FileLock单飞/清理、应用移除与永久设备移除可信终态、生产体验/范围价值及历史裁决边界。各角色须抛开既有结论主动重造第1步全部适用反例，并核查`U38-13↔U38-14`、`U38-15↔U38-16`、`U38-17↔U38-18`、六项与U38-01～U38-12、EX38-01～EX38-03、第30～37单元和范围外发布能力的直接交界。
5. 新发现首次出现即以C38-C13起的稳定编号写入正式问题证据与反证账；收口前对历轮反证、专项审查和四路记录做差异审计，每项只能以“同根合并”“当前源码证伪”或“修复后复核通过”关闭。发现真实反证时先修正对应问题的根因、方案、验收和F38矩阵，再回到第2步；任何交付物修改都会使冻结指纹与对抗结论失效。

结束条件：同一冻结指纹上的U38-13～U38-18方案全部落地，受影响闭包的最小必要验证通过，专项功能审查与四路冷启动对抗均留下完整矩阵；全部既有及新增同根反证均有耐久处置，证明五目标artifact/source/package跨locale canonical且签名bytes贯穿同一source/package/producer closure；两根三形态任一时刻每generation至多一轮自动检查，cleanup后零旧代效果；暂停的exact FileLock owner不被接替，crash/PID reuse后恰一前滚；app remove失败只补偿本操作改变的future且不确定时保留程序，成功时current/future/program共同移除；device取消只命中本次operation，在线/离线及响应丢失均耐久前滚，lost/destroy无确认旁路。U38-01～U38-12、EX38-01～EX38-03与第30～37单元结论不变，范围外发布能力未实施，六项均更新为“已验证”。满足后明确报告“U38-13～U38-18 六项问题已彻底解决”并立即停止；不得进入全单元终审或单元提交验证。
...
> **清单状态**：0 项 `[ ]`、20 项 `[x]`、0 项 `[!]`、11 项 `[~]`；U38-13～U38-18 已修复并通过专项验证，但凡受生产实现、公共合同或直接测试变化影响的节点均已作废旧证据并转为待独立复审，修复验证不直接产生`[x]`。
...
> 本轮已审完全部 18 项 `[~]`，其余 13 项 `[x]` 的登记输入未变并直接复用。状态仅表示本轮独立判定，不以专项修复验证或测试结果代替功能判断。
...
| IR38-02 | [x] | 发布身份唯一：根 package.json 的 stable 0.1.0 是 workspace、CLI、构建和运行投影唯一版本源；release sequence、五个 target、index/manifest/artifact identity 全等。反降级高水位只来自已验安装 manifest 与同 home upgrade 记录；无 home、应用移除后重装和手动兼容恢复均不得降低，错误 target、旧序列及同 identity 异 bytes fail-closed。 | 当前独立证据：raw SemVer、release sequence、target 和已验 manifest digest 继续作为 exact release identity；ProgramStore 仅把已验 manifest digest 的 64 位小写 hex 用作 filesystem key，pointer 同时保留 raw release identity 与目录。安装 receipt 的程序根外高水位、SemVer 2.0 precedence、同 identity 异 bytes和旧 sequence 拒绝均未被该存储投影改变，无 P0/P1。 |
...
| IR38-04 | [~] | ProgramArtifact v1 合同完整：exact key-set、规范 UTF-8、文件 canonical 排序、路径、权限、单文件/总大小、base64url 与展开上界受限，禁止路径穿越、链接逃逸和额外根；下载字节、manifest 摘要、stage read-back 与执行字节全等。 | 旧证据已作废：U38-13/U38-14 修改 canonical comparator、producer/decoder 与 provenance 合同；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
...
| IR38-07 | [x] | 五目标安装与 launcher：五个 target 的用户级程序根、PATH 中稳定 zz/zhixing、不可变 versions 与原子 current 同一合同；首次/重复安装验证平台签名、内嵌 manifest 和最终 bytes，只写程序层与内置 stable URL。空格/Unicode 路径成立；无 home 时只复用重复安装路径，零 host、零配对/恢复概念、零 lifecycle 或 ZHIXING_HOME 写入。 | 当前独立证据：installer 只消费已通过签名、target、digest/bytes 和 strict artifact decode 的输入；stage/version/current/previous 共用 digest 目录，stable launcher 只读取 pointer.directory 且不自行重算 raw 版本。旧安全 pointer 可读且零迁移，first/exact replay 无 home/lifecycle，空格/Unicode 路径由 path API 保持。U38-13 位于上游发布 producer，不形成安装器绕过或第二安装入口，本项自身无 P0/P1。 |
| IR38-08 | [~] | 自动维护触发正确：真实 foreground、on-demand、managed 三种本机 host 启动均异步检查，managed 按既定周期；current 启动、ready、首个输入和工作不等待网络。离线/禁用无破坏效果，无 updater daemon；pure surface 只消费 current-authority 更新投影，本机零 updater、home、module、listener。 | 旧证据已作废：U38-15 修改 automatic round 所有权及两根 teardown；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
| IR38-09 | [~] | 获取与续传安全：检查、下载、Range 续传、超时、断流、重试和容量门禁只接受同一候选 identity；网络 permit、响应头、长度、摘要和最终字节均验证，失败可重放且不污染 current。 | 旧证据已作废：U38-16 修改 FileLock record、活性谓词与平台身份 provider；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
| IR38-10 | [x] | ProgramStore 与指针耐久：stage、current、previous、active stage 的 identity、原子写、目录同步、read-back 和崩溃窗口闭合；任何响应丢失通过耐久事实前滚，不产生第二事实源。 | 当前独立证据：`programVersionDirectory()` 只接受已验 `sha256:` digest并返回完整64位小写hex，stage rename、version read-back、current/previous 和 cleanup 均消费同一目录；pointer 保留 raw release identity，generated launcher只消费 pointer.directory。原子写、目录 sync、read-back和旧安全 pointer 兼容成立，rename/pointer响应丢失可由同一耐久事实前滚，无 P0/P1。 |
| IR38-11 | [x] | ProgramUpdateReceipt 是唯一程序层更新事实：phase、notice、action 及条件字段必须逐项符合上表 exact-set，receipt 不复制 lifecycle、health、pointer 或兼容事实。它与 current/previous、已验 stage 和 active upgrade 形成唯一无副作用投影；CLI、server、status、doctor、输入区和第一方 surface 消费同一结果。固定文案可行动、零 raw，失败在成功、candidate 换代或用户明确收起前保持可见。 | 当前独立证据：receipt strict codec、pointer/stage/lifecycle/health 一致性投影、exact notice token 和 sticky action 未分叉；默认 no-genesis 单机由 `loadConfig(noAutoCreate)` 直接证明本机 current 且零 SecretStore，paired 仅走 existing-only trust/binding/key 检查。doctor 只在 current-authority RPC unavailable时消费该纯证明，non-current/坏 trust 统一 unavailable，固定 code/message/action 无 raw，无 P0/P1。 |
...
| IR38-16 | [x] | 恢复语义有限而完整：健康失败自动恢复 previous；用户可在支持窗口执行 guided restore；恢复仍走同一安全生命周期和健康验证，current/previous 交换可重放，产品不暴露 rollback 术语或无限历史版本。 | 当前独立证据：previous 与 current 均保存 raw release identity和digest目录；健康失败与 guided restore 都重入同一 upgrade lifecycle、exact host stop、pointer swap和loopback health，旧安全目录无需迁移。产品只投影“恢复上一个可用版本”及一个行动，不暴露 rollback/无限历史；响应丢失按 pointer/lifecycle achieved facts前滚，无 P0/P1。 |
| IR38-17 | [x] | 清理和资源上界：仅清理非 current、非 previous、非 active stage 的程序版本，固定批次且 read-back；失败可重入，不触碰 home、SecretStore、checkpoint 或 lifecycle evidence，磁盘和内存增长有界。 | 当前独立证据：删除集合只来自 ProgramStore versions/stage并排除 current、previous、active stage，未触碰 home、SecretStore、checkpoint 或 lifecycle evidence；`rm()`完成或抛错、下次cleanup按当前目录事实重试，128项限制并发破坏面。`readdir()`快照的O(n)内存和额外缺失read-back没有当前可达损失，已由EX38-03排除，本项无P0/P1。 |
...
| IR38-21 | [~] | 应用移除：先复用现有安全 stop 收束 current/future launch，再只移除程序工件；用户 home、配置、信任、SecretStore、checkpoint 和可恢复数据保持，效果丢失可重放。 | 旧证据已作废：U38-17 修改 managed removal prepare/rollback 与三平台 adapter；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
| IR38-22 | [~] | 永久设备移除与误操作边界：与应用移除保持独立入口、强确认和不可逆语义；只将 zz uninstall 与 zz update --rollback 作为旧别名拒绝，不能误伤当前 zz device remove --permanent。文案、help、交互和非交互参数清楚区分“移除应用并保留数据”与“永久移除设备及本机数据”。 | 旧证据已作废：U38-18 修改 device remove/continue strict RPC、journal/mesh 与交互终态；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
...
| IR38-24 | [~] | 五目标发布工件构建：每个 target 从同一 source/package closure 形成自包含 Node 22 程序树，消解 workspace:*，只含运行资产；消费平台签名/公证后才冻结最终归档 bytes、manifest 和逐文件摘要。版本、target、权限、原生件、launcher read-back 全等，开发占位 channel、开发依赖、install script、source map、测试资产和秘密不得进入正式包。 | 旧证据已作废：U38-13/U38-14 修改五目标程序树排序、source/package closure 与 tree receipt；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
| IR38-25 | [~] | release:check 是确定、受控输出且无外部发布副作用的发布门：先运行 lint/test/build，再验证同一 source/package 输入、五目标、上述 12 项 smoke、签名/公证、manifest/artifact、50 行 ledger 和文档版本；结束前复验输入未漂移，才原子写 release-report 与可选 candidate index。不依赖 Git 暂存/干净状态，不保管私钥、不调用外部发布；失败不得留下伪成功或半写正式输出。 | 旧证据已作废：U38-13/U38-14 修改 release producer、evidence 与 release:check 门禁；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
...
| IR38-27 | [~] | 上表 10 个 fault id 逐行闭合：每行分别覆盖其规范终态及适用的效果前失败、效果后响应丢失、重启和恢复；fault-update 还必须证明旧版安全可用或兼容恢复并给唯一用户行动。只有 10 行各自有结论和生产证据时通过。 | 旧证据已作废：U38-15/U38-16 修改 fault-update 的轮次清理与跨进程单飞；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
...
| IR38-29 | [~] | topology-single-machine 与 topology-paired-devices 分别闭合：anchor/executor 和 managed/on-demand/foreground 复用同一更新、lifecycle、health、兼容与诊断原语；pure surface 只走 current authority 的有限投影，本机零 updater/lifecycle/host。未启用角色零模块/监听，无漏装、双装或角色旁路。 | 旧证据已作废：U38-15 修改两根三形态更新装配与清理；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
| IR38-30 | [~] | 上表 12 个 journey id 必须逐行给出入口、用户语言、终态和直接证据；另将 12 项 release smoke exact-set 逐项映射到相关旅程。开箱、配对、日常、离线、uncertain、迁居/恢复、停机/永久移除与四个更新旅程均保持零拓扑术语、正常无噪声、失败一个下一步；只有 12 行和 12 项 smoke 各自有结论时通过。 | 旧证据已作废：U38-13/U38-15/U38-17/U38-18 修改发布、自动更新和两类移除旅程；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
| IR38-31 | [~] | 最终交付装配与证据充分：当前变更集中的 root/scripts、core、cli、mesh、owner-kernel、providers、server、tools-builtin、模块文档和 workbench 每组均反向归入 IR38-02～IR38-30；所有 production 文件有真实 producer、consumer、composition root 和公开出口，直接测试走生产路径。生成 version/channel、S7、golden、导出、ledger、guide、report 与代码一致，无孤立实现、测试专用生产旁路或范围外能力。 | 旧证据已作废：U38-13～U38-18 修改跨包生产、消费、直接测试、派生脚本和文档闭包；按修复后状态更新协议仅登记输入变化，待独立复审，不以修复验证或专项对抗直接产生通过结论。 |
...
> **独立审查结论**：未通过。31项均已完成本轮判定，20项`[x]`、11项`[!]`、零`[ ]`/`[~]`；P38-13～P38-18 已正式转存为 U38-13～U38-18，P38-19 已转存为 EX38-03，三张源问题表当前均无待转记录。阻断节点须待正式问题修复并按影响范围复审，当前不得封版。
