# 研究进度追踪 (Progress Tracker)

> 追踪各认知域的研究状态，确保系统性覆盖

## 总览

| 认知域 | 状态 | 问题数 | 已完成 | 关键阻塞 |
|--------|------|--------|--------|----------|
| 01-核心循环 | 🔶 进行中 | 3 | 2 | q03 待审阅 |
| 02-工具系统 | 🔲 待开始 | 0 | 0 | — |
| 03-上下文管理 | 🔲 待开始 | 0 | 0 | — |
| 04-提示工程 | 🔲 待开始 | 0 | 0 | — |
| 05-安全模型 | 🔲 待开始 | 0 | 0 | — |
| 06-插件架构 | 🔲 待开始 | 0 | 0 | — |
| 07-会话与记忆 | 🔲 待开始 | 0 | 0 | — |
| 08-交互界面 | 🔲 待开始 | 0 | 0 | — |
| 09-协议与通信 | 🔲 待开始 | 0 | 0 | — |
| 10-部署与配置 | 🔶 进行中 | 1 | 0 | q03 覆盖配置部分，待审阅 |

**状态图例**: 🔲 待开始 | 🔶 进行中 | ✅ 基本完成 | 🔁 持续更新

## 源码分析进度

| 目标系统 | 架构概述 | Agent Loop | Provider 层 | 设计模式 | 模块地图 | 数据流 |
|----------|---------|-----------|-------------|---------|---------|--------|
| OpenClaw | ✅ | ✅ | ✅ | ✅ | 🔲 | 🔲 |
| Claude Code | ✅ | ✅ | ✅ | ✅ | — | — |

## 设计产出进度

| 产出物 | 状态 | 依赖的认知域 |
|--------|------|-------------|
| 设计原则 | 🔲 待开始 | 需多个域的洞察积累 |
| 架构概述 | ✅ v0.3 已发布 | 01, 10 |
| ADR-001 Monorepo 结构 | ✅ 已完成 | 全局 |
| ADR-002 Provider 架构 | ✅ 已完成 | 01, 10 |
| 差异化策略 | 🔲 待开始 | 竞品分析 + 认知研究 |

## 实现进度

| 模块 | 包 | 状态 | 验证方式 |
|------|-----|------|---------|
| EventBus | `@zhixing/core` | ✅ 已完成 | 单元测试通过 |
| 核心类型 | `@zhixing/core` | ✅ 已完成 | tsc 编译通过 |
| Agent Loop | `@zhixing/core` | ✅ 已完成 | 单元测试通过（Mock LLM） |
| MockLLMProvider | `@zhixing/core` | ✅ 已完成 | Agent Loop 测试中验证 |
| Provider 层 | `@zhixing/provider-*` | 🔲 下一步 | — |
| 工具系统 | `@zhixing/tools-builtin` | 🔲 待开始 | — |
| CLI | `@zhixing/cli` | 🔲 待开始 | — |

## 里程碑

- [x] **M1**: 完成核心循环 + 工具系统的基本认知 → 理解智能体最小运行原理 ✅ 2026-04-06
- [ ] **M2**: 完成全部 10 个认知域的基本认知 → 整体架构认知建立
- [x] **M3**: 完成设计原则 + 架构概述 → 可以开始技术选型 ✅ 2026-04-06（v0.2）
- [x] **M4**: 完成首批 ADR → 可以开始项目搭建 ✅ 2026-04-06（ADR-001）
- [x] **M5**: EventBus + 核心类型 + Agent Loop MVP 完成 ✅ 2026-04-06
- [x] **M6**: Provider 层调研 + 设计完成 ✅ 2026-04-07（ADR-002）
- [ ] **M7**: 首次真实 AI 对话 → Provider 实现 + 基础工具 + CLI

---

> 每次研究完成后更新此文件，保持进度可见。
