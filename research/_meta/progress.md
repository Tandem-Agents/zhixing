...
- [x] **M27**: 消息 Outbox 设计完成 → Step 16 E2E 暴露多生产者顺序倒转，产出规格 `message-outbox.md` + `ADR-007`；跨模块影响文档同步更新（conversation-model TurnId、ADR-004 ToolContext 扩展、persistent-service §4.7 Pipeline/Outbox 职责切分） ✅ 2026-04-21
...
- [x] **M38**: 旧 skill 实现彻底清理（白底重建准备）。**最大化彻底删**——删 `core/memory/{skills-store,skill-security,effectiveness}.ts` + 4 测试 + `reflection.test.ts`；摘除共享文件 skill 接线（retriever 整体收掉，`enrichContext` 直调 scoped `PeopleStore`；memory-store/flush-engine/journal-store 去 skill 类目/提取/skillCandidates；`RunResult.injectedSkillIds`、reflection 全链路、`/skills` 命令 + audit + stale + 效果推断、system-prompt 的 skill-evolution 段、memory 工具 skill 类目）；按 Finding A 收掉退化抽象 `MemoryRetriever`/`RetrievalResult`；连带清掉 `switch-to-new-conversation` 的 reflection 残留 `lastToolEndCount`。**护栏验证**：people 检索经 `enrichContext`→`PeopleStore` 行为等价（people-store.test 兜底）、profile/journal/flush/memory 工具(profile+person) 全绿。文档：删 `skills-evolution.md`、memory-system.md 技能支柱挂废弃横幅、死链中和。全仓 grep skill 仅剩 temp-dir JSDoc 误报。**`pnpm -r build` 全绿、`pnpm -r test` 全绿（test-utils 1 / network 7 / core 98 / feishu 7 / mcp 9 / providers 11 / server 24 / tools-builtin 16 / orchestrator 21 / cli 93，零失败）** ✅ 2026-05-25
