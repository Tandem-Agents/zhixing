...
> 类型：用户协作文档<br>
> 当前进度：已处理 107 份；未提交文档专项修复已复查，5 次迁移提交的同类问题审查与补正全部完成（5/5，100%），见下方检查点；尚未开始下一个迁移单元。此百分比仅指专项复查，不是 Research 整体迁移进度。
...
正文直接陈述所研究对象的事实、结论与设计理由，不以“原稿如何归纳”“本轮核对了什么”“迁移为何删改”等迁移者的判断过程替代；迁移操作与审查过程只记入任务文档。必要的来源限制、适用条件与未决问题应保留，但不能挤占或削弱有价值的核心结论。

凝练不得损失有价值的结论、比较关系、选型依据和设计理由；旧表格或图示若承担这些职责，应校准后保留或等效重构，不能用通用概述替代具体差异，也不能仅因相关词句仍在就判定完整承接。迁移与复审须对照旧稿核查新文档是否保有相同的解释与决策价值。

历史记录以可还原性为准：有价值的原话、时间线、现场细节、证据与当时取舍不得以摘要替代，也不因不再代表现状而删除。错误推断与未经证实的归因须明确校注，不改写成已证实事实；确需脱敏或去活化时说明处理，不借此抹去记录含义。
...
第 5 次提交专项复查：用户授权检查 `e07016b1` 的全部 67 个文件变更，对照父提交的 19 份旧稿、现行正文及配套引用，已补正 12 份正文。清除配置、秘密存储、安全、子 Agent、中断、确认及工程文档中的迁移者旁白，直接表达产品要求、设计理由与当前限制；补回公开配置与秘密物理分离的原因、工作目录与内部数据目录的区别及默认选址理由、信任沉淀防首次误判固化的目的、强模型研判与降频的取舍、子执行工厂和工具白名单的理由、中断用量与宽限清理、终态事件顺序及审批沟通的核心含义。保留配置保存与换代、权限规则、子执行终态及 RPC 展示等已记录的实现差异，不恢复旧实施计划或把目标写成现状。修正后对照复查未再发现本专项同类问题，所改正文 132 个本地文件链接及差异格式检查通过。仅修改工作区，暂存区哈希与 HEAD 均未变，前 4 次补正保留；未改产品代码、未新增迁移份数。5 次提交专项复查至此完成，后续按逐份审核流程继续尚未处理的迁移单元；以下检查点保留各轮历史范围与结果。

第 4 次提交专项复查：用户授权检查 `3d7811b4` 的全部 62 个文件变更，对照父提交的 18 份旧稿、配套变更和现行正文，已补正 17 份文档。清除 Provider、工具、MCP、网络、Outbox、容错、用量展示及 Phase 2 引用中的迁移者旁白，直接说明设计事实、要求与实现差异；补回辅助调用的上下文累积成本、业务任务不宜统一封装的理由、推荐必须指定 provider/model 二元组的原因、思考配置按角色独立的意义、失败项队首重试与队尾重排的冲突、用量反馈优先级、MCP 候选判断依据及按需绑定目录的理由。保留网络选型的历史错误特征，恢复 Staging 摘要升级记录的日期、角色变化与摘要结构，并明确与现行路径的区别。修正后对照复查未再发现本专项同类问题；所改模块正文及配套新增引用共 170 个本地文件链接检查通过，差异格式检查通过。仅修改工作区，暂存区哈希与 HEAD 均未变，前 3 次补正保留；不改产品代码、不增加迁移份数。下一次待用户授权审查第 5 次 `e07016b1`；以下检查点保留各轮范围与结果。

第 3 次提交专项复查：用户授权以 `7e71cf0b` 的 55 个文件变更为范围，对照父提交旧稿与现行正文检查含义删漏、迁移者判断过程混入正文。已补正 16 份正文：清理 CLI、材料输入、多视角、技能及工作场景中的“本文只记录”“本次迁移不……”等过程说明，保留真正的适用边界与实现缺口；补回技能从一致复用到经验积累、模型增长下的特定性价值、创作知情不另加确认步骤、创作与接入护栏不对称的理由、输入补全选型比较、技能进化与 Hermes 的关键取舍，以及 grep 故障的诊断依据。外部比较限定为仓库调研版本，不宣称外部当前事实；复查时将接入报告后的同意要求明确限定为 needs-confirm，不改变 safe/escalate 分支。修正后未再发现本专项同类问题，差异格式与所改正文的本地链接检查通过。仅修改工作区，暂存区哈希与 HEAD 均未变，不改产品代码、不恢复已批准清退的旧流程、不新增迁移份数。下一次待用户授权审查第 4 次 `3d7811b4`；以下检查点保留各轮范围与结果。

第 2 次提交专项复查：用户授权检查 `dfae98fb` 的 37 个文件变更，在当前工作区修复，不改暂存区与历史。对照父提交旧稿及后续现行正文，清理持久化、CLI 视觉／输入和对话模型中的“仍须保留”“不再保留旧稿”“本节不再定义”等迁移者口吻，保留真实设计与实现差异；补回 CLI 四类场景依据、中文优先、输入留白与复制理由，生命周期的经验总结用途与装配期注册理由，启动预算与 GC 的决策理由，以及切屏故障同帧发／不发 CSI r 的正反实验细节。历史实验明确限定为该次记录，不扩为全部终端结论。用户已批准删除的空工作台、旧索引及被替代的治理流程未恢复。修正后对照复核未再发现本专项同类问题；本轮不增加迁移份数。下一次待用户授权审查第 3 次 `7e71cf0b`；以下第 1 次检查点保留当轮范围与结果。

已提交迁移专项复查（按时间顺序）：`21f1bb8a` → `dfae98fb` → `7e71cf0b` → `3d7811b4` → `e07016b1`。用户本轮仅授权检查第 1 次 `21f1bb8a` 的全部 43 个文件变更，发现问题在当前工作区补正，不改写历史或暂存区。对照其父提交旧稿，发现上下文架构的方案比较、摘要质量要求及子任务预算区分承接不足，动态注入遗漏结果／投递感知，任务列表遗漏部分展示与生命周期理由；已在原三份目标正文补齐，并将“演进中保留的判断”等迁移口吻改为直接设计取舍。依据包括上下文 v1／v2／v3、问题对齐稿、两份工具编排创新稿、摘要规格、注入规格和 Phase 1 实施稿；新增现状已核对当前摘要提示／解析器、模型调用链、调度状态源、TaskTail 与子任务预算代码。用户已批准删除的框架／路线图未恢复，哲学保持已确认内容；其他变更为导航与引用衔接，未发现同类核心遗漏。修正后对照复查及本次文件链接／差异格式检查通过。本轮不增加迁移份数；后 4 次提交尚未审查，须待用户继续授权。

当前未提交文档专项补正：用户指出动态上下文研究把直接结论替换为迁移者判断，扩查发现通道／Markdown 比较关系漏迁、安全事件转录与现场细节被摘要化，以及多篇正文夹带迁移过程说明。经用户授权，已修复 11 份正文：恢复直接结论、六平台与三套飞书实现比较、五种 Markdown 策略及依据、安全事件转录与取证线索；清理过程性噪音，保留来源限制、实现差异与历史推断校注。复查中补齐遗漏的归因线索并纠正标签可信性的表述；最终对照旧稿未再发现本专项问题，转录除域名去活化外一致，当前 31 份变更后 Markdown 文件的本地链接检查无断链，差异格式检查通过。本轮不新增处理份数，不扩目录树、不改产品代码、不改变暂存区或提交。以下较早检查点的“通过”不能覆盖本次发现的问题，以本条补正结果为准；外部历史资料的版本缺口仍按各正文标注，不宣称已复验外部最新版。

动态上下文注入研究：用户批准将 `research/source-analysis/dynamic-context-injection.md` 独立迁至 `docs/research/dynamic-context-injection-comparison.md`。保留三方比较表、机制图、源码定位、状态示意及前缀稳定／及时感知／入口一致的设计理由；区分调度与 Todo、应用侧与服务端缓存，校准未注入不等于无法查询、日期示例不等于逐轮刷新等推断。外部材料未锁定版本，显式说明来源边界及相邻研究的口径冲突；回填知行现有 `<turn-context>` 发送视图路径，旧 16d 建议不再作为待实施任务。相关提示词、缓存研究为相邻参考，不并入本单元；现行模块正文保留唯一实现权威。旧正文已清退、源码分析导航已更新，技能树未变；新增处理 1 份，本单元待审查。前一 Web 搜索工具单元已复审通过。

Web 搜索工具研究：用户批准将 `research/source-analysis/web-search-tool.md` 独立迁至 `docs/research/web-search-tool-comparison.md`，不并入 WebFetch 模块文档。保留三种执行范式、后端表、实现机制、横向对比与来源；外部材料标明 2026-05-23 原研究快照及未锁定版本的证据边界，校准接口统一不等于结果同形、客户端反向推断与安全保证。按当前内置工厂和 WebFetch 校准知行现状，旧落地建议明确为未批准研究候选，不引入功能或实施任务。旧正文清退，源码分析索引指向新正文；其他研究稿和旧索引保留，技能树未变。本单元待审查，新增处理 1 份。

Windows 状态目录迁移记录：用户批准将 `research/agent-workflow-issues/2026-08-17-windows-codex-home-migration.md` 独立迁至 `docs/postmortems/` 同名文件，并明确以历史记录完整性为先，不因年代或现状变化删减细节。原四节正文完整保留，仅调整标题、补充历史范围与经验适用边界；未重新核查本机状态、凭证或会话数据，未执行记录中的操作。导航加入新复盘索引，旧正文及 `research/agent-workflow-issues/README.md` 清退，后者已有重置复盘导航由新索引承接；新增处理 2 份，技能树未变。本单元待审查；下方三篇 CLI 诊断复盘已复审通过。

CLI 诊断复盘收尾：用户批准将 `research/postmortems/2026-05-07-anchor-empty-line.md`、`2026-05-14-confirm-input-paste-misroute.md`、`2026-05-20-confirm-input-freeze-conpty.md` 分别迁至 `docs/postmortems/` 同名文件，不合并正文；三篇及旧 `research/postmortems/README.md` 已清退，新增处理 4 份。保留失败轨迹、证据、成本、比较关系与经验，校准 tokenizer/宿主时序/ConPTY 的未证实推断；回填输入双路径与物理租约连续性的当前实现。审查发现将旧 TextStream 起首清理误写为当前生产机制，已按用户授权修正：保留 DEL/ESC 取舍，明确旧类及测试不代表生产覆盖，补充 OutputRenderer → MarkdownStream 的现行路径；未据此宣告故障或启动代码整改。新索引、AGENTS/CLAUDE 入口及协作流程目录的分工说明已同步，技能树未变；旧交付清单中的路径保留为历史记录。旧复盘目录已无文件，本单元修正后待复审，不代表整个 Research 迁移完成。

复盘与事件职责纠正：用户批准新增 `docs/postmortems/`，技能已区分“后续工作借鉴”与“事件调查留存”。原 `docs/incidents/` 下 2026-05-09 屏幕诊断、2026-07-12 API 兼容、2026-07-14 重置重放三篇已迁至 postmortems 同名文件，既有正文含义保留；凭证外发事件留在 incidents。新迁移旧 postmortems 下 2026-08-01 性能门禁、2026-08-02 验收误判、2026-08-06 验证失控三篇至新 postmortems 同名文件，保留失败轨迹、成本及时间账，校准过强规则和历史状态。六个旧位置文件删除，新增按问题信号检索的 README，相关引用同步。未改变生产代码、验证协议或 Git 状态；待审查。下方旧记录中的 incidents 是曾用位置，以本检查点为准。

公共 API 兼容盲区：用户批准将 `research/postmortems/2026-07-12-public-api-compatibility-blind-spot.md` 迁至 `docs/incidents/2026-07-12-public-api-compatibility-blind-spot.md`，旧文件已删除，旧复盘索引与文档入口已更新。保留历史失误、公开入口消费者与兼容验证教训；明确仅适用于承诺保持旧调用合同的重构，不要求永久保留旧接口或新建通用门禁。未改代码，待用户审查；前一凭证外发诱导事件已审查通过。

凭证外发诱导事件：用户批准将根级 `research/2026-06-30-prompt-injection-credential-exfil-incident.md` 校准后迁至 `docs/incidents/2026-06-30-prompt-injection-credential-exfil-incident.md`，旧文件已删除，文档入口已更新。保留时间线、载荷意图、取证定位及教训；区分历史报告、未决归因和无完成证据的建议，移除过强结论及失效链接。未重查本机或原始会话日志，未操作秘密、网络或系统配置；待用户审查。

通道平台处理记录：用户批准拆分 `research/design/channel-platforms.md` 为 `docs/research/channel-platforms.md` 与 `docs/modules/feishu/architecture.md`，旧稿已删除；同步网关文档第八节平台选择及旧首选排序，网络代理等相邻职责保留。保留平台选择依据、飞书体验目标与架构分工；核实官方资料及生产链，明确未实现的流式／编辑／表情回执、内存去重和截断边界，未核实的平台断言改为待核实问题，不沿用旧排期和合规保证。按既有技能目录归位，未改功能、未执行 Git 写操作，待用户审查。前一工程经验及关联复盘单元已审查通过。

状态、重放与合同一致性的工程经验处理记录：用户批准将 `research/design/principles/` 的 README 与三份规则整合为 `docs/engineering/state-replay-contract-lessons.md`，将 `research/agent-workflow-issues/2026-07-14-compaction-replays-destructive-reset.md` 迁至 `docs/incidents/` 同名文档，五份旧文件已删除。保留准入条件、适用边界、决策理由和事故根因；校准追加／幂等／原子性及文件登记恢复保证，同步直接引用。后经用户确认改为有条件的工程经验定位，取消案例数量门槛和强制验证手段，并同步工作台对应要求。未扩技能树、未改产品代码、未执行 Git 写操作，待用户审查。前一屏幕与 Markdown 渲染单元已审查通过。

屏幕与 Markdown 渲染处理记录：用户批准将 `research/design/problems/screen-render-architecture.md` 合并入已有 `docs/modules/cli/screen-rendering.md`，将 `research/design/specifications/markdown-stream-redesign.md` 校准为新建 `docs/modules/cli/markdown-rendering.md`，关联复盘迁至 `docs/incidents/2026-05-09-screen-render-misdiagnosis.md`；三份旧文件已删除。按技能已有职责创建 incidents 目录，未扩树。承接区域与段合同、模式及方案比较、诊断教训；校准降级、缩进、行数兜底／结束、列宽捕获和重复高亮边界，不把实现差异写成已满足保证。同步入口与直接引用，未改功能、未执行 Git 写操作，待用户审查。前一 CLI 总体架构单元已审查通过，其记录中待处理的渲染专题由本条接续。

CLI 总体架构处理记录：用户批准将 `research/design/architecture/decisions/005-cli-architecture.md` 校准后迁至新建 `docs/modules/cli/architecture.md`，旧稿已删除。保留直接启动、执行与呈现分离、需求驱动选型及上下文／连续性思想；按当前宿主 RPC、输入与输出链校准，移除旧 Ink 路线、未提供的启动参数和规模估算，同步决策索引与入口。相邻 `research/design/problems/screen-render-architecture.md`、`research/design/specifications/markdown-stream-redesign.md` 及渲染复盘本轮未迁移，后续按渲染专题整体核对。不改功能、不执行 Git 写操作，待用户审查。
...
1. 核心承接：旧文档中仍有效且有价值的需求、架构思想、关键取舍和职责边界，是否在新文档中得到准确承接。按本任务“核心依据”核对结论、比较关系与解释价值，检查迁移者的判断过程是否侵入正文。一般设计说明保留含义，不要求保留原措辞、篇幅或文件数量；历史记录另按可还原性核对原话与细节，不能以摘要代替。不能因删掉旧方案而丢掉仍成立的设计理由。
