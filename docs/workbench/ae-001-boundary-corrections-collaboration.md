...
本文只维护双方交接、问题处置和验证证据。当前任务为[通用自主性：产品与架构设计](../tasks/agent-autonomy-design-and-execution.md)的单元 3，需求、封版设计及单元边界以该任务和[需求起点](../origin/agent-autonomy.md)为准。沿用原协作文件位置；下方单元 1、2 和 AE-001 四单元记录已结项，不是本轮待办。
...
本轮交付单元 3“自主发现、接入能力并继续原任务”：让模型自主寻找、学习和组合已有工具、技能、程序与服务，确需新增能力时通过产品管理入口受控接入，生效后继续原任务。单元 1 的身份与通用自主职责、单元 2 的耐久交接与交付已完成，本轮复用且不得破坏；MCP 是当前接入实现，不是将通用自主性缩成固定场景或强制注册所有能力。封版产品与架构设计不修改；需要改变范围时交用户决定。
