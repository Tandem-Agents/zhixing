...
任务衔接与历史恢复不是同一件事。模型触发进入时应携带引发切换的用户输入，让场景知道接下来要做什么，而不复制整段主对话历史。当前 [CLI 切换处理](../../../packages/cli/src/repl.ts)只切换会话，不自动携带该输入，场景首轮由用户进入后提供。因此，场景历史恢复不能作为原任务已衔接的证明。

工作模式也应能新建、浏览和恢复对话，不应只因处于工作场景就禁用 `/new`、`/resume`。当前命令没有禁用守卫，但 `ConversationController.newConversation()` 实际新建并切到 main；这与“在当前场景作用域内新建”的要求存在差异，不能把命令可用等同于场景内新建已实现。场景默认进入仍选择 primary 会话。历史与恢复的正文由[对话持久化](../conversation/persistence.md)和[运行体生命周期](../conversation/runtime-lifecycle.md)承担。
