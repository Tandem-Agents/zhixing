...
模型工具与 `/work` 不应维护两套业务；应基于共同的原子管理能力，按使用场景作薄封装。创建后能够补绑、更换或解除目录，删除场景不能动用户目录。主模式管理全部场景，场景模式只管理自身相关属性。

`/work` 的 Ctrl+N 应理解用户自然表达，而不是把输入硬接为固定名称字段：使用 Main 档位、临时且非持久化的模型调用，只给必要上下文与工具；信息不足时澄清，执行经确认的真实创建。
...
目录是可选的工作条件，不应成为创建场景的必填流程。创建时用户未必已知道或需要哪个目录，真正需要文件能力时任务上下文才明确；因此可以先创建、使用场景，需要时再引导选择工作空间，绑定持久生效后不应在每次文件操作前重复询问。取消绑定不阻断无目录也能完成的工作，但不能以默认目录继续未经授权的文件操作。

当前已支持创建后显式绑定、更换和解除工作空间；无授权工作空间的 power profile 在装配时不提供本地文件工具。因此，“首次需要读写文件时自动询问，绑定后继续原任务”的体验尚未形成完整生产流程，不能把管理工具存在等同于该体验已实现。

目录输入的辅助识别应保守、可编辑：只在明确的目录输入语境处理可靠的引号、空白或转义格式，保留 Windows 路径分隔符；无法可靠识别时保留原输入供修正，不因辅助识别失败打断输入，也不改写普通对话正文。输入识别与绑定校验是两回事，真实的权限、类型和设备错误仍须明确反馈，不能以“任何情况不报错”掩盖失败。

终端拖拽可能只提供路径文本，不证明该路径属于执行设备。本机路径须经本机绑定管理转换，远程绑定使用目标设备的授权引用。当前智能创建会校验模型提交的本机绝对路径，通用规范化只做 trim 与 `path.normalize`；这不构成专用的拖拽／粘贴目录输入流程。[CLI 材料输入](../cli/material-input.md)处理的是本轮附件采集，不承担工作空间绑定，也不能作为上述目录输入体验已实现的证据。
