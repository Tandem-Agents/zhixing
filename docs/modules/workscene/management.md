...
目录是可选的工作条件，不应成为创建场景的必填流程。用户尚未需要文件能力时可以先创建、使用场景；需要时再引导选择工作空间，绑定持久生效后不应在每次文件操作前重复询问。取消绑定不阻断无目录也能完成的工作，但不能以默认目录继续未经授权的文件操作。

当前已支持创建后显式绑定、更换和解除工作空间；无授权工作空间的 power profile 在装配时不提供本地文件工具。因此，原设计“首次需要读写文件时自动询问，绑定后继续原任务”的体验尚未形成完整生产流程，不能把管理工具存在等同于该体验已实现。迁移保留按需引导的目标，不沿用文件工具中途弹窗、直接更新 Registry 或退回宿主 cwd 的旧实现方案。
