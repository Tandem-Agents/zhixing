...
确认负责取得用户对待执行操作的决定，而非重新定义安全策略。审批是一次沟通，不只是开关：用户应能说明拒绝原因，也应能在批准时补充要求，让模型据此调整方案；当前两条反馈链的支持程度见下文。缺少交互能力或用户未响应不能默认放宽安全边界。具体入口见[确认接入与交互](surfaces.md)，工具侧政策与批准应用见[工具与权限集成](../tools/permission-integration.md)。
...
批准时的补充说明应回送模型，而非仅留作本地日志。类型与渲染器支持 `note`，但当前常规请求不生成该选项，尚不能提供与拒绝理由对称的完整产品入口及模型反馈。`edit-then-allow` 在执行侧明确拒绝为未实现，不能因为类型存在就认为可编辑后执行。
...
信任规则与安全助理研判由[安全模块](../security/trust.md)负责；确认层不另建预审批规则库或模型分诊链。
