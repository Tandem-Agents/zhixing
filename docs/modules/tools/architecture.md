...
容器隔离不是所有部署环境都具备的前提；全面自动放行无法约束危险操作，逐次机械确认又会造成疲劳。工具安全须在实际操作边界内兼顾风险与使用成本，不把安全责任交回用户自行承担。
...
`turnId`、投递目标和可选 `commitToUser` 由执行上下文承接；工具直接掌握执行结果，可以报告自己已完成的事实，无需等待模型重新推断。只应在反馈确实成立后设置 `committedToUser`，供模型避免重复叙述。无反馈通道时仍返回正常工具结果供模型表达。
