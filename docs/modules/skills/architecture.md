...
技能的价值不止是重复执行时保持一致，更在于持续积累做事经验。模型越强，越应减少通用操作步骤，留下特定约定、踩坑原因与经过验证的路径；方法帮助模型判断，不把判断锁死成逐步照抄的流程。
...
- `skill-archive` 当前从活动目录移除条目。归档应可逆，但现有管理 API 没有恢复命令，不能把权威历史或制品仍在解释为用户已经能恢复。
