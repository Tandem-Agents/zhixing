...
保留执行机制与产品入口的分离：调用方决定任务与组合方式，子执行负责装配、权限、预算、中断和结果。用户不必编写角色或流程配置；不能从“可组合”推导出必须预建任意组合、递归或更多工具入口。
...
每次派生创建独立消息、角色、EventBus、ConfirmationBroker 和中断状态，不池化复用上下文。运行采用同进程异步执行，重叠模型与工具 I/O，不以 worker 或独立进程实现子 Agent。上下文隔离不是进程沙箱。
...
当前工厂清理 bus/broker，loop 清理定时器及监听，再 settle/release 子租约；结算或释放异常转为 failed。该异常分支提前返回，未发送 child_end，故不能承诺所有启动事件都必有配对终态事件。编排外层中止/超时可能先返回包装结果，其等待不等于底层清理已完成，详见[编排终态边界](../orchestration/architecture.md)。这些是需保留的实现差异，不是放宽终态要求。
