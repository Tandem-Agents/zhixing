...
编辑器只调用一个 `save` 入口，由 Configuration/Secret owner 检查公开配置和实际修改的凭据基线。初始配置、REPL 与 MCP 专用编辑共用这一保存机制；过期修改在发布或写入前拒绝，未修改的其他账号材料保留。

保存计划耐久接纳前失败，不改变生效来源；接纳后若中断，明确提示“修改已接纳、收尾待恢复”，重新打开配置入口或重启会完成同一计划。接纳结果未知时核对标记，不猜测未提交。公开配置与秘密物理写入仍分步执行，但在完整收束前不向新消费者返回半份来源；也不在失败时用旧快照逆向覆盖。
...
直接验证应区分：首次与再次缺字段、可选角色／通道不阻塞、无 LLM 引导、TTY／非 TTY、取消不提交、并发不同条目保存与同条目冲突、耐久接纳前后及各写入阶段的中断恢复、完成后重读失败、敏感值不出现在错误中。不得以表单填写成功证明外部服务可用或设备角色已经就绪。
