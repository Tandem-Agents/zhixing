...
状态：首发收尾，尚不可正式发布。以下清单在 2026-09-13 核查基础上同步了 2026-09-19 跨平台任务的范围变化；后文 P01～P08 保存此前准备过程及证据，不替代最新状态。

## 当前判断与待办（2026-09-19 范围更新）

原核查基线：`3f31c9b4`，加当时工作区对会话持久化说明的修订。其余首发事项未在本次重新验收。用户随后要求解除单平台绑定：交付目标为 Windows x64、macOS x64/arm64、Linux x64/arm64，宿主条件见[运行时边界](../../research/design/modules/distributed-runtime/node-runtime-boundaries.md)；仍采用 Node >=24 与 npm 全局安装，不删减既有能力。实现及本机独立审查已完成，证据见[协作记录](../workbench/ae-001-boundary-corrections-collaboration.md)；macOS/Linux 实机验收与五目标 CI 运行归入下方 R04/R05，不能沿用旧 Windows 结果宣告跨平台可发布。

### 当前事实

| 分类 | 核实结果 |
|---|---|
| 产品功能 | 本轮未确认新的首发功能阻塞，不等于全部真实旅程已验收。对比内部准备提交 `fc0de070`，27 个有变更的 TypeScript 文件经语法树去注释比较无差异；依赖与 manifest 未变。既有功能证据可按输入复用，不因文档迁移重开全项目开发 |
| 工程交付阻塞 | 本轮 `pnpm lint` 实际失败：S7 仍读取已删除的文档。其 10 项文档输入中有 4 个失效路径，见 R01。供应链边界、TLS 测试边界和秘密隔离检查已通过；单独执行 Biome 检查 1696 文件通过。不能沿用此前 G3 已闭合的总判断 |
| 发布流程缺口 | `scripts/publish-npm.mjs` 在 registry 候选的版本、integrity、version/help 检查后立即提升 CLI `latest`；没有停点供最终全局安装、真实用户旅程及托管验收完成，且该 registry 验证不等同于本地完整 package check |
| 开源文件与目录 | 中英文 README、MIT、贡献/行为/安全说明、问题/PR 模板、版本说明和包 README 已存在；本轮 16 个公开包结构测试 4/4、版本检查通过，25 份公开入口的 122 个本地文件链接有效。此结果不代表 npm 页面相对链接或真实产品操作已验收 |
| 公开渠道 | 当日匿名只读 GitHub API 返回：仓库简介仍描述 Yuling 执行运行时，homepage 为空、topics 为空、私密漏洞报告 `enabled=false`、Releases 为空；官方 npm registry 查询 `@zhixing/cli` 返回 404。未检查账号登录或写权限，不能据此认定账号不可用 |

此前讨论的启动装填阈值、分片清理和退场 Memory 不作为本轮新增功能待办。旧设计差异、文档措辞或尚缺验证都不自动等于产品缺陷；只有当前可达失败与明确首发合同相冲突，才进入修复范围。

### 执行清单

| 顺序 | 待办与类别 | 完成条件 |
|---|---|---|
| [ ] R01 | **修复迁移造成的工程门禁失效**：更新 S7 的 4 个失效文档输入及相交 golden/断言 | 将原持久化草案、工作场景管理草案、消息 Outbox、运行体生命周期文档的检查归到现行权威正文，按当前合同调整断言；不恢复旧文档、不为通过检查给正文塞旧词、不删除有效检查。`pnpm lint` 完整通过 |
| [ ] R02 | **补齐候选验收与正式推广的发布停点** | 在现有发布入口中分离候选上传/验证与 `latest` 推广；沿用精确版本、依赖拓扑、shrinkwrap、integrity 和重入冲突保护，明确使用官方 registry。候选或验收失败不得推广；直接测试证明这一边界，不建设另一套发布系统 |
| [ ] R03 | **完成公开门面与反馈入口**；可与 R01/R02 独立推进 | GitHub 简介、homepage/topics 与个人智能体定位一致；私密漏洞报告实际启用并可进入，SECURITY 与版本说明同步真实渠道；复核 GitHub 和随 npm 包发布的 README 链接、双语承诺、使用/维护入口。发布前保持“未发布”，正式交付时同步状态；不新建官网或重复教程 |
| [ ] R04 | **固定候选输入并闭合本地工程/制品证据** | R01/R02 与随包文件固定后，先确定独立预发布版本（如 `0.1.0-rc.1`），统一全部包版本，再固定 commit、锁文件及支持平台；复用同输入的功能证据，只补真实失效闭包。对当前生产依赖的新风险核实可达性，不做无差别升级。按现有 canonical 流程取得 fresh build 与 16 包安装、公开入口、helper、许可/秘密及卸载保留证据；发布脚本不无理由重复已验证的同输入全量检查 |
| [ ] R05 | **发布候选并完成真实验收**（证据缺口，不预设功能有错） | 用户账号条件和本次发布授权就绪后，将 R04 的同一候选发布到非默认 tag，保留正式 `0.1.0`。从官方 registry 安装到各声明目标的隔离 npm 全局前缀及数据目录，验证 `zz`/`zhixing`、交互配置、真实模型读写任务、继续/恢复对话、拒绝/取消/失败终态、诊断、实际托管启停、同版重装及卸载数据保留。Skills、MCP、调度、飞书、配对/跨设备与备份恢复按已承诺范围核对直接证据，只补缺失项；跨平台任务中仍有效的验收结果复用，不重复同输入旅程。用户自行输入凭据，不读取/展示秘密，不动日常使用数据 |
| [ ] R06 | **正式发布并确认公开交付** | R01～R05 完成，已确认 P0/P1 为零。由同一已验收源码切换正式版本 `0.1.0`，生成并验证最终制品及 registry 安装闭包；版本/制品变化须重验相应检查，未变业务旅程不全部重跑。通过后提升 `latest`，同步 Git 标签、GitHub Release、中英文发布状态及正式版本说明，再从默认公开入口核验安装和启动。失败停止后续推广，不覆盖已发布版本或标签 |

R05 的首次配置、真实工具效果、恢复与取消等验收在同一次候选安装旅程中完成，不另造多套全量验收。若实际发现功能缺陷，记录复现、对应合同和影响，修复并重验受影响链后回到该节点；不能以“已知限制”绕过 P0/P1，也不追加无依据的增强。

用户配合只在需要时介入：npm 登录/组织与包权限/2FA、真实模型及可选外部服务的凭据输入和调用许可，以及无法由现有登录会话完成的 GitHub 管理操作。账号事项不阻塞 R01～R04；不索取 token，不把过去的推送授权扩展成 npm 发布授权。

**清单复核结论：**六项分别解决实际门禁失败、提前推广风险、公开渠道缺口、候选证据、用户旅程和正式交付，无重复权威或“先正式发布才能验收”的循环。本次已授权的五目标交付与手动构建验证 CI 纳入 R04/R05；Research 全量迁移、目录全面美化、旧设计补齐、Memory、强沙箱、声明范围之外的新平台、原生安装器、自有更新、文档站和 CI 自动发布不进入首发关键路径。完成上述清单即发布，不等待长期维护清零。
...
| 发布门 | 必须成立 | 足够的验收证据 |
|---|---|---|
| G1 产品承诺 | 正式支持须以声明的五目标实际验收为前提，Node >=24、npm 全局安装及既有正式能力不回退。准确说明本地部署、模型服务数据流、执行/审批与隔离边界、成本前提和已知限制；不宣称尚未验收的平台或未实现的沙箱、记忆、插件平台 | 中英文入口、公开命令、当前生产能力与发布说明一致；版本号和稳定性承诺明确；macOS/Linux 仍待验收 |
| G2 用户完成事情 | 一个不了解架构的用户能按文档安装、配置所需模型、完成一个有真实工具效果的任务、查看并恢复对话；取消/失败有真实终态和明确下一步；能诊断、停用、维护和卸载且符合数据保留合同 | 各声明目标隔离数据目录中的实际产品旅程与平台服务/密钥适配证据；真实模型验证由用户控制凭据与外部调用授权，不读取或展示用户秘密。配对、渠道、恢复等已承诺能力复用有效直接证据，对失效交界补验，不临时删减承诺 |
...
| G4 制品可用 | 复用现有分发合同：16 个公开包版本、精确依赖、exports、仓库元数据和许可正确，私有测试包不发布；CLI/helper 从实际包安装可用，维护与卸载行为成立 | 同一候选的五目标 helper 完整性、各目标 package check 与安装 smoke；真实发布时再验证 registry 候选版本、integrity、CLI shrinkwrap 和候选安装，全部成立后才提升 latest。手动 CI 只构建验证，不替代真实服务及产品旅程验收 |
...
**现有能力必须保护，但不要求首发无限扩面。** 除已授权的跨平台修复外，强沙箱、新记忆/OWNWARD、声明范围之外的新平台、新交互表面、插件生态、自动更新等，不因“别人有”自动成为首发阻塞项；如发现当前安全合同确实不成立，则按真实缺陷修复，不能靠声明免责。
...
不为首发强行增加官网、文档站、CLA、复杂治理、资金页、徽章、签名公证、新安装器或 CI 发布平台。许可证、测试、安全和贡献机制必须真实有效；文件齐全或徽章齐全本身不是质量证明。本次跨平台交付使用手动 CI 构建、汇集和验证五目标产物，不执行发布；真实发布仍走已有显式授权流程。
...
- [ ] G3 工程、安全、数据和现有能力保障成立：既有功能/安全证据保留，当前 S7 文档输入失效待 R01 修复，候选证据按 R04 更新。
- [ ] G4 同候选基线的正式制品验证通过。
...
本节记录内部准备完成时的判断；2026-09-13 已发现的门禁失效与发布停点缺口，以文首 R01～R06 为当前待办。以下“无内部阻塞”“已闭合”均指当时基线。
