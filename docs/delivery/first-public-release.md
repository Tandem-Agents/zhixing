...
状态：首发收尾，尚不可正式发布。当前判断与执行顺序以下方 2026-09-13 清单为准；后文 P01～P08 保存此前准备过程及证据，不替代最新状态。

## 当前判断与待办（2026-09-13）

核查基线：`3f31c9b4`，加工作区对会话持久化说明的修订。首发范围保持 Windows 10/11 x64、Node >=24、npm 全局安装及已承诺能力，不新增产品功能。
...
| [ ] R05 | **发布候选并完成真实验收**（证据缺口，不预设功能有错） | 用户账号条件和本次发布授权就绪后，将 R04 的同一候选发布到非默认 tag，保留正式 `0.1.0`。从官方 registry 安装到隔离的 Windows npm 全局前缀及数据目录，验证 `zz`/`zhixing`、交互配置、真实模型读写任务、继续/恢复对话、拒绝/取消/失败终态、诊断、实际托管启停、同版重装及卸载数据保留。Skills、MCP、调度、飞书、配对/跨设备与备份恢复按已承诺范围核对直接证据，只补缺失项。用户自行输入凭据，不读取/展示秘密，不动日常使用数据 |
...
**清单复核结论：**六项分别解决实际门禁失败、提前推广风险、公开渠道缺口、候选证据、用户旅程和正式交付，无重复权威或“先正式发布才能验收”的循环。Research 全量迁移、目录全面美化、旧设计补齐、Memory、强沙箱、新平台、原生安装器、自有更新、文档站和 CI 平台不进入首发关键路径。完成上述清单即发布，不等待长期维护清零。
