...
12. **终审质量门（§3.13 唯一清单）**：机械触发——D3 定稿判定前，或已过门产物 diff 触及承诺/机制/验收/语义句（纯措辞豁免）；角色分离——执笔方举证、非执笔方裁判、冷读者对抗，无人自判；三法缺组不合格（符合性/闭合性——含排除负担落点与状态机实现的终局六问（通用方法）/场景走查含故障故事）；**默认冷读**＝无讨论史新会话最后一步，冷读未过门禁无效。**代码类 D3 产物**：提交前唯一时点＝冷执行（只读契约构造场景、对冻结版本跑真实接口）＋既有套件＋定向读审，门裁判宣判；探针清单在案为必要条件，读审零发现不构成可提交。
